...
// ── CRYPTO ENGINE (shadow mode — runs alongside existing Hunter) ──
let cryptoEngine = null;
try {
  cryptoEngine = require('./hunter_crypto_engine.js');
} catch (e) {
  // Engine file not deployed yet — continue without it
}
...
const CONFIG = {
  ...
  THRESHOLDS: {
    ...
    confidenceMinimum:      55,   // discovery layer — Sniper handles validation
    convictionMinimum:      50,   // discovery layer — Sniper handles validation
    executionHardFloor:     55,   // was 60
    edgeMinimumOverField:   4,    // discovery layer — don't gate on edge alone
    competitionGap:         5,
    competitionBonus:       8,
  },
...
// ── CORE CRYPTO (V12 — 25 symbols, leader-hunting not coverage) ──
// Proposed by audit. Thematically grouped.
// L1/L2 core + DeFi + RWA + AI + Infrastructure
const CORE_CRYPTO = [
  // Tier A — Core leaders (always tracked)
  'BTCUSDT','ETHUSDT','SOLUSDT','BNBUSDT','LINKUSDT','AAVEUSDT',
  // Tier B — High-conviction growth
  'UNIUSDT','ONDOUSDT','HYPEUSDT','ARBUSDT','SUIUSDT','TAOUSDT',
  'RENDERUSDT','AVAXUSDT','NEARUSDT','PENDLEUSDT',
  // Tier C — Emerging / high-beta
  'ENAUSDT','JUPUSDT','INJUSDT','APTUSDT',
  // Additional strong candidates
  'CRVUSDT','STRKUSDT','XRPUSDT','LDOUSDT',
...
const STATE = {
  ...
  btcRegime:            'NEUTRAL',
  btcScoreHistory:      [],   // last 5 BTC Hunter scores for surge detection
  symbolScoreHistory:   new Map(), // symbol -> last 5 scores for acceleration detection
  candidateStates:      new Map(), // symbol -> { state, persistence, firstSeen, lastSeen, peakScore }
  btcLastSurgeAlert:    0,    // timestamp of last BTC surge alert
  qqqRegime:            'NEUTRAL',
...
async function fetchBTCRegime() {
  try {
    ...
    // ── 24h REALITY CHECK ──────────────────────────────────
    // Price vs 24 candles ago (24h on 1h candles)
    // If BTC is down meaningfully today, don't call it BULL
    const dayAgoClose  = closes.length >= 24 ? closes[closes.length - 24] : closes[0];
    const change24h    = (last - dayAgoClose) / dayAgoClose * 100;
    log(`  BTC 24h change: ${round2(change24h)}%`);

    // EMA structure + 24h momentum combined
    const emasBullish  = last > e20 && e20 > e50;
    const emasBearish  = last < e20 && e20 < e50;
    const dayStrong    = change24h >= 1;    // up 1%+ today
    const dayWeak      = change24h <= -1;   // down 1%+ today

    if (emasBullish && !dayWeak)  return 'BULL';    // uptrend + not falling today
    if (emasBearish || dayWeak)   return 'BEAR';    // structure broken OR falling today
    return 'NEUTRAL';
  } catch (err) {
...
// ── 4H / 12H DIRECTIONAL CONTEXT ────────────────────────────
// Feature/penalty — NOT a hard gate. Measure first, gate later.
async function fetchHTFTrend(symbol, assetType) {
  try {
    let candles4h = null;
    let candles12h = null;
    if (assetType === 'crypto') {
      const [d4h, d12h] = await Promise.all([
        httpGet(`https://api.bybit.com/v5/market/kline?category=linear&symbol=${symbol}&interval=240&limit=20`),
        httpGet(`https://api.bybit.com/v5/market/kline?category=linear&symbol=${symbol}&interval=720&limit=20`),
      ]);
      if (d4h?.result?.list)  candles4h  = d4h.result.list.map(c => parseFloat(c[4])).reverse();
      if (d12h?.result?.list) candles12h = d12h.result.list.map(c => parseFloat(c[4])).reverse();
    } else {
      const data = await httpGet(`https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?interval=1h&range=5d`);
      const closes = data?.chart?.result?.[0]?.indicators?.quote?.[0]?.close?.filter(Boolean) || [];
      if (closes.length >= 8) {
        candles4h  = closes.slice(-8);
        candles12h = closes.slice(-24);
      }
    }
    function dir(arr) {
      if (!arr || arr.length < 4) return 'flat';
      const mid  = Math.floor(arr.length / 2);
      const avgF = arr.slice(0,mid).reduce((s,v)=>s+v,0)/mid;
      const avgL = arr.slice(mid).reduce((s,v)=>s+v,0)/(arr.length-mid);
      const pct  = (avgL-avgF)/avgF*100;
      return pct >= 1 ? 'up' : pct <= -1 ? 'down' : 'flat';
    }
    return { trend4h: dir(candles4h), trend12h: dir(candles12h) };
  } catch { return { trend4h: null, trend12h: null }; }
}
...
function scoreMarketContext(assetType) {
  ...
  if (STATE.btcRegime === 'BULL') {
    // BTC itself doesn't benefit from its own regime — avoid self-reinforcing scores
    score += assetType === 'crypto' ? 15 : 5;
  ...
  if (assetType === 'crypto') {
    if (STATE.cryptoBreadth >= 70) { score += 10; reasons.push('Broad crypto strength'); }
    else if (STATE.cryptoBreadth <= 30) { score -= 10; reasons.push('Crypto breadth weak'); }
  }
...
function scoreParticipation(candles) {
  ...
  // Dollar volume — scored RELATIVELY against the asset's own recent history
  // Prevents BTC always scoring 90 just for being BTC
  const dollarVol = lastVol * closes[closes.length - 1];
  const dollarVols = candles.slice(-20).map(c => c.volume * c.close);
  const dollarVolSMA = dollarVols.reduce((a,b)=>a+b,0) / dollarVols.length;
  const relDollarVol = dollarVolSMA > 0 ? dollarVol / dollarVolSMA : 1;
  if (relDollarVol >= 2.0) { score_parts.push(90); reasons.push(`Dollar volume ${round2(relDollarVol)}x own avg — surge`); }
  else if (relDollarVol >= 1.3) { score_parts.push(70); reasons.push(`Dollar volume ${round2(relDollarVol)}x own avg`); }
  else if (relDollarVol >= 0.8) { score_parts.push(50); }
  else if (dollarVol >= CONFIG.MIN_DOLLAR_VOLUME) { score_parts.push(35); reasons.push('Dollar volume below own avg'); }
...
// ── ASSET CLASS PERCENTILE ────────────────────────────────────
// Display only — raw scores used for all gate decisions.
function calcPercentile(score, assetType, allScores) {
  const cls = allScores.filter(s=>s.assetType===assetType).map(s=>s.finalScore).sort((a,b)=>a-b);
  if (cls.length < 2) return null;
  return Math.round(cls.filter(s=>s<=score).length/cls.length*100);
}
...
async function scoreAsset(symbol, assetType) {
  try {
    ...
    // Quick pre-estimate for HTF fetch threshold check
    const preEdgeApprox = (
      structure.score  * 0.15 +
      part.score       * 0.15 +
      momentum.score   * 0.15 +
      execEngine.blendedScore * 0.20 +
      risk.score       * 0.10
    );
    ...
    // ── 4H/12H CONTEXT — feature modifier, not hard gate ──
    // Audit mandate: measure first, gate later.
    // Fetch HTF only for top candidates (score ≥55) to save API calls
    let htfMod = 0;
    let trend4h = null;
    let trend12h = null;
    if (preEdgeApprox >= 55) {
      try {
        const htf = await fetchHTFTrend(symbol, assetType);
        trend4h  = htf.trend4h;
        trend12h = htf.trend12h;
        // Score direction: if momentum is positive, we're bullish
        const bullish = momentum.score >= 50;
        htfMod = htfContextModifier(trend4h, trend12h, bullish ? 'up' : 'down');
      } catch { /* non-blocking */ }
    }
    ...
    const preEdgeScore = clamp(weightedSum / weightWithoutEdge + htfMod);
    ...
    return {
      symbol,
      assetType,
      trend4h:  trend4h  || null,
      trend12h: trend12h || null,
      htfMod:   htfMod   || 0,
      preEdgeScore,
      rawScores,
      stageInfo,
...
async function runFullScan() {
  ...
  // ── BTC CORRELATION BOOST ───────────────────────────────
  // When BTC is surging (score jumped 8+ pts last scan),
  // L1 crypto assets get a score boost — they move with BTC
  // even before their individual indicators fire.
  const BTC_SURGE_BOOST   = 8;   // pts added to L1 crypto when BTC surging
  const BTC_SURGE_MINIMUM = 8;   // BTC score must jump this much to trigger
  const L1_SECTORS        = new Set(['CRYPTO_L1','CRYPTO_L2']);

  const btcHistory = STATE.btcScoreHistory || [];
  const btcCurrent = scored.find(s => s.symbol === 'BTCUSDT')?.hunterScore || 0;
  const btcPrior   = btcHistory.length >= 1 ? btcHistory[btcHistory.length - 1] : btcCurrent;
  const btcSurging = btcCurrent - btcPrior >= BTC_SURGE_MINIMUM && btcCurrent >= 60;

  // Update BTC score history (keep last 5)
  STATE.btcScoreHistory = [...btcHistory, btcCurrent].slice(-5);
  if (btcSurging) log(`🚀 BTC SURGE DETECTED: score ${round2(btcPrior)} → ${round2(btcCurrent)} (+${round2(btcCurrent - btcPrior)} pts)`);

  const withCompetition = scored.map((r) => {
    const comp = scoreCompetition(r.symbol, r.hunterScore);
    let baseScore = r.hunterScore * 0.9 + comp.score * 0.1;

    // Apply BTC correlation boost to L1/L2 crypto when BTC is surging
    if (btcSurging && r.symbol !== 'BTCUSDT') {
      const sector = SECTOR_MAP[r.symbol];
      if (L1_SECTORS.has(sector)) {
        baseScore = Math.min(100, baseScore + BTC_SURGE_BOOST);
        r.btcBoostApplied = BTC_SURGE_BOOST;
        log(`  📈 BTC boost +${BTC_SURGE_BOOST} applied to ${r.symbol}: ${round2(r.hunterScore)} → ${round2(baseScore)}`);
      }
    }

    const finalScore = clamp(baseScore);

    // Update per-symbol score history for acceleration detection
    if (!STATE.symbolScoreHistory) STATE.symbolScoreHistory = new Map();
    const _symHist = STATE.symbolScoreHistory.get(r.symbol) || [];
    STATE.symbolScoreHistory.set(r.symbol, [..._symHist, finalScore].slice(-6));

    return { ...r, comp, finalScore };
  });

  const finalCandidates = withCompetition.map((r) => {
    // ── TIERED DISCOVERY — not binary reject ─────────────────
    // Hunter = discovery engine. Sniper = validation engine.
    // Gates below flag issues for Sniper but don't erase the candidate.
    // Score drives tier. Flags inform Sniper what needs validating.
    //
    // OBSERVE:    score 50-55 — early signal, log it
    // DEVELOPING: score 55-60 — building momentum
    // CONFIRMED:  score 60+   — strong candidate
    // OPEN/WATCH: clears all gates — Sniper-ready
    let decision = 'OBSERVE';
    const rejectReasons = [];
    const sniperFlags   = []; // flags for Sniper, not rejections

    if (!isMarketOpen(r.assetType)) {
      return { ...r, decision: 'OBSERVE', rejectReasons: ['Market closed — observing only'] };
    }

    // Collect flags for Sniper — these are informational, not rejections
    const execFloor = (r.execEngine?.setupType === 'BREAKOUT_CONTINUATION' || r.execEngine?.setupType === 'MOMENTUM_IGNITION') ? 50 : CONFIG.THRESHOLDS.executionHardFloor;
    if ((r.rawScores?.executionQuality || 0) < execFloor)
      sniperFlags.push(`execution:${round2(r.rawScores?.executionQuality)} (Sniper validates)`);
    if ((r.confidence?.score || 0) < CONFIG.THRESHOLDS.confidenceMinimum)
      sniperFlags.push(`confidence:${round2(r.confidence?.score)} (Sniper validates)`);
    if ((r.conviction?.score || 0) < CONFIG.THRESHOLDS.convictionMinimum)
      sniperFlags.push(`conviction:${round2(r.conviction?.score)} (Sniper validates)`);
    if ((r.edgeResult?.edge || 0) < CONFIG.THRESHOLDS.edgeMinimumOverField)
      sniperFlags.push(`edge:${round2(r.edgeResult?.edge)} (Sniper validates)`);
    if ((r.rawScores?.structure || 0) < 55)
      sniperFlags.push(`structure:${round2(r.rawScores?.structure)} (Sniper validates)`);
    if ((r.rawScores?.risk || 0) < 50)
      sniperFlags.push(`risk:${round2(r.rawScores?.risk)} (Sniper validates)`);
    if (r.comp?.rank > 5)
      sniperFlags.push(`rank:#${r.comp?.rank} — not field leader`);

    // Assign tier — score drives state, flags inform Sniper
    const score = r.finalScore;
    r.sniperFlags = sniperFlags;

    if (sniperFlags.length === 0) {
      // Clean — all gates clear, assign OPEN/WATCH by score
      let adjustedScore = score;
      if (r.comp?.rank === 1 && (r.comp?.gap || 0) >= CONFIG.THRESHOLDS.competitionGap) {
        adjustedScore = Math.min(100, score + CONFIG.THRESHOLDS.competitionBonus);
      ...
      else decision = 'DEVELOPING';
    } else {
      // Has Sniper flags — tier by score, don't reject
      if (score >= 65)      decision = 'CONFIRMED';   // strong but needs Sniper validation
      else if (score >= 60) decision = 'DEVELOPING';
      else if (score >= 55) decision = 'DEVELOPING';
      else                  decision = 'OBSERVE';

      // Still count as rejection for calibration stats
      rejected++;
      for (const flag of sniperFlags) {
        const key = flag.split(':')[0];
        STATE.rejectionStats.set(key, (STATE.rejectionStats.get(key) || 0) + 1);
      }
    }

    return { ...r, decision, rejectReasons: sniperFlags };
  });
  ...
  const funnelComp        = funnelEdge.filter((c) => (c.comp?.rank || 99) <= 5);
  ...
  // BTC surge Telegram alert suppressed — boost still applied to L1 scores silently
  if (btcSurging) log(`🚀 BTC surge: +${round2(btcCurrent - btcPrior)} pts — L1 boost active`);
  ...
  // ── FSM STATE + OBSERVATION WRITE ───────────────────────
  // Run FSM for every scored candidate above FSM_FLOOR.
  // Write observation for DEVELOPING and CONFIRMED candidates.
  // EARLY candidates logged silently — no Telegram.
  for (const c of withData) {
    if (c.finalScore < FSM_FLOOR) continue;

    const fsmResult = updateCandidateState(c.symbol, c.finalScore, c.comp?.rank);
    const fsmState  = fsmResult.state;

    // Log observation for DEVELOPING and CONFIRMED
    if (fsmState === 'DEVELOPING' || fsmState === 'CONFIRMED') {
      const existing = STATE.candidateStates?.get(c.symbol);
      await safeRun('fsmObservation', () => logObservation(
        c.symbol,
        c.assetType,
        fsmState,
        c.finalScore,
        c.execEngine?.entryPrice || c.execution?.entry || 0,
        {
          momentum:      c.rawScores?.momentum,
          participation: c.rawScores?.participation,
          structure:     c.rawScores?.structure,
          execution:     c.rawScores?.executionQuality,
          risk:          c.rawScores?.risk,
          confidence:    c.confidence?.score,
          conviction:    c.conviction?.score,
          edge:          c.edgeResult?.edge,
          fieldAvg:      STATE.fieldAverage,
          stage:         c.stageInfo?.stage,
          state:         fsmState,
          prevState:     fsmResult.prev,
          reason:        fsmResult.transition,
          persistence:   existing?.persistence || 1,
          rank:          c.comp?.rank,
        }
      ));
    }

    // CONFIRMED → add to hunter_watchlist for SNIPER
    if (fsmState === 'CONFIRMED' && fsmResult.transition === 'DEVELOPING->CONFIRMED') {
      log(`🎯 CONFIRMED: ${c.symbol} (score ${round2(c.finalScore)}, ${STATE.candidateStates?.get(c.symbol)?.persistence || '?'} scans) — adding to hunter_watchlist`);
    }
  }
...
// ── OBSERVATION LOGGER ───────────────────────────────────────
// Full audit-specified schema. Persists every meaningful candidate.
// alert_type: MOVE_ALERT | ACCUMULATION | EARLY | DEVELOPING | CONFIRMED
async function logObservation(symbol, assetType, alertType, score, price, extras = {}) {
  if (!supabase) return;
  try {
    const id     = `obs_${symbol}_${alertType}_${Date.now()}`;
    const sector = SECTOR_MAP[symbol] || null;

    // Score acceleration from STATE history
    const hist = STATE.symbolScoreHistory?.get(symbol) || [];
    // NOTE: score_1cycle_change from in-memory history resets on restart.
    // Values will be 0 after restart until 2+ scans complete.
    // crypto_v1 engine uses Supabase-persisted history instead.
    const score1  = hist.length >= 2 ? round2(score - hist[hist.length - 1]) : null;
    const score3  = hist.length >= 4 ? round2(score - hist[hist.length - 3]) : null;
    const score5  = hist.length >= 6 ? round2(score - hist[hist.length - 5]) : null;
    const accel   = score5 !== null
      ? (score5 >= 10 ? 'surging' : score5 >= 4 ? 'rising' : score5 >= -2 ? 'stable' : score5 >= -6 ? 'fading' : 'falling')
      : (score1 !== null ? (score1 >= 3 ? 'rising' : score1 <= -3 ? 'fading' : 'stable') : null);

    await supabase.from('hunter_observations').insert({
      id,
      symbol,
      asset_type:            assetType,
      sector,
      alert_type:            alertType,
      hunter_score:          score    ? round2(score) : null,
      momentum_score:        extras.momentum    ? round2(extras.momentum)    : null,
      participation_score:   extras.participation ? round2(extras.participation) : null,
      structure_score:       extras.structure   ? round2(extras.structure)   : null,
      execution_score:       extras.execution   ? round2(extras.execution)   : null,
      risk_score:            extras.risk        ? round2(extras.risk)        : null,
      confidence:            extras.confidence  ? round2(extras.confidence)  : null,
      conviction:            extras.conviction  ? round2(extras.conviction)  : null,
      edge_over_field:       extras.edge        ? round2(extras.edge)        : null,
      field_avg:             extras.fieldAvg    ? round2(extras.fieldAvg)    : null,
      score_1cycle_change:   score1,
      score_3cycle_change:   score3,
      score_5cycle_change:   score5,
      score_acceleration:    accel,
      stage:                 extras.stage       || null,
      candidate_state:       extras.state       || alertType,
      previous_state:        extras.prevState   || null,
      state_transition_reason: extras.reason    || null,
      candidate_persistence: extras.persistence || 1,
      rank_in_field:         extras.rank        || null,
      price_at_observation:  price ? round2(price) : null,
      volume_ratio:          extras.volumeRatio ? round2(extras.volumeRatio) : null,
      atr:                   extras.atr         ? round2(extras.atr)         : null,
      volatility_state:      extras.volatilityState || null,
      price_4h_trend:        extras.trend4h     || null,
      price_12h_trend:       extras.trend12h    || null,
      price_acceleration:    extras.priceAccel  || null,
      relative_strength:     extras.rs          ? round2(extras.rs)          : null,
      btc_regime:            STATE.btcRegime,
      qqq_regime:            STATE.qqqRegimeStale ? 'CLOSED' : STATE.qqqRegime,
      market_breadth:        STATE.cryptoBreadth ? round2(STATE.cryptoBreadth) : null,
      observed_at:           new Date().toISOString(),
      outcome_checked:       false,
    });

    // Update score history for this symbol
    if (!STATE.symbolScoreHistory) STATE.symbolScoreHistory = new Map();
    const symHist = STATE.symbolScoreHistory.get(symbol) || [];
    STATE.symbolScoreHistory.set(symbol, [...symHist, score].slice(-6));

    log(`📚 Observation logged: ${symbol} ${alertType} score=${round2(score)} accel=${accel || 'n/a'}`);
  } catch (err) {
    log(`⚠️  Observation log error ${symbol}: ${err.message}`);
  }
}

async function trackPriceMoveCorrelation(symbol, currentPrice, currentScore) {

  // ── WEEKLY ACCUMULATION CHECK ─────────────────────────────
  // Catches gradual compounders like SLB — moves that build over
  // days rather than spiking in a single scan cycle.
  // Checks price 1d, 3d, and 7d ago from momentum_price_tracking.
  if (supabase && currentScore >= 60) {
    try {
      const { data: hist } = await supabase
        .from('momentum_price_tracking')
        .select('price, score, created_at')
        .eq('symbol', symbol)
      ...
      if (hist?.length >= 2) {
        const now     = Date.now();
        const windows = [
          { label: '1d', ms: 86400000,     minMove: 3  },
          { label: '3d', ms: 3 * 86400000, minMove: 5  },
          { label: '7d', ms: 7 * 86400000, minMove: 8  },
        ];

        for (const w of windows) {
          // Find the price closest to N days ago
          const target    = now - w.ms;
          const baseEntry = hist.reduce((best, h) => {
            const diff    = Math.abs(new Date(h.created_at).getTime() - target);
          ...
          const baseScore   = parseFloat(baseEntry.score);
          if (!basePrice || basePrice <= 0) continue;

          const accumMove   = ((currentPrice - basePrice) / basePrice) * 100;

          if (accumMove >= w.minMove) {
            // Genuine accumulating move — check if we already alerted today
            const alertKey = `accum_alert_${symbol}_${w.label}_${new Date().toISOString().slice(0,10)}`;
            let alreadySent = false;
            try {
              const { data: ex } = await supabase.from('hunter_calibration').select('id').eq('id', alertKey).single();
              alreadySent = !!ex;
            } catch { /* not found */ }

            if (!alreadySent && accumMove > 0 && ENV.BLUEJAM_CHAT_ID) {
              ...
              const moveEmoji  = accumMove >= 10 ? '🚀' : accumMove >= 6 ? '📈' : '⬆️';
              const _SLABELS   = {'AI_HW':'AI Hardware','AI_SW':'AI Software','DEFENCE':'Defence','SPACE':'Space','CRYPTO_DEFI':'Crypto DeFi','CRYPTO_L1':'Crypto L1','CRYPTO_L2':'Crypto L2','CRYPTO_AI':'Crypto AI','POWER':'Power & Energy','ENERGY':'Energy','HEALTH':'Healthcare','GROWTH':'Consumer Growth','LSE_MINING':'LSE Mining','LSE_BANK':'LSE Banking'};
              const sector     = SECTOR_MAP[symbol];
              const sectorLine = sector ? `Sector: ${_SLABELS[sector] || sector.replace(/_/g,' ')}` : '';
              const DISCLAIMER = '⚠️ BareTradeSignals provides educational market analysis only. Not financial advice. Not a recommendation to buy or sell. Always do your own research. Capital at risk.';

              let regimeLine;
              if (assetType === 'crypto') {
                regimeLine = STATE.btcRegime === 'BULL' ? '✅ BTC Uptrend' : STATE.btcRegime === 'BEAR' ? '🔴 BTC Downtrend' : '🟡 BTC Ranging';
              } else {
                const qqqOk = STATE.qqqRegime === 'BULL' && !STATE.qqqRegimeStale;
                regimeLine  = qqqOk ? '✅ QQQ Uptrend' : STATE.qqqRegimeStale ? '🟡 QQQ Closed' : '🟡 QQQ Ranging';
              }
              ...
              const msg = [
                ...
                `Move: +${round2(accumMove)}% over ${w.label} (gradual accumulation)`,
                ``,
                `From $${round2(basePrice)} → $${round2(currentPrice)}`,
                regimeLine,
                sectorLine,
                ``,
                `This move built steadily over ${w.label} — not a one-day spike.`,
                `——————————`,
                DISCLAIMER,
                `📸 @baretradesignals 🏹`,
              ].filter(Boolean).join('\n');
              ...
              log(`📲 Accumulation alert sent: ${symbol} +${round2(accumMove)}% over ${w.label}`);
              await logObservation(symbol, assetType, 'ACCUMULATION', currentScore, currentPrice, { reason: `+${round2(accumMove)}% over ${w.label}` });

              await supabase.from('hunter_calibration').upsert({
                id: alertKey, recorded_at: new Date().toISOString(),
                top_score: round2(currentScore), top_symbol: symbol,
              ...
              break; // only fire the longest window that qualifies per day
            }
          }
        }
      }
    } catch (accErr) {
      log(`⚠️  Accumulation check error for ${symbol}: ${accErr.message}`);
    }
  }
  // ── END WEEKLY ACCUMULATION CHECK ────────────────────────

  const prior = priceSnapshots.get(symbol);
  if (prior && prior.price > 0) {
    const movePct = ((currentPrice - prior.price) / prior.price) * 100;
    const absMove = Math.abs(movePct);
    if (absMove >= 3) {  // minimum 3% — below this is noise, not signal
      const tier = absMove >= 8 ? 8 : absMove >= 5 ? 5 : 3;
      ...
      if (prior.score >= 65 && movePct > 0 && ENV.BLUEJAM_CHAT_ID) {
        ...
        if (!alreadySent) {
          ...
          const flag       = assetType === 'crypto' ? '🪙' : assetType === 'lse' ? '🇬🇧' : '🇺🇸';
          const label      = symbol.replace('USDT','').replace('.L','');
          const moveEmoji  = absMove >= 8 ? '🚀' : absMove >= 5 ? '📈' : absMove >= 3 ? '⬆️' : '↗️';
          const _SLABELS   = {
            'AI_HW': 'AI Hardware', 'AI_SW': 'AI Software',
            'DEFENCE': 'Defence', 'SPACE': 'Space',
            'CRYPTO_INFRA': 'Crypto Infrastructure', 'CRYPTO_DEFI': 'Crypto DeFi',
            'CRYPTO_L1': 'Crypto L1', 'CRYPTO_L2': 'Crypto L2',
            'CRYPTO_AI': 'Crypto AI', 'CRYPTO_RWA': 'Crypto RWA',
            'CRYPTO_GAMING': 'Crypto Gaming', 'POWER': 'Power & Energy',
            'ENERGY': 'Energy', 'HEALTH': 'Healthcare',
            'GROWTH': 'Consumer Growth', 'LSE_MINING': 'LSE Mining',
            'LSE_BANK': 'LSE Banking',
          };
          const sector     = SECTOR_MAP[symbol];
          const sectorLine = sector ? `Sector: ${_SLABELS[sector] || sector.replace(/_/g,' ')}` : '';
          // Show relevant regime per asset type
          let regimeLine;
          if (assetType === 'crypto') {
            regimeLine = STATE.btcRegime === 'BULL'    ? '✅ BTC Uptrend — conditions favour this move'
                       : STATE.btcRegime === 'BEAR'    ? '🔴 BTC Downtrend — conditions unfavourable'
                       : '🟡 BTC Ranging';
          } else {
            const qqqOk = STATE.qqqRegime === 'BULL' && !STATE.qqqRegimeStale;
            regimeLine  = qqqOk                        ? '✅ QQQ Uptrend — conditions favour this move'
                       : STATE.qqqRegime === 'BEAR'    ? '🔴 QQQ Downtrend — conditions unfavourable'
                       : STATE.qqqRegimeStale          ? '🟡 QQQ Closed'
                       : '🟡 QQQ Ranging';
          }
          ...
          const msg = [
            ...
            `Move: +${round2(movePct)}% (${absMove >= 8 ? 'major' : absMove >= 5 ? 'significant' : absMove >= 3 ? 'notable' : 'minor'})`,
            ``,
          ...
          await logObservation(symbol, assetType, 'MOVE_ALERT', prior.score, currentPrice, { reason: `+${round2(movePct)}% single scan` });
...
function buildHunterAlert(candidate, scan) {
  ...
  const sector      = SECTOR_MAP[symbol] || null;
  const sectorLine  = sector ? `Sector: ${_SLABELS2[sector] || sector.replace(/_/g,' ')}` : '';
...
async function fireCombinedBrief(session) {
  ...
  // Show last known QQQ regime even when market closed — ⚪ alone is misleading
  const _btcIcon = STATE.btcRegime === 'BULL' ? '🟢' : STATE.btcRegime === 'BEAR' ? '🔴' : '🟡';
  const _qqqIcon = STATE.qqqRegime === 'BULL'  ? '🟢' : STATE.qqqRegime === 'BEAR'  ? '🔴' : '🟡';
  const _qqqSfx  = STATE.qqqRegimeStale ? ' (closed)' : '';
  const regimeStr = `BTC ${_btcIcon} · QQQ ${_qqqIcon}${_qqqSfx}`;
...
// Implements EARLY -> DEVELOPING -> CONFIRMED progression.
// Hunter discovers. SNIPER validates. Do not conflate the two.
//
// EARLY:      First scan above floor (score ≥ 55). Something unusual.
// DEVELOPING: 3+ scans above floor, score rising or stable.
// CONFIRMED:  5+ scans, score ≥ 65, persistence proven.
//             Candidate handed to SNIPER via hunter_watchlist/top50.
//
// State persists in STATE.candidateStates (in-memory, reset on restart).
// Transitions logged to hunter_observations.

const FSM_FLOOR         = 55;   // minimum score to enter EARLY
const FSM_DEVELOPING    = 3;    // scans above floor to become DEVELOPING
const FSM_CONFIRMED     = 5;    // scans above floor to become CONFIRMED
const FSM_CONFIRM_SCORE = 65;   // score must reach this for CONFIRMED
const FSM_DROP_SCANS    = 2;    // consecutive scans below floor to exit

function updateCandidateState(symbol, currentScore, rank) {
  if (!STATE.candidateStates) STATE.candidateStates = new Map();

  const existing = STATE.candidateStates.get(symbol);
  const now      = Date.now();

  // Below floor — mark for removal
  if (currentScore < FSM_FLOOR) {
    if (existing) {
      existing.scansBelowFloor = (existing.scansBelowFloor || 0) + 1;
      if (existing.scansBelowFloor >= FSM_DROP_SCANS) {
        STATE.candidateStates.delete(symbol);
        return { state: null, transition: 'DROPPED', prev: existing.state };
      }
    }
  ...
  if (!existing) {
    // First time above floor — EARLY
    STATE.candidateStates.set(symbol, {
      state:          'EARLY',
      persistence:    1,
      firstSeen:      now,
      lastSeen:       now,
    ...
    return { state: 'EARLY', transition: 'NEW', prev: null };
  }

  // Update existing
  existing.persistence++;
  existing.lastSeen    = now;
  existing.peakScore   = Math.max(existing.peakScore, currentScore);
  existing.scansBelowFloor = 0;

  const prev = existing.state;

  // State transitions
  if (existing.state === 'EARLY' && existing.persistence >= FSM_DEVELOPING) {
    existing.state = 'DEVELOPING';
    return { state: 'DEVELOPING', transition: 'EARLY->DEVELOPING', prev };
  }
  if (existing.state === 'DEVELOPING' && existing.persistence >= FSM_CONFIRMED && currentScore >= FSM_CONFIRM_SCORE) {
    existing.state = 'CONFIRMED';
    return { state: 'CONFIRMED', transition: 'DEVELOPING->CONFIRMED', prev };
  }

  return { state: existing.state, transition: null, prev };
}


// ── MOMENTUM EVENT LANE ───────────────────────────────────────
// Assets with abnormal price moves (≥5% in a single scan or
// ≥10% accumulation) deserve a separate fast lane.
// They bypass the full scoring funnel — structure and edge
// may not be perfect yet at the start of a major move.
// This is explicitly separate from the OPEN/WATCH pipeline.
async function checkMomentumEvents(withData, scan) {
  if (!ENV.BLUEJAM_CHAT_ID) return;

  const MOMENTUM_SCORE_MIN = 55;   // lower bar — move is the signal
  const MOVE_THRESHOLD_5M  = 5;    // % in single scan
  const DISCLAIMER = '⚠️ BareTradeSignals provides educational market analysis only. Not financial advice. Not a recommendation to buy or sell. Always do your own research. Capital at risk.';

  for (const c of withData) {
    if (c.finalScore < MOMENTUM_SCORE_MIN) continue;

    // Check recent price move from momentum_price_tracking
    const prior = priceSnapshots.get(c.symbol);
    if (!prior || !prior.price) continue;

    const movePct  = ((c.execEngine?.entryPrice || 0) - prior.price) / prior.price * 100;
    if (Math.abs(movePct) < MOVE_THRESHOLD_5M) continue;
    if (movePct <= 0) continue; // positive moves only

    // Fire momentum event alert
    const alertKey = `momentum_event_${c.symbol}_${new Date().toISOString().slice(0,10)}`;
    let alreadySent = false;
    try {
      if (supabase) {
        const { data: ex } = await supabase.from('hunter_calibration').select('id').eq('id', alertKey).single();
    ...
    const assetType = c.assetType || 'us';
    const flag      = assetType === 'crypto' ? '🪙' : assetType === 'lse' ? '🇬🇧' : '🇺🇸';
    const label     = c.symbol.replace('USDT','').replace('.L','');
    const _SLABELS  = {'AI_HW':'AI Hardware','AI_SW':'AI Software','DEFENCE':'Defence','SPACE':'Space','CRYPTO_DEFI':'Crypto DeFi','CRYPTO_L1':'Crypto L1','CRYPTO_L2':'Crypto L2','CRYPTO_AI':'Crypto AI','POWER':'Power & Energy','ENERGY':'Energy','HEALTH':'Healthcare','GROWTH':'Consumer Growth','LSE_MINING':'LSE Mining','LSE_BANK':'LSE Banking'};
    const sector    = SECTOR_MAP[c.symbol];
    const sectorLine = sector ? `Sector: ${_SLABELS[sector] || sector.replace(/_/g,' ')}` : '';

    const msg = [
      `🚨 MOMENTUM EVENT — ${flag} ${label}`,
      ``,
      `Hunter score: ${round2(c.finalScore)}`,
      `Move detected: +${round2(movePct)}% in current scan`,
      ``,
      `This is a large abnormal move — not a standard scored signal.`,
      sectorLine,
      ``,
      `——————————`,
      DISCLAIMER,
    ...
    await sendTelegram(msg, ENV.BLUEJAM_CHAT_ID);
    log(`🚨 Momentum event: ${c.symbol} +${round2(movePct)}%`);

    if (supabase) {
      try {
        await supabase.from('hunter_calibration').upsert({
          id: alertKey, recorded_at: new Date().toISOString(),
          top_score: round2(c.finalScore), top_symbol: c.symbol,
          field_avg: 0, candidate_count: 0,
          btc_regime: STATE.btcRegime, qqq_regime: STATE.qqqRegime,
          market_open_count: 0, buy_count: 0, watch_count: 0,
        });
        await logObservation(c.symbol, assetType, 'MOMENTUM_EVENT', c.finalScore,
          c.execEngine?.entryPrice || 0, { reason: `+${round2(movePct)}% abnormal move` });
      } catch { /* best effort */ }
    }
  }
}
...
async function fireAlert(candidate, scan) {
  ...
  if (execPlan && execPlan.entryPrice && execPlan.stopPrice) {
    if (execPlan.stopPrice >= execPlan.entryPrice) {
      const atype2  = candidate.assetType || 'us';
      const atrPct2 = atype2 === 'crypto' ? 0.05 : 0.03;
      execPlan = { ...execPlan, stopPrice: round2(execPlan.entryPrice * (1 - atrPct2)) };
      log(`⚠️  execPlan stop inverted for ${candidate.symbol} — ATR fallback: ${execPlan.stopPrice}`);
    }
  ...
  if (execOld && execPlan?.entryPrice && execOld.stopPrice != null) {
    if (execOld.stopPrice >= execPlan.entryPrice) {
      // Alpaca 403s cause bad candle data → inverted stops
      // Use ATR fallback: 5% below entry for crypto, 3% for stocks
      const sym        = candidate.symbol;
      const atype      = candidate.assetType || 'us';
      const atrPct     = atype === 'crypto' ? 0.05 : 0.03;
      const fallback   = round2(execPlan.entryPrice * (1 - atrPct));
      log(`⚠️  Stop inverted for ${sym} (${execOld.stopPrice} >= ${execPlan.entryPrice}) — using ATR fallback: ${fallback}`);
      execOld = { ...execOld, stopPrice: fallback };
    }
  }
...
const WATCHLIST_STALE_MS           = 12 * 60 * 60 * 1000; // 12h — was 4h, ARB was dropped too early
...
async function sendWatchlistCreated(candidate, stage) {
  log(`👀 Watchlist CREATED: ${candidate.symbol} @ ${round2(candidate.finalScore)}`);
  // Persist to Supabase so the discovery chain isn't lost
  if (supabase) {
    try {
      await supabase.from('portfolio_watchlist').upsert({
        symbol:       candidate.symbol,
        asset_type:   candidate.assetType || 'unknown',
        last_score:   round2(candidate.finalScore),
        last_seen:    new Date().toISOString(),
        watch_reason: `${stage || 'WATCH'} — score ${round2(candidate.finalScore)} | flags: ${(candidate.sniperFlags||[]).join(', ') || 'none'}`,
      }, { onConflict: 'symbol' });
    } catch (e) { log(`⚠️ Watchlist write error: ${e.message}`); }
  }
  // Also log full observation for learning loop
  await logObservation(candidate.symbol, candidate.assetType, 'WATCHLIST_CREATED',
    candidate.finalScore, candidate.execEngine?.entryPrice || 0,
    { stage, reason: `Watchlist entry at score ${round2(candidate.finalScore)}` }
  );
}

async function sendWatchlistUpdate(symbol, candidate, state) {
  const delta = round2(candidate.finalScore - state.lastAlertScore);
  log(`⬆️ Watchlist UPDATE: ${symbol} ${round2(state.lastAlertScore)}→${round2(candidate.finalScore)} (${delta >= 0 ? '+' : ''}${delta})`);
  if (supabase) {
    try {
      await supabase.from('portfolio_watchlist').upsert({
        symbol:       symbol,
        ...
        last_score:   round2(candidate.finalScore),
        last_seen:    new Date().toISOString(),
        watch_reason: `UPDATE — score ${round2(state.lastAlertScore)}→${round2(candidate.finalScore)} (${delta >= 0 ? '+' : ''}${delta})`,
      }, { onConflict: 'symbol' });
    } catch (e) { log(`⚠️ Watchlist update error: ${e.message}`); }
  }
}

async function sendWatchlistRemoved(symbol, candidate, state) {
  const currentScore = candidate?.finalScore ?? state.score;
  log(`❌ Watchlist REMOVED: ${symbol} peak=${round2(state.peakScore)} now=${round2(currentScore)}`);
  if (supabase) {
    try {
      await supabase.from('portfolio_watchlist').delete().eq('symbol', symbol);
    } catch (e) { log(`⚠️ Watchlist remove error: ${e.message}`); }
  }
}
...
async function fireLeaderboardMessage(scan) {
  ...
  const qqqWord = STATE.qqqRegimeStale ? `${STATE.qqqRegime === 'BULL' ? '🟢' : STATE.qqqRegime === 'BEAR' ? '🔴' : '🟡'} ${STATE.qqqRegime} (closed)`
  ...
  if (supabase && allScored.length > 0) {
    try {
      ...
      // ── HUNTER WATCHLIST — shared with SNIPER ───────────────
      // Write all assets scoring 60+ so SNIPER can filter its
      // universe to only momentum-confirmed assets.
      // SNIPER should ignore entries older than 30 minutes.
      try {
        const watchRows = rows
          .filter(r => parseFloat(r.score) >= 60)
          .map(r => {
            // Determine score trend from recent history
            const sym      = r.symbol;
            const history  = STATE.btcScoreHistory || [];
            const trend    = r.score >= 65 ? 'rising' : r.score >= 60 ? 'stable' : 'falling';
            return {
              symbol:        sym,
              asset_type:    r.asset_type,
              hunter_score:  parseFloat(r.score),
              score_trend:   trend,
              btc_regime:    STATE.btcRegime,
              qqq_regime:    STATE.qqqRegimeStale ? 'CLOSED' : STATE.qqqRegime,
              last_updated:  snapshotTime,
            };
          });

        if (watchRows.length > 0) {
          await supabase.from('hunter_watchlist').upsert(watchRows, { onConflict: 'symbol' });
          log(`👁 Watchlist updated: ${watchRows.length} assets (score ≥ 60) available for SNIPER`);
        }

        // Remove assets that have dropped below 55 — clean stale entries
        const activeSymbols = rows.filter(r => parseFloat(r.score) >= 55).map(r => r.symbol);
        if (activeSymbols.length > 0) {
          await supabase.from('hunter_watchlist')
            .delete()
            .not('symbol', 'in', `(${activeSymbols.map(s => `'${s}'`).join(',')})`);
        }
      } catch (wErr) {
        log(`⚠️  Watchlist write error: ${wErr.message}`);
      }

      // ── HUNTER TOP 50 — for SNIPER consumption ───────────────
      // Top 50 assets by current score across all markets.
      // SNIPER reads this table and boosts its own score for any
      // asset that appears here. Agreement between both bots = signal.
      try {
        const now7d    = new Date(Date.now() - 7 * 86400000).toISOString();

        // Build 7-day avg and top-5 appearances per symbol
        const { data: hist7d } = await supabase
          .from('hunter_leaderboard_history')
          .select('symbol, score, rank')
          .gte('snapshot_time', now7d);

        const hist7dMap = new Map();
        if (hist7d) {
          for (const h of hist7d) {
            if (!hist7dMap.has(h.symbol)) hist7dMap.set(h.symbol, { scores: [], top5: 0 });
            const d = hist7dMap.get(h.symbol);
            d.scores.push(parseFloat(h.score));
            if (h.rank <= 5) d.top5++;
          }
        }

        // Rank all assets in this snapshot by score, take top 50
        const top50rows = [...rows]
          .sort((a, b) => parseFloat(b.score) - parseFloat(a.score))
          .slice(0, 50)
          .map((r, i) => {
            const h      = hist7dMap.get(r.symbol) || { scores: [parseFloat(r.score)], top5: 0 };
            const avg7d  = round2(h.scores.reduce((a,b)=>a+b,0) / h.scores.length);
            const trend  = parseFloat(r.score) >= avg7d + 2 ? 'rising'
                         : parseFloat(r.score) <= avg7d - 2 ? 'falling' : 'stable';
            return {
              symbol:               r.symbol,
              asset_type:           r.asset_type,
              hunter_score:         parseFloat(r.score),
              ...
              score_trend:          trend,
              avg_score_7d:         avg7d,
              top5_appearances_7d:  h.top5,
              btc_regime:           STATE.btcRegime,
              qqq_regime:           STATE.qqqRegimeStale ? 'CLOSED' : STATE.qqqRegime,
              sector:               SECTOR_MAP[r.symbol] || null,
              updated_at:           snapshotTime,
            };
          });

        // Wipe and replace — always reflects current state
        await supabase.from('hunter_top50').delete().neq('symbol', '___never___');
        await supabase.from('hunter_top50').insert(top50rows);
        log(`🏆 Top 50 written for SNIPER: ${top50rows[0]?.symbol} [${top50rows[0]?.hunter_score}] → ${top50rows[49]?.symbol} [${top50rows[49]?.hunter_score}]`);
      } catch (t50Err) {
        log(`⚠️  Top50 write error: ${t50Err.message}`);
      }
      // ─────────────────────────────────────────────────────────
    } catch (err) {
...
async function fireWeeklyLeagueTable(limit = 10) {
  if (!supabase) return;
  log(`📅 Building league table (top ${limit})...`);
  ...
  try {
    ...
    // Top 10 this week
    const top10 = thisWeek.slice(0, limit);
    ...
    const lines = [
      ...
      `${dateStr}  ·  All markets combined  ·  Top ${limit}`,
      ``,
    ...
    top10.forEach((e, i) => {
      ...
      // Add percentile context to league table row
      const pct = calcPercentile(e.avg, e.assetType, thisWeek);
      const pctStr = pct !== null ? ` (${pct}th)` : '';
      lines.push(`${pos} ${flag} ${label} ${score} ${move}${pctStr}`);
    });
...
async function fireHunterWeeklyReport() {
  ...
  const dateStr = new Date().toISOString().slice(0, 10);
  const btcStr  = STATE.btcRegime === 'BULL' ? '🟢 Uptrend' : STATE.btcRegime === 'BEAR' ? '🔴 Downtrend' : '🟡 Ranging';
  const _qqqI2  = STATE.qqqRegime === 'BULL' ? '🟢' : STATE.qqqRegime === 'BEAR' ? '🔴' : '🟡';
  const qqqStr  = `${_qqqI2} ${STATE.qqqRegime}${STATE.qqqRegimeStale ? ' (closed)' : ''}`;
...
// ── OBSERVATION OUTCOME CHECKER ──────────────────────────────
// Runs every scan cycle. Checks price 24h and 48h after each
// observation was logged. Builds the learning loop.
async function checkObservationOutcomes() {
  if (!supabase) return;
  try {
    const now     = new Date();
    const ago24h  = new Date(Date.now() - 24 * 3600000).toISOString();
    const ago48h  = new Date(Date.now() - 48 * 3600000).toISOString();
    const ago72h  = new Date(Date.now() - 72 * 3600000).toISOString();

    // Find observations older than 24h that haven't been fully checked
    const { data: pending } = await supabase
      .from('hunter_observations')
      .select('*')
      .eq('outcome_checked', false)
      .lt('alerted_at', ago24h)
      .limit(10);

    if (!pending?.length) return;

    for (const obs of pending) {
      const assetType = obs.asset_type;
      let currentPrice = null;

      try {
        const candles = await fetchDailyCandles(obs.symbol, assetType);
        if (candles?.length) currentPrice = candles[candles.length - 1].close;
      } catch { /* best effort */ }

      if (!currentPrice || !obs.price_at_alert) continue;

      const outcomePct = round2((currentPrice - obs.price_at_alert) / obs.price_at_alert * 100);
      const alertAge   = (Date.now() - new Date(obs.alerted_at).getTime()) / 3600000;

      const update = {
        outcome_pct:     outcomePct,
        outcome_checked: alertAge >= 48,
      };

      // Fill outcome columns using new schema
      if (alertAge >= 24) {
        update.hit_0_5pct = outcomePct >= 0.5;
        update.hit_1pct   = outcomePct >= 1.0;
        update.hit_2pct   = outcomePct >= 2.0;
        update.hit_3pct   = outcomePct >= 3.0;
        update.mfe        = round2(Math.max(outcomePct, 0));
        update.mae        = round2(Math.min(outcomePct, 0));
      }
      if (alertAge >= 48) {
        update.eventual_outcome   = outcomePct >= 3 ? 'WIN_STRONG' : outcomePct >= 1 ? 'WIN' : outcomePct >= -1 ? 'FLAT' : outcomePct >= -3 ? 'LOSS' : 'LOSS_STRONG';
        update.outcome_horizon    = 48;
        update.outcome_checked_at = new Date().toISOString();
      }

      await supabase.from('hunter_observations')
        .update(update)
        .eq('id', obs.id);

      log(`📊 Outcome checked: ${obs.symbol} ${obs.alert_type} → ${outcomePct >= 0 ? '+' : ''}${outcomePct}% (${Math.round(alertAge)}h after alert)`);
    }
  } catch (err) {
    log(`⚠️  Outcome check error: ${err.message}`);
  }
}
  ...
  try {
    ...
    if (buys.length > 0) {
      ...
      const sectorMsg = [
            ...
            // Cluster alert suppressed — logged only
            log(`🔍 Cluster detected: ${risk.sector} (${risk.count} assets) — alert suppressed`);
          }
        }
    ...
    // Daily leaderboard messages
    {
      ...
      const _day = new Date().getUTCDay(); // 0=Sun, 6=Sat — defined here for weekend guards below
       ...
       // Daily league table — 22:00 UTC (30 mins after evening brief)
       // Shows combined top 10 with movement arrows, every day
       if (_dh === 22 && _dm < 5) {
         const _lk = `daily_league_${_dd}`;
         let _lsent = false;
         if (supabase) { try { const {data:_d} = await supabase.from('hunter_calibration').select('id').eq('id',_lk).single(); _lsent=!!_d; } catch {} }
         if (!_lsent) {
           await fireWeeklyLeagueTable();
           if (supabase) { try { await supabase.from('hunter_calibration').upsert({id:_lk,recorded_at:new Date().toISOString(),top_score:0,top_symbol:'DAILY_LEAGUE',field_avg:0,candidate_count:0,btc_regime:STATE.btcRegime,qqq_regime:STATE.qqqRegime,market_open_count:0,buy_count:0,watch_count:0}); } catch {} }
         }
       }
    }

    // Momentum event check — separate fast lane for abnormal moves
    await safeRun('momentumEvents', () => checkMomentumEvents(withData, scan));

    // ── LEADERBOARD WRITE — every scan, unconditionally ─────────
    // Previously only wrote when no buys were found (4h cooldown).
    // That meant ARB at 68-71 was never captured — the peak was invisible.
    // Now writes every scan. Telegram is suppressed inside fireLeaderboardMessage.
    ...
    // Observation outcome checker — builds the learning loop
    if (STATE.cycleCount % 4 === 0) {
      await checkObservationOutcomes();
    }

    // ── CRYPTO ENGINE (shadow mode, A/B comparison) ───────
    // Runs alongside existing Hunter. Outputs to hunter_observations
    // with engine_version='crypto_v1'. Does not affect existing logic.
    // Compare: old hunter_score vs crypto_v1 state machine outcomes.
    if (cryptoEngine && STATE.cycleCount % 2 === 0) {
      try {
        // Pass current Hunter scores so crypto engine can log them alongside new features
        const existingScores = {};
        if (withData?.length) {
          for (const c of withData) {
            if (c.assetType === 'crypto') existingScores[c.symbol] = c.finalScore;
          }
        }
        const cryptoResult = await cryptoEngine.runCryptoScan(supabase, existingScores);
        if (cryptoResult.stateChanges > 0 && ENV.BLUEJAM_CHAT_ID) {
          // Send Telegram alerts for meaningful state transitions
          for (const [symbol, state] of Object.entries(cryptoResult.states || {})) {
            const symState = cryptoEngine.CRYPTO_STATE.symbolStates.get(symbol);
            if (symState?.transition && ['AWAKENING','PRESSURE','BREAKOUT'].includes(state)) {
              await cryptoEngine.sendCryptoStateAlert(
                symbol, symState.prevState, state, symState.features || {},
                sendTelegram, ENV.BLUEJAM_CHAT_ID
              );
            }
          }
        }
        if (cryptoResult.symbolsScanned > 0) {
          log(`[CRYPTO] Scan: ${cryptoResult.symbolsScanned} symbols, ${cryptoResult.observationsWritten} obs, ${cryptoResult.stateChanges} state changes`);
        }
      } catch (cryptoErr) {
        log(`[CRYPTO] Engine error: ${cryptoErr.message}`);
      }
    }

    // Crypto outcome checker (separate from legacy checker)
    if (cryptoEngine && STATE.cycleCount % 8 === 0) {
      cryptoEngine.checkCryptoOutcomes(supabase).catch(e =>
        log(`[CRYPTO] Outcome check error: ${e.message}`)
      );
    }
    ...
    // Railway restarts or multiple instances don't double-fire.
    {
      ...
      if (_day === 5 && _hour >= 11 && _hour < 13) { // 11:00-13:00 UTC = 12:00-14:00 BST (UK Friday lunchtime)
        ...
        if (!_alreadySent) {
          log('📅 Friday window — firing top 50 league table');
          await fireWeeklyLeagueTable(50);
          if (supabase) {
            try {
              await supabase.from('hunter_calibration').upsert({
                id: _weekKey,
