 ...
 *
 * Adds actual SL/TP price levels to the OPEN alert, not just the
 * percentages — the levels were already computed and stored
 * (stop_loss_price/take_profit_price), just not shown.
 *
 * New formatPrice() scales decimal places to the coin's
 * magnitude (2dp for >=100, 4dp for >=1, 6dp for >=0.01, 8dp for
 * >=0.0001, 10dp below that) and trims trailing zeros — needed
 * because Hunter's universe spans sub-cent coins through
 * high-value ones, and a single fixed decimal count would round
 * small coins to something useless. Verified against real
 * examples (CELR ~0.0029, SKY ~0.068, HBAR ~0.087, a BTC-like
 * 43521.7, and a sub-satoshi 0.00000521) before shipping — all
 * came out clean.
 *
 * New OPEN message:
 *   Entry: {price}
