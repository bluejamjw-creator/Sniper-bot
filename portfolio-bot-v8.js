    ...
    if (absMove >= 2) {
      ...
      if (prior.score >= 65 && movePct > 0 && ENV.BLUEJAM_CHAT_ID) {
        ...
        if (!alreadySent) {
          ...
          const msg = [
            `${moveEmoji} MOVE DETECTED — ${flag} ${label}`,
            ``,
            ...
            sectorLine,
            ``,
            `Score 65+ before a move is Hunter's confirmed edge.`,
            `——————————`,
            `⚠️ Educational market analysis only · Not personalised investment advice`,
            `📸 @baretradesignals 🏹`,
          ].filter(Boolean).join('\n');

          await sendTelegram(msg, ENV.BLUEJAM_CHAT_ID);
          log(`📲 Move alert sent: ${symbol} +${round2(movePct)}% (score was ${round2(prior.score)})`);
...

      {
        ...
        for (const risk of sectorRisks) {
          ...
          if (!STATE.sectorRiskSent.has(riskKey)) {
            ...
            const sectorMsg = [
              `⚠️ SECTOR CONCENTRATION — ${risk.sector.replace('_',' ')}`,
              ``,
              `${risk.count} assets in this sector are scoring 65+ simultaneously.`,
              `Assets: ${risk.assets.join(', ')}`,
              ``,
              `This may indicate a sector-wide event (earnings, macro, rotation).`,
              `Be cautious about taking multiple positions in the same sector.`,
              ``,
              `——————————`,
              `📸 @baretradesignals 🏹`,
            ].join('\n');
