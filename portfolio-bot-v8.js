...
async function runFullScan() {
  ...
  // ── BTC SURGE ALERT ──────────────────────────────────────
  // Fire once per surge event — not per scan cycle
  if (btcSurging && ENV.BLUEJAM_CHAT_ID) {
    const surgeKey     = `btc_surge_${new Date().toISOString().slice(0,13)}`; // once per hour max
    const lastSurge    = STATE.btcLastSurgeAlert || 0;
    const hoursSinceLast = (Date.now() - lastSurge) / 3600000;

    if (hoursSinceLast >= 1) {
      STATE.btcLastSurgeAlert = Date.now();
      const btcPrice = withData.find(c => c.symbol === 'BTCUSDT')?.price || 0;
      const l1s      = withData
        .filter(c => L1_SECTORS.has(SECTOR_MAP[c.symbol]) && c.symbol !== 'BTCUSDT')
        .sort((a,b) => b.finalScore - a.finalScore)
        .slice(0, 5)
        .map(c => `${c.symbol.replace('USDT','')} [${round2(c.finalScore)}]`)
        .join(' · ');

      const surgeMsg = [
        `🚀 BTC MOMENTUM SURGE`,
        ``,
        `BTC score jumped +${round2(btcCurrent - btcPrior)} points this scan.`,
        `BTC: $${round2(btcPrice)}`,
        ``,
        `L1 crypto typically follows BTC momentum within 1-4 hours.`,
        `Watch:`,
        l1s,
        ``,
        `This is an observation — not a signal to act.`,
        `——————————`,
      ...
      await sendTelegram(surgeMsg, ENV.BLUEJAM_CHAT_ID);
      log(`📲 BTC surge alert sent`);
    }
  }
...
// ============================================================
// WEEKLY PICK — One crypto, one stock, every Friday.
// Fires after the league table in the Friday 11:00-13:00 window.
// Stores the pick in hunter_weekly_picks.
// Next Friday: reviews last week's pick before publishing new one.
// ============================================================

async function fireWeeklyPick() {
  if (!supabase) { log('⚠️  Weekly pick skipped — no Supabase'); return; }
  log('🎯 Building weekly pick...');

  try {
    const now      = new Date();
    const dateStr  = now.toISOString().slice(0, 10);
    const weekKey  = `pick_${now.getUTCFullYear()}_W${String(now.getUTCMonth()+1).padStart(2,'0')}_${String(now.getUTCDate()).padStart(2,'0')}`;

    // ── Already sent this week? ──────────────────────────────
    const { data: existing } = await supabase
      .from('hunter_weekly_picks')
      .select('id')
      .eq('week_key', weekKey)
      .single()
      .catch(() => ({ data: null }));
    ...
    if (existing) {
      log('🎯 Weekly pick already sent this week — skipping');
      return;
    }

    // ── Step 1: Pull top assets from leaderboard this week ───
    const weekStart = new Date(Date.now() - 7 * 86400000).toISOString();
    const { data: rows, error } = await supabase
      .from('hunter_leaderboard_history')
      .select('symbol, asset_type, score, snapshot_time')
      .gte('snapshot_time', weekStart)
      .order('snapshot_time', { ascending: false });

    if (error || !rows?.length) {
      log('⚠️  Weekly pick: no leaderboard data');
      return;
    }

    // Build avg score per symbol
    const scoreMap = new Map();
    for (const row of rows) {
      if (!scoreMap.has(row.symbol)) {
        scoreMap.set(row.symbol, { symbol: row.symbol, assetType: row.asset_type, scores: [] });
      }
      scoreMap.get(row.symbol).scores.push(parseFloat(row.score));
    }

    const ranked = [];
    for (const [, d] of scoreMap.entries()) {
      if (d.scores.length < 2) continue;
      const avg = round2(d.scores.reduce((a, b) => a + b, 0) / d.scores.length);
      ranked.push({ symbol: d.symbol, assetType: d.assetType, avg });
    }
    ranked.sort((a, b) => b.avg - a.avg);

    // Cross-ref with moves table — bonus for confirmed moves this week
    const { data: moveRows } = await supabase
      .from('hunter_moves')
      .select('symbol, move_pct, score_before_move')
      .gte('recorded_at', weekStart)
      .order('move_pct', { ascending: false })
      .catch(() => ({ data: [] }));

    const moveBonus = new Map();
    for (const m of (moveRows || [])) {
      const pct = parseFloat(m.move_pct);
      if (pct > 3) moveBonus.set(m.symbol, (moveBonus.get(m.symbol) || 0) + pct);
    }

    // Final score = leaderboard avg + 20% of move bonus
    function finalScore(entry) {
      const bonus = moveBonus.get(entry.symbol) || 0;
      return round2(entry.avg + bonus * 0.2);
    }

    // Separate crypto and stocks
    const topCrypto = ranked
      .filter(e => e.assetType === 'crypto')
      .sort((a, b) => finalScore(b) - finalScore(a))[0];

    const topStock = ranked
      .filter(e => e.assetType === 'us' || e.assetType === 'lse')
      .sort((a, b) => finalScore(b) - finalScore(a))[0];

    if (!topCrypto && !topStock) {
      log('⚠️  Weekly pick: no candidates found');
      return;
    }

    // ── Step 2: Get live entry prices ────────────────────────
    let cryptoEntryPrice = null;
    let stockEntryPrice  = null;

    if (topCrypto) {
      try {
        const coinId = topCrypto.symbol.replace('USDT','').toLowerCase();
        const cg = await fetchCoinGeckoPrice(coinId);
        cryptoEntryPrice = cg?.usd || null;
      } catch { /* best effort */ }
    }

    if (topStock) {
      try {
        const q = await fetchFinnhubQuote(topStock.symbol.replace('.L',''));
        stockEntryPrice = q?.price || null;
      } catch { /* best effort */ }
    }

    // ── Step 3: Review last week's pick ──────────────────────
    ...
    let prevCryptoOutcome = null;
    let prevCryptoPnl     = null;
    let prevStockOutcome  = null;
    let prevStockPnl      = null;

    if (lastPick) {
      // Crypto review
      if (lastPick.crypto_symbol && lastPick.crypto_entry_price) {
        try {
          const coinId = lastPick.crypto_symbol.replace('USDT','').toLowerCase();
          const cg = await fetchCoinGeckoPrice(coinId);
          if (cg?.usd) {
            prevCryptoPnl = round2(((cg.usd - lastPick.crypto_entry_price) / lastPick.crypto_entry_price) * 100);
            const stillRanked = ranked.filter(e => e.assetType === 'crypto').slice(0, 10)
              .some(e => e.symbol === lastPick.crypto_symbol);
            if (prevCryptoPnl <= -7)        prevCryptoOutcome = 'CLOSE ❌';
            else if (prevCryptoPnl >= 20)   prevCryptoOutcome = 'CONSIDER CLOSE 🎯';
            else if (!stillRanked)          prevCryptoOutcome = 'WATCH ⚠️';
            else                            prevCryptoOutcome = 'HOLD ✅';
          }
        } catch { /* best effort */ }
      }

      // Stock review
      if (lastPick.stock_symbol && lastPick.stock_entry_price) {
        try {
          const q = await fetchFinnhubQuote(lastPick.stock_symbol.replace('.L',''));
          if (q?.price) {
            prevStockPnl = round2(((q.price - lastPick.stock_entry_price) / lastPick.stock_entry_price) * 100);
            const stillRanked = ranked.filter(e => e.assetType === 'us' || e.assetType === 'lse').slice(0, 10)
              .some(e => e.symbol === lastPick.stock_symbol);
            if (prevStockPnl <= -7)        prevStockOutcome = 'CLOSE ❌';
            else if (prevStockPnl >= 20)   prevStockOutcome = 'CONSIDER CLOSE 🎯';
            else if (!stillRanked)         prevStockOutcome = 'WATCH ⚠️';
            else                           prevStockOutcome = 'HOLD ✅';
          }
        } catch { /* best effort */ }
      }
    }

    // ── Step 4: Store this week's pick ───────────────────────
    await supabase.from('hunter_weekly_picks').insert({
      week_key:             weekKey,
      week_start:           dateStr,
      crypto_symbol:        topCrypto?.symbol || null,
      crypto_score:         topCrypto ? finalScore(topCrypto) : null,
      crypto_entry_price:   cryptoEntryPrice,
      crypto_reasoning:     topCrypto ? `Hunter avg ${topCrypto.avg} this week${moveBonus.get(topCrypto.symbol) ? `, confirmed move +${round2(moveBonus.get(topCrypto.symbol))}%` : ''}` : null,
      stock_symbol:         topStock?.symbol || null,
      stock_score:          topStock ? finalScore(topStock) : null,
      stock_entry_price:    stockEntryPrice,
      stock_reasoning:      topStock ? `Hunter avg ${topStock.avg} this week${moveBonus.get(topStock.symbol) ? `, confirmed move +${round2(moveBonus.get(topStock.symbol))}%` : ''}` : null,
      prev_crypto_outcome:  prevCryptoOutcome,
      prev_crypto_pnl_pct:  prevCryptoPnl,
      prev_stock_outcome:   prevStockOutcome,
      prev_stock_pnl_pct:   prevStockPnl,
      hunter_confidence:    STATE.cycleCount || null,
      btc_regime:           STATE.btcRegime || null,
    }).catch(e => log(`⚠️  Weekly pick insert error: ${e.message}`));

    // ── Step 5: Build and send Telegram message ───────────────
    const lines = [
      `🏹 HUNTER WEEKLY PICK — ${dateStr}`,
      ``,
    ];
    ...
    if (lastPick) {
      lines.push(`━━━━━━━━━━━━━━━━━━`);
      lines.push(`📊 LAST WEEK REVIEW`);
      lines.push(``);
      if (lastPick.crypto_symbol && prevCryptoOutcome) {
        const sym = lastPick.crypto_symbol.replace('USDT','');
        const pnl = prevCryptoPnl >= 0 ? `+${prevCryptoPnl}%` : `${prevCryptoPnl}%`;
        lines.push(`🪙 ${sym} — ${pnl}`);
        lines.push(`→ ${prevCryptoOutcome}`);
        lines.push(``);
      }
      if (lastPick.stock_symbol && prevStockOutcome) {
        const pnl = prevStockPnl >= 0 ? `+${prevStockPnl}%` : `${prevStockPnl}%`;
        lines.push(`📈 ${lastPick.stock_symbol} — ${pnl}`);
        lines.push(`→ ${prevStockOutcome}`);
        lines.push(``);
      }
    }
    ...
    if (topCrypto) {
      const sym   = topCrypto.symbol.replace('USDT','');
      const score = finalScore(topCrypto);
      const price = cryptoEntryPrice ? ` · Entry ~$${round2(cryptoEntryPrice)}` : '';
      lines.push(`🪙 CRYPTO: ${sym}`);
      lines.push(`Score: ${score} · BTC regime: ${STATE.btcRegime || 'N/A'}${price}`);
      lines.push(`Reason: Hunter avg ${topCrypto.avg} this week${moveBonus.get(topCrypto.symbol) ? ` · confirmed +${round2(moveBonus.get(topCrypto.symbol))}% move` : ''}`);
      lines.push(``);
    }

    if (topStock) {
      const flag  = topStock.assetType === 'lse' ? '🇬🇧' : '🇺🇸';
      const score = finalScore(topStock);
      const price = stockEntryPrice ? ` · Entry ~$${round2(stockEntryPrice)}` : '';
      lines.push(`${flag} STOCK: ${topStock.symbol}`);
      lines.push(`Score: ${score} · QQQ regime: ${STATE.qqqRegime || 'N/A'}${price}`);
      lines.push(`Reason: Hunter avg ${topStock.avg} this week${moveBonus.get(topStock.symbol) ? ` · confirmed +${round2(moveBonus.get(topStock.symbol))}% move` : ''}`);
      lines.push(``);
    }

    lines.push(`━━━━━━━━━━━━━━━━━━`);
    lines.push(`⚠️ Educational market analysis only. Not financial advice. Not a recommendation to buy or sell.`);
    lines.push(`📸 @baretradesignals 🏹`);

    const msg = lines.join('\n');
    if (ENV.BLUEJAM_CHAT_ID) await sendTelegram(msg, ENV.BLUEJAM_CHAT_ID);
    log('🎯 Weekly pick sent');

  } catch (err) {
    log(`⚠️  Weekly pick error: ${err.message}`);
  }
}
