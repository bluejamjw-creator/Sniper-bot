  ...
  try {
    ...
    if (buys.length > 0) {
      ...
      if (bestWatch && ENV.BLUEJAM_CHAT_ID) {
        // Fingerprint dedup — WATCH alerts suppressed if same symbol+stage sent in last 30 mins
        const watchStage = bestWatch.stageInfo?.stage || 'UNKNOWN';
        if (checkAndSetFingerprint(bestWatch.symbol, 'WATCH', watchStage, 'watchLoop')) {
