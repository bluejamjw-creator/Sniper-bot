...
var RadarEngine = class extends Engine {
  ...
  async runScan() {
    if (!this.latestMarketState) {
      this.logger.warn("Radar has no market state from Observer yet \u2014 stock candidates will not get sector-leadership scoring this cycle, but discovery still proceeds for every registered adapter (e.g. crypto)");
    }
    ...
    for (const adapter of this.assetClassAdapters) {
      ...
      for (const raw of rawCandidates) {
        ...
        const payload = {
          ...
          sector: raw.sector,
          interest,
          marketStateId: this.latestMarketState?.timestamp ?? "no-market-state-available",
...
var activesCache = new TtlCache(10 * 60 * 1e3);
var profileCache = new TtlCache(6 * 60 * 60 * 1e3);
var quoteCache = new TtlCache(4 * 60 * 1e3);
...
async function fetchQuote(symbol) {
  return quoteCache.getOrFetch(symbol, () => fetchJson(`${BASE_URL}/quote?symbol=${encodeURIComponent(symbol)}&${apiKeyParam()}`));
}
...
var nyseTimeFormatter = new Intl.DateTimeFormat("en-US", {
  timeZone: "America/New_York",
  hour: "numeric",
  minute: "numeric",
  hour12: false
});
function isNyseOpenNow(now = /* @__PURE__ */ new Date()) {
  assertKnownYear(now);
  if (isNyseClosed(now)) return false;
  const parts = nyseTimeFormatter.formatToParts(now);
  const hour = Number(parts.find((p) => p.type === "hour").value);
  const minute = Number(parts.find((p) => p.type === "minute").value);
  const minutesSinceMidnightET = hour * 60 + minute;
  const marketOpenMinutes = 9 * 60 + 30;
  const marketCloseMinutes = 16 * 60;
  return minutesSinceMidnightET >= marketOpenMinutes && minutesSinceMidnightET < marketCloseMinutes;
}
...
var liveMarketDataPort = Object.freeze({
   ...
   */
  async getMarketSnapshot(now = /* @__PURE__ */ new Date()) {
    if (!isNyseOpenNow(now)) {
      throw new Error("NYSE is currently closed \u2014 market snapshot skipped to avoid wasting API budget outside trading hours (this is expected, not a failure)");
    }
...
function parseMarkets(rawResponse) {
  if (!Array.isArray(rawResponse)) {
    throw new Error("CoinGecko markets response was empty or malformed");
  }
  const result = {};
  for (const coin of rawResponse) {
    if (coin.symbol && typeof coin.market_cap === "number") {
      result[coin.symbol.toUpperCase()] = coin.market_cap;
    }
  }
  return result;
}
async function fetchMarkets(symbols) {
  ...
  try {
    ...
    const url = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&symbols=${encodeURIComponent(symbolsParam)}`;
    const response = await fetch(url, {
      headers: config.env.adapters.coingecko.apiKey ? { "x-cg-demo-api-key": config.env.adapters.coingecko.apiKey } : {}
    });
    if (!response.ok) {
...
var coingeckoProvider = Object.freeze({
  ...
  /**
   * Bulk market-cap lookup — ONE call for many symbols, sharing the
   * same rate limiter as every other CoinGecko call here. Deliberately
   * built this way rather than looping getCoinData() per symbol: at
   * the ~55-minute interval, one call per coin would take hours to
   * refresh even a small universe, making it impractical for any
   * real use. This is the one CoinGecko call the crypto Radar adapter
   * actually depends on.
   *
   * @param {string[]} symbols - ticker symbols, e.g. ['BTC', 'ETH']
   * @returns {Promise<Record<string, number>>} uppercased ticker symbol -> market cap in USD
   */
  async getMarketCapsBySymbols(symbols) {
    const rawResponse = await fetchMarkets(symbols);
    return parseMarkets(rawResponse);
  }
});
...
var liveUsStockAssetClassAdapter = Object.freeze({
   ...
   */
  async getCandidates(now = /* @__PURE__ */ new Date()) {
    if (!isNyseOpenNow(now)) {
      return [];
    }
    const rawActives = await fetchActives();
...
// src/adapters/liveCryptoAssetClassAdapter.js
var BINANCE_24HR_URL2 = "https://data-api.binance.vision/api/v3/ticker/24hr";
var CRYPTO_UNIVERSE = Object.freeze([
  .../* @__PURE__ */ new Set([...SECTOR_SYMBOLS.DEFI, ...SECTOR_SYMBOLS.L1, ...SECTOR_SYMBOLS.AI_CRYPTO, ...SECTOR_SYMBOLS.ORACLES])
]);
var symbolToSector = Object.freeze(
  Object.fromEntries(Object.entries(SECTOR_SYMBOLS).flatMap(([sector, symbols]) => symbols.map((s) => [s, sector])))
);
function stripUsdtSuffix(pair) {
  return pair.endsWith("USDT") ? pair.slice(0, -4) : pair;
}
var lastKnownMarketCaps = {};
async function getMarketCapsWithFallback(symbols) {
  try {
    const fresh = await coingeckoProvider.getMarketCapsBySymbols(symbols);
    if (Object.keys(fresh).length > 0) {
      lastKnownMarketCaps = { ...lastKnownMarketCaps, ...fresh };
    }
    return lastKnownMarketCaps;
  } catch {
    return lastKnownMarketCaps;
  }
}
function buildCryptoRawCandidate(ticker, marketCaps) {
  const price = Number(ticker.lastPrice);
  const highPrice = Number(ticker.highPrice);
  const baseSymbol = stripUsdtSuffix(ticker.symbol);
  return {
    symbol: ticker.symbol,
    assetClass: config.assetClasses.CRYPTO,
    price,
    avgDollarVolume: Number(ticker.quoteVolume),
    // 24hr quote volume — a real, honest proxy, NOT a 20-day average (Binance's free ticker doesn't offer that without extra historical calls)
    marketCap: marketCaps[baseSymbol] ?? 0,
    // 0 (not undefined) so a genuinely unknown coin fails the minMarketCap filter rather than silently passing an undefined comparison
    sector: symbolToSector[ticker.symbol] ?? "crypto",
    distanceFromHighPercent: highPrice > 0 ? Number(((highPrice - price) / highPrice * 100).toFixed(2)) : 0,
    hasAdapterCoverage: true,
    // confirmed via direct testing: liveTechnicalDataPort.getTechnicalSnapshot already works correctly for crypto via the existing Binance klines route in registry.js — no new Sniper work needed
    timestamp: (/* @__PURE__ */ new Date()).toISOString()
  };
}
var liveCryptoAssetClassAdapter = Object.freeze({
  assetClass: config.assetClasses.CRYPTO,
  /** @returns {Promise<import('../radar/marketAdapterPort.js').RawCandidate[]>} */
  async getCandidates() {
    const response = await fetch(BINANCE_24HR_URL2);
    if (!response.ok) {
      throw new Error(`Binance 24hr ticker request failed: ${response.status} ${response.statusText}`);
    }
    ...
    const universeSet = new Set(CRYPTO_UNIVERSE);
    const relevantTickers = allTickers.filter((t) => universeSet.has(t.symbol));
    const baseSymbols = CRYPTO_UNIVERSE.map(stripUsdtSuffix);
    const marketCaps = await getMarketCapsWithFallback(baseSymbols);
    return relevantTickers.map((ticker) => buildCryptoRawCandidate(ticker, marketCaps));
  }
});
...
function buildApp() {
  ...
  const radar = new RadarEngine({
    ...
    assetClassAdapters: [liveUsStockAssetClassAdapter, liveCryptoAssetClassAdapter],
