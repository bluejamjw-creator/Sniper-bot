/**
 * portfolio-bot-v8.js — PART 1 of 3 (Hunter V13.2)
 *
 * This is a straight 3-way split of one file, split at clean section
 * boundaries — NOT three separate modules. To deploy: concatenate all
 * three parts in order into a single portfolio-bot-v8.js and commit
 * that as one file, exactly as the existing repo has it:
 *
 *   cat portfolio-bot-v8.part1.js portfolio-bot-v8.part2.js \
 *       portfolio-bot-v8.part3.js > portfolio-bot-v8.js
 *
 * V13.2 changes vs the uploaded V13.1, each marked inline with
 * '// FIX (V13.2):'. All six were verified against the live Railway
 * logs and Supabase schema for this deployment, not just read from
 * the source:
 *
 *   1. hunter_v11_detections insert now writes 'similarity' instead
 *      of 'learned_similarity' — confirmed against the live schema
 *      and against the exact error string filling the Railway logs
 *      every cycle since deployment. (hunter_v11_snapshots' own
 *      learned_similarity write was already correct and is left
 *      untouched — that table really does have that column.)
 *
 *   2. candleLimit raised 400 -> 520 so the fetched candle window
 *      covers the 120h learning lookback with a real buffer past
 *      the 96h longest target, instead of falling ~20h short and
 *      manufacturing false-negative outcomes for older examples.
 *
 *   3. Same-candle look-ahead removed from BOTH label-resolution
 *      functions — labelRecentExamples() (hunter_v11_examples) and
 *      labelObservationHistory() (hunter_observation_history, a
 *      second, independent occurrence of the same bug found while
 *      patching the first). The future outcome window now starts
 *      strictly after the observation candle closes, not at its
 *      open.
 *
 *   4. Each learning target (+10/+50/+100/+150) is now usable by
 *      the model as soon as its own horizon resolves, instead of
 *      waiting for all four (up to 96h) to resolve together. No
 *      schema change needed — .eq(column, value) already excludes
 *      unresolved rows on its own.
 *
 *   5. hunter_v11_detections.reason is now actually populated.
 *      The column existed on the table from the start but nothing
 *      ever wrote to it — openCandidate()'s pass/fail reasons were
 *      computed fresh every cycle inside rankOpenCandidates() and
 *      discarded immediately. Confirmed live: as of this writing
 *      hunter_v11_alerts has 11 real OPEN/CLOSE rows (FILUSDT among
 *      them, correctly), but there was no equivalent record of WHY
 *      any of the other 180+ scanned assets per cycle didn't alert.
 *      Every 'why didn't X fire' question now answers with a query
 *      instead of manually reconstructing gate math from raw
 *      feature snapshots after the fact.
 *
 * Also confirmed live but NOT fixed here (flagging, not silently
 * fixing, since these need your call):
 *   - Row Level Security is disabled on all 50 public tables in
 *     this Supabase project, including every hunter_v11_* table.
 *     Anyone with the anon key can read or write every row.
 *   - FORTHUSDT has zero rows in hunter_v11_snapshots — it was
 *     never in the tracked universe, separate from any gating
 *     question.
 */
'use strict';
...
// portfolio-bot-v8.js — PART 2 of 3 (Hunter V13.2). See part 1 for
...
// portfolio-bot-v8.js — PART 3 of 3 (Hunter V13.2). See part 1 for
// concatenation instructions and the full changelog.
