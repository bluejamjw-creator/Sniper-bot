...
var MAX_POSTS_PER_DAY_SAFETY_BACKSTOP = 20;
...
function evaluateNarrativeChange(previousState, proposed, now = Date.now()) {
  ...
  if (postCountToday >= MAX_POSTS_PER_DAY_SAFETY_BACKSTOP) {
...
function moveBadge(heat, changePercent) {
  if (heat >= 80) return " \u{1F321}\uFE0F";
  if (changePercent >= 8) return " \u{1F525}";
  if (changePercent >= 5) return " \u26A1";
  if (changePercent <= -8) return " \u{1F9CA}";
  if (changePercent <= -5) return " \u{1F976}";
  return "";
}
function buildNarrativeMessage(content) {
  const lines = ["\u{1F9E0}\u{1F525} <b>MARKET INTEL UPDATE</b>", ""];
  if (content.hotSectors.length > 0) {
    lines.push("<b>Active narratives:</b>");
    for (const sector of content.hotSectors) {
      const emoji = SECTOR_EMOJI[sector] ?? "\u{1F4CC}";
      const members = (content.sectorSymbols[sector] ?? []).slice(0, 4).join(", ");
      lines.push(`  ${emoji} <b>${sector.replace("_", " ")}</b> \u2192 ${members}`);
    }
    lines.push("");
  }
  if (content.stockMovers.length > 0) {
    lines.push("\u{1F1FA}\u{1F1F8} <b>Hot stocks right now:</b>");
    for (const m of content.stockMovers) {
      const sign = m.changePercent >= 0 ? "+" : "";
      const badge = moveBadge(m.heat, m.changePercent);
      const brain = m.inSector ? " \u{1F9E0}" : "";
      lines.push(`  \u{1F680} <b>${m.symbol}</b> ${sign}${m.changePercent.toFixed(1)}%${badge}${brain}`);
    }
    lines.push("");
  }
  if (content.cryptoMovers.length > 0) {
    lines.push("\u{1FA99} <b>Elevated momentum detected:</b>");
    for (const c of content.cryptoMovers) {
      const tag = c.changePercent >= 15 ? "\u{1F525} explosive move" : c.changePercent >= 8 ? "\u26A1 strong momentum" : c.changePercent >= 0 ? "\u{1F4C8} building momentum" : c.changePercent <= -15 ? "\u{1F9CA} sharp drop" : c.changePercent <= -8 ? "\u{1F976} selling pressure" : "\u{1F4C9} pulling back";
      const sym = c.symbol.replace("USDT", "");
      lines.push(`  \u{1F4CA} <b>${sym}</b> \u2014 ${tag}`);
    }
    lines.push("");
  }
  lines.push("\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014");
  lines.push("\u26A0\uFE0F <i>Educational market commentary only \xB7 Not personalised investment advice \xB7 Not a recommendation to buy or sell</i>");
  return lines.join("\n");
}
function hasContentToSend(content) {
  return content.hotSectors.length > 0 || content.stockMovers.length > 0 || content.cryptoMovers.length > 0;
}
...
var IntelEngine = class extends Engine {
  ...
  async runDiscoveryCycle() {
    ...
    const allMoverSymbols = [...snapshot.stockMovers.map((m) => m.symbol), ...snapshot.trendingSymbols];
