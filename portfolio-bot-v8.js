...
async function sendWatchlistCreated(candidate, stage) {
  const flagMap = { crypto: '🌐', us: '🇺🇸', lse: '🇬🇧' };
  const flag = flagMap[candidate.assetType] || '🌐';
  const gap  = round2(CONFIG.THRESHOLDS.hunterScoreGood - candidate.finalScore);
  const setupLabel = (candidate.execEngine?.setupType || 'PULLBACK_CONTINUATION').replace(/_/g, ' ');

  const msg = [
    `🏹 <b>HUNTER CANDIDATE</b>`,
    ``,
    `${flag} ${candidate.symbol}`,
    `Pattern: ${setupLabel}`,
    ``,
    `Score: <b>${round2(candidate.finalScore)}</b>`,
    `Needed: <b>${CONFIG.THRESHOLDS.hunterScoreGood}</b>`,
    `Status: Monitoring for confirmation`,
    ``,
    `<i>Entry levels will be published if this setup strengthens and confirms.</i>`,
    ``,
    `——————————`,
    `⚠️ <i>Educational commentary only · Not investment advice · @baretradesignals 🏹</i>`,
  ].join('\n');

  if (checkAndSetFingerprint(candidate.symbol, 'WATCHLIST_CREATED', stage, 'lifecycleEngine')) {
...
async function sendWatchlistUpdate(symbol, candidate, state) {
  ...
  const nearTrigger = gap <= WATCHLIST_NEAR_TRIGGER_GAP;
  ...
  const msg = [
    ...
    `${flag} ${symbol}`,
    ``,
    `${round2(state.lastAlertScore)} → <b>${round2(candidate.finalScore)}</b> (${delta >= 0 ? '+' : ''}${delta})`,
    nearTrigger
      ? `Now only ${gap} point${gap === 1 ? '' : 's'} from trigger.`
      : `Gap to trigger: ${gap} points.`,
    ``,
    nearTrigger ? `Interesting. Worth watching closely.` : `Improving steadily.`,
    ``,
    `——————————`,
    `⚠️ <i>Educational commentary only · Not investment advice · @baretradesignals 🏹</i>`,
  ].join('\n');
  ...
  await sendTelegram(msg, ENV.BLUEJAM_CHAT_ID);
  log(`⬆️ Watchlist UPDATE: ${symbol} ${round2(state.lastAlertScore)}→${round2(candidate.finalScore)}`);
}
...
async function sendWatchlistRemoved(symbol, candidate, state) {
  ...
  const msg = [
    ...
    `${round2(state.peakScore)} → ${round2(currentScore)}`,
    ``,
...
async function fireLeaderboardMessage(scan) {
  ...
  if (candidates.length > 0) {
    candidates.forEach((c, i) => {
      ...
      lines.push(`${i+1}. <b>${c.symbol}</b> ${score}${deltaStr} ${tag} ${gap > 0 ? `(need +${gap})` : ''}`);
    });
    lines.push(``);
