...
async function fireLeaderboardMessage(scan) {
  ...
  if (candidates.length > 0) {
    candidates.forEach((c, i) => {
      ...
      lines.push(`${rankEmoji[i] || `${i+1}.`} ${mIcon} ${c.symbol}   ⭐${score}  ${tag}`);
    });
  } else {
  ...
  if (candidates.length > 0) {
    ...
    lines.push(`🏆 Strongest Leader`);
    lines.push(`${best.symbol}`);
    lines.push(``);

    // Fastest accelerator — highest score delta from watchlist tracking
    ...
    if (fastestSymbol) {
      lines.push(`⚡ Fastest Accelerator`);
      lines.push(`${fastestSymbol} (▲ +${fastestDelta})`);
      lines.push(``);
    }

    // Market breadth from candidates
    const improving = candidates.filter(c => {
      const tracked = STATE.watchlistState.get(c.symbol);
      return tracked?.createdScore != null && (c.finalScore || 0) > tracked.createdScore;
    }).length;
    lines.push(`📈 Market Breadth`);
    lines.push(`${improving} / ${candidates.length} assets improving`);
    lines.push(``);
  ...
  if (supabase && candidates.length > 0) {
    try {
      ...
      log(`📊 Writing leaderboard snapshot: ${rows.length} rows, top=${rows[0]?.symbol} score=${rows[0]?.score}`);
      ...
      else log(`📊 Leaderboard snapshot saved (snapshot_id=${snapshotId})`);
    } catch (err) {
      log(`⚠️  Leaderboard history error: ${err.message}`);
    }
  } else {
    log(`📊 Leaderboard history skipped — supabase=${!!supabase} candidates=${candidates.length}`);
  }
