...
async function fireDailyLeaderboard(market) {
  ...
  const CRYPTO_UNIVERSE = new Set(['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'TAOUSDT', 'FETUSDT', 'ONDOUSDT', 'LINKUSDT', 'MORPHOUSDT', 'PENDLEUSDT', 'AAVEUSDT', 'AVAXUSDT', 'XRPUSDT', 'TIAUSDT', 'SUIUSDT', 'SEIUSDT', 'INJUSDT', 'NEARUSDT', 'TONUSDT', 'HYPEUSDT', 'JUPUSDT', 'ENAUSDT', 'ZROUSDT', 'GRASSUSDT', 'HBARUSDT', 'QNTUSDT']);
  const US_UNIVERSE     = new Set(['NVDA', 'AVGO', 'ANET', 'VRT', 'TSM', 'AMD', 'ARM', 'CRDO', 'PLTR', 'CRWD', 'DDOG', 'SNOW', 'SOUN', 'TEM', 'APP', 'RKLB', 'LUNR', 'ASTS', 'OKLO', 'LEU', 'BWXT', 'NNE', 'SMR', 'CEG', 'ETN', 'PWR', 'FIX', 'RTX', 'AVAV', 'KTOS', 'HOOD', 'UBER', 'DUOL', 'HIMS', 'IONQ', 'META', 'MSFT']);
  const universeFilter  = market === 'crypto' ? s => CRYPTO_UNIVERSE.has(s)
                        : market === 'us'     ? s => US_UNIVERSE.has(s)
                        : s => s.includes('.L');
  ...
  // Week start — last Monday 00:00 UTC
  const daysSinceMon = now.getUTCDay() === 0 ? 6 : now.getUTCDay() - 1;
  const weekStart = new Date(now);
  weekStart.setUTCDate(now.getUTCDate() - daysSinceMon);
  weekStart.setUTCHours(0, 0, 0, 0);

  // Month start — 1st of current month 00:00 UTC
  const monthStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1));

  const marketLabel = { crypto: '🪙 CRYPTO', us: '🇺🇸 US STOCKS', lse: '🇬🇧 LSE' }[market];
  ...
  function fmtRow(emoji, sym, score, delta) {
    const label = sym.replace('USDT','').padEnd(8);
    return `${emoji} ${label}   [${score}]   ${vel(delta)}`;
  }

  try {
    // Single query covers month + week + today
    const since = new Date(monthStart).toISOString();
    const { data: allRows, error } = await supabase
      .from('hunter_leaderboard_history')
      .select('symbol, score, rank, snapshot_id, snapshot_time, asset_type')
      .eq('asset_type', market)
      .gte('snapshot_time', since)
      .order('snapshot_time', { ascending: true });

    if (error || !allRows || allRows.length === 0) {
      log(`📊 Daily ${market}: no data`); return;
    }

    const filtered = allRows.filter(r => universeFilter(r.symbol));
    const snapIds  = [...new Set(filtered.map(r => r.snapshot_id))];
    const latestId = snapIds[snapIds.length - 1];
    const priorId  = snapIds.length >= 2 ? snapIds[snapIds.length - 2] : null;

    // Latest scores map
    const latestScores = new Map();
    filtered.filter(r => r.snapshot_id === latestId)
      .forEach(r => latestScores.set(r.symbol, round2(parseFloat(r.score))));

    // Prior snapshot scores (for today velocity)
    const priorScores = new Map();
    if (priorId) filtered.filter(r => r.snapshot_id === priorId)
      .forEach(r => priorScores.set(r.symbol, round2(parseFloat(r.score))));

    // Week start scores
    const weekSnapIds = [...new Set(filtered
      .filter(r => new Date(r.snapshot_time) >= weekStart)
      .map(r => r.snapshot_id))];
    const weekStartScores = new Map();
    if (weekSnapIds[0]) filtered.filter(r => r.snapshot_id === weekSnapIds[0])
      .forEach(r => weekStartScores.set(r.symbol, round2(parseFloat(r.score))));

    // Month start scores
    const monthStartScores = new Map();
    if (snapIds[0]) filtered.filter(r => r.snapshot_id === snapIds[0])
      .forEach(r => monthStartScores.set(r.symbol, round2(parseFloat(r.score))));

    // Build cumulative scores per symbol for each timeframe
    function cumulativeRank(rows, startTime) {
      const map = new Map();
      for (const r of rows.filter(x => !startTime || new Date(x.snapshot_time) >= startTime)) {
        if (!map.has(r.symbol)) map.set(r.symbol, 0);
        map.set(r.symbol, map.get(r.symbol) + parseFloat(r.score));
      }
      return [...map.entries()]
        .sort((a,b) => b[1]-a[1])
        .map(([sym, cum]) => ({ symbol: sym, cumScore: round2(cum), score: latestScores.get(sym) || 0 }));
    }

    const todayTop   = filtered.filter(r => r.snapshot_id === latestId)
      .sort((a,b) => a.rank - b.rank)
      .slice(0, 3)
      .map(r => ({ symbol: r.symbol, score: round2(parseFloat(r.score)) }));

    const weekTop    = cumulativeRank(filtered, weekStart).slice(0, 3);
    const monthTop   = cumulativeRank(filtered, monthStart).slice(0, 3);

    const todayTop3  = new Set(todayTop.map(d => d.symbol));
    const weekTop3   = new Set(weekTop.map(d => d.symbol));
    const monthTop3  = new Set(monthTop.map(d => d.symbol));

    // ── Notable mentions logic ───────────────────────────────
    const mentions = [];
    const allSymbols = new Set(filtered.map(r => r.symbol));

    for (const sym of allSymbols) {
      const cur   = latestScores.get(sym);
      const prior = priorScores.get(sym);
      const wBase = weekStartScores.get(sym);
      const mBase = monthStartScores.get(sym);
      if (!cur) continue;

      const todayVel  = prior != null ? round2(cur - prior) : null;
      const weekVel   = wBase != null ? round2(cur - wBase) : null;
      const monthVel  = mBase != null ? round2(cur - mBase) : null;
      const label     = sym.replace('USDT','');

      // 🚀 Accelerating across all three timeframes
      if (todayVel >= 6 && weekVel >= 8 && monthVel >= 10 &&
          !todayTop3.has(sym) && !weekTop3.has(sym) && !monthTop3.has(sym)) {
        mentions.push(`🚀 ${label}  [${cur}]  Today ▲+${todayVel}  Week ▲+${weekVel}  Month ▲+${monthVel} — accelerating across all timeframes`);
        continue;
      }

      // ⚡ Fast mover outside top 3 on any timeframe
      // Exclude if already shown in today's top 3
      if (!todayTop3.has(sym) &&
          ((todayVel >= 8) ||
          (weekVel  >= 10 && !weekTop3.has(sym)) ||
          (monthVel >= 12 && !monthTop3.has(sym)))) {
        const best = Math.max(todayVel||0, weekVel||0, monthVel||0);
        const tf   = best === todayVel ? 'today' : best === weekVel ? 'WTD' : 'MTD';
        mentions.push(`⚡ ${label}  [${cur}]  ▲ +${best} ${tf} — accelerating outside top 3`);
        continue;
      }

      // 🔄 Divergence — strong monthly but fading today
      if (monthTop3.has(sym) && !todayTop3.has(sym) && todayVel != null && todayVel < -5) {
        mentions.push(`🔄 ${label}  [${cur}]  Monthly leader ▼ ${todayVel} today — rotation signal`);
        continue;
      }

      // 📉 Was in top 3 yesterday, dropping sharply
      const priorTop3Syms = filtered.filter(r => r.snapshot_id === priorId && r.rank <= 3).map(r => r.symbol);
      if (priorTop3Syms.includes(sym) && !todayTop3.has(sym) && todayVel != null && todayVel < -5) {
        mentions.push(`📉 ${label}  [${cur}]  ▼ ${todayVel} — dropped from top 3`);
        continue;
      }

      // ⚠️ Strong this week, absent from monthly top 3 (short-term spike)
      if (weekTop3.has(sym) && !monthTop3.has(sym) && weekVel >= 8) {
        mentions.push(`⚠️ ${label}  [${cur}]  Strong WTD ▲+${weekVel} but not in monthly top 3 — watch for continuation`);
      }
    }

    // ── Build message ────────────────────────────────────────
    const lines = [
      `🏹 ${marketLabel} DAILY — ${timeStr}`,
      ``,
      `TODAY'S TOP 3`,
      ``,
    ];

    todayTop.forEach((d, i) => {
      lines.push(fmtRow(rankEmoji[i], d.symbol, d.score, priorScores.get(d.symbol) != null ? round2(d.score - priorScores.get(d.symbol)) : null));
    });

    // On Mondays, week-to-date = today so show last week instead
    const isMonday = now.getUTCDay() === 1;
    if (isMonday) {
      // Fetch last week's data (Mon-Sun prior week)
      const lastWeekEnd   = new Date(weekStart); lastWeekEnd.setUTCSeconds(-1);
      ...
      const { data: lwRows } = await supabase
        ...
        .select('symbol, score, rank, snapshot_id, snapshot_time, asset_type')
        .eq('asset_type', market)
        .gte('snapshot_time', lastWeekStart.toISOString())
        .lte('snapshot_time', lastWeekEnd.toISOString())
        .order('snapshot_time', { ascending: true });

      const lwFiltered = (lwRows || []).filter(r => universeFilter(r.symbol));
      const lwMap = new Map();
      for (const r of lwFiltered) {
        if (!lwMap.has(r.symbol)) lwMap.set(r.symbol, 0);
        lwMap.set(r.symbol, lwMap.get(r.symbol) + parseFloat(r.score));
      }
      const lwTop = [...lwMap.entries()]
        .sort((a,b) => b[1]-a[1])
        .slice(0, 3)
        .map(([sym]) => ({ symbol: sym, score: latestScores.get(sym) || 0 }));

      lines.push(``, `━━━━━━━━━━━━━━━━━━`, `LAST WEEK (for reference)`, ``);
      if (lwTop.length > 0) {
        lwTop.forEach((d, i) => lines.push(fmtRow(rankEmoji[i], d.symbol, d.score, null)));
      } else {
        lines.push(`No last week data yet`);
      }
    } else {
      lines.push(``, `━━━━━━━━━━━━━━━━━━`, `WEEK TO DATE`, ``);
      weekTop.forEach((d, i) => {
        lines.push(fmtRow(rankEmoji[i], d.symbol, d.score, weekStartScores.get(d.symbol) != null ? round2(d.score - weekStartScores.get(d.symbol)) : null));
      });
    }

    lines.push(``, `━━━━━━━━━━━━━━━━━━`, `MONTH TO DATE`, ``);
    monthTop.forEach((d, i) => {
      lines.push(fmtRow(rankEmoji[i], d.symbol, d.score, monthStartScores.get(d.symbol) != null ? round2(d.score - monthStartScores.get(d.symbol)) : null));
    });

    if (mentions.length > 0) {
      lines.push(``, `━━━━━━━━━━━━━━━━━━`, `👁 NOTABLE`, ``);
      mentions.slice(0, 4).forEach(m => lines.push(m)); // max 4 mentions
    }
    ...
    log(`📊 Daily ${market} leaderboard sent`);

  } catch (err) {
    log(`⚠️  Daily ${market} leaderboard error: ${err.message}`);
  }
}


// ============================================================
// PORTFOLIO WATCH — 5-state portfolio manager
// Daily at 09:00 UTC. Bluejam only (personal).
// States: Consider Buying → Hold → Take Profit → Set Stop → Time to Sell
// ============================================================

async function runPortfolioWatch() {
  if (!supabase) return;
  log('💼 Running Portfolio Watch...');

  const WATCH_UNIVERSE = new Set(['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'TAOUSDT', 'FETUSDT', 'ONDOUSDT', 'LINKUSDT', 'MORPHOUSDT', 'PENDLEUSDT', 'AAVEUSDT', 'AVAXUSDT', 'XRPUSDT', 'TIAUSDT', 'SUIUSDT', 'SEIUSDT', 'INJUSDT', 'NEARUSDT', 'TONUSDT', 'HYPEUSDT', 'JUPUSDT', 'ENAUSDT', 'ZROUSDT', 'GRASSUSDT', 'HBARUSDT', 'QNTUSDT', 'NVDA', 'AVGO', 'ANET', 'VRT', 'TSM', 'AMD', 'ARM', 'CRDO', 'PLTR', 'CRWD', 'DDOG', 'SNOW', 'SOUN', 'TEM', 'APP', 'RKLB', 'LUNR', 'ASTS', 'OKLO', 'LEU', 'BWXT', 'NNE', 'SMR', 'CEG', 'ETN', 'PWR', 'FIX', 'RTX', 'AVAV', 'KTOS', 'HOOD', 'UBER', 'DUOL', 'HIMS', 'IONQ', 'META', 'MSFT']);
  const since14 = new Date(Date.now() - 14 * 24 * 60 * 60 * 1000).toISOString();

  const { data: allRows, error } = await supabase
    .from('hunter_leaderboard_history')
    .select('symbol, asset_type, rank, score, snapshot_id, snapshot_time')
    .gte('snapshot_time', since14)
    .order('snapshot_time', { ascending: true });

  if (error || !allRows?.length) { log('💼 Portfolio Watch: no data'); return; }

  const rows = allRows.filter(r => WATCH_UNIVERSE.has(r.symbol));
  const snapIds = [...new Set(rows.map(r => r.snapshot_id))];
  const latestId = snapIds[snapIds.length - 1];

  const symbolMap = new Map();
  for (const row of rows) {
    if (!symbolMap.has(row.symbol)) {
      symbolMap.set(row.symbol, { symbol: row.symbol, assetType: row.asset_type, snapshots: [] });
    }
    symbolMap.get(row.symbol).snapshots.push({
      score: parseFloat(row.score), rank: row.rank, snapshotId: row.snapshot_id
    });
  }

  const alerts = [];

  for (const [sym, d] of symbolMap.entries()) {
    const snaps = d.snapshots;
    if (snaps.length < 3) continue;

    const scores       = snaps.map(s => s.score);
    const ranks        = snaps.map(s => s.rank);
    const last3Scores  = scores.slice(-3);
    const last3Ranks   = ranks.slice(-3);
    const currentScore = scores[scores.length - 1];
    const currentRank  = ranks[ranks.length - 1];
    const startScore   = scores[0];
    const peakScore    = Math.max(...scores);
    const scoreRise    = round2(currentScore - startScore);
    const dropFromPeak = round2(peakScore - currentScore);
    const top5count    = snaps.filter(s => s.rank <= 5).length;

    const rising   = last3Scores[2] > last3Scores[1] && last3Scores[1] > last3Scores[0];
    const falling  = last3Scores[2] < last3Scores[1] && last3Scores[1] < last3Scores[0];
    const peaked   = peakScore > 62 && dropFromPeak >= 8;
    const plateau  = Math.abs(last3Scores[2] - last3Scores[0]) < 3 && currentScore >= 55;
    const collapsed = currentScore < 50 && dropFromPeak >= 15;
    const outOfTop10 = last3Ranks.every(r => r > 10);

    const label     = sym.replace('USDT', '');
    const assetIcon = d.assetType === 'crypto' ? '🪙' : d.assetType === 'lse' ? '🇬🇧' : '📈';

    if (collapsed || (outOfTop10 && dropFromPeak >= 20)) {
      alerts.push({ priority: 1, symbol: sym, msg:
        `🚨 PORTFOLIO WATCH — TIME TO SELL\n\n${assetIcon} ${label}\n\nScore collapsed: [${round2(peakScore)}] → [${round2(currentScore)}]\nDropped out of top 10 for 3 sessions\n\nHunter says sell. Leadership gone.\n\n——————————\n📸 @baretradesignals 🏹`
      });
    } else if (peaked && dropFromPeak >= 12 && currentRank > 5) {
      alerts.push({ priority: 2, symbol: sym, msg:
        `⚠️ PORTFOLIO WATCH — SET YOUR STOP\n\n${assetIcon} ${label}\n\nScore: [${round2(peakScore)}] peak → [${round2(currentScore)}] now\nFading from top 5\n\nMomentum weakening. Tighten your stop.\n\n——————————\n📸 @baretradesignals 🏹`
      });
    } else if (peaked && dropFromPeak >= 8 && currentRank > 3 && currentScore >= 55) {
      alerts.push({ priority: 3, symbol: sym, msg:
        `💰 PORTFOLIO WATCH — TAKE PROFIT\n\n${assetIcon} ${label}\n\nScore peaked: [${round2(peakScore)}] — now [${round2(currentScore)}] and falling\nDropped from top 3, rank now #${currentRank}\n\nMomentum shifting. Good time to take profit.\n\n——————————\n📸 @baretradesignals 🏹`
      });
    } else if (currentScore >= 60 && currentRank <= 5 && top5count >= 3 && plateau) {
      alerts.push({ priority: 4, symbol: sym, msg:
        `✅ PORTFOLIO WATCH — HOLD\n\n${assetIcon} ${label}\n\nScore: [${round2(currentScore)}] — holding strong\nTop 5 in ${top5count} of ${snaps.length} sessions\n\nHold for now. Set a stop loss if you haven't already.\n\n——————————\n📸 @baretradesignals 🏹`
      });
    } else if (scoreRise >= 15 && currentScore >= 60 && top5count >= 3 && rising) {
      alerts.push({ priority: 5, symbol: sym, msg:
        `🎯 PORTFOLIO WATCH\n\n${assetIcon} ${label}\n\nHunter has been tracking ${label} for ${snaps.length} sessions.\nScore building: [${round2(startScore)}] → [${round2(currentScore)}]\nPersistent top-5 leader — ${top5count} appearances\n\n💡 Consider buying.\n\n——————————\n📸 @baretradesignals 🏹`
      });
    }
  }
  ...
  if (alerts.length > 0) {
    ...
    for (const alert of alerts.slice(0, 5)) {
      if (ENV.BLUEJAM_CHAT_ID) await sendTelegram(alert.msg, ENV.BLUEJAM_CHAT_ID);
      await new Promise(r => setTimeout(r, 1500));
      log(`💼 Portfolio Watch: ${alert.symbol} (priority ${alert.priority})`);
    }
  } else {
    log('💼 Portfolio Watch: no signals today');
  }
}
...
async function fireHunterWeeklyReport() {
  ...
  log('📅 Building Hunter weekly report (Market Intelligence + Planner Guidance)...');

  // Fixed universe — exclude dynamic injections and LSE
  const REPORT_CRYPTO   = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'TAOUSDT', 'FETUSDT', 'ONDOUSDT', 'LINKUSDT', 'MORPHOUSDT', 'PENDLEUSDT', 'AAVEUSDT', 'AVAXUSDT', 'XRPUSDT', 'TIAUSDT', 'SUIUSDT', 'SEIUSDT', 'INJUSDT', 'NEARUSDT', 'TONUSDT', 'HYPEUSDT', 'JUPUSDT', 'ENAUSDT', 'ZROUSDT', 'GRASSUSDT', 'HBARUSDT', 'QNTUSDT'];
  const REPORT_US       = ['NVDA', 'AVGO', 'ANET', 'VRT', 'TSM', 'AMD', 'ARM', 'CRDO', 'PLTR', 'CRWD', 'DDOG', 'SNOW', 'SOUN', 'TEM', 'APP', 'RKLB', 'LUNR', 'ASTS', 'OKLO', 'LEU', 'BWXT', 'NNE', 'SMR', 'CEG', 'ETN', 'PWR', 'FIX', 'RTX', 'AVAV', 'KTOS', 'HOOD', 'UBER', 'DUOL', 'HIMS', 'IONQ', 'META', 'MSFT'];
  const REPORT_UNIVERSE = new Set([...REPORT_CRYPTO, ...REPORT_US]);
  ...
  if (rows.length === 0) {
    log('⚠️  Hunter weekly report: no fixed-universe rows found');
    return;
  }
  ...
  const snapshotsWithUS = new Set(rows.filter(r => r.asset_type === 'us').map(r => r.snapshot_id)).size;
  ...
  for (const row of rows) {
    if (!symbolMap.has(row.symbol)) {
      symbolMap.set(row.symbol, {
        ...
        firstRank: null, lastRank: null, firstScore: null, lastScore: null,
      });
    }
  ...
  const mostPersistent = [...assets].sort((a,b) => b.top5count - a.top5count).slice(0, 5);
  ...
  // Capital rotation — compare first-half vs second-half avg score per asset
  ...
  for (const d of assets) {
    const firstRows  = rows.filter(r => r.symbol === d.symbol && firstHalfIds.has(r.snapshot_id));
    const secondRows = rows.filter(r => r.symbol === d.symbol && secondHalfIds.has(r.snapshot_id));
    if (!firstRows.length || !secondRows.length) continue;
    const firstAvg  = firstRows.reduce((s,r)=>s+parseFloat(r.score),0)/firstRows.length;
    const secondAvg = secondRows.reduce((s,r)=>s+parseFloat(r.score),0)/secondRows.length;
    const delta = secondAvg - firstAvg;
    if (delta > 3)       rotatingIn.push({ symbol: d.symbol, delta: round2(delta) });
    else if (delta < -3) rotatingOut.push({ symbol: d.symbol, delta: round2(delta) });
    else                 stable.push(d.symbol);
  }
  ...
  const marketStructure = avgScoreAll >= 65 ? 'Strong' : avgScoreAll >= 55 ? 'Moderate' : 'Weak';

  // Hunter Confidence — composite of breadth + leader persistence + market structure
  const topPersistence = champion ? round2((champion.top5count / totalSnapshots) * 100) : 0;
  const confScore = Math.round((breadthPct * 0.4) + (topPersistence * 0.4) + (avgScoreAll * 0.2));
  const hunterConf = confScore >= 65 ? 'HIGH' : confScore >= 45 ? 'MEDIUM' : 'LOW';

  // Hunter Flow Reliability — based on data volume and consistency
  const reliability = totalSnapshots >= 30 ? '70%' : totalSnapshots >= 15 ? '50%' : '30%';

  // Hunter Rating — capped at 100
  const hofScore = champion ? Math.min(100, round2(
    ((champion.rank1count / totalSnapshots) * 40) +
    ((champion.top5count  / totalSnapshots) * 35) +
    (Math.min(avgScoreFn(champion) / 100, 1) * 25)
  )) : 0;

  // Planner guidance — based on rotation and persistence
  ...
  const plannerMaintain = mostPersistent
  ...
  // Facts only. No investment decisions.
  // ════════════════════════════════════════════════════════════
  const part1 = [
    ...
    `📡 Flow Reliability: ${reliability} (${totalSnapshots} snapshots)`,
    ``,
    `━━━━━━━━━━━━━━━━━━`,
    `🏛 HALL OF FAME`,
    ``,
  ...
  if (champion) {
    part1.push(`👑 Champion: ${champion.symbol.replace('USDT','')}`);
    part1.push(`🥇 #1 Finishes: ${champion.rank1count}`);
    part1.push(`🔥 Top 5 Appearances: ${champion.top5count}/${totalSnapshots}`);
    part1.push(`⭐ Avg Score: ${round2(avgScoreFn(champion))}`);
    part1.push(`🏅 Hunter Rating: ${hofScore}/100`);
  }

  part1.push(``, `━━━━━━━━━━━━━━━━━━`, `📊 WEEKLY RANKINGS`, ``);
  part1.push(`🔥 Most Persistent:`);
  mostPersistent.forEach(d => {
    const pct = Math.round((d.top5count / totalSnapshots) * 100);
    part1.push(`   ${d.symbol.replace('USDT','').padEnd(8)} ${pct}% top-5  avg ⭐${round2(avgScoreFn(d))}`);
  });

  part1.push(``);
  if (mostWins?.rank1count > 0) part1.push(`🏆 Most #1 Finishes: ${mostWins.symbol.replace('USDT','')} (${mostWins.rank1count})`);
  if (biggestClimber?.rankDelta > 0) part1.push(`📈 Biggest Climber: ${biggestClimber.symbol.replace('USDT','')} (▲${biggestClimber.rankDelta} places)`);
  if (biggestFaller?.rankDelta < 0)  part1.push(`📉 Biggest Faller: ${biggestFaller.symbol.replace('USDT','')} (▼${Math.abs(biggestFaller.rankDelta)} places)`);
  if (newEntrants.length) part1.push(`🆕 New Top 10: ${newEntrants.join('  ')}`);
  if (dropouts.length)    part1.push(`❌ Dropped: ${dropouts.join('  ')}`);
  ...
  part1.push(`Structure: ${marketStructure}`);
  part1.push(`Breadth: ${breadthPct}% of assets strengthened`);
  part1.push(`Avg Score: ${avgScoreAll}`);
  if (snapshotsWithUS < totalSnapshots * 0.3) {
    part1.push(`⚠️ US data thin — ${snapshotsWithUS}/${totalSnapshots} snapshots (holiday/timing)`);
  }

  part1.push(``, `——————————`);
  part1.push(`⚠️ Educational market analysis only · Not personalised investment advice · Not a recommendation to buy or sell.`);
  part1.push(``);
  part1.push(`📸 Analysis by @baretradesignals 🏹`);

  // ════════════════════════════════════════════════════════════
  // PART 2 — PLANNER GUIDANCE
  // Hunter Flow signals only. No BUY/SELL. Portfolio Engine decides.
  // ════════════════════════════════════════════════════════════
  const part2 = [
    `🏹 HUNTER WEEKLY REPORT`,
    `PART 2 — PLANNER GUIDANCE`,
    ``,
    `🤖 For use with Crypto Cycle Planner only.`,
    `Hunter reports market signals. Portfolio Engine makes decisions.`,
    ``,
    `━━━━━━━━━━━━━━━━━━`,
    `Hunter Confidence: ${hunterConf}`,
    `Hunter Flow Weight: 30%`,
    `Flow Reliability: ${reliability}`,
    ``,
    `━━━━━━━━━━━━━━━━━━`,
  ...
  else part2.push(`• None this week`);
  ...
  part2.push(
    ``,
    `━━━━━━━━━━━━━━━━━━`,
    `Only increase overall conviction when at least two agree:`,
    `• Hunter Flow ← this report`,
    `• Plex/Google Narrative`,
    `• Technical Structure`,
    ``,
    `——————————`,
    `⚠️ Educational market analysis only · Not personalised investment advice · Not a recommendation to buy or sell.`,
  ...
  if (ENV.BLUEJAM_CHAT_ID) {
    ...
    await new Promise(r => setTimeout(r, 2000)); // 2s gap between messages
  ...
  log('📅 Hunter weekly report sent (Part 1 + Part 2)');
}
...
async function mainCycle() {
  ...
  try {
    ...
    // Daily leaderboard messages
    {
      ...
      // Crypto 08:00 UTC
      if (_dh === 8 && _dm < 5) {
        const _ck = `daily_crypto_${_dd}`;
        let _sent = false;
        if (supabase) { try { const {data:_d} = await supabase.from('hunter_calibration').select('id').eq('id',_ck).single(); _sent=!!_d; } catch {} }
        if (!_sent) {
          await fireDailyLeaderboard('crypto');
          if (supabase) { try { await supabase.from('hunter_calibration').upsert({id:_ck,recorded_at:new Date().toISOString(),top_score:0,top_symbol:'DAILY_CRYPTO',field_avg:0,candidate_count:0,btc_regime:STATE.btcRegime,qqq_regime:STATE.qqqRegime,market_open_count:0,buy_count:0,watch_count:0}); } catch {} }
        }
      }
      // LSE 16:30 UTC
      if (_dh === 16 && _dm >= 30 && _dm < 35) {
        const _lk = `daily_lse_${_dd}`;
        let _sent = false;
        if (supabase) { try { const {data:_d} = await supabase.from('hunter_calibration').select('id').eq('id',_lk).single(); _sent=!!_d; } catch {} }
        if (!_sent) {
          await fireDailyLeaderboard('lse');
          if (supabase) { try { await supabase.from('hunter_calibration').upsert({id:_lk,recorded_at:new Date().toISOString(),top_score:0,top_symbol:'DAILY_LSE',field_avg:0,candidate_count:0,btc_regime:STATE.btcRegime,qqq_regime:STATE.qqqRegime,market_open_count:0,buy_count:0,watch_count:0}); } catch {} }
        }
      }
      // US 21:00 UTC
      if (_dh === 21 && _dm < 5) {
        const _uk = `daily_us_${_dd}`;
        let _sent = false;
        if (supabase) { try { const {data:_d} = await supabase.from('hunter_calibration').select('id').eq('id',_uk).single(); _sent=!!_d; } catch {} }
        if (!_sent) {
          await fireDailyLeaderboard('us');
          if (supabase) { try { await supabase.from('hunter_calibration').upsert({id:_uk,recorded_at:new Date().toISOString(),top_score:0,top_symbol:'DAILY_US',field_avg:0,candidate_count:0,btc_regime:STATE.btcRegime,qqq_regime:STATE.qqqRegime,market_open_count:0,buy_count:0,watch_count:0}); } catch {} }
        }
      }
    }
