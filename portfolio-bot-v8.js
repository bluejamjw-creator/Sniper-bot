...
async function fireDailyLeaderboard(market) {
  ...
  // Status based on Hunter score trend across last 4 snapshots.
  // Labels describe SCORE momentum, not price direction.
  // Score reflects: volume, structure, breadth, EMA positioning.
  // A rising score means conditions are improving — not that price is up today.
  function getStatus(snapshots) {
    if (snapshots.length < 2) return { label: 'New entry',      emoji: '🆕' };
    ...
    if (allUp  && delta >= 4)  return { label: 'Conditions improving strongly', emoji: '🔥' };
    if (allUp  && delta >= 1)  return { label: 'Conditions improving',          emoji: '📈' };
    if (allDown && delta <= -4) return { label: 'Conditions deteriorating',     emoji: '🚨' };
    if (allDown && delta <= -1) return { label: 'Conditions weakening',         emoji: '📉' };
    if (Math.abs(delta) <= 0.5) return { label: 'Conditions stable',           emoji: '➡️' };
    if (delta > 0)              return { label: 'Conditions picking up',        emoji: '📈' };
    return                             { label: 'Conditions cooling',           emoji: '⚠️' };
  }
  ...
  try {
    ...
    const briefNote = {
      crypto: `Hunter's top 5 crypto by momentum score.`,
      us:     `Hunter's top 5 US stocks by momentum score.`,
      lse:    `Hunter's top 5 LSE stocks by momentum score.`,
    }[market] || '';
...
async function mainCycle() {
  ...
  try {
    ...
    if (buys.length === 0 && noTradeAge >= CONFIG.NO_TRADE_COOLDOWN_MS) {
      ...
      if (shouldSend) {
        // fireLeaderboardMessage suppressed
