// ============================================================
// THE HUNTER â€” BareTradeSignals Opportunity Engine
// Deploys as: hunter.js (or map your start command to this file)
...
const ENV = {
  ...
  // Accept both ALPACA_API_KEY and ALPACA_KEY
  ALPACA_API_KEY:         process.env.ALPACA_API_KEY || process.env.ALPACA_KEY,
  ALPACA_SECRET_KEY:      process.env.ALPACA_SECRET_KEY || process.env.ALPACA_SECRET,
...
// ============================================================
// SECTION 6: DATA FETCHERS â€” CRYPTO (Bybit primary, Binance fallback)
// NOTE: Railway IPs are US-based; Binance blocks US ranges.
//       Bybit has no geo-restrictions â€” it is the primary source.
// ============================================================
...
// CoinGecko simple price â€” used for BTC regime + breadth fallback
...
// CoinGecko OHLC â€” 1 day of hourly data for regime detection
async function fetchCoinGeckoOHLC(coinId, days = 2) {
  try {
    const key = ENV.COINGECKO_API_KEY ? `?x_cg_demo_api_key=${ENV.COINGECKO_API_KEY}&` : '?';
    const url = `https://api.coingecko.com/api/v3/coins/${coinId}/ohlc${key}vs_currency=usd&days=${days}`;
    const raw = await httpGet(url);
    if (!Array.isArray(raw) || raw.length === 0) return null;
    return raw.map((c) => ({
      time:   c[0],
      ...
      low:    c[3],
      close:  c[4],
      volume: 1, // CoinGecko OHLC doesn't include volume
    }));
  } catch {
    return null;
  }
...
// Bybit symbol â†’ CoinGecko id mapping for top assets
...
// ============================================================
// SECTION 7: DATA FETCHERS â€” US STOCKS (Alpaca primary)
// ============================================================
...
// ============================================================
// SECTION 8: DATA FETCHERS â€” MARKET CONTEXT
...
async function fetchBTCRegime() {
  try {
    ...
    if (!candles || candles.length < 20) {
      log(`âš ï¸  BTC regime: no data from any source â€” keeping ${STATE.btcRegime}`);
    ...
    log(`âš ï¸  BTC regime error: ${err.message}`);
    return STATE.btcRegime || 'NEUTRAL';
  }
}
...
async function fetchQQQRegime() {
  try {
    ...
    if (!candles || candles.length < 20) {
      log(`âš ï¸  QQQ regime: no data from any source â€” keeping ${STATE.qqqRegime}`);
    ...
    log(`âš ï¸  QQQ regime error: ${err.message}`);
    return STATE.qqqRegime || 'NEUTRAL';
  }
}
...
async function fetchCryptoBreadth() {
  ...
  if (checked === 0) {
    log(`âš ï¸  Breadth: no data from any source â€” keeping ${STATE.cryptoBreadth}%`);
...
// ============================================================
// SECTION 10: ENGINE 1 â€” MARKET CONTEXT (weight 0.08)
// ============================================================
...
// ============================================================
// SECTION 11: ENGINE 2 â€” STRUCTURE (weight 0.12)
// ============================================================
...
function scoreStructure(candles) {
  ...
  if (bbLast.width !== null) {
    ...
    if (bbLast.width < avgWidth * 0.7) {
      score_parts.push(80);
      reasons.push('BB compression â€” coiling for move');
    } else if (bbLast.width > avgWidth * 1.5) {
      score_parts.push(40);
      reasons.push('BB expanded â€” volatile');
    } else {
      score_parts.push(60);
    }
  }
  ...
  if (nearResistance) { score_parts.push(35); reasons.push('Near resistance â€” caution'); }
...
// ============================================================
// SECTION 12: ENGINE 3 â€” PARTICIPATION (weight 0.13)
// ============================================================
...
function scoreParticipation(candles) {
  ...
  if (relVol >= 3) { score_parts.push(95); reasons.push(`Relative volume ${round2(relVol)}x â€” exceptional`); }
  else if (relVol >= 2) { score_parts.push(80); reasons.push(`Relative volume ${round2(relVol)}x â€” strong`); }
  ...
  else { score_parts.push(20); reasons.push('Low dollar volume â€” liquidity risk'); }
...
// ============================================================
// SECTION 13: ENGINE 4 â€” MOMENTUM (weight 0.12)
// ============================================================
...
function scoreMomentum(candles) {
  ...
  if (rsiLast >= 55 && rsiLast <= 75) { score_parts.push(80); reasons.push(`RSI ${round2(rsiLast)} â€” bullish zone`); }
  else if (rsiLast > 75) { score_parts.push(45); reasons.push(`RSI ${round2(rsiLast)} â€” overbought`); }
  else if (rsiLast >= 45) { score_parts.push(55); }
  else { score_parts.push(25); reasons.push(`RSI ${round2(rsiLast)} â€” weak`); }
  ...
  if (roc20 > 15) { score_parts.push(30); reasons.push('Potentially extended â€” 20-period move large'); }
  else if (roc20 > 8) { score_parts.push(55); }
  else { score_parts.push(70); reasons.push('Fresh move â€” not extended'); }
...
// ============================================================
// SECTION 14: ENGINE 5 â€” STAGE CLASSIFICATION (weight 0.08)
// ============================================================
...
// ============================================================
// SECTION 15: ENGINE 6 â€” EXECUTION QUALITY (weight 0.22 â€” highest)
// ============================================================
...
function scoreExecutionQuality(candles) {
  ...
  if (pctAboveEMA >= 0 && pctAboveEMA <= 1.5) {
    score_parts.push(90);
    reasons.push('Tight to EMA20 â€” ideal entry zone');
  } else if (pctAboveEMA > 1.5 && pctAboveEMA <= 3) {
    score_parts.push(65);
    reasons.push('Slightly extended from EMA20');
  } else if (pctAboveEMA > 3) {
    score_parts.push(30);
    reasons.push(`Extended ${round2(pctAboveEMA)}% from EMA20 â€” risky entry`);
  } else {
    score_parts.push(40);
    reasons.push('Below EMA20 â€” structure weak');
  }
  ...
  if (pctAboveVWAP >= 0 && pctAboveVWAP <= 1.5) {
    score_parts.push(85);
    reasons.push('Close to VWAP â€” good R:R anchor');
  } else if (pctAboveVWAP > 1.5 && pctAboveVWAP <= 3) {
    score_parts.push(60);
  } else if (pctAboveVWAP > 3) {
    score_parts.push(25);
    reasons.push(`${round2(pctAboveVWAP)}% above VWAP â€” stretched`);
  } else {
    score_parts.push(50);
  }
  ...
  if (rrRatio >= 2.5) {
    score_parts.push(90);
    reasons.push(`R:R ${round2(rrRatio)}:1 â€” excellent`);
  } else if (rrRatio >= 1.8) {
    score_parts.push(70);
    reasons.push(`R:R ${round2(rrRatio)}:1 â€” acceptable`);
  } else if (rrRatio >= 1.3) {
    score_parts.push(45);
    reasons.push(`R:R ${round2(rrRatio)}:1 â€” marginal`);
  } else {
    score_parts.push(15);
    reasons.push(`R:R ${round2(rrRatio)}:1 â€” poor`);
  }
  ...
  if (closePosition >= 0.7 && upperWick <= 0.2) {
    ...
    score_parts.push(25);
    reasons.push('Weak candle close â€” selling pressure visible');
  }
...
// ============================================================
// SECTION 15b: EXECUTION ENGINE â€” ORDER TYPE & ENTRY METHOD
// ============================================================
...
const MAX_SPREAD_FOR_MARKET = 0.004; // 0.4% â€” reject MARKET if spread exceeds this
...
function buildExecutionPlan(candles, setupType, execution, assetType) {
  ...
  switch (setupType) {
    ...
    case SETUP_TYPES.BREAKOUT_CONTINUATION: {
      ...
      executionPlan   = `Place BUY STOP at $${entryPrice} (just above resistance $${round2(nearestResistance)}). Only fills if level actually breaks. Cancel immediately if price closes back below resistance. Do not anticipate â€” wait for confirmation.`;
      executionScore  = 75;
      break;
    }
    ...
    case SETUP_TYPES.MOMENTUM_IGNITION: {
      if (spread > MAX_SPREAD_FOR_MARKET) {
        rejectEntry    = true;
        rejectReason   = `Spread ${round2(spread * 100)}% exceeds MARKET order threshold â€” execution quality too poor`;
        ...
        chaseLimit     = null;
        cancelCondition = `Abort if spread widens above 0.4% at execution. Momentum decays fast â€” if not in within 1 candle, reassess.`;
        executionPlan   = `Enter at MARKET immediately. Spread ${round2(spread * 100)}% â€” within threshold. Speed is priority. Abort if spread widens at execution.`;
        executionScore  = 70;
      }
      break;
    }
    ...
    case SETUP_TYPES.PARABOLIC_EXTENSION: {
      rejectEntry    = true;
      rejectReason   = 'Parabolic extension â€” trade already happened. Wait for pullback to EMA20/VWAP before considering entry.';
      orderType      = ORDER_TYPES.NO_ENTRY;
      entryPrice     = null;
      chaseLimit     = null;
      cancelCondition = 'N/A â€” no entry on parabolic extension';
...
// ============================================================
// SECTION 16: ENGINE 7 â€” RISK (weight 0.10)
// ============================================================
...
function scoreRisk(candles, assetType) {
  ...
  if (avgDollarVol >= CONFIG.MIN_DOLLAR_VOLUME * 5) {
    ...
    score_parts.push(20);
    reasons.push('Low liquidity â€” execution risk');
  }
  ...
  else { score_parts.push(30); reasons.push('Wide spread â€” slippage risk'); }

  // Volatility (ATR %)
  if (atrPct <= 1.5) { score_parts.push(80); reasons.push('Low ATR â€” controlled volatility'); }
  else if (atrPct <= 3) { score_parts.push(60); }
  else if (atrPct <= 5) { score_parts.push(40); }
  else { score_parts.push(15); reasons.push(`High ATR ${round2(atrPct)}% â€” elevated risk`); }
...
// ============================================================
// SECTION 17: ENGINE 8 â€” DECISION STABILITY
// ============================================================

function scoreDecisionStability(candles, baseScores) {
  // Simulate adverse scenarios â€” does the setup survive?
  const reasons = [];
...
// ============================================================
// SECTION 18: ENGINE 9 â€” EDGE SCORE
// ============================================================
...
function scoreEdge(hunterScore) {
  ...
  let score = 50;
  if (edge >= 25) { score = 95; reasons.push(`${round2(edge)} points above field average â€” exceptional edge`); }
  else if (edge >= CONFIG.THRESHOLDS.edgeMinimumOverField) { score = 75; reasons.push(`${round2(edge)} points above field â€” solid edge`); }
  else if (edge >= 5) { score = 55; reasons.push(`${round2(edge)} points above field â€” marginal edge`); }
  else { score = 25; reasons.push(`Only ${round2(edge)} points above field â€” insufficient edge`); }

  return { score: clamp(score), edge, reasons };
}

// ============================================================
// SECTION 19: ENGINE 10 â€” COMPETITION ENGINE
// ============================================================
...
function scoreCompetition(symbol, hunterScore) {
  ...
  if (rank === 1 && gap >= CONFIG.THRESHOLDS.competitionGap) {
    ...
    score = 45;
    reasons.push(`Ranked #${rank} â€” not clear best`);
  } else {
    score = 20;
    reasons.push(`Ranked #${rank} â€” better setups exist`);
  }
...
// ============================================================
// SECTION 20: ENGINE 11 â€” CONFIDENCE
// ============================================================
...
function scoreConfidence(allScores) {
  ...
  else reasons.push('Mixed signals â€” confidence reduced');

  return { score: clamp(alignmentScore), stdDev, reasons };
}

// ============================================================
// SECTION 21: ENGINE 12 â€” CONVICTION
// ============================================================
...
function scoreConviction(allScores, stage, stability) {
  ...
  if (!passesFinalTest) {
    ...
    reasons.push('FAILS final conviction test â€” would not be happy with loss tomorrow');
  } else {
...
async function scoreAsset(symbol, assetType) {
  try {
    ...
    // Weighted composite (without edge/competition â€” those need field context)
...
async function injectDynamicAssets() {
  try {
    // Use Bybit 24hr tickers â€” no geo-restrictions
    ...
    if (data?.result?.list) {
      ...
      for (const mover of movers) {
        ...
        if (!CORE_CRYPTO.includes(mover.symbol) && !STATE.dynamicAssets.has(mover.symbol)) {
          ...
          log(`ðŸ”¥ Dynamic inject: ${mover.symbol} (+${pct}%)`);
        }
      }
    }
...
function buildHunterAlert(candidate, scan) {
  ...
  // â”€â”€ Grade â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const isElite   = finalScore >= CONFIG.THRESHOLDS.hunterScoreElite;
  const gradeIcon = isElite ? 'ðŸ’ŽðŸ”¥' : 'ðŸ”¥';
  const gradeLabel = isElite ? 'A* SETUP DETECTED' : 'A SETUP DETECTED';

  // â”€â”€ Flag + market label â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const flagMap = { crypto: 'ðŸŒ', us: 'ðŸ‡ºðŸ‡¸', lse: 'ðŸ‡¬ðŸ‡§' };
  const marketMap = { crypto: 'CRYPTO', us: 'US', lse: 'LSE' };
  const flag   = flagMap[assetType]   || 'ðŸŒ';
  const market = marketMap[assetType] || assetType.toUpperCase();

  // â”€â”€ Prices â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Setup type â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Candle momentum â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Regime label â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const regimeLabel = assetType === 'crypto'
    ? STATE.btcRegime.toLowerCase()
    : STATE.qqqRegime.toLowerCase();

  // â”€â”€ Setup analysis text â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const setupAnalysis = {
    PULLBACK_CONTINUATION: `Pullback Continuation â€” algorithm identified a pullback to the 20-period EMA within an uptrend. Scanner flagged potential continuation from this level.`,
    BREAKOUT_CONTINUATION: `Breakout Continuation â€” price is testing a key resistance level with volume confirmation. Scanner flagged a potential breakout entry above structure.`,
    MOMENTUM_IGNITION:     `Momentum Ignition â€” algorithm detected a surge in volume and price velocity. Scanner flagged an accelerating move with strong participation.`,
    PARABOLIC_EXTENSION:   `Parabolic Extension â€” no entry. Trade has already happened. Waiting for pullback or consolidation.`,
  };
  const analysis = setupAnalysis[setupType] || setupAnalysis.PULLBACK_CONTINUATION;

  // â”€â”€ Grade descriptor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  const gradeDesc = isElite
    ? 'A* grade â€” all algorithmic conditions aligned.'
    : 'A grade â€” strong setup, all primary conditions met.';

  // â”€â”€ Execution note â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const lines = [
    `ðŸš¨ ${gradeIcon} ${gradeLabel}`,
    ``,
    `${flag} ${market} | ${symbol}`,
    `ðŸŽ¯ ${setupLabel}`,
    ``,
    `ðŸ’° Entry zone $${entry} (${orderLabel.toLowerCase()} area)`,
    `ðŸ›¡ï¸ SL:    $${stop} (${slPct}%)`,
    `ðŸŽ¯ TP1:   $${tp1} (+${tp1Pct}%)`,
    `ðŸš€ TP2:   $${tp2} (+${tp2Pct}%)`,
    `âš–ï¸ R:R:   ${rr}`,
    `â­ Score: ${round2(finalScore)}`,
    `ðŸ“Š Vol:   ${relVol}x`,
    ``,
    `ðŸ“š <b>SETUP ANALYSIS</b>`,
    ``,
    `ðŸŽ¯ ${analysis}`,
    ``,
    `ðŸ“Š Volume ${relVol}x â€” ${volDesc} relative volume. Strong participation detected.`,
    ``,
    `${flag} Market regime: ${regimeLabel}`,
    ``,
    `â­ ${gradeDesc}`,
    ``,
    `âš¡ Candle momentum: ${candleMom > 0 ? '+' : ''}${candleMom}%`,
    ``,
    `ðŸ“‹ Execution: ${execNote}`,
    ``,
    ...
    ``,
    `â€”â€”â€”â€”â€”â€”â€”â€”â€”â€”`,
    `âš ï¸ Educational market commentary only Â· Not personalised investment advice Â· Not a recommendation to buy or sell`,
    `ðŸ“¸ Analysis by @baretradesignals`,
  ];
...
function buildNoTradeMessage(scan) {
  const totalScanned = scan?.totalAssets || 'â€”';
  const cycles = STATE.cycleCount;
  return [
    `â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”`,
    `ðŸ•µï¸ THE HUNTER â€” NO TRADE`,
    `â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”`,
    ``,
    ...
    ``,
    `Every candidate was measured across 12 engines. None cleared all gates. The field average sits at <b>${round2(STATE.fieldAverage)}</b> â€” below the conviction threshold.`,
    ``,
...
async function fireAlert(candidate, scan) {
  if (!checkAlertCooldown()) {
    log(`Alert suppressed â€” cooldown/daily limit active`);
    return;
  }
  ...
  log(`âœ… Alert fired for ${candidate.symbol} (score ${round2(candidate.finalScore)})`);
}

async function fireNoTradeMessage(scan) {
  const msg = buildNoTradeMessage(scan);
  if (ENV.BLUEJAM_CHAT_ID) await sendTelegram(msg, ENV.BLUEJAM_CHAT_ID);
  log('ðŸ“­ No trade message sent');
}
...
async function storeAlert(candidate, scan) {
  ...
  try {
    ...
    else log(`ðŸ“š Alert stored in Supabase`);
  } catch (err) {
...
async function trackOutcomes() {
  ...
  try {
    ...
    log(`ðŸ“Š Outcome tracking complete`);
  } catch (err) {
...
async function mainCycle() {
  STATE.cycleCount++;
  log(`\nâ•â•â•â• CYCLE ${STATE.cycleCount} â•â•â•â•`);
  ...
  try {
    ...
    if (buys.length > 0) {
      ...
      const best = buys[0];
      log(`ðŸŽ¯ BUY candidate: ${best.symbol} (score ${round2(best.finalScore)})`);

      // Final sanity â€” competition check
      ...
      if (gap >= CONFIG.THRESHOLDS.competitionGap) {
        ...
        log(`Competition gap too small (${round2(gap)}) â€” holding fire`);
      }
    } else if (scan.watches?.length > 0) {
      log(`ðŸ‘ ${scan.watches.length} assets on WATCH â€” not firing`);
    } else {
...
async function start() {
  ...
  log('â•‘     THE HUNTER â€” v1.0.0          â•‘');
  log('â•‘     BareTradeSignals Engine      â•‘');
  log('â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•');
  log('');
  log(`Mode: ${ENV.HUNTER_LIVE ? 'ðŸ”´ LIVE' : 'ðŸŸ¡ BLUEJAM TEST'}`);
  ...
  // ENV diagnostics
  log('ENV CHECK:');
  log(`  BOT_TOKEN        : ${ENV.BOT_TOKEN        ? 'âœ…' : 'âŒ missing'}`);
  log(`  BLUEJAM_CHAT_ID  : ${ENV.BLUEJAM_CHAT_ID  ? 'âœ…' : 'âŒ (tried BLUEJAM_CHAT_ID + CHAT_ID)'}`);
  log(`  CHANNEL_CHAT_ID  : ${ENV.CHANNEL_CHAT_ID  ? 'âœ…' : 'âšª not set'}`);
  log(`  ALPACA_API_KEY   : ${ENV.ALPACA_API_KEY   ? 'âœ…' : 'âŒ missing'}`);
  log(`  ALPACA_SECRET    : ${ENV.ALPACA_SECRET_KEY ? 'âœ…' : 'âŒ missing'}`);
  log(`  FINNHUB_API_KEY  : ${ENV.FINNHUB_API_KEY  ? 'âœ…' : 'âšª not set'}`);
  log(`  COINGECKO_KEY    : ${ENV.COINGECKO_API_KEY ? 'âœ…' : 'âšª not set (free tier used)'}`);
  log(`  SUPABASE_URL     : ${ENV.SUPABASE_URL     ? 'âœ…' : 'âŒ missing'}`);
  log(`  SUPABASE_KEY     : ${ENV.SUPABASE_SERVICE_KEY ? 'âœ…' : 'âŒ (tried SERVICE_ROLE_KEY + SERVICE_KEY + ANON_KEY)'}`);
  log(`  HUNTER_LIVE      : ${ENV.HUNTER_LIVE      ? 'ðŸ”´ LIVE' : 'ðŸŸ¡ bluejam only'}`);
  log('');

  if (!ENV.BOT_TOKEN) log('âš ï¸  WARNING: BOT_TOKEN not set â€” Telegram disabled');
  if (!supabase) {
    log('âš ï¸  WARNING: Supabase not configured â€” learning engine disabled');
    log('         Check SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in Railway vars');
  }

  // â”€â”€ DATA HEALTH CHECK â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  log('ðŸ”¬ Running data health check...');
  let healthPassed = 0;
  ...
  try {
    ...
    if (bybitTest && bybitTest.length > 0) {
      log(`  Bybit        : âœ… (BTC last: $${round2(bybitTest[bybitTest.length-1].close)})`);
      healthPassed++;
    } else {
      log('  Bybit        : âŒ returned no data');
    }
  } catch (e) { log(`  Bybit        : âŒ ${e.message}`); }

  // Test Alpaca
  try {
    const alpacaTest = await fetchAlpacaBars('QQQ', '15Min', 5);
    if (alpacaTest && alpacaTest.length > 0) {
      log(`  Alpaca       : âœ… (QQQ last: $${round2(alpacaTest[alpacaTest.length-1].close)})`);
      healthPassed++;
    } else {
      log('  Alpaca       : âŒ returned no data');
    }
  } catch (e) { log(`  Alpaca       : âŒ ${e.message}`); }

  // Test Yahoo
  try {
    const yahooTest = await fetchYahooCandles('SPY', '15m');
    if (yahooTest && yahooTest.length > 0) {
      log(`  Yahoo        : âœ… (SPY last: $${round2(yahooTest[yahooTest.length-1].close)})`);
      healthPassed++;
    } else {
      log('  Yahoo        : âŒ returned no data');
    }
  } catch (e) { log(`  Yahoo        : âŒ ${e.message}`); }

  // Test CoinGecko
  try {
    const cgTest = await fetchCoinGeckoPrice('bitcoin');
    if (cgTest?.usd) {
      log(`  CoinGecko    : âœ… (BTC: $${round2(cgTest.usd)}, 24h: ${round2(cgTest.usd_24h_change)}%)`);
      healthPassed++;
    } else {
      log('  CoinGecko    : âŒ returned no data');
    }
  } catch (e) { log(`  CoinGecko    : âŒ ${e.message}`); }

  // Test Binance (may fail on Railway â€” that's expected)
  try {
    const binanceTest = await fetchBinanceCandles('BTCUSDT', '15m', 5);
    if (binanceTest && binanceTest.length > 0) {
      log(`  Binance      : âœ… (accessible from this server)`);
    } else {
      log('  Binance      : âš ï¸  no data (likely geo-blocked â€” Bybit handles crypto instead)');
    }
  } catch (e) { log(`  Binance      : âš ï¸  ${e.message} (expected on Railway US IPs)`); }

  log(`Health check: ${healthPassed}/4 primary sources online`);
  if (healthPassed === 0) {
    log('ðŸš¨ CRITICAL: No data sources available. Check network settings.');
  }
  log('');
