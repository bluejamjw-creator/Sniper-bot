...
function buildHunterAlert(candidate, scan) {
  ...
  const setupAnalysis = {
    PULLBACK_CONTINUATION: 'Pullback continuation into the 20 EMA within an established uptrend.',
    BREAKOUT_CONTINUATION: 'Breakout continuation above a key resistance level with volume confirmation.',
    MOMENTUM_IGNITION:     'Momentum ignition detected — surge in volume and price velocity.',
    PARABOLIC_EXTENSION:   'Parabolic extension — no entry. Waiting for pullback or consolidation.',
  };
  ...
  const execBullets = [];
  if (orderLabel === 'LIMIT') {
    execBullets.push(`• Buy Limit: $${entry}`);
    if (execEngine?.chaseLimit) execBullets.push(`• Do not chase above $${round2(execEngine.chaseLimit)}`);
    execBullets.push(`• Cancel if price closes above $${round2(entry * 1.015)} before entry`);
    execBullets.push(`• Setup expires at end of session if unfilled`);
  } else if (orderLabel === 'STOP') {
    execBullets.push(`• Buy Stop: $${entry}`);
    execBullets.push(`• Only fills if resistance actually breaks`);
    execBullets.push(`• Cancel if price closes back below entry level`);
  } else {
    execBullets.push(`• Market entry — act immediately`);
    execBullets.push(`• Abort if spread widens at execution`);
  }
  ...
  const lines = [
    ...
    `🎯 ${setupLabel}`,
    ``,
    `💰 Entry: $${entry} (${orderLabel.toLowerCase()} area)`,
    `🛡️ Stop Loss: $${stop} (${slPct}%)`,
    `🎯 TP1: $${tp1} (+${tp1Pct}%)`,
    `🚀 TP2: $${tp2} (+${tp2Pct}%)`,
    `⚖️ Risk / Reward: ${rr}`,
    ...
    `🎯 ${analysis}`,
    ``,
    `📊 Strong participation (${relVol}× average volume).`,
...
function buildNoTradeMessage(scan) {
  ...
  // What's missing — identify the dominant rejection reason
  const missingLines = [];
  ...
  if (bestSymbol && bestScore) {
    lines.push(`🏆 Strongest asset:`);
    lines.push(`${bestSymbol}`);
    lines.push(`⭐ Score: ${bestScore}`);
  }
...
async function fireLeaderboardMessage(scan) {
  ...
  // Regime labels — consistent ALL CAPS with emoji
  const qqqLabel = STATE.qqqRegimeStale
    ? '⚪ CLOSED (last: ' + (STATE.qqqRegime || 'unknown') + ')'
    : STATE.qqqRegime === 'BULL' ? '🟢 BULL' : STATE.qqqRegime === 'BEAR' ? '🔴 BEAR' : '🟡 NEUTRAL';
  const btcLabel = STATE.btcRegime === 'BULL' ? '🟢 BULL' : STATE.btcRegime === 'BEAR' ? '🔴 BEAR' : '🟡 NEUTRAL';

  const rankEmoji = ['1️⃣','2️⃣','3️⃣','4️⃣','5️⃣'];

  const lines = [
    `🏹 HUNTER LEADERBOARD — ${timeStr}`,
    ``,
    `📊 QQQ: ${qqqLabel}`,
    `₿ BTC: ${btcLabel}`,
    ``,
  ...
  // Compact ranked list — velocity appended when available
  if (candidates.length > 0) {
    candidates.forEach((c, i) => {
      ...
      const tag     = score >= CONFIG.THRESHOLDS.hunterScoreGood ? '🔥 BUY'
                    ...
                    : score >= 65 ? '📡 CLOSE' : '💤 FORMING';
      const mIcon   = (c.assetType || c.type) === 'crypto' ? '🪙' : '📈';
      // Score velocity — show if watchlist delta exists
      const tracked = STATE.watchlistState.get(c.symbol);
      let velStr = '';
      if (tracked?.createdScore != null) {
        const delta = round2(score - tracked.createdScore);
        velStr = delta > 0 ? `   ▲+${delta}` : delta < 0 ? `   ▼${delta}` : '';
      }
      lines.push(`${rankEmoji[i] || `${i+1}.`} ${mIcon} ${c.symbol}   ⭐${score}  ${tag}${velStr}`);
    });
  } else {
  ...
  if (candidates.length > 0) {
    ...
    // Refinement: single line, not two lines
    lines.push(`🏆 Strongest Leader: ${best.symbol}`);
    lines.push(``);

    // Fastest accelerator — only shown when delta data exists
    let fastestSymbol = null, fastestDelta = 0;
    for (const c of candidates) {
      const tracked = STATE.watchlistState.get(c.symbol);
      if (tracked?.createdScore != null) {
        const delta = round2((c.finalScore || 0) - tracked.createdScore);
        if (delta > fastestDelta) { fastestDelta = delta; fastestSymbol = c.symbol; }
      }
    }
    if (fastestSymbol) {
      lines.push(`⚡ Fastest Accelerator: ${fastestSymbol} (▲ +${fastestDelta})`);
      lines.push(``);
    }

    // Market breadth — use full universe count, not just top 5
    const totalScored = allScored.length;
    const improving = allScored.filter(c => {
      const tracked = STATE.watchlistState.get(c.symbol);
      return tracked?.createdScore != null && (c.finalScore || 0) > tracked.createdScore;
    }).length;
    if (totalScored > 0) {
      lines.push(`📈 Market Breadth: ${improving} / ${totalScored} tracked assets improving`);
    } else {
      lines.push(`📈 Market Breadth: ${improving} of today's Top 5 improving`);
    }
    lines.push(``);

    // BUY status
    const hasBuy = candidates.some(c => (c.finalScore || 0) >= CONFIG.THRESHOLDS.hunterScoreGood);
    if (hasBuy) {
      lines.push(`🔥 BUY candidate confirmed — see alert`);
    } else {
      lines.push(`⏳ No BUY candidates yet.`);
      lines.push(`Best: ${candidates[0]?.symbol} (${round2(candidates[0]?.finalScore || 0)})`);
    }
  }
  ...
  // ── Leaderboard history persistence — FULL UNIVERSE ─────────────────
  // Write EVERY scored asset to Supabase, not just top 5.
  // Telegram gets top 5 only. Supabase gets the complete ranking so
  // weekly league tables, consistency scores, and rank velocity can
  // be computed later. ~58 rows/snapshot x 6/day = ~350 rows/day.
...
async function fireWeeklyReport() {
  if (!supabase) {
    log('⚠️  Weekly report skipped — no Supabase');
    return;
  }

  log('📅 Building weekly Crypto Cycle Planner report...');

  // Fetch last 7 days of leaderboard history for the 11 coins
  ...
  if (error || !rows || rows.length === 0) {
    log(`⚠️  Weekly report: no data (${error?.message || 'empty result'})`);
    return;
  }

  // Count distinct snapshots for denominator
  const allSnapshots = new Set(rows.map(r => r.snapshot_id));
  const totalSnapshots = allSnapshots.size;
  log(`📅 Weekly report: ${rows.length} rows across ${totalSnapshots} snapshots`);

  // Build per-coin stats
  const coinData = {};
  for (const coin of CYCLE_PLANNER_COINS) {
    const coinRows = rows.filter(r => r.symbol === coin).sort((a, b) =>
    ...
    // Rotation: compare first-half avg vs second-half avg
    const mid          = Math.floor(scores.length / 2);
    const firstHalf    = scores.slice(0, mid);
    const secondHalf   = scores.slice(mid);
    const firstAvg     = firstHalf.length ? firstHalf.reduce((a,b)=>a+b,0)/firstHalf.length : avgScore;
    const secondAvg    = secondHalf.length ? secondHalf.reduce((a,b)=>a+b,0)/secondHalf.length : avgScore;
    ...
    coinData[coin] = {
      appearances: coinRows.length,
      top5count:   top5rows.length,
      rank1count:  rank1rows.length,
      avgScore, avgRank, peakScore,
      streak, trend, persistence,
      rotation, direction,
    };
  }

  // Sort coins by persistence score descending
  ...
  const lines = [
    ...
    ``,
    `📊 QQQ: ${STATE.qqqRegimeStale ? '⚪ CLOSED' : STATE.qqqRegime === 'BULL' ? '🟢 BULL' : STATE.qqqRegime === 'BEAR' ? '🔴 BEAR' : '🟡 NEUTRAL'}`,
    `₿ BTC: ${STATE.btcRegime === 'BULL' ? '🟢 BULL' : STATE.btcRegime === 'BEAR' ? '🔴 BEAR' : '🟡 NEUTRAL'}`,
    `📅 ${totalSnapshots} snapshots over 7 days`,
    ``,
    `━━━━━━━━━━━━━━━━━━`,
    `🏆 RANKINGS`,
    ``,
  ...
  sorted.forEach((coin, i) => {
    ...
    const dirStr     = d.direction === 'Improving' ? '📈' : d.direction === 'Weakening' ? '📉' : '➡️';

    // Strip USDT suffix for display
    const label = coin.replace('USDT', '');

    lines.push(`${rankEmoji[i] || `${i+1}.`} ${label}`);
    lines.push(`   Persistence: ${d.persistence}/100  Avg Score: ${d.avgScore}`);
    lines.push(`   Top 5: ${d.top5count}/${totalSnapshots}  #1 Finishes: ${d.rank1count}  Avg Rank: ${d.avgRank}`);
    lines.push(`   Streak: ${streakStr}  Rotation: ${rotStr}  ${dirStr} ${d.direction}`);
    lines.push(``);
  });

  // Missing coins (no data this week)
  const missing = CYCLE_PLANNER_COINS.filter(c => coinData[c] === null);
  if (missing.length > 0) {
    lines.push(`━━━━━━━━━━━━━━━━━━`);
    lines.push(`⚠️ No data this week: ${missing.map(c => c.replace('USDT','')).join(', ')}`);
    lines.push(``);
  }
  ...
  if (streakers.length) lines.push(`🔥 Momentum Streaks (3+): ${streakers.join('  ')}`);
  if (inbound.length) lines.push(`🔼 Capital Rotating In: ${inbound.join('  ')}`);
  if (outbound.length) lines.push(`🔽 Rotating Out: ${outbound.join('  ')}`);
  if (improving.length) lines.push(`📈 Improving This Week: ${improving.join('  ')}`);
  ...
  log('📅 Weekly Crypto Cycle Planner report sent');
}
...
async function sendOutcomeAlert(alert, outcomeType, hitPrice, pctMove) {
  ...
  const label     = outcomeType === 'TP2' ? 'TP2 HIT' : outcomeType === 'TP1' ? 'TP1 HIT' : 'STOPPED OUT';
  ...
  const insight = isWin
    ? `Leadership remained intact after entry, supporting the continuation thesis.`
    : `Setup did not follow through. Leadership thesis invalidated.`;
  ...
  try {
    ...
    // Railway restarts or multiple instances don't double-fire.
    {
      ...
      if (_day === 5 && _hour >= 11 && _hour < 13) { // 11:00-13:00 UTC = 12:00-14:00 BST (UK Friday lunchtime)
        ...
        if (!_alreadySent) {
          log('📅 Friday window — firing weekly Crypto Cycle Planner report');
