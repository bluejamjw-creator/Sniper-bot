...
const CORE_CRYPTO = [
  // Tier A — core liquidity anchors
  'BTCUSDT','ETHUSDT','SOLUSDT',
  // Tier B — liquid majors with real institutional flow
  'BNBUSDT','XRPUSDT','LINKUSDT','AVAXUSDT','SUIUSDT','HYPEUSDT',
  // Tier C — narrative-driven, thinner liquidity, smaller allocation
  'AAVEUSDT','UNIUSDT','ONDOUSDT','MKRUSDT','TAOUSDT','RENDERUSDT',
];

const CORE_US_STOCKS = [
  // Tier A+ — AI infrastructure cluster (Hunter's strongest hunting ground)
  'NVDA','MSFT','AVGO','TSM','ANET','MRVL',
  // Tier A — relative strength + institutional sponsorship
  'AMD','AMZN','META','GOOG','PLTR','VRT','ORCL',
  // Tier B — liquid, real theme, slightly thinner sponsorship
  'DELL','MU','SMCI','ARM','INTC','AAPL',
  // Defense — clean institutional trend behavior
  'LMT','RTX','NOC','GD','BA',
  // Power / Grid infrastructure
  'GEV','CEG','VST','ETN','PWR',
  // Financials — added for regime detection (sector leadership/laggard signal)
  'JPM','GS',
  // Energy — added for regime detection
  'XOM',
];
...
// Records every fetch failure with full diagnostic detail. Surfaced
// via the leaderboard/diagnostic logs so outages are visible, not silent.
...
async function fetchBybitCandles(symbol, interval = '15', limit = 100) {
  try {
    ...
    // No throw, but empty/malformed response — still worth recording
...
// CoinGecko demo/free tier allows roughly 30 calls/min, but during an
// outage Hunter was calling it on every single cycle (every 90-180s)
// across multiple concurrent scan paths (BTC regime + breadth + any
// dynamic injection), compounding into the 429s seen in production.
// This gate enforces a minimum spacing between calls regardless of how
// many code paths want to call it — protects the rate limit even when
// Bybit/Binance are down and CoinGecko becomes the only crypto fallback.
...
// Tracks whether we've already warned about a misconfigured base URL —
// avoids spamming the same diagnostic every single cycle.
...
async function fetchAlpacaBars(symbol, timeframe = '15Min', limit = 100) {
  try {
    ...
    // Defend against the exact misconfiguration seen in production:
    // ALPACA_BASE_URL set to the TRADING API (paper-api.alpaca.markets)
    // instead of the MARKET DATA API (data.alpaca.markets). These are
    // different Alpaca products with different hosts — using the trading
    // host for market data calls produces a 404 on every request.
    if (base.includes('paper-api.alpaca.markets') || base.includes('api.alpaca.markets')) {
      ...
      base = 'https://data.alpaca.markets'; // auto-correct so Hunter keeps working regardless
    }
    // Also strip any trailing /v2 the env var might already include —
    // the code appends '/v2/stocks/...' itself, so a base ending in /v2
    // would otherwise double up to '/v2/v2/...' and 404.
...
// STATE.qqqRegimeStale flags when the current regime label is NOT fresh —
// either because US markets are closed (weekend/holiday) or because the
// last live fetch genuinely failed. Without this flag, Hunter happily
// displayed Friday's "BULL" label all through a closed weekend, since
// QQQ stops trading but the in-memory STATE.qqqRegime value just sits
// there unchanged. The leaderboard and any regime-dependent scoring
// should treat a stale QQQ regime as NEUTRAL, not as a live signal.
async function fetchQQQRegime() {
  // QQQ doesn't trade outside US market hours (incl. holidays). Don't
  // even attempt a fetch — there's nothing fresh to get, and re-using
  // the last known label would misrepresent it as current.
  if (!isMarketOpen('us')) {
  ...
  try {
    ...
    STATE.qqqRegimeStale = false; // fresh live data — label is current
    if (last > e20 && e20 > e50) return 'BULL';
    if (last < e20 && e20 < e50) return 'BEAR';
    return 'NEUTRAL';
  } catch (err) {
...
function scoreMarketContext(assetType) {
  ...
  // Skip entirely when stale (weekend/holiday/fetch failure) — a frozen
  // Friday "BULL" label shouldn't grant crypto a scoring bonus on a
  // Saturday just because nobody refreshed the number.
  ...
  // Session timing (UTC) — now routes through isMarketOpen() for a single
  // source of truth that includes holiday awareness. Previously this
  // duplicated the time-only check with no holiday calendar, meaning a
  // closed market (e.g. Juneteenth) still scored "+10 market open".
  if (assetType === 'us') {
    if (isMarketOpen('us')) {
...
// ============================================================
// Without this, isMarketOpen() treats every holiday as a normal trading
// day — scanning a frozen/closed market, burning API credits on stale
// data, and posting "live" intel that's actually yesterday's close.
// Crypto has no holidays (24/7/365) so no calendar needed there.
//
// Dates are 'YYYY-MM-DD' in UTC. Source: NYSE/Nasdaq official holiday
// schedule and London Stock Exchange holiday schedule.
// IMPORTANT: This list must be updated yearly. A stale list silently
// reverts to "no holiday awareness" for the missing year — same failure
// mode as having no calendar at all, but harder to notice.
...
// NYSE/Nasdaq early-close days (1pm ET close) — market still "open" but
// only for a shortened session. Not currently used as a hard block, but
// tracked here for future use (e.g. tightening the close-time check).
...
function isMarketOpen(assetType) {
  ...
  if (assetType === 'us') {
    if (isUSMarketHoliday(now)) return false; // NYSE/Nasdaq closed — saves scan credits
    return totalMin >= 13*60+30 && totalMin < 20*60;
  }
  if (assetType === 'lse') {
    if (isUKMarketHoliday(now)) return false; // LSE closed — saves scan credits
    return totalMin >= 8*60 && totalMin < 16*60+30;
  }
  return true; // crypto always open — no holidays
}
...
function buildExecutionPlan(candles, setupType, execution, assetType) {
  ...
  //
  // MULTIPLIER UPDATE (two independent research passes — Plex + GPT, both
  // converged on the same direction): widen TP1/TP2 relative to SL rather
  // than just scaling everything up. Old 1.5/2/4 gave TP1 R:R=1.33 (43%
  // breakeven win rate needed). New 1.8/3/6 gives TP1 R:R=1.67 (37.5%
  // breakeven) — same risk exposure per trade, meaningfully better reward
  // profile, well within both models' recommended "universal compromise"
  // range (Plex: 1.8-2.5x SL / GPT: 1.5-2.0x SL, both ~2.5-3x TP1).
  //
  // KNOWN REMAINING ISSUE (not fixed by this multiplier change alone):
  // candles passed into this function are 15-minute crypto candles, so
  // ATR(14) here measures volatility over ~3.5 hours, not a full session.
  // This is why BTC's stop showed as only 0.2% — the ATR INPUT is small,
  // not just the multiplier. Widening the multiplier helps but the
  // structural fix is using a longer-period ATR (e.g. 1h or 4h candles)
  // for the execution plan specifically, even though scoring still uses
  // 15m data. Flagged for a follow-up change — not done here because it
  // requires fetching a second candle set and more testing before going live.
  const entryRef   = entryPrice || execution.entry;
  const atrForStop = atr[atr.length - 1];
  const SL_ATR_MULT  = 1.8;  // was 1.5
  const TP1_ATR_MULT = 3.0;  // was 2.0
  const TP2_ATR_MULT = 6.0;  // was 4.0
...
async function runFullScan() {
  ...
  // Build universe — skip closed-market assets ENTIRELY here, not just in
  // scoring. Previously isMarketOpen() only affected the market-context
  // score, but scoreAsset() still fetched candles for every US/LSE symbol
  // even when the market was shut (holiday or outside hours). That means
  // scan credits were being burned 24/7 regardless of whether the market
  // was actually open. Filtering here means closed-market assets are
  // never fetched, never scored, never counted toward API usage.
  const usOpen  = isMarketOpen('us');
  const lseOpen = isMarketOpen('lse');
  if (!usOpen)  log('⏸️  US market closed (holiday or outside hours) — skipping 32 US assets, saving scan credits');
  if (!lseOpen) log('⏸️  LSE market closed (holiday or outside hours) — skipping 21 LSE assets, saving scan credits');

  const universe = [
    ...CORE_CRYPTO.map((s) => ({ symbol: s, type: 'crypto' })), // crypto always scanned — 24/7
  ...
  const finalCandidates = withCompetition.map((r) => {
    ...
    // PASS — apply competition bonus then tier
      ...
      else if (adjustedScore >= CONFIG.THRESHOLDS.hunterScoreWatch) decision = 'WATCH'; // 75+ watch (was 65+)
      else decision = 'WAIT';
    }
  ...
  //
  // FIXED: previously only logged the single field-wide top score and
  // its symbol. This made it IMPOSSIBLE to later ask "what was the
  // stock-only ceiling on these days?" — if crypto happened to be
  // scoring higher than stocks in a given cycle, the stock data was
  // invisible, since only the overall winner got recorded. Now logs
  // top score PER asset class separately, so any future backtest can
  // isolate crypto vs US vs LSE cleanly without this ambiguity.
  ...
  await safeRun('calibrationTracker', () => recordCalibrationSnapshot({
    ...
    // New: per-asset-class top scores, null if that class had no
    // scored candidates this cycle (e.g. US/LSE closed for holiday)
...
// ── Missing helper — caused "safeRun is not defined" crash in V8b ──
// safeRun existed in the Sniper bot but was never defined in Hunter.
// Added here: runs a function, catches any error, logs it, never throws.
...
async function recordCalibrationSnapshot(snapshot) {
  ...
  try {
    const row = {
      ...
      // New columns — see ALTER TABLE in calibration_tables.sql.
      // Nullable: a class may have no scored candidates this cycle
      // (e.g. US/LSE closed for weekend/holiday).
...
function buildHunterAlert(candidate, scan) {
  ...
  // ── Grade ──────────────────────────────────────────────────────────────
  // Replaced "🚨🔥 A SETUP DETECTED" — this was the single biggest
  // psychological trigger in the whole message. "SETUP DETECTED" with a
  // siren emoji reads as "act now" regardless of the actual evidence
  // behind it. Calmer framing doesn't change what fired or why, only
  // how urgently it's presented — the goal is for the alert to feel like
  // information, not an instruction.
  const isElite   = finalScore >= CONFIG.THRESHOLDS.hunterScoreElite;
  const isRare    = finalScore >= 80; // rarity tier, distinct from elite/buy threshold
  const gradeIcon = isElite ? '💎' : '🏹';
  const gradeLabel = isRare ? 'RARE SETUP CONFIRMED' : isElite ? 'STRONG SETUP CONFIRMED' : 'SETUP CONFIRMED';
  ...
  // ── Prices ────────────────────────────────────────────────────────────
  // BUG FIX: stop/tp1/tp2 were reading from `execution.stopPrice` — the
  // OLD `last`-based calculation from scoreExecutionQuality(). Entry was
  // correctly reading from `execEngine.entryPrice` (the V7d fix), but
  // nothing wired the corrected stop/TP from execEngine into this alert.
  // Result: entry from one calculation, stop/TP from a different,
  // unrelated one — exactly how HYPEUSDT got stop ABOVE entry on a long.
  // The hard validation gate (TRADE STRUCTURE INVALID) checks
  // execPlan.stopPrice vs execPlan.entryPrice — both from execEngine —
  // so it never saw this mismatch because it wasn't checking what
  // actually got displayed in the message text.
  ...
  // Final safety net — should never trigger given the fix above, but if
  // entry/stop ever end up inverted for any reason, refuse to display the
  // alert with broken numbers rather than send something unusable.
  if (stop >= entry) {
    log(`🚫 ALERT DISPLAY BLOCKED: ${candidate.symbol} would show stop=$${stop} >= entry=$${entry} — refusing to send malformed alert`);
    return null;
  }
...
async function fireAlert(candidate, scan) {
  ...
  // Single biggest requested change: don't publish trade parameters on
  // the first cycle a symbol crosses BUY. Many setups spike for one
  // cycle then disappear — these are the "one-cycle wonders" that drive
  // overtrading and fee bleed without ever being genuinely tradeable.
  // A setup that's still above threshold on the NEXT cycle has actually
  // persisted, which is a meaningfully different (stronger) signal than
  // a single momentary score crossing.
  //
  // Mechanics: track every symbol that crosses BUY threshold. If it's
  // the FIRST time seen this cycle, record it and return WITHOUT firing
  // — this is intentional silence, not a bug. Only on the SECOND
  // consecutive cycle (still above threshold) does the alert actually
  // fire. If the symbol drops below threshold in between, the counter
  // resets — it has to re-prove itself from scratch, not pick up where
  // it left off.
  ...
  if (!tracker || tracker.lastCycleSeen !== STATE.cycleCount - 1) {
    // First time crossing threshold, OR there was a gap (missed a cycle,
    // meaning it dropped below threshold at some point) — start fresh.
    ...
    return; // do NOT fire yet — wait for persistence
  }

  // Second (or later) consecutive cycle above threshold — confirmed.
  tracker.consecutiveCycles++;
  tracker.lastCycleSeen = STATE.cycleCount;
  if (tracker.consecutiveCycles < 2) {
    return; // still shouldn't happen given the logic above, but safe
  }
  log(`✅ ${candidate.symbol} confirmed for ${tracker.consecutiveCycles} consecutive cycles — proceeding to alert`);
  // Clear tracker now that it's fired — checkAlertCooldown handles
  // re-alert suppression from here, this tracker's job is done.
  ...
  // FIXED: previously only checked execEngine (the corrected calculation),
  // but buildHunterAlert was displaying execution.stopPrice (a different,
  // older, last-based calculation) — so this gate could pass while the
  // actual Telegram message still showed an inverted stop/entry (see
  // HYPEUSDT incident). Now checks BOTH sources since either could be
  // what ends up in the rendered message.
  ...
  if (execPlan && execPlan.entryPrice && execPlan.stopPrice) {
    ...
    // Minimum R:R raised from 1.0 to 1.5 per converged research (Plex +
    // GPT both independently recommended 1.5 as the floor, 1.67-2.0 as
    // preferred, for pullback-continuation setups specifically). At the
    // OLD 1.5/2/4 ATR multipliers, R:R was always exactly 1.33 — meaning
    // this 1.0 floor never actually filtered anything. At the NEW
    // 1.8/3/6 multipliers R:R is 1.67 by construction, so this gate is
    // now a meaningful safety check rather than a no-op.
    if (execPlan.rrRatio < 1.5) {
      log(`🚫 R:R TOO LOW: ${candidate.symbol} R:R=${execPlan.rrRatio} — alert suppressed (min 1.5 required)`);
      return;
    }
  }
  // Also validate the legacy execution object, since buildHunterAlert
  // falls back to it with `??` whenever execEngine fields are missing —
  // an undetected fallback here is exactly how HYPEUSDT got through.
  ...
  if (!msg) {
    // buildHunterAlert's own safety net caught an inverted stop/entry
    // that the earlier execPlan check missed (different source object —
    // see comments in buildHunterAlert). Don't send a malformed alert.
...
// ═══════════════════════════════════════════════════════════
// Replaces "post every WATCH every cycle" with a proper state machine.
// Each symbol tracked in STATE.watchlistState transitions through:
//
//   (untracked) --score≥65--> WATCHING --score≥78--> TRIGGERED
//                                |  ^
//                                |  | +5pts OR within 2pts of 78
//                                v  |
//                            (silent, unless update-worthy)
//                                |
//                                +--score drops ≥10pts or stage degrades-->
//                                REMOVED --(untracked)
//
// Only FOUR message types fire, each at most once per transition:
//   👀 HUNTER WATCH      — first time a symbol enters WATCHING
//   ⬆️ HUNTER UPDATE      — score improved ≥5pts, OR now within 2pts of BUY
//   🎯 HUNTER ENTRY       — score crossed the BUY threshold (handled by
//                           the existing BUY alert path — this just clears
//                           the watchlist state since it's no longer "watching")
//   ❌ HUNTER REMOVED     — score dropped ≥10pts from its watchlist peak,
//                           or stage degraded to SLEEPING/FAILURE/EXHAUSTION
//
// Everything else stays silent. Expected volume reduction: ~80-90% fewer
// messages, matching the explicit goal — event-driven, not scan-driven.
// ═══════════════════════════════════════════════════════════

const WATCHLIST_UPDATE_SCORE_DELTA = 5;   // post UPDATE if score improves by this much
const WATCHLIST_NEAR_TRIGGER_GAP   = 2;   // post UPDATE if within this many points of BUY
const WATCHLIST_REMOVE_SCORE_DROP  = 10;  // post REMOVED if score falls this much from peak
const WATCHLIST_DEGRADED_STAGES    = new Set(['SLEEPING', 'FAILURE', 'EXHAUSTION']);
const WATCHLIST_STALE_MS           = 4 * 60 * 60 * 1000; // auto-remove from tracking after 4h silence
...
async function processWatchlistLifecycle(candidates) {
  ...
  // Index this cycle's candidates by symbol for quick lookup
  const bySymbol = new Map();
  for (const c of candidates) bySymbol.set(c.symbol, c);

  // ── 1. Process every symbol currently being tracked ────────────────
  for (const [symbol, state] of STATE.watchlistState.entries()) {
    const current = bySymbol.get(symbol);

    // Symbol no longer appears in scan results at all (delisted from
    // universe, data failure, etc.) — let it go stale silently rather
    // than firing a REMOVED alert for something that may just be a
    // transient data gap.
    if (!current) {
    ...
    // ── TRIGGERED: crossed into BUY territory ─────────────────────────
    // The actual BUY alert is fired separately by the existing fireAlert
    // path — this just removes the symbol from watchlist tracking since
    // it's graduated out of "watching" status.
    ...
    // ── REMOVED: degraded meaningfully from peak ──────────────────────
    ...
    // ── UPDATE: improved ≥5pts since last alert, or now within 2pts of BUY ──
    ...
    // Update tracking state regardless of whether an alert fired
    state.score    = score;
    state.stage    = stage;
    state.peakScore = Math.max(state.peakScore, score);
  }

  // ── 2. Detect NEW candidates entering WATCHING for the first time ──
  for (const c of candidates) {
    if (c.finalScore < watchThreshold || c.finalScore >= buyThreshold) continue;
    if (STATE.watchlistState.has(c.symbol)) continue; // already tracked
...
async function sendWatchlistCreated(candidate, stage) {
  // WATCH alerts removed from Telegram output per product decision:
  // a WATCH message with entry/stop/targets behaves like a trade signal
  // regardless of the disclaimer — it creates FOMO and early entries.
  // The lifecycle engine still runs (feeding leaderboard deltas and
  // calibration), but individual WATCH events stay internal only.
  // Subscribers hear nothing until a genuine BUY fires.
...
async function sendWatchlistUpdate(symbol, candidate, state) {
  // Internal only — see sendWatchlistCreated for explanation.
...
async function fireLeaderboardMessage(scan) {
  // BUG FIX: leaderboard was showing "No scored candidates" despite
  // hunter_calibration recording real scores of 70-76 every cycle.
  //
  // Two compounding bugs:
  // 1. STATE.leaderboard was only written when scan.watches?.length > 0
  //    — meaning any cycle with top score below WATCH threshold (75)
  //    left it empty, even when real 70-74 candidates existed.
  // 2. The fallback referenced scan?.allCandidates, which doesn't exist
  //    in the scan return object — the real field is scan.candidates.
  //    So the fallback ALWAYS returned [] silently.
  //
  // Fix: leaderboard always takes the top 5 from scan.candidates
  // (all scored assets, regardless of tier), with STATE.leaderboard
  // as a secondary source only if scan.candidates isn't available.
  // This matches what calibration_table records and what users expect
  // to see — the actual top performers, not just the WATCH+ slice.
  const allScored = [...(scan?.candidates || [])].sort((a,b) => b.finalScore - a.finalScore);
  const candidates = allScored.length > 0
    ? allScored.slice(0, 5)
  ...
  // QQQ regime is meaningless when US markets are closed (weekend/holiday).
  // Show "CLOSED" instead of replaying Friday's stale BULL/BEAR label —
  // a stale label displayed as live is actively misleading, not just
  // imprecise, since the person reading it has no way to tell the
  // difference from a genuinely current reading.
  const qqqLabel = STATE.qqqRegimeStale
    ? '⚪ CLOSED (last: ' + (STATE.qqqRegime || 'unknown').replace('risk_off','RISK OFF') + ')'
    : STATE.qqqRegime === 'BULL' ? '🟢 BULL' : STATE.qqqRegime === 'risk_off' ? '🔴 RISK OFF' : '🟡 NEUTRAL';
  ...
  if (candidates.length > 0) {
    candidates.forEach((c, i) => {
      ...
      const tag   = score >= CONFIG.THRESHOLDS.hunterScoreGood ? '🔥 BUY'
                  ...
                  : score >= 65 ? '📡 CLOSE'
                  : '💤 FORMING';
      const marketIcon = c.assetType === 'crypto' ? '🪙' : '📈';
      const tracked = STATE.watchlistState.get(c.symbol);
      let deltaStr = '';
      if (tracked && tracked.createdScore != null) {
        const delta = round2(score - tracked.createdScore);
        deltaStr = ` (${delta >= 0 ? '+' : ''}${delta})`;
      }
      lines.push(`${i+1}. ${marketIcon} <b>${c.symbol}</b>  ${score}${deltaStr} ${tag}`);
    });
    lines.push(``);
...
async function storeAlert(candidate, scan) {
  ...
  try {
    ...
    // BUG FIX: was reading entry/stop/t1/t2 from the stale `execution`
    // object (scoreExecutionQuality's last-based calculation), same class
    // of bug as buildHunterAlert. This corrupted the Supabase learning
    // engine with wrong stop/TP prices — meaning trackOutcomes() above
    // would check TP/SL hits against the WRONG levels, silently
    // invalidating the calibration data being built up over time.
...
// ── Outcome confirmation alert ────────────────────────────────────────
// Fires once when a tracked alert hits TP1, TP2, or SL. Closes the loop
// on every BUY alert sent — previously the only way to know an outcome
// was to query Supabase directly, which nobody does in real time.
async function sendOutcomeAlert(alert, outcomeType, hitPrice, pctMove) {
  ...
  const label     = outcomeType === 'TP2' ? 'TARGET 2 HIT' : outcomeType === 'TP1' ? 'TARGET 1 HIT' : 'STOPPED OUT';
  ...
  const lines = [
    ...
    `Entry: ${formattedEntry}  →  ${outcomeType}: ${formattedHit}`,
    `Result: ${resultStr}`,
    ``,
    `Score at entry: ${round2(alert.hunter_score)} | Setup held ${round2((Date.now() - new Date(alert.created_at).getTime())/60000)} min`,
    ``,
    `——————————`,
    `⚠️ Educational market commentary only · Not personalised investment advice`,
    `📸 Analysis by @baretradesignals 🏹`,
  ...
  // This is the core research dataset for eventually proving whether
  // Top 3 / Top 5 / Top 10 momentum leadership portfolios outperform.
  // Previously the leaderboard existed only in Telegram — each 4-hour
  // snapshot was sent and immediately discarded. Without this write,
  // questions like "does rank #1 outperform rank #5?" or "does score 76
  // behave differently from score 72?" are permanently unanswerable for
  // the period before this fix was deployed.
  //
  // Schema: hunter_leaderboard_history (create with leaderboard_history.sql)
  // One row per ranked position per snapshot — not one row per snapshot —
  // so each symbol's rank, score, and context are queryable independently.
  if (supabase && candidates.length > 0) {
    try {
      ...
      if (error) log(`⚠️  Leaderboard history write failed: ${error.message}`);
      else log(`📊 Leaderboard snapshot saved: ${rows.length} positions (snapshot_id=${snapshotId})`);
    } catch (err) {
      log(`⚠️  Leaderboard history error (non-fatal): ${err.message}`);
    }
  }
  ...
  try {
    ...
    for (const alert of pending) {
      try {
        ...
        // ── Outcome confirmation alert ──────────────────────────────────
        // Previously trackOutcomes() only wrote to Supabase silently —
        // no way to know from Telegram whether a trade actually hit
        // target or got stopped out. Send a confirmation the FIRST time
        // an outcome flips true (checked via alert.target1_hit etc.,
        // the PRE-update value, so this only fires once per event).
        //
        // AUDIT FIX: mfe/mae are computed as max(highs)/min(lows) over a
        // lookback window — they don't tell us WHICH happened first in
        // real time. If both stop and TP1 fall within the same window
        // (common with wide-range candles in volatile crypto), the old
        // if/else chain always reported the target as a win, even though
        // the stop may have triggered first and the trade was actually
        // a loss. Now: if stoppedOut is true at all, report SL — the
        // conservative assumption when sequence is ambiguous. Only report
        // a target hit when stop was NOT also touched in this window.
        ...
        if (newlyStopped) {
          // Stop takes priority regardless of whether a target also shows
          // as hit in this window — can't confirm the target came first.
...
async function mainCycle() {
  ...
  STATE.cycleCount++;
  // Instance ID stamped on every cycle — per the duplicate-scanner
  // sanity check: if two different instance IDs ever appear logging
  // the SAME cycle number, that's hard proof of overlapping scanners
  // (not just a fast restart loop). grep logs for "CYCLE N" and check
  // whether more than one Instance= value appears for the same N.
  ...
  try {
    ...
    if (buys.length > 0) {
      ...
      // Lifecycle engine replaces the old "post WATCH every cycle" model.
      // Processes ALL watch-tier candidates (not just the single best),
      // since real spam was multiple symbols (SOL/BTC/AVAX/ETH) each
      // repeating independently, not just one symbol repeating. Only
      // fires on state transitions: new entry, +5pt improvement, within
      // 2pts of trigger, or removal — silent otherwise. ~80-90% fewer
      // messages than the old every-cycle approach.
    ...
    // Provider health summary — every 10 cycles, makes outages visible
    // instead of buried in individual fetch logs. Answers: which source,
    // what error, how long has it been down.
...
// INSTANCE_ID now declared at top of file (see line ~21) — needed early
// for provider-failure logging during startup health check.
let IS_LEADER     = false;
let lockHandle    = null;
...
async function start() {
  ...
  // ── RESTART COUNTER — restart-loop diagnostic ───────────────────────
  // Each boot increments a Supabase counter so restart frequency is
  // visible across the WHOLE test window, not just within one instance's
  // own logs (which disappear when the container dies). Answers Q1 from
  // the diagnostic plan directly: "how many unique instance IDs per hour?"
  // Simple read-then-write — fine for this low-frequency diagnostic use;
  // a rare race under concurrent restarts just undercounts slightly,
  // which doesn't materially affect the conclusion either way.
  if (supabase) {
    try {
  ...
  // ── DATA HEALTH CHECK ──────────────────────────────────────────────────
  // Per restart-loop diagnostic: every container boot makes ONE request to
  // each of these 5 providers, regardless of whether a scan cycle ever
  // runs. If Railway is restarting this container frequently (health
  // check failures, OOM, crash loop, etc.), startup alone compounds into
  // real request volume — e.g. 60 restarts/day × 5 requests = 300 extra
  // hits/day on top of normal scan traffic, before counting duplicate
  // instances or scan frequency at all.
  log('🔬 Running data health check...');
  let healthPassed = 0;
  let startupRequestCount = 0; // increments once per provider call below
