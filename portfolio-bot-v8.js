/**
 * portfolio-bot-v8.js — PART 1 of 3 (latest)
 *
 * Extends the relative-strength research flag from last pass —
 * still purely observational, still zero effect on what opens.
 *
 * To deploy:
 *   cat portfolio-bot-v8.part1.js portfolio-bot-v8.part2.js \
 *       portfolio-bot-v8.part3.js > portfolio-bot-v8.js
 *
 * detection.experimentFlags now also stores:
 *   - relative_strength_value: the raw number, not just the
 *     boolean — avoids a new migration if the threshold or
 *     bucket boundaries need revisiting later.
 *   - relative_strength_bucket: RS_WEAK (<=0) / RS_EARLY (<2%)
 *     / RS_HEALTHY (2-5%) / RS_EXTENDED (>=5%) — finer-grained
 *     than the single boolean, for the eventual re-audit.
 *   - priority_before_penalty / priority_after_hypothetical_
 *     penalty: candidatePriority() called an extra time (it's
 *     pure, same pattern as the existing openCandidate() double
 *     -call above it) with an 8-point penalty applied only to
 *     the hypothetical figure. This answers a sharper question
 *     than outcome R alone: would an RS_EXTENDED penalty have
 *     actually changed which candidates win a slot under the
 *     2-per-cycle cap, not just whether the trade itself looks
 *     worse in isolation. The 8-point penalty is NOT applied to
 *     any real ranking anywhere in this file.
 *
 * Schema unchanged — experiment_flags is already jsonb from the
 * prior migration, no new ALTER needed for these extra keys.
 */
'use strict';
