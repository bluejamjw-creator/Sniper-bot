...
async function runFullScan() {
  ...
  // ── DYNAMIC UNIVERSE ───────────────────────────────────────
  // Refresh every UNIVERSE_REFRESH_CYCLES cycles, otherwise use cache.
  // Falls back to Supabase snapshot if API fetch fails.
  const shouldRefresh = STATE.cycleCount % DYN.UNIVERSE_REFRESH_CYCLES === 0;
  if (shouldRefresh) {
    await refreshDynamicUniverse();
  }
  const dynUniverse = await getDynamicUniverse();

  const cryptoSymbols = dynUniverse.crypto;
  const usSymbols     = usOpen  ? dynUniverse.us  : [];
  const lseSymbols    = lseOpen ? dynUniverse.lse : [];

  if (!usOpen)  log(`⏸️  US market closed — skipping ${dynUniverse.us.length} US assets`);
  if (!lseOpen) log(`⏸️  LSE market closed — skipping ${dynUniverse.lse.length} LSE assets`);
...
async function fireDailyLeaderboard(market) {
  ...
  function fmtRow(emoji, sym, score, delta) {
    ...
    // Fix: never show 0 score — use dash if no current score
    const scoreDisplay = score > 0 ? `[${score}]` : '[--]';
    return `${emoji} ${label}   ${scoreDisplay}   ${velStr(delta)}`;
  }
  ...
  try {
    ...
    if (filtered.length === 0) { log(`📊 Daily ${market}: no universe data`); return; }
    ...
    // Week rows
    const weekRows = filtered.filter(r => new Date(r.snapshot_time) >= weekStart);
    const weekSnapIds = [...new Set(weekRows.map(r => r.snapshot_id))];

    // Week start scores — first snapshot of the week
    const weekStartScores = new Map();
    if (weekSnapIds.length > 0) {
      filtered.filter(r => r.snapshot_id === weekSnapIds[0])
        .forEach(r => weekStartScores.set(r.symbol, round2(parseFloat(r.score))));
    }

    // Month start scores — first snapshot of the month
    const monthSnapIds = [...new Set(filtered.map(r => r.snapshot_id))];
    const monthStartScores = new Map();
    if (monthSnapIds.length > 0) {
      filtered.filter(r => r.snapshot_id === monthSnapIds[0])
        .forEach(r => monthStartScores.set(r.symbol, round2(parseFloat(r.score))));
    }

    // ── TODAY: ranked by latest snapshot score ───────────────
    const todayRows = filtered.filter(r => r.snapshot_id === latestId)
      .sort((a,b) => parseFloat(b.score) - parseFloat(a.score))
      .slice(0, 3);

    // ── WEEK TO DATE: ranked by avg score this week, velocity vs Monday ──
    const weekMap = new Map();
    for (const row of weekRows) {
      if (!weekMap.has(row.symbol)) weekMap.set(row.symbol, []);
      weekMap.get(row.symbol).push(parseFloat(row.score));
    }
    const weekTop = [...weekMap.entries()]
      .filter(([sym]) => latestScores.has(sym)) // only show if in latest snapshot
      .map(([sym, scores]) => {
        const avgScore = round2(scores.reduce((a,b)=>a+b,0)/scores.length);
        const curScore = latestScores.get(sym) || 0;
        const wBase    = weekStartScores.get(sym);
        // Weekly velocity: how much has score moved since Monday?
        const weekVel  = wBase != null ? (curScore - wBase) : 0;
        // Blend: 50% avg score + 50% current score, penalise negative weekly velocity
        // An asset down 16 pts WTD cannot be ranked #1 regardless of avg
        const penalty  = weekVel < 0 ? weekVel * 1.5 : weekVel * 0.5;
        const rankScore = round2((avgScore * 0.5 + curScore * 0.5) + penalty);
        return { symbol: sym, avgScore, curScore, weekVel: round2(weekVel), rankScore };
      })
      .filter(d => d.weekVel >= -5) // exclude assets in clear weekly freefall
      .sort((a,b) => b.rankScore - a.rankScore)
      .slice(0, 3);

    // ── MONTH TO DATE: ranked by avg score + monthly velocity blend ──
    const monthMap = new Map();
    for (const row of filtered) {
      if (!monthMap.has(row.symbol)) monthMap.set(row.symbol, []);
      monthMap.get(row.symbol).push(parseFloat(row.score));
    }
    const monthTop = [...monthMap.entries()]
      .filter(([sym]) => latestScores.has(sym)) // only show if in latest snapshot
      .map(([sym, scores]) => {
        const avgScore = round2(scores.reduce((a,b)=>a+b,0)/scores.length);
        const curScore = latestScores.get(sym) || 0;
        const mBase    = monthStartScores.get(sym);
        const monthVel = mBase != null ? (curScore - mBase) : 0;
        const penalty  = monthVel < 0 ? monthVel * 1.5 : monthVel * 0.5;
        const rankScore = round2((avgScore * 0.5 + curScore * 0.5) + penalty);
        return { symbol: sym, avgScore, curScore, monthVel: round2(monthVel), rankScore };
      })
      .filter(d => d.monthVel >= -8) // exclude assets in clear monthly freefall
      .sort((a,b) => b.rankScore - a.rankScore)
      .slice(0, 3);

    // ── NOTABLE MENTIONS ─────────────────────────────────────
    const todayTop3  = new Set(todayRows.map(r => r.symbol));
    const weekTop3   = new Set(weekTop.map(d => d.symbol));
    const monthTop3  = new Set(monthTop.map(d => d.symbol));
    const mentions   = [];

    for (const sym of latestScores.keys()) {
      if (todayTop3.has(sym)) continue; // already shown
      const cur    = latestScores.get(sym) || 0;
      ...
      // 🚀 Accelerating across all timeframes
      if (todayVel >= 6 && weekVel >= 8 && monthVel >= 10) {
        mentions.push(`🚀 ${label}  [${cur}]  Today ▲+${todayVel}  WTD ▲+${weekVel}  MTD ▲+${monthVel} — accelerating across all timeframes`);
        continue;
      }
      // ⚡ Fast mover on any timeframe
      if ((todayVel >= 8) || (weekVel >= 10 && !weekTop3.has(sym)) || (monthVel >= 12 && !monthTop3.has(sym))) {
      ...
      // 📉 Was in prior top 3, dropped sharply
      const priorTop = filtered.filter(r => r.snapshot_id === priorId && parseFloat(r.score) >= 60)
        .sort((a,b)=>parseFloat(b.score)-parseFloat(a.score)).slice(0,3).map(r=>r.symbol);
      if (priorTop.includes(sym) && todayVel != null && todayVel < -5) {
        mentions.push(`📉 ${label}  [${cur}]  ▼ ${todayVel} — dropped from top 3`);
        continue;
      }
      // ⚠️ Strong WTD but not monthly
    ...
    // ── Build message ────────────────────────────────────────
    const lines = [`🏹 ${marketLabel} DAILY — ${timeStr}`, ``, `TODAY'S TOP 3`, ``];

    todayRows.forEach((r, i) => {
      const score = round2(parseFloat(r.score));
      const delta = priorScores.has(r.symbol) ? round2(score - priorScores.get(r.symbol)) : null;
      lines.push(fmtRow(rankEmoji[i], r.symbol, score, delta));
    });

    lines.push(``, `━━━━━━━━━━━━━━━━━━`);

    // Monday: show last week instead of WTD
    if (isMonday) {
      ...
      const lwLatestId = lwFiltered[0]?.snapshot_id;
      const lwTop = lwFiltered.filter(r => r.snapshot_id === lwLatestId)
        .sort((a,b)=>parseFloat(b.score)-parseFloat(a.score)).slice(0,3);

      lines.push(`LAST WEEK (for reference)`, ``);
      if (lwTop.length > 0) {
        lwTop.forEach((r, i) => lines.push(fmtRow(rankEmoji[i], r.symbol, round2(parseFloat(r.score)), null)));
      } else {
        lines.push(`No last week data`);
      }
    } else if (weekTop.length > 0) {
      lines.push(`WEEK TO DATE`, ``);
      weekTop.forEach((d, i) => {
        const wDelta = weekStartScores.has(d.symbol) ? round2(d.curScore - weekStartScores.get(d.symbol)) : null;
        lines.push(fmtRow(rankEmoji[i], d.symbol, d.curScore, wDelta));
      });
    } else {
      lines.push(`WEEK TO DATE`, ``, `No data yet this week`);
    }

    lines.push(``, `━━━━━━━━━━━━━━━━━━`, `MONTH TO DATE`, ``);
    if (monthTop.length > 0) {
      monthTop.forEach((d, i) => {
        const mDelta = monthStartScores.has(d.symbol) ? round2(d.curScore - monthStartScores.get(d.symbol)) : null;
        lines.push(fmtRow(rankEmoji[i], d.symbol, d.curScore, mDelta));
      });
    } else {
      lines.push(`No month data yet`);
    }

    if (mentions.length > 0) {
      lines.push(``, `━━━━━━━━━━━━━━━━━━`, `👁 NOTABLE`, ``);
      mentions.slice(0, 4).forEach(m => lines.push(m));
    }
    ...
    log(`⚠️  Daily ${market} error: ${err.message}`);
  }
}
...
const DYN = {
  ...
  CRYPTO_MIN_MARKET_CAP_M:  50,    // $50M minimum market cap
  CRYPTO_MIN_VOL_24H_M:      5,    // $5M minimum 24h volume
...
async function runNewEntrantAlerts(symbolMap, snapshots) {
  ...
  // Symbols in latest snapshot, top 10
  ...
  for (const [sym, { assetType, snapshots: snaps }] of symbolMap.entries()) {
    ...
    if (latest && latest.rank <= INTEL.NEW_ENTRANT_TOP_N) {
...
async function runTop3BreakoutAlerts(symbolMap, snapshots) {
  ...
  // Top 3 in latest snapshot
  const latestTop3 = new Map();
  for (const [sym, { assetType, snapshots: snaps }] of symbolMap.entries()) {
    const latest = snaps.find(s => s.snapshotId === latestSnapId);
    if (latest && latest.rank <= INTEL.TOP3_RANK_THRESHOLD) {
...
async function runDivergenceAlerts(symbolMap) {
  ...
  for (const [sym, { assetType, snapshots }] of symbolMap.entries()) {
    ...
    // High flow, low price movement = divergence
    ...
    const msg = [
      `📡 DIVERGENCE SIGNAL`,
      ``,
      `${flag} ${label} — High Hunter flow, price not yet moving.`,
      ``,
      `Hunter score:    ${round2(currentScore)}`,
      `Top-5 this week: ${top5count} appearances`,
      `Price move (7d): ${round2(pricePctMove)}%`,
      ``,
      `Hunter is detecting institutional interest that has not yet appeared in price.`,
      `This pattern often precedes a meaningful move. Watch closely.`,
      ``,
      `This is not a trade signal — it is an observation.`,
      ``,
      `——————————`,
      `⚠️ Educational market commentary only · Not personalised investment advice`,
      `📸 @baretradesignals 🏹`,
...
async function fireWeeklyMomentumReport(symbolMap) {
  ...
  lines.push(`——————————`);
  lines.push(`⚠️ Educational market commentary only · Not personalised investment advice`);
...
function pmBuildOpenMessage(symbol, assetType, score, reasons, atrStop, swingLow) {
  ...
  return [
    ...
    swingLine,
    ``,
    `Silence means HOLD. You will only hear from me when something changes.`,
    ``,
    `——————————`,
...
function pmBuildCloseMessage(symbol, assetType, score, reason, position, evidence) {
  ...
  return [
    ...
    `Why: ${reason}`,
    ``,
    `Score at entry: ${round2(position.entry_score)}`,
    `Score now:      ${round2(score)}`,
    `Held:           ${heldDays} day${heldDays !== 1 ? 's' : ''}`,
    ``,
    `Trigger: ${evidence.trigger || 'objective failure'}`,
    ``,
    `Looking for the next OPEN. Silence until then.`,
    ``,
    `——————————`,
    `⚠️ Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell`,
    `📸 @baretradesignals 🎯`,
...
function pmBuildWeeklyReview(positions, watchlist, marketContext) {
  ...
  lines.push(``);
  lines.push(`——————————`);
  lines.push(`⚠️ Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell`);
  lines.push(`📸 @baretradesignals 🎯`);
...
async function mainCycle() {
  ...
  try {
    ...
    // Daily leaderboard messages
    {
      ...
      // LSE 16:30 UTC — weekdays only (LSE closed Sat/Sun)
      if (_dh === 16 && _dm >= 30 && _dm < 35 && _day !== 0 && _day !== 6) {
      ...
      // US 21:00 UTC — weekdays only (US markets closed Sat/Sun)
      if (_dh === 21 && _dm < 5 && _day !== 0 && _day !== 6) {
    ...
    // Railway restarts or multiple instances don't double-fire.
    {
      ...
      if (_day === 5 && _hour >= 11 && _hour < 13) { // 11:00-13:00 UTC = 12:00-14:00 BST (UK Friday lunchtime)
        ...
        if (!_alreadySent) {
          ...
          const _wmsMap = await intelLoadFullHistory(14);
          await fireWeeklyMomentumReport(_wmsMap);
