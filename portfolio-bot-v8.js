...
async function runFullScan() {
  ...
  // ── UNIVERSE ASSEMBLY ─────────────────────────────────────
  // Crypto: always use CORE_CRYPTO fixed list — dynamic fetch
  //   pulls too much junk from Bybit (thousands of coins, poor
  //   quality filter). Fixed list is curated and reliable.
  // US/LSE: dynamic fetch via FMP/Yahoo when markets open.
  const cryptoSymbols = CORE_CRYPTO;

  // Dynamic refresh for US and LSE only
  const shouldRefresh = STATE.cycleCount % DYN.UNIVERSE_REFRESH_CYCLES === 0;
  if (shouldRefresh && (usOpen || lseOpen)) {
    await refreshDynamicUniverse();
  }
  const dynUniverse = await getDynamicUniverse();
  const usSymbols  = usOpen  ? (dynUniverse.us.length  >= 5 ? dynUniverse.us  : CORE_US_STOCKS) : [];
  const lseSymbols = lseOpen ? (dynUniverse.lse.length >= 3 ? dynUniverse.lse : CORE_LSE_STOCKS) : [];
...
function buildHunterAlert(candidate, scan) {
  ...
  // Plain English BUY alert — for dummies, not traders
  const label      = symbol.replace('USDT', '');
  const whyBuy     = relVol >= 1.8
    ? `Volume is ${relVol}× above normal — real money is moving in.`
    : `Momentum building steadily over multiple scans.`;
  const riskLine   = `If it drops to $${stop} (${Math.abs(slPct)}% down), exit. That is your limit.`;
  const targetLine = `Target $${tp2} — that is ${tp2Pct}% from here.`;
  const regimeLine = STATE.btcRegime === 'BULL' ? `Market is in good shape. Conditions favour this move.`
                   : STATE.btcRegime === 'BEAR' ? `Market is weak. Be cautious — smaller position if you act.`
                   : `Market is mixed. Keep it small.`;

  const lines = [
    `🟢 BUY SIGNAL — ${flag} ${label}`,
    ``,
    `What to do: Buy now around $${entry}.`,
    ``,
    whyBuy,
    regimeLine,
    ``,
    `If it works: Sell at $${tp2} (+${tp2Pct}%).`,
    riskLine,
    ``,
    `——————————`,
...
async function fireDailyLeaderboard(market) {
  ...
  // ── STATUS LABELS ─────────────────────────────────────────
  // Based on score velocity over the last 3 snapshots.
  // Plain English — no jargon.
  function getStatus(snapshots) {
    if (snapshots.length < 2) return { label: 'New',     emoji: '🆕' };
    const scores  = snapshots.slice(-4).map(s => s.score);
    const first   = scores[0];
    const last    = scores[scores.length - 1];
    const delta   = last - first;
    const allUp   = scores.every((s, i) => i === 0 || s >= scores[i - 1]);
    const allDown = scores.every((s, i) => i === 0 || s <= scores[i - 1]);

    if (allUp  && delta >= 4)  return { label: 'Gaining fast',  emoji: '🔥' };
    if (allUp  && delta >= 1)  return { label: 'Rising',        emoji: '📈' };
    if (allDown && delta <= -4) return { label: 'Exit now',     emoji: '🚨' };
    if (allDown && delta <= -1) return { label: 'Fading',       emoji: '📉' };
    if (Math.abs(delta) <= 0.5) return { label: 'Holding',     emoji: '➡️' };
    if (delta > 0)              return { label: 'Improving',    emoji: '📈' };
    return                             { label: 'Softening',    emoji: '⚠️' };
  }

  // ── HOW MANY CONSECUTIVE DAYS IN TOP 5 ────────────────────
  function daysLeading(snapshots) {
    let count = 0;
    for (let i = snapshots.length - 1; i >= 0; i--) {
      if (snapshots[i].rank <= 5) count++;
      else break;
    }
    return count;
  }

  if (market === 'crypto') await loadSniperRegimes();
  const marketLabel = { crypto: '🌐 CRYPTO', us: '🇺🇸 US STOCKS', lse: '🇬🇧 LSE' }[market];
  const now         = new Date();
  const dateStr     = now.toISOString().slice(0, 10);

  try {
    // Load last 14 days of data for this market
    ...
    if (error || !data?.length) {
      log(`📊 Daily ${market}: no data`);
      return;
    }

    // Build per-symbol snapshot history
    const symbolMap = new Map();
    for (const row of data) {
      if (!symbolMap.has(row.symbol)) symbolMap.set(row.symbol, []);
      symbolMap.get(row.symbol).push({
    ...
    const latestSnapId = data[data.length - 1].snapshot_id;

    // Build today's rankings from latest snapshot
    const today = [];
    for (const [sym, snaps] of symbolMap.entries()) {
      const latest = snaps.find(s => s.snapshotId === latestSnapId);
      if (!latest) continue;
      const status  = getStatus(snaps);
      const days    = daysLeading(snaps);
      today.push({
        symbol: sym,
        score:  round2(latest.score),
        rank:   latest.rank,
        status,
        days,
        snaps,
    ...
    // Sort by current score, take top 5
    today.sort((a, b) => b.score - a.score);
    const top5 = today.slice(0, 5);

    if (top5.length === 0) {
      log(`📊 Daily ${market}: no assets in latest snapshot`);
      return;
    }

    // ── BUILD MESSAGE ──────────────────────────────────────────
    const rankNum = ['1.','2.','3.','4.','5.'];
    const lines = [
      `🏹 DAILY BRIEF — ${dateStr}`,
      ``,
      marketLabel,
    ...
    for (let i = 0; i < top5.length; i++) {
      const a     = top5[i];
      const label = a.symbol.replace('USDT', '');
      const daysStr   = a.days >= 2 ? ` · ${a.days}d leading` : '';
      // Add SNIPER regime tag for crypto if available and fresh
      let regimeTag = '';
      if (market === 'crypto') {
        const sr = getSniperRegime(a.symbol);
        if (sr) {
          regimeTag = sr.regime === 'TRENDING'  ? ' ⚡' :
                      sr.regime === 'ROTATING'  ? ' 🔄' :
                      sr.regime === 'DEAD_CAT'  ? ' ☠️' : '';
        }
      }
      lines.push(`${rankNum[i]} ${a.status.emoji} ${label.padEnd(7)} ${a.status.label}${daysStr}${regimeTag}`);
    }

    lines.push(``);
    lines.push(`——————————`);
    lines.push(`⚠️ Educational market analysis only · Not personalised investment advice`);
    lines.push(`📸 @baretradesignals 🏹`);

    const msg = lines.join('\n');
    if (ENV.BLUEJAM_CHAT_ID) await sendTelegram(msg, ENV.BLUEJAM_CHAT_ID);
    log(`📊 Daily ${market} brief sent`);
...
function pmBuildOpenMessage(symbol, assetType, score, reasons, atrStop, swingLow) {
  ...
  return [
    `🟢 BUY — ${flag} ${label}`,
    ``,
    `What to do: Add this to your portfolio now.`,
    ``,
    `Why Hunter likes it:`,
...
function pmBuildCloseMessage(symbol, assetType, score, reason, position, evidence) {
  ...
  return [
    `🔴 SELL — ${flag} ${label}`,
    ``,
    `What to do: Sell now. Exit your position.`,
    ``,
    `Why: ${reason}`,
    ``,
    holdVerdict,
    ``,
    `No action needed until you see a BUY signal.`,
    ``,
    `——————————`,
    `⚠️ Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell`,
    `📸 @baretradesignals 🏹`,
...
async function mainCycle() {
  ...
  try {
    ...
    // Daily leaderboard messages
    {
      ...
      // Crypto 08:00 UTC — daily, including weekends (crypto never closes)
      if (_dh === 8 && _dm < 5) {
    ...
    if (buys.length === 0 && noTradeAge >= CONFIG.NO_TRADE_COOLDOWN_MS) {
      ...
      if (shouldSend) {
        // fireLeaderboardMessage suppressed — daily brief handles this now
