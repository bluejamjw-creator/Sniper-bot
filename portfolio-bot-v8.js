// ================================================================
// PORTFOLIO BOT V8 — MONTHLY PIE MANAGER
// Trading 212 stock pie + Revolut crypto pie
// Monthly picks via live momentum + narrative data
// Weekly swap alerts when holdings go cold
// Plain English instructions throughout
// ================================================================

console.log("🥧 PORTFOLIO BOT V8 STARTING...");
...
app.get("/",       (_req, res) => res.status(200).send("PORTFOLIO BOT V8 alive"));
...
const PIE_SIZE_STOCKS  = 5;
const PIE_SIZE_CRYPTO  = 5;
...
function scoreStock(symbol, candles, quote, hotSectors) {
  ...
  const mom5  = (last - prev5)  / prev5;   // 5-day momentum
  const mom20 = (last - prev20) / prev20;  // 20-day momentum

  const avgVol     = volumes.slice(0, -1).reduce((a, b) => a + b, 0) / (volumes.length - 1);
  const currentVol = volumes.at(-1);
  const volRatio   = avgVol > 0 ? currentVol / avgVol : 1;

  // Trend: price above 10-day average
...
app.listen(PORT, "0.0.0.0", async () => {
  console.log(`🥧 Portfolio Bot V8 health server on port ${PORT}`);
  await send(
    "\uD83E\uDD67 <b>PORTFOLIO BOT V8 LIVE</b>\n" +
    "Monthly pie picks \u2022 Weekly swap alerts\n" +
    `Stock budget: ${gbp(STOCK_BUDGET)}/month \u2022 Crypto budget: ${gbp(CRYPTO_BUDGET)}/month\n\n` +
