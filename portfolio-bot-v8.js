...
/**
 * HUNTER V13 — crypto-only early-move discovery + learning engine
 *
...
// Centralized table names for maintainability
const TABLES = {
  snapshots: 'hunter_v11_snapshots',
  examples: 'hunter_v11_examples',
  detections: 'hunter_v11_detections',
  alerts: 'hunter_v11_alerts',
  positions: 'hunter_v11_positions',
  watchlist: 'hunter_watchlist',
  universe: 'hunter_dynamic_universe',
  observationHistory: 'hunter_observation_history',
  predictiveStats: 'hunter_predictive_statistics',
  denylist: 'hunter_v11_denylist',
};
...
if (ENV.SUPABASE_URL && ENV.SUPABASE_KEY) {
  ...
  console.warn('[HUNTER] Supabase not configured; persistence disabled.');
}
    ...
    const req = https.get(
      {
        ...
        headers: {
          'User-Agent': 'Hunter/13.0 crypto-only',
...
async function discoverUniverse() {
  ...
  console.log(`[HUNTER] Universe: ${selected.length} assets (liquidity: ${Math.min(140, byLiquidity.length)}, movement: ${Math.min(80, byMovement.length)})`);
...
async function getLearningModel() {
  ...
  if (error) {
    console.error(
      `[DB] ${TABLES.examples}: ${error.message}`
    );
...
async function refreshPredictiveStatistics() {
  ...
  const { data, error } = await supabase
    .from(TABLES.observationHistory)
  ...
  for (const [low, high] of bins) {
    ...
    const winRate = wins.length && gains.length ? wins.length / gains.length : null;
    const avgWin = wins.length ? avg(wins) : null;
    const avgLoss = losses.length ? avg(losses) : null;

    await dbUpsert(
      TABLES.predictiveStats,
      {
        period_start: start,
        period_end: end,
        score_bin_low: low,
