...
const CONFIG = {
  ...
  CYCLE_MS_BEAR:        300_000,      // 5 mins when BEAR — no point hammering
...
async function runFullScan() {
  ...
  const finalCandidates = withCompetition.map((r) => {
    ...
    else if (r.rawScores?.executionQuality < CONFIG.THRESHOLDS.executionHardFloor) {
    ...
    // PASS — tier the output
    else {
      if (r.finalScore >= CONFIG.THRESHOLDS.hunterScoreElite) decision = 'BUY';      // 88+ elite
      else if (r.finalScore >= CONFIG.THRESHOLDS.hunterScoreGood) decision = 'BUY';  // 78+ strong
      else if (r.finalScore >= CONFIG.THRESHOLDS.hunterScoreWatch) decision = 'WATCH'; // 65+ watch
      else decision = 'WAIT';
    }
  ...
  const funnelComp        = funnelEdge.filter((c) => (c.comp?.rank === 1 || c.comp?.rank === 0) && (c.comp?.gap || 0) >= T.competitionGap);
  ...
  log(`  Funnel: total=${withData.length} → market_open=${funnelMarketOpen.length} → exec≥${T.executionHardFloor}:${funnelExec.length} → conf≥${T.confidenceMinimum}:${funnelConf.length} → conv≥${T.convictionMinimum}:${funnelConv.length} → edge≥${T.edgeMinimumOverField}:${funnelEdge.length} → comp_gap≥${T.competitionGap}:${funnelComp.length} → str≥55:${funnelStructure.length} → risk≥50:${funnelRisk.length}`);
...
function buildWatchAlert(candidate, scan) {
  ...
  return [
    ...
    ``,
    `Score: <b>${round2(finalScore)}</b> (below BUY threshold of 78)`,
    `Confidence: ${round2(confidence?.score)} | Conviction: ${round2(conviction?.score)} | Edge: +${round2(edgeResult?.edge)}`,
    ``,
    `💰 Potential entry: $${entry}  🛡️ Stop: $${stop}  🎯 T1: $${t1}  ⚖️ R:R: ${rr}:1`,
    ``,
    `Component scores: Str:${round2(rawScores?.structure)} | Part:${round2(rawScores?.participation)} | Mom:${round2(rawScores?.momentum)} | Exec:${round2(rawScores?.executionQuality)} | Risk:${round2(rawScores?.risk)}`,
    ``,
    `<i>WATCH only — does not meet BUY threshold. Monitor for improvement.</i>`,
    `——————————`,
...
async function mainCycle() {
  ...
  try {
    ...
    if (buys.length > 0) {
      ...
      // Fire best WATCH to bluejam only (never to live channel)
    ...
    // No-trade message — time-based, max once per 4 hours
...
// ============================================================
// SINGLETON LOCK — prevents duplicate Railway instances
// Uses Supabase as a distributed lock. On startup, tries to
// claim the lock. If another instance owns it and is still
// alive (heartbeat < 60s old), this instance exits.
// The lock is refreshed every 30s so a crashed instance
// releases the lock automatically after 60s.
// ============================================================

const INSTANCE_ID = `hunter-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
let lockHandle = null;

async function acquireSingletonLock() {
  if (!supabase) {
    log('⚠️  No Supabase — singleton lock disabled (watch for duplicates)');
    return true; // allow start without lock if no Supabase
  }
  try {
    const now = new Date().toISOString();
    const cutoff = new Date(Date.now() - 60000).toISOString(); // 60s ago

    // Check if another live instance holds the lock
    ...
    if (existing && existing.heartbeat > cutoff && existing.instance_id !== INSTANCE_ID) {
      log(`🔒 Another instance is running (${existing.instance_id}, heartbeat: ${existing.heartbeat})`);
      log('   This instance will exit. Only one Hunter should run at a time.');
      log('   If the other instance crashed, it will release the lock in 60s.');
      return false; // another live instance owns the lock
    }

    // Claim or take over the lock
    ...
    log(`🔓 Singleton lock acquired (${INSTANCE_ID})`);

    // Heartbeat every 30s to prove this instance is alive
    lockHandle = setInterval(async () => {
      try {
        await supabase.from('hunter_lock').update({
          heartbeat: new Date().toISOString(),
        }).eq('id', 'singleton').eq('instance_id', INSTANCE_ID);
      } catch { /* heartbeat failure is non-fatal */ }
    }, 30000);

    return true;
  } catch (err) {
    log(`⚠️  Singleton lock error: ${err.message} — proceeding without lock`);
    return true; // fail open rather than preventing startup
  }
}
...
async function releaseSingletonLock() {
  ...
  if (!supabase) return;
  try {
    await supabase.from('hunter_lock')
      .delete()
      .eq('id', 'singleton')
      .eq('instance_id', INSTANCE_ID);
    log('🔓 Singleton lock released');
  } catch { /* best effort */ }
}
...
async function start() {
  ...
  // Singleton lock — exit if another instance is already running
  const lockAcquired = await acquireSingletonLock();
  if (!lockAcquired) {
    log('Exiting — another Hunter instance is active.');
    process.exit(0);
  }
