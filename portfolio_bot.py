...
def score_signal(s):
    vol = s.get("volRatio", 1.5)
    # Handle both numeric and string volRatio
...
def analyse():
    ...
    signals = get_signals()
    if signals:
        # FIX 1 & 2: sort best first, slice from front
        ...
        for sig in ranked[:5]:   # FIX 1: top 5 strongest, not weakest
            asset      = sig.get("asset", "?")
            entry      = sig.get("entry", "?")
            sl         = sig.get("sl", "?")
            tp         = sig.get("tp", "?")
        ...
        if trim_actions:
            ...
            for t, st, p in trim_actions:
                ...
                all_lines.append(
                    ...
                    f"in Pot {best_route[0]} (confidence {best.get('confidence','?')}%)"
                )
