...
#  TICKER FIXES:
#  LSE-listed ETPs need .L suffix for Yahoo Finance
#  Crypto uses CoinGecko-style Yahoo tickers e.g. RENDER-USD not RNDR-USD
# ================================================================

POT_1_CORE = {
    "SMH":    0.29,   # VanEck Semiconductors ETF  — 29% (NASDAQ)
    "EQQQ.L": 0.25,   # Invesco Nasdaq-100 ETF      — 25% (LSE)
    "INDA":   0.20,   # iShares MSCI India ETF      — 20% (NASDAQ)
    "PLTR":   0.13,   # Palantir                    — 13%
    "VRT":    0.08,   # Vertiv Holdings             —  8%
    "ETHC.L": 0.05,   # 21Shares ETH Staking ETP    —  5% (LSE)
}
...
POT_3_CRYPTO = {
    "RENDER-USD": 0.333,  # Render    — GPU compute  — 33%
...
def check_stagnant(holdings, pot_num, signals):
    ...
    for ticker in holdings:
        ...
        if abs(change_60d) <= STAGNANT_PCT:
            replacement = ""
            if best_sig:
                replacement = f"\n🔄 Best signal now: {best_sig.get('asset')} — possible replacement"
            msg = (
...
def analyse_pot(name, holdings, pot_type, pot_num):
    ...
    for ticker, alloc in holdings.items():
        ...
        if not data:
            lines.append(f"  {ticker:<16} — no data")
            continue
        ...
        lines.append(f"  {ticker:<16} {alloc_str:<5} {pct_str:<8} {status}")
        if note:
            lines.append(f"                 ↳ {note}")
    ...
    if signals:
        ...
        if trim_actions and ranked:
            ...
            for t, st, p in trim_actions:
                all_lines.append(
                    f"  Trim {t} → redeploy into {best_asset} "
