...
def drift_report(total, values, targets):
    ...
    for pot in POT_ORDER:
        ...
        for ticker, w in pot_targets.items():
            ...
            drift = pct - float(w)
            rows.append(f"{ticker:<6} {pct:>5.1f}% / {float(w):>5.1f}%  {drift:+5.1f}%")
    ...
    for pot in POT_ORDER:
        ...
        for ticker, weight in targets.get(pot, {}).items():
            ...
            if price and price > 0:
                units = amount / price
                rows.append(f"{ticker:<6} {fmt_money(amount):>9}  {fmt_units(units):>10}")
            else:
                missing.append(f"{ticker}  {fmt_money(amount)}  no data")
