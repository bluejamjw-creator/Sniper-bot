...
def normalize_all(symbol):
    return (symbol or "").replace("USDT", "").upper().strip()
...
def calculate_portfolio():
    ...
    for t, units in holdings.items():
        ...
        n = normalize_all(t)
        price = get_crypto_data(n) if n in COINGECKO_IDS else get_stock_data(t)
        if price is None:
            continue
