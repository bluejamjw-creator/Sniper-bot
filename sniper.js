...
console.log(`
🚀 SNIPER BUILD INFO
Version:    V104.0 (Growth Mode -- Execution Engine -- Validation Phase: executionScore classification, sniper_signal_decisions logging, outcome tracker, intel noise reduction)
...
app.get("/",       (_req, res) => res.status(200).send("SNIPER V104 alive"));
...
// V104: reasons that remain genuine hard safety stops (unchanged from V103).
// Anything hitting these gates never reaches quality scoring at all.
const SAFETY_GATE_REASONS = new Set([
  "momentum_low", "momentum_decay", "no_setup", "extension", "parabolic", "invalid_atr"
]);

function reject(reason, asset = "", market = "", extra = {}) {
  ...
  // V104: log EVERY rejected asset to Supabase, not just quality-assessed ones.
  // Fire-and-forget -- logSignalDecision() handles its own error logging.
  if (asset) {
    const qualityFlags = extra.qualityFlags || {};
    logSignalDecision({
      asset, market,
      decisionClass: SAFETY_GATE_REASONS.has(reason) ? "SAFETY" : "REJECT",
      safetyGate: reason,
      qualityFlags,
      flagCount: Object.values(qualityFlags).filter(Boolean).length,
      rawScore: extra.rawScore,
      grade: extra.grade,
    }).catch(e => log(`❌ logSignalDecision (reject) failed for ${asset}: ${e.message}`));
  }
}

// ── V104: near-miss tracking (in-memory, resets daily) ────────────
// Feeds the "Near misses today" section of the daily block stats report.
let nearMissLog = [];
let nearMissLogDate = new Date().toDateString();

function resetNearMissLogIfNewDay() {
  const today = new Date().toDateString();
  if (today !== nearMissLogDate) {
    nearMissLog = [];
    nearMissLogDate = today;
  }
}

function addNearMiss(signal) {
  resetNearMissLogIfNewDay();
  // V105 FIX: analyse() runs every scan cycle (12-90s) but candles only close
  // every 5-15min, so the same unchanged candle was getting logged as a "new"
  // near-miss on every cycle. Confirmed with real data: ARBUSDT and IONQ each
  // appeared 4x in one day's near-miss list with IDENTICAL score/execScore --
  // that's the same candle re-scanned, not 4 independent readings. Skip if
  // the most recent entry for this asset has the same score+execScore.
  const lastForAsset = [...nearMissLog].reverse().find(n => n.asset === signal.asset);
  if (lastForAsset && lastForAsset.score === signal.score && lastForAsset.executionScore === signal.executionScore) {
    return;
  }
  nearMissLog.push({
    asset: signal.asset,
    ...
    flags: signal.qualityFlags,
    rr: signal.rr,
    volRatio: signal.volRatio,
    decisionClass: signal.decisionClass,
    timestamp: nowIso(),
  });
  if (nearMissLog.length > 200) nearMissLog.shift();
}

function getTopNearMisses(n = 8) {
  resetNearMissLogIfNewDay();
  return [...nearMissLog].sort((a,b) => (b.executionScore||0) - (a.executionScore||0)).slice(0, n);
}

// V104: 2hr dedup so the same GOOD candidate doesn't alert repeatedly
const goodAlertCooldown = new Map(); // asset -> last-alerted timestamp
const GOOD_DEDUP_MS = 2 * 60 * 60 * 1000;

function isGoodOnCooldown(asset) {
  const last = goodAlertCooldown.get(asset);
  return !!last && (Date.now() - last) < GOOD_DEDUP_MS;
}

function markGoodAlerted(asset) {
  goodAlertCooldown.set(asset, Date.now());
}

// ── V104: Supabase decision logging ────────────────────────────────
// CRITICAL BUG FIX: previous version's insert was missing a semicolon on the
// destructure, which meant insert failures were silently swallowed. Always
// use `const { error } = await supabase.from(...).insert([...]);` and check it.
async function logSignalDecision(payload) {
  if (!supabaseEnabled) return;
  try {
    const row = {
      // -- V104 columns --
      decision_class:   payload.decisionClass || null,
      safety_gate:      payload.safetyGate || null,
      quality_flags:    payload.qualityFlags || {},
      flag_rr:          !!(payload.qualityFlags && payload.qualityFlags.rr),
      flag_risk_pct:    !!(payload.qualityFlags && payload.qualityFlags.riskPct),
      flag_expansion:   !!(payload.qualityFlags && payload.qualityFlags.expansion),
      flag_resistance:  !!(payload.qualityFlags && payload.qualityFlags.resistance),
      flag_score:       !!(payload.qualityFlags && payload.qualityFlags.score),
      flag_grade_b:     !!(payload.qualityFlags && payload.qualityFlags.gradeB),
      flag_count:       payload.flagCount || 0,
      raw_sniper_score: payload.rawScore ?? null,
      execution_score:  payload.executionScore ?? null,
      score_breakdown:  payload.scoreBreakdown || {},
      engine_version:   "V104",
      // -- dual-write: old column names, kept alive for backward compatibility --
      symbol:       payload.asset,
      asset:        payload.asset,
      decision:     payload.decisionClass || null,
      // V104 FIX (root cause of ELITE/GOOD/WATCH never appearing in this table
      // at all): 'reason' and 'decision' are pre-existing NOT NULL columns.
      // safetyGate is legitimately null for anything that wasn't rejected --
      // which meant every single ELITE/GOOD/WATCH insert violated the NOT
      // NULL constraint and failed silently (Telegram alerts don't depend on
      // the Supabase write, so nothing looked broken from the outside). Falls
      // back to setupType, then decisionClass, so 'reason' is never null.
      reason:       payload.safetyGate || payload.setupType || payload.decisionClass || "quality_assessed",
      reject_reason: payload.safetyGate || null,
      score:        payload.rawScore ?? null,
      confidence:   payload.grade || null,
      btc_regime:   typeof btcTrend !== "undefined" ? btcTrend : null,
      btc_trend:    typeof btcTrend !== "undefined" ? btcTrend : null,
      qqq_regime:   typeof qqqTrend !== "undefined" ? qqqTrend : null,
      qqq_trend:    typeof qqqTrend !== "undefined" ? qqqTrend : null,
      entry_price:  payload.entry ?? null,
      entry:        payload.entry ?? null,
      stop_price:   payload.sl ?? null,
      sl:           payload.sl ?? null,
      target_price: payload.tp ?? null,
      tp:           payload.tp ?? null,
      market:       payload.market || null,
      created_at:   nowIso(),
    };
    const { error } = await supabase.from("sniper_signal_decisions").insert([row]);
    if (error) {
      log(`❌ Supabase insert FAILED (sniper_signal_decisions) for ${payload.asset}: ${error.message}`);
    }
  } catch (e) {
    log(`❌ logSignalDecision exception for ${payload.asset}: ${e.message}`);
  }
}

// V104: plain-English translations for reject reasons -- daily block stats
// used to just dump raw filter names, which meant nothing to Trey or anyone
// else without the codebase open in front of them.
const REJECT_REASON_PLAIN = {
  momentum_low:               "Low price momentum. Most assets didn't move enough to form a valid setup.",
  momentum_decay:             "Momentum faded before a setup could confirm.",
  no_setup:                   "No recognisable setup pattern formed.",
  extension:                  "Price already extended too far to enter safely.",
  parabolic:                  "Parabolic/exhaustion move -- entering here would be chasing.",
  invalid_atr:                "Volatility (ATR) data was invalid or unavailable for this asset.",
  risk_pct:                   "Stop distance implied more risk than the market's max allowed.",
  rr_too_low:                 "Reward-to-risk didn't clear the minimum bar.",
  no_expansion:               "Target too close given current volatility -- unlikely to be worth the trade.",
  global_floor:                "Score didn't clear the global minimum quality bar.",
  score_floor:                "Score didn't clear this market's minimum quality bar.",
  crypto_grade_a_suppressed:  "Crypto candidate below the stricter crypto quality floor.",
  poor_execution_severe:      "Execution window was too broken to place a sane order.",
  late_breakout:              "Breakout confirmed too late to get a clean entry.",
  pullback_neutral_btc:       "Pullback setup skipped -- BTC regime not supportive.",
  macro_protection:           "Macro capital-protection mode was active.",
};

function plainReject(reason) {
  return REJECT_REASON_PLAIN[reason] || reason;
}

async function sendRejectStatsSummary() {
  const entries = Object.entries(rejectStats).sort((a,b) => b[1]-a[1]);
  const nearMisses = getTopNearMisses(8);
  if (entries.length === 0 && nearMisses.length === 0) return;
  ...
  if (entries.length > 0) {
    for (const [k,v] of entries) {
      parts.push(`  <b>${v}x</b> ${escapeHtml(plainReject(k))}`);
    }
    parts.push("");
    // Market breakdown
    ...
    parts.push("No rejections logged today.", "");
  }

  // V104: near-miss tracking -- GOOD/WATCH candidates that came close
  if (nearMisses.length > 0) {
    parts.push("<b>🎯 Near misses today</b>");
    for (const nm of nearMisses) {
      const flagStr = Object.entries(nm.flags || {}).filter(([,v]) => v).map(([k]) => k).join(", ") || "none";
      parts.push(`  <b>${escapeHtml(nm.asset)}</b> — score ${nm.score} | execScore ${nm.executionScore} | ${escapeHtml(nm.setupType || "")} | flags: ${escapeHtml(flagStr)}`);
    }
    parts.push("");
  }
  ...
  parts.push("");
  parts.push("📈 V104 Observation — No parameter changes recommended. Continue collecting data for 100–200 trades before reviewing thresholds.");
...
async function sendEndOfDayReport() {
  ...
  // V104 FIX: originally gated on dailyStats.signalsFired, a field that was
  // never incremented anywhere in the file (and whose reset function was
  // never called either) -- so the gate was silently broken. Now checks real
  // trade data instead. dailyStats/resetDailyStats() have since been removed
  // as dead code entirely.
  const nearMissesToday = getTopNearMisses(8);
  const firedToday = trades.some(t => t.sentAt && new Date(t.sentAt).toDateString() === today);
  if (!firedToday && nearMissesToday.length === 0) {
    log("📋 EOD report skipped -- no trades fired and no near misses today");
    return;
  }
...
let lastMacroAlert   = 0;         // throttle protection alerts (V104: 4hr)
let lastMacroRegimeState = "";    // V104: tracks btc|qqq state so cooldown gates on it changing
...
async function saveTrades(trades) {
  ...
  if (!supabaseEnabled) return;
  // V105 FIX: this used to be one batch upsert for the whole array. If a
  // single row was malformed for any reason, the ENTIRE batch failed and
  // NOTHING persisted -- which matches what was observed (trades table stuck
  // at 0 rows despite real signals firing, with no way to tell which row or
  // why since the error only ever referenced the whole batch). Per-row
  // upserts isolate failures to the one bad row and surface its specific
  // error, instead of silently blocking every trade behind it forever.
  let ok = 0, failed = 0;
  for (const row of b) {
    try {
      const { error } = await retry(() => supabase.from("trades").upsert([row], { onConflict: "id" }));
      if (error) {
        failed++;
        log(`❌ Supabase saveTrades error on ${row.id}: ${error.message} (code: ${error.code})`);
      } else {
        ok++;
      }
    } catch (e) {
      failed++;
      log(`❌ Supabase saveTrades exception on ${row.id}: ${e.message}`);
    }
  }
  if (failed > 0) {
    log(`⚠️ Supabase trades saved ${ok}/${b.length} -- ${failed} row(s) failed, see errors above`);
  } else if (ok > 0) {
    log(`✅ Supabase trades saved (${ok} records)`);
  }
}
...
function narrativeHasChanged(sectors, stockMovers, cryptoMovers, lseMovers = []) {
  ...
  // V104 FIX: this was hard-capped at 4/day, shared across sectors/stocks/
  // crypto/LSE combined. Real data showed it exhausting by mid-to-late
  // morning most days, after which EVERY subsequent check returned false
  // regardless of how explosive a new mover was -- only hardcoded
  // forceNarrativeAlert triggers (market open) still got through. The
  // material-change logic below (new sector / big new mover / 3hr gap)
  // is the actual anti-spam mechanism and was already working correctly;
  // this raw counter was short-circuiting it before it could run. Raised
  // to 20 as a sanity backstop, not the real gate.
  if (narrativePostCount >= 20) return false;
...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = []) {
  ...
  if (!isWeekend() && topMovers.length > 0) {
    ...
    // V104: before 12:30 UK, m.pct (today's % change) is often still last
    // trading session's close-to-close figure -- stale, not current. Only
    // pre-market data (already genuinely current) or post-12:30 today data
    // gets shown, and each is labelled so subscribers know which it is.
    const stockMoversAfter1230 = getUkMinutes() >= 750; // 12:30 UK
    parts.push("🇺🇸 <b>Hot stocks right now:</b>");
    for (const m of ranked) {
      const pctStr  = m.pmPct
        ? `+${m.pmPct.toFixed(1)}% (pre-market)`
        : (stockMoversAfter1230 && m.pct)
          ? `${m.pct >= 0 ? "+" : ""}${m.pct.toFixed(1)}% (today)`
          : "";
...
async function sendWeeklyAudit() {
  ...
  // V104: moved from public channel to Bluejam only
  await sendPrivate(lines(parts.filter(Boolean)));
  log("📊 Weekly self-audit sent (Bluejam)");
}

// ── V104: Health dashboard ──────────────────────────────────────────
// Renamed/rebuilt for V104. Queries sniper_signal_decisions for this week's
// ELITE/GOOD/WATCH counts and the highest execution score seen. Bluejam only,
// runs alongside the weekly self-audit on Sundays.
let lastHealthDashboardDate = null;
...
async function sendV104HealthDashboard() {
  ...
  if (!supabaseEnabled) {
    log("📋 V104 health dashboard skipped -- Supabase disabled");
    return;
  }
  ...
  try {
    ...
    const { data: rows, error } = await supabase
      ...
      .select("decision_class, execution_score")
      .gte("created_at", weekAgo);

    if (error) {
      log(`❌ V104 health dashboard fetch failed: ${error.message}`);
      return;
    }

    const counts = { ELITE:0, GOOD:0, WATCH:0, REJECT:0, SAFETY:0 };
    let topScore = 0;
    for (const r of (rows || [])) {
      if (counts[r.decision_class] !== undefined) counts[r.decision_class]++;
      if (Number.isFinite(r.execution_score) && r.execution_score > topScore) topScore = r.execution_score;
    }

    const parts = [
      `🩺 <b>V104 HEALTH DASHBOARD</b>`,
      `<i>Last 7 days — sniper_signal_decisions</i>`,
      ``,
      `ELITE: <b>${counts.ELITE}</b> | GOOD: <b>${counts.GOOD}</b> | WATCH: <b>${counts.WATCH}</b>`,
      `REJECT: ${counts.REJECT} | SAFETY: ${counts.SAFETY}`,
      `Top execution score this week: <b>${topScore}</b>`,
      ``,
      `📈 V104 Observation — No parameter changes recommended. Continue collecting data for 100–200 trades before reviewing thresholds.`,
    ];
    await safeRun("v104HealthDashboard", () => sendPrivate(lines(parts)));
  } catch (e) {
    log(`❌ V104 health dashboard exception: ${e.message}`);
  }
}
...
// V104 FIX: the midweek/week-in-review digest was labelling movers using
// getMoveMaturity(candles) -- current CANDLE structure -- instead of the
// actual weekly % move being displayed. A stock up +15% on the week could
// still show "Flat/consolidating" if its most recent candles happened to be
// quiet. This labels off the weekly % change itself instead. (Replaces the
// old maturityToCautionLine(), which is no longer referenced anywhere.)
function weeklyMoveCautionLine(changePct) {
  const abs = Math.abs(changePct);
  if (abs >= 15) return "⚠️ Big weekly move — extended, caution if still holding, watch for reversal";
  if (abs >= 7)  return "📈 Solid weekly move — healthy trend, still has room if volume holds";
  if (abs >= 3)  return "🌱 Modest weekly move — early stage, room to continue";
  return "😴 Flat/consolidating — no clear direction this week";
}
...
async function buildPerformanceDigest(title, subtitle) {
  ...
  if (topCrypto.length > 0) {
    ...
    for (const c of topCrypto) {
      ...
      parts.push(`    ${weeklyMoveCautionLine(c.changePct)}`);
    }
    parts.push(``);
  }
  ...
  if (topStocks.length > 0) {
    ...
    for (const s of topStocks) {
      ...
      parts.push(`    ${weeklyMoveCautionLine(s.changePct)}`);
    }
    parts.push(``);
  }
...
async function checkMarketAlerts() {
  ...
  for (const { key, isOpen } of checks) {
    ...
    if (nowOpen && !marketWasOpen[key]) {
      ...
      if (key !== "CRYPTO") {
        ...
        // V104: Market Open moved to Bluejam only -- Opening Bell Intel already covers this publicly
    ...
    if (!nowOpen && marketWasOpen[key]) {
      ...
      if (key !== "CRYPTO") {
        ...
        // V104: resolve outcomes for unresolved ELITE/GOOD/WATCH rows, Bluejam summary
  ...
  // V104: health dashboard every Sunday, Bluejam only
  await safeRun("v104HealthDashboard", sendV104HealthDashboard);
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  // V104 FIX (root cause of the momentum_low/no_setup flood): `last` above is
  // the CURRENTLY FORMING candle -- Binance/Yahoo/Alpaca all return the
  // in-progress bar as the most recent kline when no endTime is bound. Every
  // HOT-loop scan (every 12s) was re-reading that same half-built candle and
  // computing "momentum" off it, which is why 88,111 candidates died at
  // momentum_low/no_setup over 8 days: it wasn't measuring trend, it was
  // sampling noise inside a candle that hadn't finished forming yet, dozens
  // of times before it even closed. `last`/`prev` above are kept as-is for
  // anything downstream that legitimately wants the live tick (entry pricing,
  // breakout confirmation). Momentum itself now uses only CLOSED bars.
  const closedLast = closes.at(-2);
  const closedPrev = closes.at(-3);
  if (!Number.isFinite(closedLast) || !Number.isFinite(closedPrev) || closedPrev <= 0) return null;
  const momentum = (closedLast - closedPrev) / closedPrev;
  ...
  const atr = calculateATR(candles, ATR_PERIOD);
  if (!atr || atr <= 0) { reject("invalid_atr", asset, market); return null; }
  ...
  const maxRisk = market === "CRYPTO" ? 0.08
                ...
                : market === "US" ? 0.04
                : 0.06; // LSE
  // V104: riskPct <= 0 is a genuinely invalid trade (already guarded above by
  // sl >= entry) so that stays a hard stop. Exceeding maxRisk is now a quality
  // flag, not an instant kill -- the composite executionScore decides its fate.
  if (riskPct <= 0) { reject("risk_pct", asset, market); return null; }
  const qualityFlags = {};
  qualityFlags.riskPct = riskPct > maxRisk;
  if (qualityFlags.riskPct) {
    checkpoint("risk_pct", false, `riskPct=${(riskPct*100).toFixed(2)}% > max ${(maxRisk*100).toFixed(2)}% -- flagged, not rejected`);
  }
  ...
  qualityFlags.rr = rr < minRRforMarket;
  if (qualityFlags.rr) {
    checkpoint("rr", false, `rr=${rr} < min ${minRRforMarket} -- flagged, not rejected`);
  }
  ...
  qualityFlags.expansion  = targetDistance < atr * 1.5;
  qualityFlags.resistance = resistanceRoom < atr * 1.0 && setupType !== "BREAKOUT_CONTINUATION";
  if (qualityFlags.expansion) {
    checkpoint("expansion", false, `target ${targetDistance.toFixed(4)} < 1.5 ATR ${(atr*1.5).toFixed(4)} -- flagged, not rejected`);
  }
  if (qualityFlags.resistance) {
    checkpoint("resistance", false, `resistance too close ${resistanceRoom.toFixed(4)} < 1 ATR ${atr.toFixed(4)} -- flagged, not rejected`);
  }
  if (!qualityFlags.expansion && !qualityFlags.resistance) {
  ...
  // V100 FIX: accurate reject attribution. Previously any crypto candidate
  // failing ANY condition got mislabeled "crypto_grade_a_suppressed" if it also
  // happened to be under the crypto threshold -- even when the real cause was
  // the global floor. Now checks causes in priority order (most fundamental
  // first) and logs the actual numbers so the reason is never misleading.
  if (score < MIN_GLOBAL_SCORE) {
    checkpoint("score", false, `${score} < global floor ${MIN_GLOBAL_SCORE}`);
    reject("global_floor", asset, market, { qualityFlags: { score: true }, rawScore: score, grade });
    return null;
  }
  if (score < scoreFloor) {
    checkpoint("score", false, `${score} < market floor ${scoreFloor}`);
    reject("score_floor", asset, market, { qualityFlags: { score: true }, rawScore: score, grade });
    return null;
  }
  if (market === "CRYPTO" && !cryptoGradeOk) {
    checkpoint("score", false, `${score} < crypto floor ${MIN_CRYPTO_SCORE} (grade=${grade})`);
    reject("crypto_grade_a_suppressed", asset, market, { qualityFlags: { score: true, gradeB: grade==="B" }, rawScore: score, grade });
    return null;
  }
  // V104 FIX: this used to hard-reject EVERY grade-B candidate regardless of
  // score, contradicting the comment above it (which says score>=88 "should
  // still pass"). Real data showed WLD hit 89 and NVDA hit 88 -- both inside
  // real, still-developing moves (checked against actual chart timing, not
  // already-topped-and-reversed) -- and both got killed purely on the B label
  // after already clearing every numeric floor. Now a flag, not a kill: the
  // numeric floors above already do the quality gating, so this stacks into
  // flagCount instead (a solo gradeB flag still allows GOOD; ELITE still
  // needs flagCount===0, so a clean A-grade candidate is still preferred).
  qualityFlags.gradeB = grade === "B";
  if (qualityFlags.gradeB) {
    checkpoint("score", false, `${score} -- B grade, flagged not rejected (floor=${scoreFloor})`);
  } else {
  ...
  // V104 FIX: combinedScore was referenced here but never defined anywhere in
  // V103 -- a ReferenceError on this line would have thrown on every single
  // call to analyse(), before any candidate could ever be returned. hunterScore
  // is what "combined" was always meant to mean here (both prior blocks compute
  // it as one score derived from confidence/exec/edge/conviction combined).
  log(`⚡ EXEC ${asset}: order=${execOrderType} execQ=${execQuality} setup=${score} combined=${hunterScore}`);
  ...
  if (execQuality < 40) {
    ...
    reject("poor_execution_severe", asset, market, { qualityFlags: { score: false }, rawScore: score, grade });
    return null;
  }

  log(`✅ ${asset} ${market} ${setupType} score=${score} grade=${grade} execQ=${execQuality} hunter=${hunterScore}`);

  // ── V104: EXECUTION SCORE + DECISION CLASSIFICATION ────────────
  // Setup Quality 35% + Execution Window 25% (rr+expansion) + Regime Alignment 20% + Conviction 20%
  // V105 FIX: this was Math.min(100, score-50)/50*100 -- the cap applied to
  // the INTERMEDIATE value before scaling, not the final result. For any raw
  // score above 100 (grade A/A*), this let setupQuality exceed 100 (e.g.
  // score=128 -> min(100,78)/50*100 = 156), which fed straight into
  // executionScore and let it exceed its intended 0-100 scale. Confirmed live:
  // an MSTR ELITE signal with score=128 and a 33x (!) volume ratio came out at
  // executionScore=107 -- above the scale entirely, and specifically for the
  // kind of extreme-score, extreme-volume candidate where an inflated score is
  // most likely to be masking risk rather than reflecting quality. Cap now
  // applies to the final scaled result instead.
  const setupQuality = Math.min(100, Math.max(0, score - 50) / 50 * 100);

  const rrQuality  = qualityFlags.rr        ? 0 : 100;
  const expQuality = (qualityFlags.expansion || qualityFlags.resistance) ? 0 : 100;
  const execWindow = (rrQuality + expQuality) / 2;

  const regimeState = market === "CRYPTO" ? btcTrend : market === "US" ? qqqTrend : "neutral";
  const regimeScore = (regimeState === "strong_bull") ? 100
                     : (regimeState === "bull")        ? 80
                     : (regimeState === "bear" || regimeState === "risk_off") ? 20
                     : 50; // neutral / unknown

  const heatNorm = Math.min(100, getHeat(asset));
  const volNorm  = Math.min(100, (volRatio / 3) * 100);
  const conviction = (heatNorm * 0.6) + (volNorm * 0.4);

  const executionScore = Math.round(
    (setupQuality * 0.35) + (execWindow * 0.25) + (regimeScore * 0.20) + (conviction * 0.20)
  );

  // V105 CANDIDATE -- SHADOW MODE ONLY. Does not affect decisionClass, executionScore,
  // or any live alert. Logged alongside the real score so it can be compared against
  // actual outcomes once there's a proper sample (Perplexity-recommended minimum:
  // 50 to re-evaluate, 100+ before touching the live formula). Peaked curve centered
  // ~1.8x RVOL, decaying above 2.5x -- current live volNorm rewards volume linearly
  // with no penalty for going higher, which live data (n=7, too small to act on
  // alone) suggested may be scoring climactic/exhaustion volume as bullish conviction.
  const shadowVolScore = 100 * Math.exp(-((Math.log(volRatio) - Math.log(1.8)) ** 2) / (2 * 0.35 * 0.35));
  const shadowFollowThrough = volRatio <= 2.5 || (closes.at(-2) > closes.at(-3) && closes.at(-3) > closes.at(-4));
  const shadowConviction = (heatNorm * 0.6) + (shadowVolScore * 0.4);
  const shadowExecutionScore = Math.round(
    (setupQuality * 0.35) + (execWindow * 0.25) + (regimeScore * 0.20) + (shadowConviction * 0.20)
  ) - (volRatio > 2.5 && !shadowFollowThrough ? 10 : 0);

  const flagCount = [qualityFlags.rr, qualityFlags.riskPct, qualityFlags.expansion, qualityFlags.resistance, qualityFlags.gradeB]
    .filter(Boolean).length;

  let decisionClass;
  if (flagCount === 0 && executionScore >= 65)      decisionClass = "ELITE";
  else if (flagCount <= 1 && executionScore >= 45)  decisionClass = "GOOD";
  else if (executionScore >= 25)                    decisionClass = "WATCH";
  else                                               decisionClass = "REJECT";

  const scoreBreakdown = {
    setupQuality, execWindow, regimeScore, conviction,
    rrQuality, expQuality, heatNorm, volNorm,
    rvol: volRatio, rr, atrExpansion: targetDistance / atr,
    heat: getHeat(asset), momentum, riskPct, setupType,
    rawSniperScore: score,
    // V105 shadow-mode fields -- for future analysis only, ignore for live decisions
    shadowVolScore, shadowFollowThrough, shadowConviction, shadowExecutionScore,
  };

  log(`🧮 EXEC SCORE ${asset}: setupQ=${setupQuality.toFixed(0)} execW=${execWindow.toFixed(0)} regime=${regimeScore} conviction=${conviction.toFixed(0)} → execScore=${executionScore} flags=${flagCount} class=${decisionClass} [shadow execScore=${shadowExecutionScore}]`);

  await logSignalDecision({
    asset, market, decisionClass, safetyGate: null, setupType,
    qualityFlags, flagCount, rawScore: score, executionScore,
    scoreBreakdown, grade, entry, sl, tp,
  });

  if (decisionClass === "REJECT") {
    return null;
  }
  ...
  const result = {
    ...
    qualityFlags, flagCount, executionScore, decisionClass, scoreBreakdown,
  };

  if (decisionClass === "WATCH") addNearMiss(result);
...
// Fires once daily at US close. Finds unresolved ELITE/GOOD/WATCH rows from
// the past 5 days, fetches current price, calculates outcome_pct, writes it
// back, and posts a brief Bluejam win/loss summary by classification.
let lastOutcomeTrackerDate = null;

async function runOutcomeTracker() {
  if (!supabaseEnabled) return;
  const today = new Date().toDateString();
  if (lastOutcomeTrackerDate === today) return;
  lastOutcomeTrackerDate = today;
  ...
  try {
    ...
    const { data: rows, error } = await supabase
      ...
      .is("outcome_filled", null)
      .gte("created_at", fiveDaysAgo)
      .not("entry_price", "is", null);

    if (error) {
      log(`❌ Outcome tracker fetch failed: ${error.message}`);
      return;
    }
    if (!rows || rows.length === 0) {
      log("📋 Outcome tracker: no unresolved rows to fill");
      return;
    }

    const summary = { ELITE:{w:0,l:0}, GOOD:{w:0,l:0}, WATCH:{w:0,l:0} };
    let filled = 0;

    for (const row of rows) {
      const asset  = row.symbol || row.asset;
      const market = row.market;
      if (!asset || !market) continue;
      const livePrice = await fetchLivePrice(asset, market);
      if (!Number.isFinite(livePrice)) continue;

      const entry = row.entry_price ?? row.entry;
      if (!Number.isFinite(entry) || entry <= 0) continue;
      const outcomePct = ((livePrice - entry) / entry) * 100;

      const { error: updateErr } = await supabase
        .from("sniper_signal_decisions")
        .update({ outcome_pct: outcomePct, outcome_filled: true, outcome_price: livePrice, outcome_checked_at: nowIso() })
        .eq("id", row.id);
      if (updateErr) {
        log(`❌ Outcome tracker update failed for ${asset}: ${updateErr.message}`);
        continue;
      }
      filled++;
      const cls = row.decision_class;
      if (summary[cls]) {
        if (outcomePct >= 0) summary[cls].w++; else summary[cls].l++;
      }
    }

    log(`📊 Outcome tracker: filled ${filled}/${rows.length} rows`);

    const parts = [
      `📊 <b>V104 OUTCOME TRACKER</b>`,
      `<i>Unresolved signals from the past 5 days, resolved today</i>`,
      ``,
      ...Object.entries(summary).map(([cls,{w,l}]) => `${cls}: ${w}W / ${l}L`),
      ``,
      `Rows resolved: ${filled}/${rows.length}`,
    ];
    await safeRun("outcomeTracker", () => sendPrivate(lines(parts)));
  } catch (e) {
    log(`❌ Outcome tracker exception: ${e.message}`);
  }
}

// V105: GOOD candidates no longer fire immediately. They're held and checked
// against the next closed candle -- green closes it out with the alert, red
// keeps it waiting (capped, so a stale setup can't sit forever and fire on
// an outdated entry/SL/TP). Detects "a new candle closed" by watching the
// last-closed candle's price change between checks, since not every candle
// fetcher carries a timestamp field.
const pendingGoodCandidates = new Map(); // asset -> { signal, lastClose, waits, firstSeenAt }
const GOOD_CONFIRM_MAX_WAITS = 3;
const GOOD_CONFIRM_MAX_AGE_MS = 45 * 60 * 1000; // give up after 45 min regardless

function queueGoodCandidate(signal, candles) {
  if (pendingGoodCandidates.has(signal.asset)) return; // already waiting on this asset
  const lastClosed = candles.at(-2); // candles.at(-1) is the still-forming bar
  if (!lastClosed) return;
  pendingGoodCandidates.set(signal.asset, {
    signal, lastClose: lastClosed.close, waits: 0, firstSeenAt: Date.now(),
  });
  log(`⏳ GOOD candidate ${signal.asset} detected -- holding for next candle confirmation`);
}

async function checkPendingGoodCandidate(asset, candles) {
  const pending = pendingGoodCandidates.get(asset);
  if (!pending) return;

  if (Date.now() - pending.firstSeenAt > GOOD_CONFIRM_MAX_AGE_MS) {
    pendingGoodCandidates.delete(asset);
    log(`⌛ GOOD candidate ${asset} expired waiting for confirmation -- dropped`);
    return;
  }

  const lastClosed = candles.at(-2);
  const prevClosed  = candles.at(-3);
  if (!lastClosed || !prevClosed || lastClosed.close === pending.lastClose) return; // same candle, nothing new yet

  const isGreen = lastClosed.close > prevClosed.close;
  if (isGreen) {
    pendingGoodCandidates.delete(asset);
    await handleGoodCandidate(pending.signal);
    log(`🟢 GOOD candidate ${asset} confirmed on green candle -- alerted`);
  } else {
    pending.waits++;
    pending.lastClose = lastClosed.close;
    if (pending.waits >= GOOD_CONFIRM_MAX_WAITS) {
      pendingGoodCandidates.delete(asset);
      log(`🔴 GOOD candidate ${asset} still red after ${pending.waits} candle(s) -- dropped`);
    } else {
      log(`🔴 GOOD candidate ${asset} red candle -- waiting (${pending.waits}/${GOOD_CONFIRM_MAX_WAITS})`);
    }
  }
}

// ── V104: GOOD candidate handling ──────────────────────────────────
// GOOD candidates (flagCount<=1, execScore>=45) never go public -- Bluejam
// only, and deduped 2hrs per asset so the same near-elite setup doesn't spam.
async function handleGoodCandidate(signal) {
  addNearMiss(signal);

  if (isGoodOnCooldown(signal.asset)) {
    log(`⏳ GOOD candidate ${signal.asset} on 2hr dedup cooldown -- skipping Bluejam alert`);
    return;
  }
  markGoodAlerted(signal.asset);

  const sym   = signal.asset.replace("USDT","");
  const flags = Object.entries(signal.qualityFlags || {}).filter(([,v]) => v).map(([k]) => k);
  const flagStr = flags.length > 0 ? flags.join(", ") : "none";

  const msg = lines([
    `🟡 <b>GOOD CANDIDATE</b> — ${escapeHtml(sym)} (${signal.market})`,
    `<i>Bluejam only — below public ELITE threshold</i>`,
    ``,
    `Setup: <b>${escapeHtml(signal.setupType)}</b> | Grade: ${signal.grade}`,
    `Score: ${signal.score} | Execution Score: <b>${signal.executionScore}</b>`,
    `R:R: ${signal.rr} | Vol: ${signal.volRatio?.toFixed(2)}x`,
    `Entry: ${formatPrice(signal.entry, signal.market)} | SL: ${formatPrice(signal.sl, signal.market)} | TP: ${formatPrice(signal.tp, signal.market)}`,
    `Flags: ${escapeHtml(flagStr)}`,
  ]);
  await safeRun(`goodCandidate${signal.asset}`, () => sendPrivate(msg));
  log(`🟡 GOOD candidate alerted to Bluejam: ${signal.asset} execScore=${signal.executionScore} flags=${flagStr}`);
}
...
async function scan() {
  ...
  async function processAsset(asset, market, fetcher) {
    ...
    // V105: resolve any pending GOOD confirmation for this asset first, using
    // this cycle's fresh candle data -- independent of what this cycle's own
    // classification comes out as, since the pending candidate may no longer
    // score GOOD by the time its confirmation candle closes.
    if (pendingGoodCandidates.has(asset)) {
      await checkPendingGoodCandidate(asset, candles);
    }

    const signal  = await analyse(asset, candles, market, _qqqMomThisCycle);
    if (!signal) return;

    // V104: decisionClass routes the candidate. ELITE flows into the existing
    // pickTopSignals -> processSignals public-alert pipeline unchanged. GOOD
    // gets queued for next-candle confirmation (V105) rather than alerting
    // immediately. WATCH is logged silently via the near-miss tracker
    // (already added inside analyse() for WATCH). REJECT never reaches here
    // -- analyse() returns null for it.
    if (signal.decisionClass === "GOOD") {
      queueGoodCandidate(signal, candles);
      return;
    }
    if (signal.decisionClass === "WATCH") {
      return; // already recorded in nearMissLog by analyse()
    }
    results.push(signal); // ELITE
  }
...
async function sniperScan() {
  ...
  for (const [asset, data] of SNIPER_ASSETS.entries()) {
    try {
      ...
      // V105 FIX: this loop was pushing EVERY non-null signal -- ELITE, GOOD,
      // and WATCH alike -- straight into results, and processSignals() below
      // sends a PUBLIC channel alert for everything it's given with no
      // decisionClass check of its own. GOOD/WATCH candidates caught by this
      // loop were firing to the public channel as if they were ELITE. Now
      // routed the same way the main scan loop already handles it.
      if (pendingGoodCandidates.has(asset)) {
        await checkPendingGoodCandidate(asset, candles15m);
      }
      if (signal.decisionClass === "GOOD") {
        queueGoodCandidate(signal, candles15m);
        continue;
      }
      if (signal.decisionClass === "WATCH") {
        continue; // already recorded in nearMissLog by analyse()
      }
      results.push(signal); // ELITE
...
async function warmScan() {
  ...
  for (const { asset } of warmAssets) {
    try {
      ...
      // V105 FIX: same bug as sniperScan -- this loop pushed every non-null
      // signal straight to processSignals(), which fires a public channel
      // alert unconditionally. GOOD/WATCH now routed correctly instead.
      if (pendingGoodCandidates.has(asset)) {
        await checkPendingGoodCandidate(asset, candles);
      }
      if (signal.decisionClass === "GOOD") {
...
async function safeCycle() {
  ...
  try {
    ...
    // V104: MARKET UPDATE / SCANNER RESUMING public alerts removed entirely --
    // regime doesn't actually block trades, so these were misleading subscribers
    // into thinking the scanner had stopped. Replaced with internal log lines
    // only. Cooldown extended 3hr -> 4hr and now also gates on the regime state
    // actually changing, not just re-firing on the same BTC/QQQ state.
    const alertCooldown = Date.now() - lastMacroAlert > 4 * 60 * 60 * 1000;
    const nowProtecting = isMacroProtecting();
    const regimeState   = `${btcTrend}|${qqqTrend}`;

    if (nowProtecting && !macroProtecting && alertCooldown && regimeState !== lastMacroRegimeState) {
      macroProtecting = true;
      lastMacroAlert  = Date.now();
      lastMacroRegimeState = regimeState;
      const cryptoBlocked = isCryptoBlocked();
      const usBlocked     = isUSBlocked();
      log(`📊 MARKET UPDATE (internal only) -- crypto=${cryptoBlocked} us=${usBlocked} btc=${btcTrend} qqq=${qqqTrend}`);

    } else if (!nowProtecting && macroProtecting) {
      macroProtecting = false;
      lastMacroAlert  = Date.now();
      lastMacroRegimeState = regimeState;
      log(`✅ SCANNER RESUMING (internal only) -- btc=${btcTrend} (${btcMacroTrend} 4h) qqq=${qqqTrend} (${qqqBullCount} stable readings)`);
    }
    ...
    // V104: hourly reject summary removed entirely -- daily block stats at US close covers it
