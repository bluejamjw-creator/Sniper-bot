...
async function postLeaderboardEvent(event) {
  ...
  // ── Immediate alerts -- high signal only ─────────────────────────
  ...
  if (event.type === "topN_change") {
    ...
    const parts = [`🔥 <b>MOMENTUM LEADERSHIP CHANGE</b>`, ``, `<b>${marketLabel} Top ${limit}</b>`, ``];
    ...
    parts.push(``);
    parts.push(`<b>Current Leaders:</b>`);
    const rankEmoji = ["🥇","🥈","🥉","4️⃣","5️⃣"];
    event.newTopN.forEach((a,i) => parts.push(`${rankEmoji[i]||""} ${escapeHtml(a.replace("USDT",""))}`));
    parts.push(``, LEADERBOARD_DISCLAIMER);
    await sendPrivate(lines(parts));
    return;
  }

  // ── Low priority -- buffer for hourly digest ─────────────────────
  // state_change, milestone, exit: individually noisy, valuable as a summary.
  if (["state_change","milestone","exit"].includes(event.type)) {
    const label = event.type === "state_change" ? `${sym} ${event.from||"?"} ➜ ${event.to}`
                : event.type === "milestone"    ? `${sym} held ${event.state} for ${event.streak} scans`
                : `${sym} exited (was ${event.previousState}, ${event.streak} scans)`;
...
async function sendLeaderboardDigest() {
  ...
  try {
    ...
    const { data: fullData, error: fullErr } = await supabase
      .from("momentum_leaderboard")
      .select("asset, market, state, previous_state, streak_scans, current_score, rank")
      .order("current_score", { ascending: false })
      .limit(20);
    ...
    if (crypto.length > 0) {
      ...
      crypto.forEach((r, i) => {
        ...
        const scans = r.streak_scans > 1 ? ` · ${r.streak_scans} scans` : "";
        const arrow = trendArrow(r);
        parts.push(`${rankEmoji[i]||`${i+1}.`} <b>${escapeHtml(sym)}</b> ${emoji} ${escapeHtml(r.state||"")}${scans} ${arrow}`);
      });
      parts.push(``);
    }
    ...
    if (stocks.length > 0) {
      ...
      stocks.forEach((r, i) => {
        ...
        const scans = r.streak_scans > 1 ? ` · ${r.streak_scans} scans` : "";
        const arrow = trendArrow(r);
        parts.push(`${rankEmoji[i]||`${i+1}.`} <b>${escapeHtml(r.asset)}</b> ${emoji} ${escapeHtml(r.state||"")}${scans} ${arrow}`);
      });
      parts.push(``);
    }
