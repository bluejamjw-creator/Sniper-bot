...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = []) {
  ...
  if (!isWeekend() && topMovers.length > 0) {
    const ranked = topMovers.slice(0, 6).map(sym => {
      ...
      // V98: structure check -- top pick should have positive day move or strong pre-market
      // Prevents AVAV-style: big pre-market mover but reversed and sitting below EMA all day
    ...
    // Top pick: prefer stocks with clean structure, fall back to strongest mover
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  // V98: SUSTAINED MOMENTUM -- catches gradual moves across multiple candles
  // BTC $1k move in 45 mins = ~1.3% over 3 candles, each ~0.4% individually → momentum_low
  // Multi-candle check: if last 3 candles sum to 1%+ AND each candle ≥ 0.3%, treat as momentum
  const momentum3c = closes.length >= 4
    ? (closes.at(-1) - closes.at(-4)) / closes.at(-4)
    : momentum;
  const eachCandleOk = closes.length >= 4
    ? [0,1,2].every(i => Math.abs((closes.at(-1-i) - closes.at(-2-i)) / closes.at(-2-i)) >= 0.003)
    : false;
  const sustainedMomentum = Math.abs(momentum3c) >= 0.010 && eachCandleOk;
  const effectiveMomentum = sustainedMomentum
    ? Math.max(Math.abs(momentum), Math.abs(momentum3c) / 3) * Math.sign(momentum3c)
    : momentum;
...
server = app.listen(PORT, "0.0.0.0", async () => {
  ...
  await safeRun("bootTelegram", () => sendPrivate(lines([
    "\uD83D\uDE80 SNIPER V98 LIVE -- QUALITY SIGNAL ENGINE",
    "",
    "V98: BTC risk_off = -25 score penalty (quality gate active)",
    "V98: A/A* grade only (score 90+/100+) -- B grade suppressed",
    "V98: R:R minimum 1.8 -- expansion filter active",
    "V98: 45 min opening window -- intel stocks get limit entry",
    "V98: Intel limit entry = EMA20 + 30% of gap to price",
    "V98: Force close direction-based not flatness-based",
    "V98: Replacement threshold 20 points -- no casual swaps",
    "V98: Multi-candle momentum -- catches sustained BTC moves",
    "V98: Alpaca SIP feed -- full consolidated bars",
    "V98: Polygon + Yahoo + Stooq fallback chain",
