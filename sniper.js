...
// FIXES CRYPTO POOL BUG: STATIC_CRYPTO_FALLBACK was seeded into
// every refresh AND happened to be exactly 26 entries (the cap),
// so trendingCoins could never contribute a single new coin --
// crypto pool was effectively frozen at the same 26 symbols.
// Now matches STOCK_POOL exactly: static list is boot-seed only,
// refresh is fully dynamic from trendingCoins, with a 3-coin
// (BTC/ETH/SOL) floor only if CoinGecko returns fewer than 5.
// Plus: watchlist_history, false_breakout gate, V94 reversion,
// and all prior session fixes.
// ================================================================
...
async function buildRejectInsightLines() {
  ...
  try {
    ...
    const { data, error } = await retry(() => supabase
      ...
      .in("reason", ["momentum_low", "no_setup"])
...
async function refreshDynamicPools(forceNarrativeAlert = false) {
  ...
  try {
    ...
    CRYPTO_PAIRS = dedup([...trendingCoins]).slice(0, 26);
