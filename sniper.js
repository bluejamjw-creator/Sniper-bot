...
setInterval(()=>{
  ...
  for(const [k,v] of CACHE.entries()){
    if(now-v.t>CACHE_TTL_MS) CACHE.delete(k);
  }
},600000);
...
async function fetchStock(symbol){
  ...
  try{
    ...
    const closes=data.split("\n").slice(1).map(l=>Number(l.split(",")[4]));
    const r=summarise(symbol,closes,"Stooq");
...
function buildSignal(m,type){
  ...
  const tp=Math.max(m.high*1.02,entry*1.05);
  ...
  if(momentum<0.008) return null;

  const trendRange=(m.high-m.low)/m.low;
  if(trendRange<0.06) return null;
