...
console.log("🚀 SNIPER V28 STARTING...");
...
const STOCK_SESSION_MODE = "regular"; // regular | premarket | off
...
function isUsRegularSessionNow() {
  ...
  const inBSTSeason = month >= 3 && month <= 10;
  if (!inBSTSeason) return false;
...
function tradeStats(trades) {
  ...
  const netR = closed.reduce((sum, t) => {
    if (t.result === "TP") return sum + (t.riskReward || 0);
    if (t.result === "SL") return sum - 1;
    return sum;
  }, 0);
...
async function processSignals(signals) {
  ...
  for (const s of signals) {
    ...
    const sessionTag = s.type === "stock" && !isUsRegularSessionNow() ? "
Session: PRE-MARKET / AFTER-HOURS" : "
Session: REGULAR";
    const msg = `${s.type === "crypto" ? "🚀 CRYPTO BREAKOUT" : "📈 STOCK BREAKOUT"}${sessionTag}
...
async function sendPerformanceSummaries() {
  ...
  const stateFile = path.join(DATA_DIR, "summary_state.json");
...
app.get("/", (_req, res) => res.send("SNIPER V28 RUNNING 🚀"));
