...
const MIN_CANDLES              = 40; // V97: lowered from 60 -- Yahoo 15m returns ~52 candles (2 days), was blocking all opening window signals
const ATR_PERIOD               = 14;
...
async function fetchUSData(symbol) {
  ...
  if (!candles) {
    ...
    if (candles) log(`📊 ${symbol} -- Yahoo ${interval} (${candles.length} candles)`);
  }
