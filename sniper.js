...
// ================================================================
// ANALYSIS — WITH REJECTION LOGGING
// ================================================================
...
function analyse(asset, candles, market) {
  ...
  if (!candles || candles.length < MIN_CANDLES) {
    console.log(`REJECTED ${asset} — insufficient candles (${candles?.length ?? 0} < ${MIN_CANDLES})`);
    return null;
  }

  // ── 2. REGIME ────────────────────────────────────────────────
  if (!regimePass(market)) {
    const regime = market === "CRYPTO" ? btcTrend : qqqTrend;
    console.log(`REJECTED ${asset} — regime not passing (${regime})`);
    return null;
  }
  ...
  if (!Number.isFinite(last) || !Number.isFinite(prev) || prev <= 0) {
    console.log(`REJECTED ${asset} — invalid price data`);
    return null;
  }
  ...
  if (!strongTrend) {
    if (ema20 <= ema50) {
      console.log(`REJECTED ${asset} — EMA20 ${ema20.toFixed(4)} below EMA50 ${ema50.toFixed(4)}`);
    } else if (last <= ema20) {
      console.log(`REJECTED ${asset} — price ${last.toFixed(4)} below EMA20 ${ema20.toFixed(4)}`);
    } else {
      console.log(`REJECTED ${asset} — EMA20 not rising (prev ${ema20Prev.toFixed(4)} >= now ${ema20.toFixed(4)})`);
    }
    return null;
  }
  ...
  const extLimit = extensionLimitForMarket(market);

  if (extension > extLimit) {
    console.log(`REJECTED ${asset} — overextended (${(extension * 100).toFixed(1)}% > ${(extLimit * 100).toFixed(0)}% limit)`);
    return null;
  }
  ...
  // V70.1: tightened nearEMA20 — was 2.5%/2.0%, now 1.5%/1.2%
  // Ensures limit price is very close to current price when signal fires
  // Prevents the "price already moved away" problem on execution
  const nearEMA20 =
    market === "CRYPTO"
      ? Math.abs(last - ema20) / ema20 <= 0.015
      : Math.abs(last - ema20) / ema20 <= 0.012;
  ...
  if (!pullbackSignal && !breakoutSignal) {
    const reasons = [];
    if (!nearEMA20) reasons.push(`not near EMA20 (${(extension * 100).toFixed(1)}% away)`);
    if (!pulledBackRecently) reasons.push("no recent pullback to EMA20");
    if (volRatio < pullbackVolForMarket(market)) reasons.push(`vol too low for pullback (${volRatio.toFixed(2)}x < ${pullbackVolForMarket(market)}x)`);
    if (!(last > breakoutLevel)) reasons.push(`price ${last.toFixed(4)} below breakout ${breakoutLevel.toFixed(4)}`);
    if (breakoutDistance > BREAKOUT_MAX_DISTANCE) reasons.push(`breakout distance too far (${(breakoutDistance * 100).toFixed(1)}%)`);
    if (volRatio < breakoutVolForMarket(market)) reasons.push(`vol too low for breakout (${volRatio.toFixed(2)}x < ${breakoutVolForMarket(market)}x)`);
    console.log(`REJECTED ${asset} — no setup: ${reasons.join(", ")}`);
    return null;
  }
  ...
  if (pullbackSignal && !breakoutSignal) {
    ...
    entryType = "LIMIT BUY";
    // V70.1: entry at last * 0.998 (0.2% below current)
    // was ema20 * 1.002 which could be far below current price
    // now fills much faster when signal fires
    entry = last * 0.998;
  } else {
  ...
  // ── 8a. PULLBACK QUALITY FILTERS ─────────────────────────────
  // Fix for SUI-type signals: weak momentum + lower lows = downtrend not pullback

  if (setupType === "PULLBACK_CONTINUATION") {

    // Minimum momentum — 0.14% is noise, need real buying pressure
    if (momentum < 0.003) {
      console.log(`REJECTED ${asset} — pullback momentum too weak (${(momentum * 100).toFixed(2)}% < 0.30%)`);
      return null;
    }

    // Lower lows check — three consecutive lower lows = downtrend, not pullback
    const recentLowerLows =
      lows.at(-1) < lows.at(-2) &&
      lows.at(-2) < lows.at(-3);

    if (recentLowerLows) {
      console.log(`REJECTED ${asset} — recent lower lows (downtrend not pullback)`);
      return null;
    }
  ...
  if (!atr || atr <= 0) {
    console.log(`REJECTED ${asset} — ATR calculation failed`);
    return null;
  }
  ...
  if (!Number.isFinite(sl) || sl >= entry) {
    console.log(`REJECTED ${asset} — invalid stop (sl ${sl?.toFixed(4)} >= entry ${entry.toFixed(4)})`);
    return null;
  }
  ...
  if (riskPct <= 0 || riskPct > 0.12) {
    console.log(`REJECTED ${asset} — risk out of bounds (${(riskPct * 100).toFixed(1)}%)`);
    return null;
  }
  ...
  // ── PASS ─────────────────────────────────────────────────────
  console.log(`✅ SIGNAL ${asset} — ${setupType} | entry ${entry.toFixed(4)} | RR ${rr} | vol ${volRatio.toFixed(2)}x | score pending`);
...
async function scan() {
  ...
  console.log(`--- SCAN START | BTC: ${btcTrend} | QQQ: ${qqqTrend} ---`);

  async function processAsset(asset, market, fetcher) {
    if (cooldown(asset, alerts, market)) {
      console.log(`SKIPPED ${asset} — cooldown active`);
      return;
    }
    if (dedupeOpenTradeExists(asset, trades)) {
      console.log(`SKIPPED ${asset} — open trade exists`);
      return;
    }
  ...
  console.log(`--- SCAN END | ${results.length} signal(s) ---`);
