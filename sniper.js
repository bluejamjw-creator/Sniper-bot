...
// FIXED: manageTrades uses saved `type` only
// FIXED: removed portfolio swap branch
// FIXED: long-only SL/TP logic
// FIXED: stocks display in USD, crypto displays in GBP
// ================================================================

console.log("ðŸš€ SNIPER V24.4 STARTING...");
...
const MAX_SIGNALS = Number(process.env.MAX_SIGNALS || 2);
...
async function fetchStockCandles(symbol) {
  try {
    ...
    return closes
      .map((c, i) => ({ c: +c, h: +highs[i], v: +(vols[i] || 0) }))
...
function analyseAsset(asset, candles, type = "crypto") {
  ...
  const entry = last;
  const stopPct = type === "crypto" ? 0.025 : 0.03;
  const takePct = Math.max(minMove, stopPct * 1.5);
  ...
  for (const t of trades) {
    ...
    try {
      ...
      if (t.type === "stock") {
        ...
        console.error("UNKNOWN_TRADE_TYPE:", t.asset, t.type);
        continue;
      }
      ...
      if (price >= t.tp) {
        ...
        t.closedAt = nowIso();
        changed = true;
        await send(`âŒ SL HIT: ${t.asset}`);
      }
    } catch (e) {
      console.error("MANAGE_ERROR:", t.asset, t.type, e.message);
    }
  }
...
async function runCycle() {
  ...
  try {
    ...
    for (const s of signals) {
      ...
      const entryLine = s.type === "crypto" ? fmtGBP(toGBP(s.entry)) : fmtUSD(s.entry);
      const slLine = s.type === "crypto" ? fmtGBP(toGBP(s.sl)) : fmtUSD(s.sl);
      const tpLine = s.type === "crypto" ? fmtGBP(toGBP(s.tp)) : fmtUSD(s.tp);
...
app.get("/", (_req, res) => res.send("SNIPER V24.4 RUNNING ðŸš€"));
