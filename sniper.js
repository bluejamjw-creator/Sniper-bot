// ================================================================
// SNIPER V84
// + Smart API rotation: TwelveData and Alpha Vantage alternate
//   per symbol so neither hits daily rate limit
// + Usage counters reset at midnight UTC
// + Yahoo Finance as free emergency fallback if both exhausted
// + Finnhub stock filter: only clean 1-5 char NYSE/NASDAQ tickers
//   no OTC junk symbols flooding the pool
// + Stock pool capped at 25 core names to stay within free limits
// + All V83 logic preserved:
//   - Exceptional crypto override (2.5% + 2x vol bypasses BTC)
//   - 15-min candles for US opening window 14:30-16:00 UK
//   - Correct UK market hours throughout
//   - Momentum Ignition setup type
//   - Dynamic pools via CoinGecko + Finnhub
//   - Trading coach lessons on signal and close
// ================================================================

console.log("🚀 SNIPER V84 STARTING...");
...
app.get("/",       (_req, res) => res.status(200).send("SNIPER V84 alive"));
...
async function refreshDynamicPools() {
  ...
  const [trendingStocks, topMovers, cryptoMovers] = await Promise.all([
    getTrendingStocks(),
    getTopMovers(),
    getCryptoMovers()
  ]);
  ...
  // No narrative alerts on weekends — markets are closed
  if (isWeekend()) {
    console.log("📅 Weekend — skipping narrative alert");
    return;
  }

  if (HOT_SECTORS.length > 0 || topMovers.length > 0 || cryptoMovers.length > 0) {
...
app.listen(PORT, "0.0.0.0", async () => {
  ...
  await send(
    "\uD83D\uDE80\uD83D\uDD25 SNIPER V84 LIVE\n" +
    "Smart API rotation active \u2022 Yahoo Finance fallback ready\n" +
    "Stock pool capped at 25 clean tickers \u2022 No more OTC junk\n" +
    "Crypto override, 15-min open window, UK hours all correct"
  );
