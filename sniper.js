...
// Lower thresholds across the board or nothing ever fires
const LSE_BREAKOUT_VOL    = 1.15;  // was 1.25
const LSE_PULLBACK_VOL    = 1.05;  // was 1.10
const BREAKOUT_DIST_LSE   = 0.025; // was 0.04
const EXTENSION_LIMIT_LSE = 0.06;  // was 0.08
const MOMENTUM_CAP_LSE    = 0.04;
const MIN_LSE_MOMENTUM    = 0.004; // was 0.008 — UK stocks move slower
const MIN_PULLBACK_MOM_LSE = 0.004;
...
const MIN_US_SCORE     = 76;  // was 78 — less filtering, more activity
...
let cycleCount      = 0; // used to throttle watchlist alerts (every 5 cycles = ~5 mins)
...
// Keeps the channel alive between confirmed entries
async function sendWatchlistAlert(alerts, trades) {
  ...
  if (isCryptoWindowOpen()) {
    for (const sym of CRYPTO_PAIRS.slice(0, 15)) {
      try {
        ...
        // Near EMA20 pullback with rising structure and decent volume
        const nearEMA     = distFromEMA < 0.025;
        const risingEMA   = ema20v > ema50v;
        const volBuilding = volRat >= 1.2 && volRat < 1.6; // building but not breakout yet

        if (nearEMA && risingEMA && volBuilding) {
          const name = sym.replace("USDT","");
          watching.push(`  \uD83E\uDE99 <b>${escapeHtml(name)}</b> — pullback to EMA, vol ${volRat.toFixed(1)}x`);
        }
      } catch { /* skip */ }
    }
  }
  ...
  if (isWeekday() && isUSMarketOpen()) {
    for (const sym of STOCK_POOL.slice(0, 12)) {
      try {
        ...
        if (nearEMA && risingEMA && volBuilding) {
          watching.push(`  \uD83C\uDDFA\uD83C\uDDF8 <b>${escapeHtml(sym)}</b> — near EMA, vol ${volRat.toFixed(1)}x`);
        }
      } catch { /* skip */ }
    }
  }
  ...
  const parts = [
    ...
    ``,
    `These setups are forming but haven\u2019t confirmed yet:`,
    ``
  ];
  parts.push(...watching.slice(0, 6));
  parts.push(``, `\uD83D\uDCA1 Entry alert fires when breakout confirms on next scan`);
  await safeRun("watchlistAlert", () => sendBoth(lines(parts)));
  log(`👀 Watchlist alert sent — ${watching.length} building setups`);
}
...
async function safeCycle() {
  ...
  try {
    ...
    if (Array.isArray(signals) && signals.length > 0) {
      ...
      // Every 5 cycles (~5 mins) send watchlist alert if no signal fired
      if (cycleCount % 5 === 0) {
        const alerts = await loadAlerts();
        const trades = await loadTrades();
...
server = app.listen(PORT, "0.0.0.0", async () => {
  ...
  await safeRun("bootTelegram", () => sendPrivate(lines([
    "\uD83D\uDE80\uD83D\uDD25 <b>SNIPER V91 LIVE \u2014 MOMENTUM MATURITY ENGINE</b>",
    "",
