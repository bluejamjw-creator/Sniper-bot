...
async function logSignalDecision(payload) {
  ...
  try {
    const row = {
      ...
      reason:       payload.safetyGate || null,
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  log(`🧮 EXEC SCORE ${asset}: setupQ=${setupQuality.toFixed(0)} execW=${execWindow.toFixed(0)} regime=${regimeScore} conviction=${conviction.toFixed(0)} → execScore=${executionScore} flags=${flagCount} class=${decisionClass}`);

  await logSignalDecision({
    asset, market, decisionClass, safetyGate: null,
...
async function scan() {
  ...
  async function processAsset(asset, market, fetcher) {
    ...
    // gets a deduped Bluejam-only heads-up. WATCH is logged silently via the
    // near-miss tracker (already added inside analyse() for WATCH). REJECT
    // never reaches here -- analyse() returns null for it.
    if (signal.decisionClass === "GOOD") {
      await handleGoodCandidate(signal);
      return;
    }
