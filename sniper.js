...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = [], triggerReason = "") {
  ...
  if (!isWeekend() && topMovers.length > 0) {
    ...
    // Only show stock movers if US is open or within 2hrs of open (pre-market)
    // Before 12:30 UK the % is yesterday's close -- stale and misleading
    const ukMinsNow = getUkMinutes();
    const nearUSOpen = ukMinsNow >= 750; // 12:30 UK onwards
    if (!isUSMarketOpen() && !nearUSOpen) {
      log("📊 Stock movers skipped -- too early, % would be yesterday\'s close");
      // fall through without adding stock section
    } else {
    const sessionLabel = isUSMarketOpen() ? "today" : "pre-market";
    parts.push(`🇺🇸 <b>Stocks attracting attention</b> <i>(${sessionLabel})</i>:`);
  ...
  if (filteredCryptoMovers.length > 0) {
    ...
    for (const c of filteredCryptoMovers) {
      ...
      const period  = isUSMarketOpen() ? "today" : "prev session";
      parts.push(`  ${badge} <b>${escapeHtml(sym)}</b> +${c.changePct.toFixed(1)}% <i>(${period})</i>`);
...
async function safeCycle() {
  ...
  try {
    ...
    if (nowProtecting && !macroProtecting && alertCooldown) {
      ...
      // V104: Market Update alert removed entirely.
      // Regime doesn't block trades, Opening Bell Intel covers conditions.
      log(`🛡️ Macro protection active (internal only -- no public alert)`);

    } else if (!nowProtecting && macroProtecting) {
      macroProtecting = false;
      lastMacroAlert  = Date.now();
      log(`✅ Macro protection lifted (internal only)`);
    }
