...
// TWO evidence-based fixes from real diagnostic data: (1) EMA
// crossover now allows 0.3% tolerance instead of a strict cross --
// real data showed 38%% of momentum-positive ema_fail rejects were
// within 0.3%% of crossing with the rising-EMA condition already
// satisfied. (2) Volume-data fallback extended from HIGH_BETA-only
// to all US stocks -- CRWD showed the same missing-data problem on
// a non-HIGH_BETA name. no_setup investigated separately and left
// unchanged -- real data confirmed it is a genuine volume shortfall,
// not a near-miss pattern. Plus all prior session fixes.
// ================================================================
...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = []) {
  ...
  if (!isWeekend() && topMovers.length > 0) {
    ...
    for (const sym of topMovers.slice(0,5)) {
      ...
      intelRows.push({ category: "US_MOVER", symbol: sym, tag: "gaining momentum", change_pct: stockMoverPcts[sym] ?? null });
    }
    parts.push("");
  }
