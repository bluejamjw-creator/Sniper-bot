...
// rejection_outcomes now also records MFE/MAE in both %% and
// R-multiples over the full 24h walk -- makes NEITHER outcomes
// informative too (median MFE near 1.4R vs 0.4R tell very
// different stories). Episode-clustering (grouping consecutive
// rejections of the same move into one opportunity) intentionally
// left as an analysis-time SQL technique, not baked into the
// write path. Plus all prior session fixes.
// ================================================================
...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = []) {
  ...
  if (!isWeekend() && topMovers.length > 0) {
    ...
    for (const sym of topMovers.slice(0,5)) {
      ...
      parts.push(`  🚀 <b>${escapeHtml(sym)}</b> — gaining momentum${preMarketTag}`);
      intelRows.push({ category: "US_MOVER", symbol: sym, tag: `gaining momentum${preMarketTag}`, change_pct: stockMoverPcts[sym] ?? null });
    }
    parts.push("");
  }
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  if (supabaseEnabled) {
    try {
      const grind = computeGrindMetrics(closes);
      if (grind) {
  ...
  for (const check of checks) {
    try {
      ...
      const selectFields = check.field === "checked_24h"
        ? "id, symbol, market, price_at_rejection, hypothetical_sl, hypothetical_tp"
      ...
      for (const row of rows) {
        try {
          ...
          // V102: at 24h, walk the available candle history checking high/low
          // against hypothetical SL/TP in chronological order -- APPROXIMATION,
          // not a precise from-rejection backtest, since candle objects don't
          // consistently carry a timestamp field across every fetch path. Uses
          // whatever depth the provider returns (~60-100 candles).
