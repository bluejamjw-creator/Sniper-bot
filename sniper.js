...
// Adds: reject diagnostic instrumentation for momentum_low/no_setup
// (nearest-miss detail, batched into sniper_signal_decisions,
// engine_version="V101"), Discord alongside Telegram for
// #signals + #market-intel, and all prior session fixes.
// ================================================================
...
// V102 candidate rollout stage -- controls whether structure quality can
// score or hard-reject. Bump manually as confidence builds (see rollout notes).
// "log"   -- compute + attach only, no scoring, no rejects
// "score" -- add to score, no rejects
// "gate"  -- add to score AND hard-reject on falseBreakout/weakBase
...
function passesStructureSafetyGates(structureQuality) {
  ...
  if (STRUCTURE_QUALITY_MODE !== "gate") return { ok: true, reasons: [] };
  const gatedReasons = new Set(["false_breakout", "weak_base"]);
  const reasons = (structureQuality.rejectReasons || []).filter(r => gatedReasons.has(r));
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  if (momentum <= 0) {
    ...
    if (market !== "LSE") {
      ...
      rejectDiagnostics.push({
        ...
        notes: `momentum ${(momentum*100).toFixed(3)}% (need > 0%, soft-penalty floor ${(minMomentum*100).toFixed(3)}%)`
      });
    }
    return null;
  ...
  if (!setupType) {
    ...
    rejectDiagnostics.push({
      ...
      notes: `break=${breakoutSignal} pull=${pullbackSignal} ign=${ignitionSignal} tr=${trendResumption} gap=${gapContinuationSignal} broad=${broadMomentum} `
           + `| vol=${volRatio.toFixed(2)}x (breakoutNeed=${breakoutVolNeeded.toFixed(2)} pullbackNeed=${pullbackVolNeeded.toFixed(2)} ignitionNeed=2.50) `
           + `| breakDist=${(breakoutDistance*100).toFixed(2)}% (limit=${(breakoutDistLimit*100).toFixed(2)}%) `
           + `| nearEMA=${nearEMA20} | mom=${(momentum*100).toFixed(2)}% (ignitionNeed=${(ignitionMomThreshold*100).toFixed(2)}%)`
    });
    return null;
  }
  ...
  // ── V102 CANDIDATE: STRUCTURE QUALITY ENGINE ─────────────────
  // Stage 1 (current, STRUCTURE_QUALITY_MODE = "log"): compute + attach only.
  // No score impact, no rejects, until logged data justifies turning it on.
  ...
  const combinedScore = hunterScore; // FIX: was referenced below before being defined -- ReferenceError on every call
