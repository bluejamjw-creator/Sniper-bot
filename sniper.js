// ================================================================
// SNIPER V78
// + Wider breakout distance detection (catch moves in motion)
// + Higher vol floor (1.4x crypto, 1.3x US for breakouts)
// + Score floor raised to 78 globally
// + Force close tightened: 4hrs / 20% progress
// + Stronger momentum confirmation required
// + Emojis restored (mixed style)
// + Crypto window 06:00–22:00 UTC
// + Market open/close alerts all three markets
// ================================================================

console.log("🚀 SNIPER V78 STARTING...");
...
app.get("/", (_req, res) => res.status(200).send("SNIPER V78 alive"));
...
// ── WIDENED: catch breakouts already in motion ───────────────────
// Old: 0.015 / 0.02 / 0.025
const BREAKOUT_MAX_DISTANCE_CRYPTO = 0.03;
const BREAKOUT_MAX_DISTANCE_US     = 0.04;
const BREAKOUT_MAX_DISTANCE_LSE    = 0.04;

// ── RAISED: higher volume required to confirm breakout ───────────
// Old crypto: 1.30, US: 1.15, LSE: 1.10
const CRYPTO_PULLBACK_VOL  = 1.20;
const US_PULLBACK_VOL      = 1.20;
const LSE_PULLBACK_VOL     = 1.10;

const CRYPTO_BREAKOUT_VOL  = 1.60;  // was 1.30
const US_BREAKOUT_VOL      = 1.40;  // was 1.15
const LSE_BREAKOUT_VOL     = 1.25;  // was 1.10

// ── RAISED: momentum override now needs stronger confirmation ────
const STOCK_MOMENTUM_OVERRIDE_ENABLED  = true;
const STOCK_OVERRIDE_MIN_MOMENTUM      = 0.015; // was 0.01
const STOCK_OVERRIDE_MIN_VOL           = 1.40;  // was 1.20
const STOCK_OVERRIDE_MAX_DISTANCE      = 0.025; // was 0.02
...
// ── RAISED: global score floor 72 → 78, markets tightened ───────
const MIN_GLOBAL_SCORE = 78;  // was 72
const MIN_US_SCORE     = 80;  // was 74
const MIN_LSE_SCORE    = 76;  // was 70
...
// ── RAISED: stronger candle momentum required ────────────────────
// Old US: 0.01, LSE: 0.005
const MIN_US_MOMENTUM  = 0.015; // was 0.01
const MIN_LSE_MOMENTUM = 0.008; // was 0.005

const MIN_PULLBACK_MOMENTUM_CRYPTO = 0.010; // was 0.008
const MIN_PULLBACK_MOMENTUM_US     = 0.015; // was 0.01
const MIN_PULLBACK_MOMENTUM_LSE    = 0.008; // was 0.005
...
// ── TIGHTENED: kill stale trades faster ─────────────────────────
// Old: 6hrs / 30% progress
const FORCE_CLOSE_HOURS    = 4;   // was 6
const FORCE_CLOSE_PROGRESS = 0.2; // was 0.3
...
// ── Market state tracking for open/close alerts ──────────────────
...
// ── Market open/close alert messages ────────────────────────────
...
function buildMarketCloseMessage(market) {
  if (market === "CRYPTO") {
    return lines([
      "🔴💤 <b>CRYPTO WINDOW CLOSED</b>",
      "",
      "😴 Scanning paused until 06:00 UTC"
    ]);
  }
  if (market === "US") {
    return lines([
      "🔴📉 <b>US MARKET CLOSED</b>",
      "",
      "🏁 NYSE/NASDAQ session ended"
    ]);
  }
  if (market === "LSE") {
    return lines([
      "🔴🏦 <b>LSE MARKET CLOSED</b>",
      "",
...
async function checkMarketAlerts() {
  ...
  for (const { key, isOpen } of checks) {
    ...
    const wasOpen = marketWasOpen[key];

    if (nowOpen && !wasOpen) {
      await send(buildMarketOpenMessage(key));
    } else if (!nowOpen && wasOpen) {
      await send(buildMarketCloseMessage(key));
    }
...
// ── Signal & trade messages with mixed emoji style ───────────────
...
function setupEmoji(setupType) {
  if (setupType === "BREAKOUT_CONTINUATION") return "💥";
  if (setupType === "PULLBACK_CONTINUATION") return "🎯";
  if (setupType === "MOMENTUM_BREAKOUT")     return "🔥";
  return "📊";
}

function marketEmoji(market) {
...
function buildSignalMessage(signal, size) {
  return lines([
    ...
    `${setupEmoji(signal.setupType)} ${escapeHtml(signal.setupType)} • ${escapeHtml(signal.entryType)}`,
    ``,
...
function analyse(asset, candles, market) {
  ...
  // ── Bonus: tight breakout distance = cleaner setup ───────────
  if (breakoutDistance > 0 && breakoutDistance <= 0.01) score += 4;

  // ── Bonus: strong vol on breakout specifically ───────────────
  if (setupType !== "PULLBACK_CONTINUATION" && volRatio >= 2.0) score += 4;
...
app.listen(PORT, "0.0.0.0", async () => {
  ...
  await send("🚀🔥 SNIPER V78 LIVE — higher standards, better trades");
  await runCycle();
  setInterval(async () => {
