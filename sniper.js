...
function addHeat(asset, points, reason) {
  ...
  const newScore = Math.min(150, decayed + points);
...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = []) {
  ...
  if (!isWeekend() && topMovers.length > 0) {
    ...
    for (const m of ranked) {
      ...
      const heat   = getHeat(m.sym) >= HEAT_HOT_THRESHOLD ? " \uD83D\uDD25" : "";
      parts.push(`  \uD83D\uDE80 <b>${escapeHtml(m.sym)}</b> ${escapeHtml(pctStr)}${fire}${brain}${heat}`);
    }
  ...
  if (filteredCryptoMovers.length > 0) {
    ...
    for (const c of filteredCryptoMovers) {
      ...
      const heat = getHeat(c.symbol) >= HEAT_HOT_THRESHOLD ? " \uD83D\uDD25" : "";
      const tag  = c.changePct >= 15 ? "\uD83D\uDD25 explosive move"
                 : c.changePct >= 8  ? "\u26A1 strong momentum"
                 : "\uD83D\uDCC8 building momentum";
      parts.push(`  \uD83D\uDCCA <b>${escapeHtml(sym)}</b> \u2014 ${tag}${heat}`);
    }
    parts.push("");
  }
  ...
  // ── HEAT INDEX -- show assets building momentum before signal fires ─
  const hotAssets = getHotAssets().slice(0, 5);
  if (hotAssets.length > 0) {
    parts.push("\uD83D\uDD25 <b>Heat Index:</b>");
    for (const h of hotAssets) {
      const bars  = "\u2588".repeat(Math.floor(h.heat / 15)).padEnd(10, "\u2591");
      const sym   = h.asset.replace("USDT","");
      parts.push(`  <code>${escapeHtml(sym.padEnd(6))} ${bars} ${Math.round(h.heat)}</code>`);
    }
    parts.push("");
  }
...
// ================================================================
// V99: NO REGRET ENGINE
// Final quality gate before any signal fires
// "Would a professional quant desk take this trade right now?"
// Asks 5 questions -- any critical failure = suppress
// ================================================================
function passesNoRegretCheck(signal, allSignals) {
  const reasons = [];

  // Q1: Is there enough room to target 1.8R? (already checked but confirm)
  if (!Number.isFinite(signal.rr) || signal.rr < 1.8) {
    reasons.push(`R:R ${signal.rr?.toFixed(1)} < 1.8`);
  }

  // Q2: Is momentum still positive? (not a stale setup)
  if (signal.momentum <= 0) {
    reasons.push(`momentum flat/negative`);
  }

  // Q3: Is this meaningfully better than other candidates in same market?
  const sameMarket = allSignals.filter(s => s.market === signal.market && s !== signal);
  if (sameMarket.length > 0) {
    const bestOther = Math.max(...sameMarket.map(s => s.score));
    if (bestOther > signal.score + 15) {
      reasons.push(`better candidate exists (score ${bestOther} vs ${signal.score})`);
    }
  }

  // Q4: Not a pure late breakout with no heat backing it
  const heat = getHeat(signal.asset);
  if (signal.setupType === "BREAKOUT_CONTINUATION" && heat < 20 && signal.score < 95) {
    reasons.push(`late breakout with low heat (heat=${heat.toFixed(0)})`);
  }

  // Q5: Volume is genuine (not a single spike)
  if (signal.volRatio < 1.2) {
    reasons.push(`volume weak (${signal.volRatio?.toFixed(1)}x)`);
  }
  ...
  log(`✅ NO REGRET: ${signal.asset} passes all checks`);
  return true;
}
...
function pickTopSignals(results) {
  ...
  return selected.sort((a,b) => (b.opportunityRank||0) - (a.opportunityRank||0));
}
