...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = [], triggerReason = "") {
  ...
  if (triggerReason === "regime change") {
    ...
    if (btcChanged) {
      ...
      parts.push(`₿ <b>Bitcoin</b> changed from ${fromLabel} to ${toLabel} ${toE}`, ``);
      parts.push(`🧠 <b>What does that mean?</b>`);
      if (btcTrend==="bull"||btcTrend==="strong_bull")
        parts.push(`The overall crypto market is becoming healthier. Historically, more quality trading opportunities appear when Bitcoin is in a bullish trend.`);
    ...
    if (qqqChanged) {
      ...
      parts.push(``, `📈 <b>US stocks (QQQ)</b> changed from ${fromLabel} to ${toLabel} ${toE}`, ``);
      parts.push(`🧠 <b>What does that mean?</b>`);
...
async function sendV103HealthDashboard() {
  const isSunday = new Date().getDay() === 0;
  if (!isSunday) return;
  const today = new Date().toISOString().slice(0, 10);
  ...
  const thisWeek    = trades.filter(t => {
    ...
    const d = new Date(t.closedAt || t.sentAt);
    const now = new Date();
  ...
  // Highest score seen this week -- from calibration/signal log
  const allScores   = thisWeek.map(t => t.score).filter(Boolean);
  const topScore    = allScores.length > 0 ? Math.max(...allScores) : null;

  const intelCount  = narrativePostCount; // posts since last reset (today)

  const parts = [
    `📊 <b>V103 HEALTH DASHBOARD</b>`,
    `<i>Week ending ${new Date().toLocaleDateString("en-GB", {day:"numeric", month:"short"})}</i>`,
    ``,
    `🎯 <b>Trade alerts issued:</b>  ${alerts.length}  ${alerts.length >= 2 ? "✅" : alerts.length >= 1 ? "🟡" : "🔴"}`,
    `   Target: 2–4 per week`,
    ``,
    `✅ <b>Win rate:</b>  ${winRate !== null ? winRate+"%" : "No closed trades yet"}  ${winRate >= 60 ? "✅" : winRate >= 50 ? "🟡" : winRate !== null ? "🔴" : "⚪"}`,
    `   Target: ≥60%`,
    ``,
    `🧠 <b>Intel posts today:</b>  ${intelCount}`,
    `   Target: event-driven, not repetitive`,
    ``,
    `🚫 <b>Top rejection reason:</b>  ${topBlocker}`,
    `   Watch for: global_floor or rr_too_low dominating`,
    ``,
    `📈 <b>Highest score seen:</b>  ${topScore !== null ? topScore : "No data yet"}`,
    `   Watch for: scores clustering near 75 (Growth Mode floor)`,
    ``,
    `——————————`,
    `<i>V103 Growth Mode: Global 75 · Crypto 75 · RR 1.75/1.6 · Elite threshold: ranking only</i>`,
  ];

  await sendPrivate(lines(parts));
  log("📊 V103 health dashboard sent to Bluejam");
}
...
async function checkMarketAlerts() {
  ...
  await safeRun("healthDashboard", sendV103HealthDashboard);
