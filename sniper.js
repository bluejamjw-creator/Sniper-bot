...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  if (heatScore >= HEAT_HOT_THRESHOLD) {
    const heatBonus = Math.min(15, Math.floor(heatScore / 10));
    score += heatBonus;
    log(`🔥 HEAT BONUS: ${asset} +${heatBonus} (heat=${heatScore.toFixed(0)})`);
  }
...
function shouldForceClose(trade, livePrice) {
  ...
  // ── CRYPTO: hierarchical structure-based exits ───────────────────
  // TON/BTC pattern: builds all day, runs overnight
  // Time-based 2hr cap was killing these trades
  // Exit hierarchy: structure break → regime flip → stale fallback
  if (trade.market === "CRYPTO") {
    // L2: Active breakdown -- structure clearly broken
    if (ageMins >= 30 && pricePct < -0.025)                        return true; // down 2.5%+ fast
    if (ageHrs  >= 2  && pricePct < -0.015)                        return true; // still losing after 2hrs
    // BTC regime deteriorated AND losing ground
    if (ageHrs  >= 1  && pricePct < -0.010 && btcTrend==="risk_off") return true;
    // L4: Stale fallback -- only after genuine long hold with zero movement
    if (ageHrs  >= 6  && mfePct < 0.003)                           return true; // 6hrs, never moved
    if (ageHrs  >= 12)                                              return true; // absolute cap
    return false;
  }
