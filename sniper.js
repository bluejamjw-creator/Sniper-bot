...
async function refreshDynamicPools(forceNarrativeAlert = false) {
  ...
  try {
    ...
    // V97: DexScreener supplements CoinGecko for early momentum detection
    const dexTrending = await getDexScreenerTrending();
    CRYPTO_PAIRS = dedup([...STATIC_CRYPTO_FALLBACK, ...trendingCoins, ...dexTrending.map(s => `${s}USDT`)]).slice(0, 22);
...
async function updateQQQTrend() {
  if (!ENABLE_US || !isUSMarketOpen()) {
    ...
    // Don't reset, just leave qqqTrend as-is from last update
    return;
  }
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  // Only ignition/breakout setups allowed on regime override
