...
function loadSignals(){
  try{return JSON.parse(fs.readFileSync(SIGNAL_FILE));}
  catch{return [];}
}
...
function loadTrades(){
  try{return JSON.parse(fs.readFileSync(TRADES_FILE));}
  catch{return [];}
}
...
function exportDashboard(){
  ...
  const formatted = trades
    ...
    .map(t => ({
      ...
      date: new Date(t.closedTime).toISOString()
    }));
...
function isPullback(m){
  ...
  return depth>0.02 && depth<0.06 && price>m.sma;
}

function isMomentumBreakout(m){
  const price = m.last;

  const nearHigh = price > m.high * 0.95;

  const momentum = (m.last - m.prev) / m.prev;
  const prevMove = (m.prev - m.closes.at(-3)) / m.closes.at(-3);

  return nearHigh && momentum > 0.015 && momentum > prevMove;
}
...
function buildSignal(m,type){
  ...
  if(!ema20 || !ema50 || ema20<=ema50) return null;
  ...
  if(breakout && momentum < 0.015) return null;

  const sl = breakout ? entry * 0.94 : Math.min(m.low*0.995,entry*0.97);
  const tp = breakout ? entry * 1.20 : Math.max(m.high * 1.02, entry * 1.05);
