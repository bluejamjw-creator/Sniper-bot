...
// MAJOR FIX: daily insight query was hitting Supabase default
// row cap (1000) with one unlimited combined query -- momentum_low
// volume (7000+/day) starved ema_fail/no_setup/regime of any real
// data (ema_fail showed 11 vs real 837). Now: exact count + 500-row
// sample per reason, scales regardless of volume. Also adds
// false_breakout diagnostic logging + insight (was previously
// invisible -- jumped 13->478 with zero explanation available).
// Plus crypto breadth failure logging, signalsFired counter fix,
// trades.sentToChannel column fix, and all prior session fixes.
// ================================================================
...
async function buildRejectInsightLines() {
  ...
  try {
    const [momLow, noSetup, emaFail, regime, falseBreakout] = await Promise.all(
      ["momentum_low", "no_setup", "ema_fail", "regime", "false_breakout"].map(getReasonStats)
    );
