...
//       actually changes, suppressed outside 08:00-21:00 UK + weekends
//   ✦ Crypto movers shown in narrative alert with % + momentum tag
//   ✦ Regime override lowered: 1.2% candle + 1.8× vol on 15-min
...
function getUtcMinutes()  { const n = new Date(); return n.getUTCHours() * 60 + n.getUTCMinutes(); }
...
function isMarketOpen(market) {
  ...
  if (market === "CRYPTO") return mins >= 300 && mins < 1260; // 06:00–22:00 UK BST
  if (isWeekend()) return false;                              // stocks never open weekends
  if (market === "US")  return mins >= 810 && mins < 1200;   // 14:30–21:00 UK BST
  if (market === "LSE") return mins >= 420 && mins < 930;    // 08:00–16:30 UK BST
  return false;
}
...
function isUSOpeningWindow()  {
  ...
  return m >= 810 && m < 900; // 14:30–16:00 UK
}

// alertable: weekday 08:00–21:00 UK, or weekend for crypto only
function isAlertableHours(market = "ALL") {
  if (market === "CRYPTO") {
    // Crypto alerts allowed all week during window
    return isCryptoWindowOpen();
  }
  // Stock/general alerts: weekdays only, 08:00–21:00 UK
  if (isWeekend()) return false;
...
// ── 24hr momentum cache (Binance — always free) ──────────────────
...
async function refreshDynamicPools() {
  ...
  try {
    ...
    // Stock pool and narrative: weekdays only
    if (!isWeekend()) {
      ...
      // Narrative alert: weekday alertable hours + change detection
      if (isAlertableHours("ALL")) {
      ...
      const cryptoMovers = await getCryptoMovers();
      if (narrativeHasChanged([], [], cryptoMovers) && isCryptoWindowOpen()) {
...
async function checkMarketAlerts() {
  ...
  for (const { key, isOpen } of checks) {
    ...
    if (nowOpen  && !marketWasOpen[key]) log(`🟢 ${key} open`);
    if (!nowOpen &&  marketWasOpen[key]) log(`🔴 ${key} closed`);
...
function analyse(asset, candles, market) {
  ...
  const extensionLimit  = market === "CRYPTO" ? CRYPTO_15M_EXTENSION_LIMIT : market === "US" ? EXTENSION_HARD_LIMIT_US : EXTENSION_HARD_LIMIT_LSE;
...
async function checkMomentumWatch() {
  ...
  // Crypto — always check during window
  ...
  // Stocks — weekdays only
  if (!isWeekend() && isUSMarketOpen() && canUseFinnhub()) {
...
function buildCloseMessage(trade, reason, closePrice) {
  ...
  return lines([
    ...
    `\uD83D\uDCC8 MFE: ${escapeHtml(mfe)}`,
    ``,
    buildCloseLesson(trade, reason, closePrice)
  ]);
}
...
function cooldown(asset, alerts, market) {
  ...
  return hoursAgo(last.sentAt) < (market === "CRYPTO" ? CRYPTO_COOLDOWN : market === "LSE" ? LSE_COOLDOWN : US_COOLDOWN);
}
...
async function scan() {
  ...
  // Crypto: always scan during window (weekends included)
...
async function safeCycle() {
  ...
  try {
    ...
    await safeRun("momentumWatch",  checkMomentumWatch);
...
server = app.listen(PORT, "0.0.0.0", async () => {
  ...
  await safeRun("bootTelegram", () => send(
    "\uD83D\uDE80\uD83D\uDD25 <b>SNIPER V88 LIVE</b>\n" +
    "15-min crypto candles \u2022 20-coin pool \u2022 24hr momentum aware\n" +
    "Weekend mode: crypto only \u2022 Heating up alerts active\n" +
    "Narrative alerts: change detection only \u2022 No noise\n" +
    "Next message will be a trade signal."
  ));
