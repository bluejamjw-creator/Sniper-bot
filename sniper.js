...
let cycleCount      = 0;
let lastSignalTime    = 0;    // timestamp of last fired signal
let lastWatchlistTime = 0;    // timestamp of last watchlist alert sent
let lastWatchlistKey  = "";   // fingerprint of last watchlist content — prevents repeats
...
async function safeCycle() {
  ...
  try {
    ...
    if (Array.isArray(signals) && signals.length > 0) {
      ...
      const quietEnough = lastSignalTime === 0 ? (Date.now() - lastWatchlistTime > 60*60*1000) : quietMs > 90*60*1000;
      if (quietEnough) {
        const alerts = await loadAlerts();
        const trades = await loadTrades();
