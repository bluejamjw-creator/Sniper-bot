// ================================================================
// SNIPER V90 — MOMENTUM MATURITY ENGINE
// ----------------------------------------------------------------
...
console.log("🚀 SNIPER V90 STARTING...");
...
app.get("/",       (_req, res) => res.status(200).send("SNIPER V90 alive"));
...
const EXTENSION_LIMIT_HB    = 0.20; // 20% normal, 30% opening window
const EXTENSION_LIMIT_HB_OW = 0.30;
...
const BREAKOUT_DIST_LSE  = 0.04;
const EXTENSION_LIMIT_LSE = 0.08;
const MOMENTUM_CAP_LSE   = 0.04;
const MIN_LSE_MOMENTUM   = 0.008;
const MIN_PULLBACK_MOM_LSE = 0.008;
...
// OSMO +200% = exhaustion score 8-9 → blocked
// SUI +11% building = exhaustion score 0-1 → passes
function isParabolicExhaustion(candles, pct24h) {
...
function getMomentumMaturityBonus(candles, pct24h, symbol) {
  ...
  if (maturity === "climax")       bonus -= 15; // Stage 4 — penalise hard

  // ATR units — volatility-adjusted extension
  if (atrUnits > 6)      bonus -= 12; // Way too extended
  else if (atrUnits > 4) bonus -= 6;  // Extended
  else if (atrUnits > 2) bonus += 4;  // Meaningful move
  else                   bonus += 8;  // Small vs ATR — likely early
...
// ── Market open/close + forced pool refresh ───────────────────────
...
function analyse(asset, candles, market) {
  ...
  if (isParabolicExhaustion(candles, pct24h)) {
  ...
  if (!pullbackSignal && !breakoutSignal && !strongMomentumOverride && !ignitionSignal && !gapContinuationSignal && !firstPullbackCont) {
    log(`⛔ ${asset} — no setup (break=${breakoutSignal} pull=${pullbackSignal} gap=${gapContinuationSignal} ign=${ignitionSignal} fpc=${firstPullbackCont} vol=${volRatio.toFixed(2)}x)`);
    return null;
  }
...
server = app.listen(PORT, "0.0.0.0", async () => {
  log(`🚀 SNIPER V90 on port ${PORT}`);
  await safeRun("bootTelegram", () => send(lines([
    "\uD83D\uDE80\uD83D\uDD25 <b>SNIPER V90 LIVE \u2014 MOMENTUM MATURITY ENGINE</b>",
    "",
    "\uD83C\uDF9EFD Dual-mode scanner: Trend Continuation + Momentum Runner",
    "\uD83D\uDCCA Move maturity: base \u2192 expansion \u2192 continuation \u2192 climax",
    "\uD83D\uDEAB Parabolic exhaustion hard block (OSMO-type blocked)",
    "\uD83D\uDCC8 HIGH_BETA adaptive thresholds: ASTS, RKLB, IONQ, OKLO...",
    "\uD83D\uDE80 GAP_CONTINUATION setup type added",
    "\u274C Heating up alerts: permanently removed"
  ])));
  await sleep(STARTUP_DELAY_MS);
  await safeRun("initialPools",  refreshDynamicPools);
  ready = true;
  log("\u2705 SNIPER V90 READY");
