  ...
  try {
    ...
    if (supabaseEnabled) {
      try {
        ...
        if (error) {
          log(`⚠️ Supabase connection test failed: ${error.message}`);

          await sendPrivate(
            `⚠️ <b>Supabase connection failed</b>\n${error.message}`
          );
        } else {
    ...
    await sendPrivate(
      `🚀 <b>SNIPER V91 ONLINE</b>\n` +
      `Dual-mode momentum engine active\n` +
      `Crypto: ${CRYPTO_PAIRS.length} pairs\n` +
      `Stocks: ${STOCK_POOL.length} symbols\n` +
      `LSE: ${LSE_POOL.length} symbols`
    );
    ...
    log("BOOT FAILURE:", err?.message || err);

    await sendPrivate(
      `❌ <b>SNIPER V91 BOOT FAILURE</b>\n${err?.message || err}`
    );
