...
const SCAN_INTERVAL_BEAR_MS    = 90000;   // 90s in bear -- was 5mins, too slow to catch early moves
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  if (isMarketBlocked(market)) {
    // V100 ELITE RUNNER OVERRIDE: if something is screaming on huge volume,
    // let it through regardless of macro. RKLB +5% on 3x volume doesn't care
    // that QQQ is neutral. The market is telling us something.
    const eliteRunner = momentum > 0.05 && volRatio >= 3.0;
    if (!eliteRunner) {
      reject("macro_protection", asset, market);
      return null;
    }
    log(`🚀 ELITE RUNNER OVERRIDE: ${asset} mom=${(momentum*100).toFixed(2)}% vol=${volRatio.toFixed(1)}x -- bypassing macro gate`);
  }

  // ── Regime ────────────────────────────────────────────────────
  // V100 SOFTENED: BTC risk_off = hard block (confirmed downtrend, don't trade crypto)
  // BTC neutral = score penalty only -- ranging BTC with strong individual momentum is tradeable
  // BTC bull/strong_bull = full green light
  if (market === "CRYPTO") {
    const btcBull    = btcTrend === "bull" || btcTrend === "strong_bull";
    const btcRiskOff = btcTrend === "risk_off" || btcMacroTrend === "risk_off";
    if (btcRiskOff) {
      checkpoint("regime", false, `btc=${btcTrend} macro=${btcMacroTrend} -- risk_off hard block`);
      reject("btc_not_bull", asset, market); return null;
    }
    if (!btcBull) {
      // Neutral -- apply score penalty but don't kill the trade
      checkpoint("regime", false, `btc=${btcTrend} -- neutral, -15 score penalty`);
      softPenalties.btcNeutral = 15;
    } else {
  ...
  const totalSoftPenalty = (momentumPenalty||0) + (emaPenalty||0) + (volumePenalty||0) + (heatPenalty||0) + (softPenalties.btcNeutral||0);
