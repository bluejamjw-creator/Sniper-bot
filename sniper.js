// ================================================================
// SNIPER V96 -- MOMENTUM MATURITY ENGINE
// ----------------------------------------------------------------
// V96 changes from V95:
//   - MIN_CRYPTO_SCORE raised 72->78 -- fewer higher quality signals
//   - PULLBACK_CONTINUATION requires genuine 1%+ dip from recent high
//     prevents firing on sideways consolidation near EMA
//   - Strict single pending per market -- hard block, no exceptions
//     eliminates pile-up of unfilled pending trades
// ================================================================
...
console.log("🚀 SNIPER V96 STARTING...");
...
app.get("/",       (_req, res) => res.status(200).send("SNIPER V96 alive"));
...
const MIN_US_SCORE_HB       = 70;   // V95: lowered from 74 -- high-beta needs room to breathe
...
const MIN_CRYPTO_SCORE = 78;  // V96: raised from 72 -- fewer but higher quality signals
const MIN_US_SCORE     = 72;  // V95: lowered from 76 -- matches crypto floor
...
async function analyse(asset, candles, market) {
  ...
  if (extension > extensionLimit) {
    log(`BLOCKED ${asset} | reason=extension | ext=${(extension*100).toFixed(2)}% limit=${(extensionLimit*100).toFixed(2)}% mode=${useMode2?"2":"1"}`);
    return null;
  }
  ...
  if (isRegimeOverride && setupType === "PULLBACK_CONTINUATION") {
    log(`⛔ ${asset} -- PULLBACK_CONTINUATION blocked on BTC neutral regime override`);
    return null;
  }
  ...
  if (riskPct <= 0 || riskPct > 0.08) return null; // V96: max 8% stop -- 15% was way too permissive
  ...
  // V94: RELATIVE STRENGTH vs QQQ -- leader detection
  // HIGH_BETA stocks stronger than QQQ get significant bonus
  if (market === "US" && highBeta) {
    const qqqMom = await getQQQMomentum();
  ...
  if (score < MIN_GLOBAL_SCORE || score < scoreFloor) {
    log(`BLOCKED ${asset} | reason=score | score=${score} floor=${scoreFloor} setup=${setupType}`);
    return null;
  }
...
async function scan() {
  ...
  async function processAsset(asset, market, fetcher) {
    ...
    const signal  = await analyse(asset, candles, market);
    if (!signal) return;
    results.push(signal);
  }
...
server = app.listen(PORT, "0.0.0.0", async () => {
  log(`🚀 SNIPER V96 on port ${PORT}`);
  await safeRun("bootTelegram", () => sendPrivate(lines([
    "\uD83D\uDE80 SNIPER V96 LIVE -- MOMENTUM MATURITY ENGINE",
    "",
