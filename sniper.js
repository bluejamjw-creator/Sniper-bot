// ================================================================
// SNIPER V83
// + Exceptional crypto override — momentum 2.5%+ and volume 2x+
//   bypasses BTC regime filter for A* setups
//   Catches explosive altcoin moves like ONDO regardless of BTC trend
// + All V82 logic preserved
// ================================================================

console.log("🚀 SNIPER V83 STARTING...");
...
app.get("/",       (_req, res) => res.status(200).send("SNIPER V83 alive"));
...
function isCryptoWindowOpen() {
  // 06:00–22:00 UK (BST) = 05:00–21:00 UTC
  const h = new Date().getUTCHours();
  return h >= 5 && h < 21;
}

function isUSMarketOpen() {
  // NYSE/NASDAQ: 14:30–21:00 UK (BST) = 13:30–20:00 UTC
  const now = new Date();
  const day = now.getUTCDay();
  if (day === 0 || day === 6) return false;
  const m = now.getUTCHours() * 60 + now.getUTCMinutes();
  return m >= 810 && m < 1200; // 13:30–20:00 UTC
}

function isUSOpeningWindow() {
  // First 90 mins after US open: 14:30–16:00 UK = 13:30–15:00 UTC
  const now = new Date();
  const day = now.getUTCDay();
  if (day === 0 || day === 6) return false;
  const m = now.getUTCHours() * 60 + now.getUTCMinutes();
  return m >= 810 && m < 900; // 13:30–15:00 UTC
}

function isLSEMarketOpen() {
  // LSE: 08:00–16:30 UK (BST) = 07:00–15:30 UTC
  const now = new Date();
  const day = now.getUTCDay();
  if (day === 0 || day === 6) return false;
  const m = now.getUTCHours() * 60 + now.getUTCMinutes();
  return m >= 420 && m < 930; // 07:00–15:30 UTC
}
...
async function fetchUSData(symbol) {
  // V82: Use 15-min candles for first 90 mins after US open, 1h otherwise
  const use15min = isUSOpeningWindow();
  const interval  = use15min ? "15min" : "1h";
  const outputsize = use15min ? 40 : 70; // 40 x 15min = 10 hours of data
  const cacheKey  = `us_${symbol}_${interval}`;

  const cached = getCached(cacheKey);
  if (cached) return cached;

  let candles = await fetchTwelveDataSeries(symbol, interval, outputsize);
...
// ── Exceptional override thresholds ─────────────────────────────
// If a crypto setup is this strong it fires regardless of BTC regime
const REGIME_OVERRIDE_MIN_VOL      = 2.0;   // must be 2x+ volume
const REGIME_OVERRIDE_MIN_MOMENTUM = 0.025; // must be 2.5%+ candle
const REGIME_OVERRIDE_SCORE_FLOOR  = 85;    // must score 85+ (A* only)
...
function analyse(asset, candles, market) {
  ...
  // ── Regime check with exceptional override ──────────────────
  // Normally blocked if BTC isn't bullish — but if momentum and
  // volume are exceptional we let it through as a special case
  ...
  if (exceptionalCrypto && !regimePass(market)) {
    console.log(`\u26A1 Regime override for ${asset} — vol ${volRatio.toFixed(2)}x mom ${(momentum*100).toFixed(2)}% bypasses BTC filter`);
  }
...
app.listen(PORT, "0.0.0.0", async () => {
  ...
  await send(
    "\uD83D\uDE80\uD83D\uDD25 SNIPER V83 LIVE\n" +
    "Exceptional crypto override active \u2022 ONDO-type moves now caught\n" +
    "2.5%+ momentum + 2x volume bypasses BTC regime filter"
  );
