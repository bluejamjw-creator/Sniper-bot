...
if(!fs.existsSync(STATS_FILE)) fs.writeFileSync(STATS_FILE,JSON.stringify({balance:START_BALANCE},null,2));
...
function analyse(symbol, closes){
  ...
  const trendScore =
    trendStrength > 0.02 ? 25 :
    trendStrength > 0.01 ? 15 : 0;
...
async function manageTrades(){
  ...
  for(const t of trades){
    ...
    // TP
    if(price >= t.tp2 && !t.tp_hit){
      t.tp_hit = true;
      stats.balance *= 1.05;
      send(`💰 TP ${t.asset} | Balance £${stats.balance.toFixed(2)}`);
    }
    ...
    if(price <= t.sl){
      t.status = "closed";
      stats.balance *= 0.98;
      send(`❌ SL ${t.asset} | Balance £${stats.balance.toFixed(2)}`);
    }
  }
