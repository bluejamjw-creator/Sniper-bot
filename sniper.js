...
async function logSignalDecision(d) {
  ...
  try {
    ...
    log(`⚠️ Signal decision log failed: ${e.message}`);
  }
}
...
function narrativeHasChanged(sectors, stockMovers, cryptoMovers, lseMovers = []) {
  ...
  const tier1 = newSector || newStockIn || stockLeft || newCryptoIn || cryptoLeft || regimeChanged;
  ...
  const anyChange =
    ...
    regimeChanged;
  const tier3 = !tier1 && !heatJumped && timeSinceLast >= 2 * 60 * 60 * 1000 && anyChange;
...
async function checkMarketAlerts() {
  ...
  for (const { key, isOpen } of checks) {
    ...
    if (nowOpen && !marketWasOpen[key]) {
      ...
      if (key !== "CRYPTO") {
        const msgs = {
          US:  lines([`🟢🇺🇸 <b>US MARKET OPEN</b>`, ``, `📡 QQQ: <b>${escapeHtml(qqqTrend.replace("_"," ").toUpperCase())}</b> · ₿ BTC: <b>${escapeHtml(btcTrend.replace("_"," ").toUpperCase())}</b>`, `🔍 Scanning ${STOCK_POOL.length} stocks · ${CRYPTO_PAIRS.length} crypto · ${LSE_POOL.length} LSE`, ``, `Sniper is now active. A trade alert is only issued if all entry conditions confirm.`, ``, `——————————`, `📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`]),
          LSE: lines([`🟢🇬🇧 <b>LSE MARKET OPEN</b>`, ``, `🔍 Scanning ${LSE_POOL.length} UK stocks`, ``, `——————————`, `📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`])
        };
