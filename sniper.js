...
async function sendRejectStatsSummary() {
  ...
  const lines2 = [
    ...
    "",
    "💡 Top blocker = priority fix for tomorrow"
  ];
  await safeRun("rejectStats", () => sendPrivate(lines(lines2)));
}
...
async function fetchTwelveDataSeries(symbol, interval="1h", outputsize=70) {
  ...
  // Hard daily credit guard -- never exceed 700 credits regardless of window
...
async function fetchYahooFinanceSeries(symbol, interval="1h") {
  ...
  const range         = yahooInterval==="5m"?"2d":yahooInterval==="15m"?"5d":"30d";
  try {
    const { data } = await retry(() =>
...
async function fetchUSData(symbol) {
  ...
  // TwelveData nuclear guard -- ONLY use TD credits 14:00-15:30 UK
  const ukMins = getUkMinutes();
  ...
  if (!candles) { log(`🆓 Yahoo 1h for ${symbol}`); candles = await fetchYahooFinanceSeries(symbol, interval); }
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  if (market === "CRYPTO") {
    ...
    if (momentumDecaying) {
      reject("momentum_decay", asset); return null;
    }
  ...
  if (momentum <= minMomentum) {
    if (market !== "LSE") reject("momentum_low", asset); return null;
    return null;
  }
  ...
  if (useMode2 && inUSOpen) {
    ...
    // LSE mode -- relaxed EMA check for UK grinders
    emaOk = ema20 > ema50 && ema20 > ema20Prev;
  } else {
  ...
  if (!emaOk && !isTrendResumptionCandidate) {
    reject("ema_fail", asset); return null;
  }
  ...
  if (extension > extensionLimit) {
    reject("extension", asset); return null;
  }
  ...
  if (!regimePass(market) && !exceptionalCrypto) {
    reject("regime", asset); return null;
  }
  ...
  if (!setupType) {
    reject("no_setup", asset); return null;
  }
  ...
  if (riskPct <= 0 || riskPct > maxRisk) { reject("risk_pct", asset); return null; }

  const tp = entry + riskAmt * TARGET_R;
  const rr = +((((tp - entry)/entry)/riskPct).toFixed(1));
  if (rr < MIN_RR) { reject("rr_too_low", asset); return null; }
  ...
  if (score < MIN_GLOBAL_SCORE || score < scoreFloor) {
    reject("score_floor", asset); return null;
  }
...
async function processSignals(signals) {
  ...
  for (const signal of signals) {
    ...
    if (pendingTrades.length >= 1) continue; // hard block -- one pending at a time
