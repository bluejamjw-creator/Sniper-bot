...
async function fetchUSData(symbol) {
  ...
  // TwelveData guard -- only use TD credits in active trading windows
  // 07:30-11:30 UK (LSE + pre-market awareness) and 14:00-21:00 UK (US session)
  // Prevents burning 800 daily credits during quiet hours when Yahoo is sufficient
  const ukMins = getUkMinutes();
  const inTDWindow = isWeekday() && (
    (ukMins >= 450 && ukMins < 690) ||   // 07:30-11:30 UK
    (ukMins >= 840 && ukMins < 1260)     // 14:00-21:00 UK
  );
...
async function fetchLSEData(symbol) {
  ...
  const cached   = getCached(cacheKey);
  if (cached) return cached;
  // TwelveData guard -- only use TD credits 07:30-11:30 UK (LSE active window)
  const ukMins = getUkMinutes();
  const inTDWindow = isWeekday() && ukMins >= 450 && ukMins < 690; // 07:30-11:30 UK
  let candles = inTDWindow ? await fetchTwelveDataSeries(symbol,"1h",70) : null;
