...
function alertCooldownActive(asset, alerts) {
  ...
  return found ? hoursAgo(found.sentAt) < COOLDOWN_HOURS : false;
}

function needsExecutionUpdate(alert, trades) {
  if (!alert || alert.kind !== "entry" || alert.updated) return false;
  const trade = trades.find(t => t.asset === alert.asset && t.status === "open");
  return !!trade && minutesAgo(alert.sentAt) >= EXECUTION_CHECK_MINUTES;
}

function overnightUpdateNeeded(trade) {
  return !!trade && trade.status === "open" && hoursAgo(trade.createdAt) >= OVERNIGHT_HOURS;
}
...
function makeSummary(label, trades) {
  ...
  return `📊 ${label.toUpperCase()} PERFORMANCE
...
async function scan() {
  ...
  for (const pair of CRYPTO_PAIRS) {
    ...
    try {
      const signal = analyse(pair, await fetchCrypto(pair), "crypto");
  ...
  if (stockAllowed()) {
    for (const stock of STOCK_POOL) {
      ...
      try {
        const signal = analyse(stock, await fetchStock(stock), "stock");
        if (signal) results.push(signal);
      } catch {}
    }
  }
...
async function processSignals(signals) {
  ...
  for (const s of signals) {
    ...
    const msg = `${s.type === "crypto" ? "🚀 CRYPTO BREAKOUT" : "📈 STOCK BREAKOUT"}${formatSessionTag(s.type)}
...
app.get("/", (_req, res) => res.send("SNIPER V29 RUNNING 🚀"));
