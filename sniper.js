// ================================================================
// SNIPER V98 -- MOMENTUM MATURITY ENGINE
// ----------------------------------------------------------------
...
console.log("🚀 SNIPER V98 STARTING...");
...
app.get("/",       (_req, res) => res.status(200).send("SNIPER V98 alive"));
...
const SCAN_TIMEOUT_MS          = 75000;  // V98: raised from 55s -- LSE Stooq fallback needs more time
...
const MIN_RR            = 1.8;  // V98: raised from 1.5 -- quality over quantity
const TARGET_R          = 1.8;
...
const MIN_GLOBAL_SCORE = 65;        // V98: lowered from 70 -- give stocks room
const MIN_CRYPTO_SCORE = 78;        // kept high -- crypto quality over quantity
const MIN_US_SCORE     = 68;        // V98: lowered from 72
const MIN_LSE_SCORE    = 60;        // V98: lowered from 64
...
async function fetchLSEData(symbol) {
  ...
  const cached   = getCached(cacheKey);
  if (cached) return cached;
  // V98: LSE Yahoo throttling causes scan timeouts -- add Stooq fallback
  // Yahoo sequential failures = 70+ seconds per scan cycle killing the bot
  let candles = null;
  try {
    candles = await Promise.race([
      fetchYahooFinanceSeries(symbol, "1h"),
      new Promise((_, rej) => setTimeout(() => rej(new Error("LSE Yahoo timeout")), 3000))
    ]);
  } catch { /* Yahoo throttled or timed out -- fall through to Stooq */ }
  if (!candles) {
    try {
      const stooqSym = symbol.replace(".L", ".uk").toLowerCase();
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  if (!regimePassRelaxed(market, setupType, volRatio) && !exceptionalCrypto) {
  ...
  const breakoutSignal = last >= breakoutThreshold &&
    ...
    (market !== "US" || !inUSOpen || minsIntoSession >= 15);
  ...
  const pullbackSignal = nearEMA20 && pulledBackRecently && volRatio >= pullbackVolNeeded && genuineDip
    && (market !== "US" || !inUSOpen || minsIntoSession >= 30);
  ...
  // V98: opening window HIGH_BETA gets wider ATR multiplier
  // IONQ wicked through 1.5% SL then recovered -- opening bell hunts stops
  // Extra room needed to survive the initial shake before the real move
  const openingMult = (market === "US" && highBeta && inUSOpen) ? 1.8 : 1.0;
  ...
  // V98: setup-specific risk limits
  ...
  // V98: dynamic crypto TP based on BTC regime
  ...
  if (market === "CRYPTO") {
    if      (btcTrend === "strong_bull") score += 15;
    else if (btcTrend === "bull")        score += 10;
    else if (btcTrend === "neutral")     score += 0;
    else if (btcTrend === "risk_off")    score -= 25; // V98: middle ground -- blocks weak setups, allows genuine runners
  }
  if (market === "US") {
    if      (qqqTrend === "strong_bull") score += 15;
    else if (qqqTrend === "bull")        score += 10;
    else if (qqqTrend === "neutral")     score += 0;
    else if (qqqTrend === "risk_off")    score -= 25;
  }
  ...
  // V98: B grade signals suppressed -- only A and A* published to channel
  // Quality over quantity -- subscribers get fewer but higher conviction signals
  const scoreFloor = minScoreForMarket(market, asset);
  if (score < MIN_GLOBAL_SCORE || score < scoreFloor || grade === "B") {
    if (grade === "B") {
      checkpoint("score", false, `${score} -- B grade suppressed (min A required)`);
...
async function sendPerformanceReport() {
  try {
    ...
    const closed = trades.filter(t =>
      ...
      new Date(t.closedAt).getTime() > thirtyDaysAgo
    );
    ...
    await safeRun("perfReport", () => sendChannel(lines(parts)));
  } catch (e) {
...
server = app.listen(PORT, "0.0.0.0", async () => {
  log(`🚀 SNIPER V98 on port ${PORT}`);
  await safeRun("bootTelegram", () => sendPrivate(lines([
    "\uD83D\uDE80 SNIPER V98 LIVE -- REGIME-FREE SIGNAL ENGINE",
    "",
    "V98: Regime gate removed -- momentum and volume filter quality",
    "V98: BTC/QQQ no longer block signals -- real movers fire regardless",
    "V98: Alpaca SIP feed -- full consolidated bars",
    "V98: Massive/Polygon wired for standard US stocks",
    "V98: Stooq added as Yahoo throttling fallback",
    "V98: Market breakdown in reject stats (CRYPTO/US/LSE)",
    "V98: HIGH_BETA checkpoint logging for SMCI/ASTS/IONQ",
    "V98: All messages updated for legal compliance",
