...
const HIGH_BETA_STOCKS = new Set([
  "ASTS","RKLB","LUNR","IONQ","OKLO","SMR","AVAV","ACHR","SPCX","SPCE",
...
const LSE_POOL = [
  ...
  // Banks -- rate sensitivity, can trend (expanded)
  "BARC.L","LLOY.L","NWG.L","HSBA.L","LGEN.L",
  ...
  // Pharma/consumer
  "AZN.L","ULVR.L",
  // New additions -- liquid momentum candidates
  "IAG.L",   // aviation, clean continuation when sector flows
  "JD.L",    // retail momentum, volume leader
  "RPI.L",   // liquid, real momentum behaviour
];
...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = []) {
  ...
  if (!isWeekend() && topMovers.length > 0) {
    const ranked = topMovers.slice(0, 6).map(sym => {
      ...
      // V98: structural check -- stock should have positive intraday move
      // AVAV flagged as top pick but was down on the day -- can't fire a signal below EMA
      // If pct is available and negative, flag as broken structure
      const structureOk = pct >= 0 || pmPct >= 3; // positive day move OR strong pre-market
    ...
    for (const m of ranked) {
      ...
      const fire     = Math.abs(m.pmPct||m.pct) >= 8 ? " \uD83D\uDD25" : Math.abs(m.pmPct||m.pct) >= 5 ? " \u26A1" : "";
      const brain    = m.inSector ? " \uD83E\uDDE0" : "";
      parts.push(`  \uD83D\uDE80 <b>${escapeHtml(m.sym)}</b> ${escapeHtml(pctStr)}${fire}${brain}`);
    }
    // Top pick prioritises stocks with clean structure
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  // V98: SUSTAINED MOMENTUM -- catches gradual moves that spread across multiple candles
  // BTC $1k move in 45 mins = ~1.3% over 3 candles, each ~0.4% individually (momentum_low)
  // Multi-candle check: if last 3 candles sum to 1%+ with volume, treat as momentum
  const momentum3c = closes.length >= 4
    ? (closes.at(-1) - closes.at(-4)) / closes.at(-4)
    : momentum;
  const sustainedMomentum = Math.abs(momentum3c) >= 0.010 && Math.abs(momentum) >= 0.003;
  // Use the stronger of single-candle or sustained momentum for filtering
  const effectiveMomentum = sustainedMomentum ? Math.max(Math.abs(momentum), Math.abs(momentum3c) / 3) * Math.sign(momentum3c) : momentum;
  ...
  // ── Minimum momentum ─────────────────────────────────────────
  // Uses effectiveMomentum -- catches sustained moves across multiple candles
  // BTC $1k in 45 mins fires even if each individual candle looked quiet
  const minMomentum = market === "CRYPTO" ? CRYPTO_15M_MIN_MOMENTUM
                    : (highBeta && inUSOpen) ? 0
                    : inUSOpen              ? 0.003
                    : 0;
  if (effectiveMomentum <= minMomentum) {
    checkpoint("momentum", false, `${(momentum*100).toFixed(2)}% (eff:${(effectiveMomentum*100).toFixed(2)}%) <= ${(minMomentum*100).toFixed(2)}%`);
    if (market !== "LSE") reject("momentum_low", asset, market); return null;
    return null;
  }
  checkpoint("momentum", true, `${(effectiveMomentum*100).toFixed(2)}%`);
