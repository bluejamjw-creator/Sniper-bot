// ================================================================
// SNIPER V22
// Stable multi-source breakout scanner with fee-aware filtering
// Binance Vision + KuCoin + Static fallback + CoinGecko BTC backup
// Includes debug rejection stats and duplicate-trade protection
// ================================================================

console.log("🚀 SNIPER V22 STARTING...");
...
const MIN_CONFIDENCE = Number(process.env.MIN_CONFIDENCE || 48);
const MAX_SIGNALS = Number(process.env.MAX_SIGNALS || 3);
const MAX_MIN_STAKE = Number(process.env.MAX_MIN_STAKE || 100);
...
async function safeBinanceGet(endpoint, params = {}) {
  for (const base of BINANCE_BASES) {
    try {
      ...
      console.error(`BINANCE_FAIL ${base}${endpoint}:`, code);
    }
  }
...
async function fetchCandles(symbol) {
  try {
    return await fetchCandlesBinance(symbol);
  } catch (e) {
    console.error(`CANDLES_BINANCE_FAIL ${symbol}:`, e.message);
  }

  try {
    return await fetchCandlesKuCoin(symbol);
  } catch (e) {
    console.error(`CANDLES_KUCOIN_FAIL ${symbol}:`, e.message);
  }
...
function analyse(symbol, candles) {
  ...
  const priorWindow = highs.slice(-21, -1);
  if (!priorWindow.length) {
    incReject("noPriorHigh");
    return null;
  }

  const priorHigh = Math.max(...priorWindow);
  const breakout = last >= priorHigh * 0.995;
  ...
  if (momentum > 0.0015) confidence += 10;
  if (momentum > 0.003) confidence += 8;
  if (volRatio > 1.1) confidence += 8;
...
async function runCycle() {
  if (running) {
    console.log("SKIP: cycle already running");
    return;
  }
  ...
  try {
    ...
    for (const s of signals) {
      const alreadyOpen = trades.find(
        t => t.asset === s.asset && t.status !== "closed"
      );
