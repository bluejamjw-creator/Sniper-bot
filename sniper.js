...
// SNIPER V89
// ----------------------------------------------------------------
// V89 is the definitive clean build. Based on V88 but with every
// fix properly applied from scratch. Nothing left over.
//
// ✦ BST-accurate UK time via Intl.DateTimeFormat Europe/London
// ✦ Named gate functions: shouldScanCrypto/US/LSE, shouldSendStockAlerts
// ✦ Crypto scans 15-min candles
// ✦ BTC trend uses 1h candles for regime classification
// ✦ Expanded 20-coin static crypto pool
// ✦ 24hr momentum score boost (5/10/20% day)
// ✦ Dynamic injection — hot movers added to pool silently, no Telegram
// ✦ Weekend mode — crypto only, no stock scanning or noise
// ✦ Forced pool refresh at each market open
// ✦ US opening window (14:30-16:00 UK):
//     - EMA structure relaxed — just needs rising EMA + above EMA50
//     - Extension limit raised to 18%
//     - Momentum cap raised to 15%
//     - Ignition threshold lowered to 1.5%, no expandingRange required
// ✦ Narrative alert: change detection only, suppressed weekends/out of hours
// ✦ Hot stocks show % in market intel
// ✦ Crypto movers in narrative alert with % + tag
// ✦ SL/TP show % on signal and fill messages
// ✦ Risk % of account instead of unit size
// ✦ Instagram-ready close message with @tradesfordummies
// ✦ Market open/close Telegram alerts with forced pool refresh on open
// ✦ Duplicate signal fix — 30-min dedup window + pending trade check
// ✦ Cooldown checks asset+market combination
// ✦ HEATING UP alerts: fully removed
// ✦ V87 stable engine: graceful shutdown, safeRun, withTimeout,
//     cycleLoop, Finnhub rate-limit blocking, /ready endpoint
// ================================================================

console.log("🚀 SNIPER V89 STARTING...");
...
app.get("/",       (_req, res) => res.status(200).send("SNIPER V89 alive"));
...
// ── Crypto 15-min thresholds ─────────────────────────────────────
const CRYPTO_15M_MIN_MOMENTUM    = 0.008;
...
const REGIME_OVERRIDE_MIN_MOMENTUM = 0.012;
...
// ── Dynamic injection — silent, no Telegram ──────────────────────
...
// ── Narrative alert ──────────────────────────────────────────────
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}) {
  ...
  if (!isWeekend() && topMovers.length > 0) {
    ...
    for (const sym of topMovers.slice(0, 5)) {
      ...
      const pctStr = pct ? ` +${pct.toFixed(1)}%` : "";
      parts.push(`  \uD83D\uDE80 <b>${escapeHtml(sym)}</b>${escapeHtml(pctStr)}`);
    }
    parts.push("");
  }
...
async function refreshDynamicPools(forceNarrativeAlert = false) {
  ...
  try {
    ...
    if (!isWeekend()) {
      ...
      const [trendingStocks, topMoversResult] = await Promise.all([getTrendingStocks(), getTopMovers()]);
      ...
      for (const sector of HOT_SECTORS) {
        ...
        log(`💉 Sector inject ${sector}: ${syms.join(", ")}`);
      }
      ...
      if (shouldSendStockAlerts()) {
        ...
        if (forceNarrativeAlert || narrativeHasChanged(HOT_SECTORS, topMovers, cryptoMovers)) {
          log("🔔 Sending narrative alert");
          await sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts);
        } else {
          log("🔕 Narrative unchanged — silent");
        }
      }
    } else {
      log(`✅ Weekend — Crypto pool: ${CRYPTO_PAIRS.length} pairs`);
      const cryptoMovers = await getCryptoMovers();
...
// ── Market open/close alerts + forced pool refresh on open ───────
...
function analyse(asset, candles, market) {
  ...
  // Minimum momentum per market / timeframe
  const minMomentum = market === "CRYPTO"                    ? CRYPTO_15M_MIN_MOMENTUM
                    : market === "US" && isUSOpeningWindow() ? 0.008
                    : 0;
  ...
  // US opening window: relaxed EMA — rising EMA20 + price above EMA50 is enough
  const inUSOpen = market === "US" && isUSOpeningWindow();
  const emaOk    = inUSOpen
    ? (ema20 > ema20Prev && last > ema50)
    : (ema20 > ema50 && last > ema20 && ema20 > ema20Prev);

  if (!emaOk) {
    log(`⛔ ${asset} — EMA structure fail (openingWindow=${inUSOpen})`);
    return null;
  }
  ...
  // Extension limit — raised during US opening window for gap-up stocks
  const extension      = (last - ema20) / ema20;
  const extensionLimit = market === "CRYPTO" ? CRYPTO_15M_EXTENSION_LIMIT
                       : market === "US"     ? (inUSOpen ? 0.18 : EXTENSION_HARD_LIMIT_US)
                       : EXTENSION_HARD_LIMIT_LSE;
  if (extension > extensionLimit) {
    log(`⛔ ${asset} — extended ${(extension*100).toFixed(1)}% > ${extensionLimit*100}%`);
    return null;
  }

  // Exceptional crypto regime override
  ...
  if (!regimePass(market) && !exceptionalCrypto) {
    log(`⛔ ${asset} — regime blocked (btcTrend=${btcTrend})`);
    return null;
  }
  ...
  // Ignition — lower threshold + no expandingRange during US opening window
  const ignitionMomThreshold = market === "CRYPTO" ? 0.02 : inUSOpen ? 0.015 : 0.025;
  const ignitionSignal =
    momentum > ignitionMomThreshold &&
    volRatio > 2.0 &&
    (inUSOpen ? true : (last > ema20 && expandingRange));
  ...
  if (!pullbackSignal && !breakoutSignal && !strongMomentumOverride && !ignitionSignal) {
    log(`⛔ ${asset} — no setup (break=${breakoutSignal} pull=${pullbackSignal} mom=${strongMomentumOverride} ign=${ignitionSignal} vol=${volRatio.toFixed(2)}x)`);
    return null;
  }
  ...
  // Momentum cap — raised during US opening window for parabolic gap-ups
  const momentumCap = setupType === "MOMENTUM_IGNITION" ? 0.20
                    : market === "CRYPTO"               ? CRYPTO_15M_MAX_MOMENTUM
                    : (market === "US" && inUSOpen)     ? 0.15
                    : market === "US"                   ? MAX_MOMENTUM_BAR_US
                    : MAX_MOMENTUM_BAR_LSE;
...
function buildSignalLesson(signal) {
  ...
  if (signal.market === "CRYPTO") {
    ...
    if (pct24 && pct24 >= 0.05) {
      parts.push(`\n\uD83D\uDCC8 <b>24hr move: +${(pct24*100).toFixed(1)}%</b> \u2014 building all day. 15-min candle caught the breakout.`);
    }
...
function buildCloseMessage(trade, reason, closePrice) {
  ...
  else if (isLoss) parts.push(``, `\uD83D\uDEE1\uFE0F <b>Stop did its job.</b> Loss capped. Capital protected for the next setup.`);
  else if (reason === "FORCE CLOSE") parts.push(``, `\u23F1\uFE0F <b>Force closed.</b> No progress after ${heldHrs}hrs. Freed capital for better setups.`);
  else if (reason === "PENDING_EXPIRED") parts.push(``, `\u231B <b>Order expired.</b> Entry level never reached. No capital at risk.`);

  if (isWin || isLoss) {
    parts.push(``, `\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014`, `\uD83D\uDCF8 <i>Signal by @tradesfordummies</i>`);
  }
  return lines(parts);
}
...
async function closeTrade(trades, trade, closePrice, reason) {
  ...
  await safeRun(`close${trade.asset}`, () => send(buildCloseMessage(trade, reason, closePrice)));
...
async function processSignals(signals) {
  ...
  for (const signal of signals) {
    ...
    const sentAt  = nowIso();
    // 30-min dedup window — prevents same asset firing twice in one session
    const alertId = `${signal.asset}-${signal.market}-${Math.floor(new Date(sentAt).getTime() / (30*60000))}`;
    if (alerts.some(a => a.id === alertId)) {
      log(`⏳ Dedup blocked ${signal.asset}`);
      continue;
    }
    // Extra safety — no second signal if pending trade already exists
    if (trades.some(t => t.asset === signal.asset && t.status === "PENDING")) {
      log(`⏳ Pending exists for ${signal.asset}`);
      continue;
    }
...
server = app.listen(PORT, "0.0.0.0", async () => {
  log(`🚀 SNIPER V89 on port ${PORT}`);
  await safeRun("bootTelegram", () => send(lines([
    "\uD83D\uDE80\uD83D\uDD25 <b>SNIPER V89 LIVE</b>",
    "",
    "Telegram: market intel \u2022 open/close \u2022 trade signals only",
    "Heating up alerts: \u274C permanently removed",
    "15-min crypto \u2022 BST-accurate time \u2022 opening window fixes active",
    "Dynamic injection silent \u2022 Duplicate signal fix active"
  ])));
  await sleep(STARTUP_DELAY_MS);
  await safeRun("initialPools",  refreshDynamicPools);
  ready = true;
  log("\u2705 SNIPER V89 READY");
