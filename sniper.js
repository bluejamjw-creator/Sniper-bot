...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  if (market === "CRYPTO" && avgVol > 0) {
    ...
    // V99.1: relaxed from AND to OR -- strong single candle OR two moderate candles
    // Previous: both required -- blocked too many legitimate grinding moves (NEAR, JUP, STRK)
...
function buildSignalMessage(signal, size) {
  ...
  // V100: show SNIPER tag if fired from hot loop
  ...
  if (shouldSendStockAlerts() || shouldSendCryptoAlerts()) {
    ...
    for (const { asset, heat } of warmCandidates) {
      ...
      safeRun("convictionAlert", () => sendChannel(lines([
        `👀 <b>SNIPER WATCHING</b>`,
        ``,
        `${market} | <b>${escapeHtml(sym)}</b>`,
        `🌡️ Heat: ${Math.round(heat)} — building conviction`,
        ``,
        `Intel engine has flagged this asset as a potential session leader.`,
        `Sniper is monitoring for entry confirmation.`,
        ``,
        `⚡ Entry alert fires if setup confirms on next scan`,
        `——————————`,
      ...
      log(`👀 Conviction alert: ${asset} (heat: ${Math.round(heat)})`);
    }
  }
}
