...
const MIN_CRYPTO_SCORE = 100; // V99.1: A* only for crypto -- quality gate raised
...
async function sendEndOfDayReport() {
  ...
  // Plain English. Explains what happened like a human would.
  ...
  const forceClosed = todayTrades.filter(t => t.outcome === "FORCE_CLOSE");
  ...
  const emaFails      = rejectStats["ema_fail"] || 0;
  const momLow        = rejectStats["momentum_low"] || 0;

  // Plain English condition reading
  const marketMood = momLow > 2000
    ? "The market was very choppy today — most stocks had no clear direction."
    : momLow > 1000
    ? "Conditions were mixed — some moves but nothing clean enough to trade."
    : "Market conditions were reasonable — good setups were available.";

  const emaComment = emaFails > 300
    ? "Most stocks were trading below their trend lines, which is a warning sign."
    : emaFails > 100
    ? "Some stocks were below trend — the scanner correctly avoided these."
    : "Most stocks were in healthy trends.";

  // Build the report
  const parts = [
    `📈 <b>DAILY REPORT — ${new Date().toLocaleDateString("en-GB", {weekday:"long", day:"numeric", month:"long"})}</b>`,
    ``,
    `<b>🌍 What happened today</b>`,
    `${marketMood}`,
    `${emaComment}`,
    ``,
  ];
  ...
  if (total === 0 && openTrades.length === 0) {
    parts.push(`<b>📊 Signals</b>`);
    parts.push(`No trades completed today. The scanner was waiting for better conditions — sometimes doing nothing is the right move.`);
    parts.push(``);
  } else {
    parts.push(`<b>📊 Today's trades</b>`);
    if (total > 0) {
  ...
  if (best && best.realizedR > 0) {
    ...
    parts.push(`${best.asset.replace("USDT","")} ${pct} — setup worked as planned.`);
    parts.push(``);
  }
  ...
  if (worst && worst.realizedR < 0) {
    ...
    const reason = worst.outcome === "FORCE_CLOSE"
      ? "The trade showed no progress so the bot exited early to protect capital."
      : "Price hit the stop loss. Loss was capped as planned.";
  ...
  parts.push(`<b>🔍 Why the scanner said no</b>`);
  if (topBlocker === "macro_protection") {
    parts.push(`The market was in a downtrend today — the scanner paused all new entries to protect capital. This is intentional. A day with no signals in a falling market is a good day.`);
  } else if (topBlocker === "momentum_low") {
    parts.push(`Most assets were rejected because price wasn't moving — the scanner only enters when there's real momentum, not just hope.`);
  } else if (topBlocker === "ema_fail") {
    parts.push(`Most assets were below their trend lines — the scanner avoids buying things that are already in a downtrend.`);
  } else if (topBlocker === "single_vol_candle") {
    parts.push(`Volume confirmation was missing on most setups — the scanner needs to see real buying interest, not just a single spike.`);
  } else if (topBlocker === "btc_not_bull") {
    parts.push(`Bitcoin wasn't in a clear uptrend — the scanner holds off on crypto signals when BTC is uncertain.`);
  } else {
    parts.push(`The scanner applied its quality filters and most setups didn't pass. Quality over quantity.`);
  }
  parts.push(``);
  ...
  if (hotAssets.length > 0) {
    const names = hotAssets.map(a => a.asset.replace("USDT","")).join(", ");
    parts.push(`<b>👀 Watching tomorrow</b>`);
    parts.push(`The intel engine has flagged: ${names}`);
    parts.push(`These names have been active today and could set up overnight or at open.`);
    parts.push(``);
  }

  parts.push(`<b>💡 Remember</b>`);
  parts.push(`The scanner doesn't trade every day — it waits for high quality setups. A day with no signals is not a bad day. It means the conditions weren't right.`);
  parts.push(``);
  parts.push(`——————————`);
...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = []) {
  ...
  if (filteredCryptoMovers.length > 0) {
    ...
    for (const c of filteredCryptoMovers) {
      ...
      const isHot    = getHeat(c.symbol) >= HEAT_HOT_THRESHOLD;
      const heatTag  = isHot ? " 🌡️ HOT" : "";
      // If HOT, upgrade the tag emoji to 🔥 regardless of % move
      const tag  = isHot                  ? "🔥 HOT candidate"
                 ...
                 : "📉 pulling back";
      const heatScore = isHot ? ` (heat: ${Math.round(getHeat(c.symbol))})` : "";
      parts.push(`  📊 <b>${escapeHtml(sym)}</b> — ${tag}${heatScore}`);
    }
    parts.push("");
  }
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  // V99.1: Two consecutive above-average volume candles required for crypto
  // Single spike can be a stop hunt -- two candles = genuine participation
  if (market === "CRYPTO" && avgVol > 0) {
    const prevVolRatio = prevVol / avgVol;
    const twoCandles   = volRatio >= 1.5 || (volRatio >= 1.2 && prevVolRatio >= 1.1);
    if (!twoCandles) {
      reject("single_vol_candle", asset, market);
      return null;
    }
  }

  // V100 HARDENED: Rule 5 -- Momentum without participation is noise
  // Price rising + volume declining = distribution, not accumulation. REJECT.
  if (avgVol > 0 && prevVol > 0 && last > (candles.at(-2)?.close || 0)) {
    const volTrend = lastVol / prevVol;
    if (volTrend < 0.65) {
      checkpoint("vol_trend", false, `price up but volume dropped to ${volTrend.toFixed(2)}x prior`);
      reject("volume_declining_on_move", asset, market);
      return null;
    }
  }
  ...
  // ── V100: MARKET-SPECIFIC REGIME GATE ────────────────────────
  // BTC controls crypto. QQQ controls US stocks. No crossover.
  // Protecting capital is better than a bad trade.
  if (isMarketBlocked(market)) {
    reject("macro_protection", asset, market);
    return null;
  }

  // ── Regime ────────────────────────────────────────────────────
  // V99.1: BTC must be bull for ALL crypto entries -- no neutral override
  // Neutral BTC = ranging market = stop hunts = lower win rate
  if (market === "CRYPTO") {
    const btcBull = btcTrend === "bull" || btcTrend === "strong_bull";
    if (!btcBull) {
      checkpoint("regime", false, `btc=${btcTrend} -- must be bull`);
      reject("btc_not_bull", asset, market); return null;
    }
    checkpoint("regime", true, `btc=${btcTrend}`);
  } else {
    if (qqqTrend === "risk_off") {
      checkpoint("regime", false, `qqq=risk_off`);
      reject("regime", asset, market); return null;
    }
    checkpoint("regime", true, `qqq=${qqqTrend}`);
  }
  ...
  // V99.1: minimum heat source required for crypto entries
  // Asset must have at least one watchlist or intel mention (heat > 0)
  // Catches completely cold breakouts with no prior scanner conviction
  if (market === "CRYPTO") {
    const heatScore = getHeat(asset);
    if (heatScore < MIN_HEAT_FOR_CRYPTO) {
      checkpoint("heat", false, `heat=${heatScore.toFixed(0)} < min ${MIN_HEAT_FOR_CRYPTO}`);
      reject("no_heat", asset, market); return null;
    }
  ...
  // V99.1: crypto requires A* only -- A grade suppressed for crypto
  // Stocks still publish A and A* -- only crypto tightened
  const cryptoGradeOk = market !== "CRYPTO" || grade === "A*";
  ...
  //
  const confidenceScore = Math.min(100, Math.round((score / 130) * 100));
  ...
  // V100 HARDENED: Rule 3 -- Execution Score < 75 = REJECT regardless of setup quality
  if (execQuality < 75) { reject("poor_execution_hardened", asset, market); return null; }
...
function passesNoRegretCheck(signal, allSignals) {
  ...
  // Rule 1/2: Only the elite survive -- must be best in market
  ...
  if (sameMarket.length > 0) {
    ...
    if (bestOther > thisScore + 10) {
      reasons.push(`not the best setup — better candidate exists (${bestOther.toFixed(0)} vs ${thisScore.toFixed(0)})`);
    }
  }
...
async function sendMorningBriefing() {
  ...
  const lines = movers.slice(0, 8).map(m => {
    ...
    const elite = m.changePct >= 10 ? " 🔥" : m.changePct >= 7 ? " ⚡" :
                  m.changePct <= -10 ? " 🧊" : m.changePct <= -7 ? " 🥶" : ""; // 🔥⚡ up, 🧊🥶 down
...
async function safeCycle() {
  ...
  try {
    ...
    if (nowProtecting && !macroProtecting && alertCooldown) {
      ...
      const lines2 = [
        `🛡️ <b>CAPITAL PROTECTION MODE</b>`,
        ``,
        `The scanner is protecting your capital.`,
        `No new entries until conditions improve.`,
        ``,
      ];
      if (cryptoBlocked) {
        lines2.push(`🪙 <b>Crypto paused</b> — ${escapeHtml(getProtectionReason("CRYPTO"))}`);
      }
      if (usBlocked) {
        lines2.push(`🇺🇸 <b>US stocks paused</b> — ${escapeHtml(getProtectionReason("US"))}`);
      }
      lines2.push(``);
      lines2.push(`Intel and watchlist alerts continue as normal.`);
      lines2.push(`The scanner will post when conditions improve.`);
      lines2.push(``);
      lines2.push(`——————————`);
      ...
      await safeRun("macroLift", () => sendChannel(lines([
        ...
        `The scanner is back to hunting setups.`,
        ``,
