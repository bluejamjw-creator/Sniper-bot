// ================================================================
// SNIPER V95 -- MOMENTUM MATURITY ENGINE
// ----------------------------------------------------------------
// V95 changes from V94:
//   - TREND_RESUMPTION setup type for HIGH_BETA gap-day continuation
//     Gap up at open, holds above session low, consolidates, resumes
//     No EMA20 proximity required -- uses intraday structure instead
//     Catches IONQ/LUNR/ASTS/RKLB gap-and-hold patterns
//   - Intraday high/low tracking for US stocks during opening window
//   - Gap-day consolidation detection: impulsive open + pause + resume
//   - Score boost when stock is in morning intel movers list
//   - Score floors lowered: US 76->72, HIGH_BETA 74->70
//   - US cooldown reduced 4hrs->2hrs
//   - CONDITIONAL BUY fires on vol>=1.8x regardless of BTC trend
//     INJ lesson: strong momentum = conditional not limit
// ================================================================
...
console.log("🚀 SNIPER V95 STARTING...");
...
app.get("/",       (_req, res) => res.status(200).send("SNIPER V95 alive"));
...
const APPROVED_CRYPTO_PREFIXES = [
  ...
  "SUI","NEAR","RENDER","ONDO","INJ","TIA","FET",
  "TAO","ARB","OP","JUP","TON","BNB","ATOM",
  "SEI","APT","AAVE","UNI","MKR","PEPE"
];
...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = []) {
  ...
  if (!isWeekend() && topMovers.length > 0) {
    // V94: Rank movers by signal strength using pre-market data + narrative bonus
    // Tells subscriber which name to focus on before the open
    const ranked = topMovers.slice(0, 6).map(sym => {
      ...
      // Strength score: pre-market move + sector narrative + intraday move
...
async function fetchUSData(symbol) {
  ...
  // TwelveData nuclear guard -- ONLY use TD credits 14:00-16:00 UK
  // This is the US opening window where candle quality actually matters for signals
  // Everything outside this window uses Yahoo -- preserves all 800 credits for the moment that counts
  const ukMins = getUkMinutes();
  const inTDWindow = isWeekday() && ukMins >= 840 && ukMins < 960; // 14:00-16:00 UK only
...
async function fetchLivePrice(asset, market) {
  try {
    ...
    // TD guard -- only use TD for live prices during US opening window
    // Outside this window Yahoo is fast enough and free
    const ukMins = getUkMinutes();
    const inTDWindow = isWeekday() && ukMins >= 840 && ukMins < 960;
...
async function analyse(asset, candles, market) {
  ...
  if (useMode2 && inUSOpen) {
    ...
    // Full mode=1 structure (last > ema20) kills everything in sideways UK market
    // Just needs trend direction: EMA20 > EMA50 and EMA20 rising
    emaOk = ema20 > ema50 && ema20 > ema20Prev;
  } else {
  ...
  if (avgVol > 0 && lastVol > 0) {
    ...
    // HIGH_BETA opening window with no volume data -- use range expansion as proxy
  ...
  if (!pullbackSignal && !breakoutSignal && !strongMomentumOverride && !ignitionSignal && !gapContinuationSignal && !firstPullbackCont && !preBreakoutExpansion && !trendResumption) {
    log(`⛔ ${asset} -- no setup (break=${breakoutSignal} pull=${pullbackSignal} gap=${gapContinuationSignal} ign=${ignitionSignal} fpc=${firstPullbackCont} pbe=${preBreakoutExpansion} tr=${trendResumption} vol=${volRatio.toFixed(2)}x)`);
    return null;
  }
...
async function processSignals(signals) {
  ...
  for (const signal of signals) {
    ...
    // Only block on pending if it's fresh -- stale pending (>30 mins crypto, >2hrs stocks) shouldn't block new signals
    const pendingTrades = trades.filter(t => t.market===signal.market && t.status==="PENDING");
    const freshPending  = pendingTrades.filter(t => {
      const ageHrs = hoursAgo(t.sentAt);
      return signal.market === "CRYPTO" ? ageHrs < 0.5 : ageHrs < 2;
    });
    if (freshPending.length >= 1) continue;
...
server = app.listen(PORT, "0.0.0.0", async () => {
  log(`🚀 SNIPER V95 on port ${PORT}`);
  await safeRun("bootTelegram", () => sendPrivate(lines([
    "\uD83D\uDE80 SNIPER V95 LIVE -- MOMENTUM MATURITY ENGINE",
    "",
