...
// Catches the pullback after the opening spike rather than chasing
...
function shouldForceClose(trade, livePrice) {
  ...
  const maePct    = trade.maePct || 0;
  const progress  = progressToTarget(trade, livePrice);
  const pricePct  = (livePrice - trade.entry) / trade.entry;

  // V98: REWRITTEN FORCE CLOSE -- based on direction + time, not just flatness
  // Key insight: flat ≠ bad. Going against you = bad. Give trades room to develop.
  // LUNR was flat for 20 mins then moved -- old logic closed it too early.

  // Universal hard cap -- nothing stays open more than 4 hours
  if (ageHrs >= 4) return true;

  // MOMENTUM_IGNITION -- needs to prove direction but gets more time than before
  // If it's going against you: close faster. If flat/positive: give it time.
  if (trade.setupType === "MOMENTUM_IGNITION") {
    if (ageMins >= 15 && pricePct < -0.008) return true;  // actively losing 0.8%+ → close fast
    if (ageMins >= 30 && pricePct < -0.004) return true;  // still negative after 30 mins → close
    if (ageMins >= 45 && mfePct < 0.003)    return true;  // 45 mins and never showed anything → close
    if (ageMins >= 60)                       return true;  // hard 60 min cap for ignition
    return false;
  }

  // GAP_CONTINUATION -- similar to ignition, needs early confirmation
  if (trade.setupType === "GAP_CONTINUATION") {
    if (ageMins >= 20 && pricePct < -0.010) return true;  // gapping down 1%+ → close
    if (ageMins >= 60 && mfePct < 0.005)    return true;  // 1 hour, never moved → close
    if (ageHrs >= 2)                         return true;  // hard 2hr cap
    return false;
  }
  ...
  // BREAKOUT_CONTINUATION -- give more time, close if going against
  if (trade.setupType === "BREAKOUT_CONTINUATION") {
    if (ageMins >= 30 && pricePct < -0.010) return true;  // breaking down 1%+ after 30 mins → close
    if (ageHrs >= 2 && progress < 0.15)     return true;  // 2hrs, barely moved → close
    if (ageHrs >= 3)                         return true;  // hard 3hr cap
    return false;
  }

  // PULLBACK_CONTINUATION -- slowest, most patient
  if (trade.setupType === "PULLBACK_CONTINUATION") {
    if (ageMins >= 45 && pricePct < -0.012) return true;  // going against meaningfully after 45 mins → close
    if (ageHrs >= 3 && progress < 0.15)     return true;  // 3hrs, no progress → close
    if (ageHrs >= 4)                         return true;  // hard 4hr cap
    return false;
  }
...
async function sendPerformanceReport() {
  try {
    ...
    // V98: only count trades from V98 launch -- V97 test trades skew the stats
    const v98Launch = new Date("2026-06-03T07:00:00Z").getTime();
    const thirtyDaysAgo = Date.now() - 30 * 24 * 60 * 60 * 1000;
    const since = Math.max(v98Launch, thirtyDaysAgo);
...
async function sendMorningBriefing() {
  ...
  const lines = movers.slice(0, 8).map(m => {
    ...
    const arrow = m.changePct >= 0 ? "\uD83D\uDCC8" : "\uD83D\uDCC9"; // 📈 or 📉
    const elite = Math.abs(m.changePct) >= 10 ? " \uD83D\uDD25" : Math.abs(m.changePct) >= 7 ? " \u26A1" : ""; // 🔥 10%+, ⚡ 7%+
