...
console.log("🚀 SNIPER V99 STARTING...");
...
app.get("/",       (_req, res) => res.status(200).send("SNIPER V99 alive"));
...
const STATIC_STOCK_FALLBACK = [
  ...
  "NVDA","PLTR","AMD","MSFT","META","AVGO","ANET","CRWD","APP",
...
function shouldScanCrypto() {
  ...
  if (mins >= CRYPTO_DEAD_ZONE_START && mins < CRYPTO_DEAD_ZONE_END) {
    ...
    const hasHotAsset = getTopHeat(1).some(a => a.score >= 80);
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  if (market === "CRYPTO" && avgVol > 0) {
    ...
    const twoCandles   = volRatio >= 1.3 && prevVolRatio >= 1.1;
    if (!twoCandles) {
...
server = app.listen(PORT, "0.0.0.0", async () => {
  log(`🚀 SNIPER V99.1 on port ${PORT}`);
  await safeRun("bootTelegram", () => sendPrivate(lines([
    "🚀 SNIPER V99.1 LIVE -- WIN RATE MODE",
    "",
    "TARGET: 65-70% WIN RATE",
    "Five new filters on crypto entries:",
    "✅ (1) A* only for crypto (score 100+)",
    "✅ (2) BTC must be bull -- neutral = no crypto signal",
    "✅ (3) Min heat score required -- cold breakouts blocked",
    "✅ (4) Dead zone 01:00-07:00 UK (HOT assets exempt)",
    "✅ (5) Two consecutive above-avg volume candles",
    "",
    "V99 modules all preserved:",
    "✅ Heat Score + Leader Mode + Trade State Engine",
    "✅ No Regret Engine + Opportunity Ranking",
    "✅ HEALTHY_CONSOLIDATION → hold (ARB/SOL fix)",
    "",
    "US/LSE unchanged -- A and A* still published",
    "",
