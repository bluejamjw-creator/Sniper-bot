// ================================================================
// SNIPER V79
// + BREAKOUT_CONTINUATION and MOMENTUM_BREAKOUT now use MARKET BUY
//   and fill immediately at signal price — no more missed entries
// + PULLBACK_CONTINUATION keeps LIMIT BUY as before
// + All V78 quality filters retained
// ================================================================

console.log("🚀 SNIPER V79 STARTING...");
...
app.get("/", (_req, res) => res.status(200).send("SNIPER V79 alive"));
...
// ── Signal message — MARKET BUY shows fill instruction clearly ───
...
function analyse(asset, candles, market) {
  ...
  // ── KEY CHANGE: MARKET BUY for breakouts, LIMIT BUY for pullbacks
  const entryType = setupType === "PULLBACK_CONTINUATION" ? "LIMIT BUY" : "MARKET BUY";

  // Entry price: use current close for market buys, EMA touch for limits
  const entry = setupType === "PULLBACK_CONTINUATION"
    ? ema20 * 1.002
    : last; // market fill at current candle close
...
async function manageTrades() {
  ...
  for (const trade of trades) {
    ...
    if (trade.status === "PENDING") {
      ...
      // ── MARKET BUY: fill immediately on next cycle check ────────
      const filled =
        trade.entryType === "MARKET BUY"
          ? true  // always fills — instruction sent, assume executed
      ...
      const driftTooFar =
        trade.entryType === "LIMIT BUY"
          ...
          : false; // no drift check for market buys
...
app.listen(PORT, "0.0.0.0", async () => {
  ...
  await send("🚀🔥 SNIPER V79 LIVE — market fills active");
  await runCycle();
  setInterval(async () => {
