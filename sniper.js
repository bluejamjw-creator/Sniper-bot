// ================================================================
// SNIPER V94 -- MOMENTUM MATURITY ENGINE
// ----------------------------------------------------------------
// V94 changes from V93:
//   - Pre-market price awareness: Finnhub live quote used during
//     opening window to detect real move before Yahoo candles update
//   - Relative strength scoring: HIGH_BETA stocks scored vs QQQ
//     SMCI +6% with QQQ +0.5% = leader bonus, much stronger signal
//   - Momentum persistence: higher lows after expansion = bonus
//     catches institutional accumulation before public breakout
//   - ATR-adjusted extension: HIGH_BETA extension limit now dynamic
//     based on ATR units not fixed %, prevents blocking CLSK-style moves
//   - Opening drive detection improved: pre-market move feeds into
//     gap detection so GAP_CONTINUATION fires earlier in session
// ================================================================
...
console.log("🚀 SNIPER V94 STARTING...");
...
app.get("/",       (_req, res) => res.status(200).send("SNIPER V94 alive"));
...
async function analyse(asset, candles, market) {
  ...
  const entryType = setupType === "PULLBACK_CONTINUATION"
    ? (market === "CRYPTO" && isBtcBull ? "CONDITIONAL BUY" : "LIMIT BUY")
    : "MARKET BUY";
  ...
  const entry = setupType === "PULLBACK_CONTINUATION"
    ? (market === "CRYPTO" && isBtcBull ? last * 1.005 : ema20 * 1.008)
    : last;
  ...
  // V94: PRE-MARKET STRENGTH BONUS -- if Finnhub showed big pre-market move
  // this is confirmed institutional interest before the open
  if (market === "US" && highBeta && inUSOpen) {
    const pmData = getPreMarketPrice(asset);
    if (pmData) {
      if (pmData.changePct >= 8)      score += 12; // exceptional pre-market
      else if (pmData.changePct >= 5) score += 8;  // strong pre-market
      else if (pmData.changePct >= 3) score += 4;  // notable pre-market
...
server = app.listen(PORT, "0.0.0.0", async () => {
  log(`🚀 SNIPER V94 on port ${PORT}`);
  await safeRun("bootTelegram", () => sendPrivate(lines([
    "\uD83D\uDE80 SNIPER V94 LIVE -- MOMENTUM MATURITY ENGINE",
    "",
    "Dual-mode scanner: Trend Continuation + Momentum Runner",
    "V94: Pre-market price awareness (Finnhub -> opening window)",
    "V94: Relative strength vs QQQ -- leader detection",
    "V94: Momentum persistence -- higher lows = institutional",
    "V94: ATR-adjusted extension limits -- CLSK-style moves allowed",
    "TwelveData: active window only (14:00-16:00 UK)",
    "Morning briefing: HIGH_BETA movers at 08:00 UK daily",
