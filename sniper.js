...
async function sendEndOfDayReport() {
  ...
  parts.push(`——————————`);
  parts.push(`⚠️ <i>Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell.</i>`);
...
async function sendWatchlistAlert(alerts, trades) {
  ...
  parts.push(``, `——————————`, `⚠️ <i>Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`);
...
function narrativeHasChanged(sectors, stockMovers, cryptoMovers, lseMovers = []) {
  ...
  // Hard cap -- max 6 intel posts per day (raised from 4 -- V103)
  if (narrativePostCount >= 6) return false;

  // Minimum 2 hours between posts with same narrative (lowered from 3hr -- V103)
  ...
  // V103: 2-hour fallback pulse -- post even if content unchanged so subscribers
  // always get a signal on active days rather than silence on slow ones.
  const twoHourFallback = timeSinceLast >= 2 * 60 * 60 * 1000;
  const shouldPost = newSector || bigNewMover || newExplosive || (minGapPassed && anyChange) || twoHourFallback;
  ...
  if (shouldPost) {
    ...
    narrativePostCount++;
    log(`🧠 Intel post #${narrativePostCount} today (${newSector?"new sector":bigNewMover?"new mover":newExplosive?"explosive crypto":twoHourFallback?"2hr pulse":"scheduled"})`);
  }
  return shouldPost;
}
...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = []) {
  ...
  // V103: Opening Bell fires once per trading day -- time-check alone caused duplicates on every scan
  const ukMins = getUkMinutes();
  ...
  if (openingBellDue) openingBellSentDate = todayStr; // mark as sent before post fires

  // Time-aware header -- gives each Intel post a distinct identity
  const isOpeningBell = openingBellDue;
  const isMidday      = !isWeekend() && isUSMarketOpen() && ukMins >= 1650 && ukMins < 1710; // ~17:30-18:30 UK
  const isClosingBell = !isWeekend() && isUSMarketOpen() && ukMins >= 1920 && ukMins < 1980; // ~20:00-21:00 UK
...
const LEADERBOARD_DISCLAIMER = `⚠️ <i>Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell.</i>\n@baretradesignals 🎯`
// V103: standard footer for all public alerts -- disclaimer + standalone signature
const ALERT_FOOTER = [`——————————`, `⚠️ <i>Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`];
...
async function checkMarketAlerts() {
  ...
  for (const { key, isOpen } of checks) {
    ...
    if (nowOpen && !marketWasOpen[key]) {
      ...
      if (key !== "CRYPTO") {
        const msgs = {
          US:  lines([`🟢🇺🇸 <b>US MARKET OPEN</b>`, ``, `📡 QQQ: <b>${escapeHtml(qqqTrend.replace("_"," ").toUpperCase())}</b> · ₿ BTC: <b>${escapeHtml(btcTrend.replace("_"," ").toUpperCase())}</b>`, `🔍 Scanning ${STOCK_POOL.length} stocks · ${CRYPTO_PAIRS.length} crypto · ${LSE_POOL.length} LSE`, ``, `Sniper is now active. A trade alert is only issued if all entry conditions confirm.`, ``, `——————————`, `⚠️ <i>Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`]),
          LSE: lines([`🟢🇬🇧 <b>LSE MARKET OPEN</b>`, ``, `🔍 Scanning ${LSE_POOL.length} UK stocks`, ``, `——————————`, `⚠️ <i>Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`])
        };
    ...
    if (!nowOpen && marketWasOpen[key]) {
      ...
      if (key !== "CRYPTO") {
        const msgs = {
          US:  lines([`🔴🇺🇸 <b>US MARKET CLOSED</b>`, ``, `Sniper has completed today\'s US session scan.`, ``, `——————————`, `⚠️ <i>Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`]),
          LSE: lines([`🔴🇬🇧 <b>LSE MARKET CLOSED</b>`, ``, `——————————`, `⚠️ <i>Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`])
        };
...
async function sendMorningBriefing() {
  ...
  msgParts.push(``, `——————————`, `⚠️ <i>Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`);
...
async function safeCycle() {
  ...
  try {
    ...
    if (nowProtecting && !macroProtecting && alertCooldown) {
      ...
      lines2.push(``);
      lines2.push(`——————————`);
      lines2.push(`⚠️ <i>Educational market commentary only · Not personalised investment advice · Not a recommendation to buy or sell.</i>`);
