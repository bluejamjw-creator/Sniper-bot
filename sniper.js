...
const supabase = supabaseEnabled
  ? createClient(SUPABASE_URL, SUPABASE_SERVICE_KEY, {
      ...
      global: { headers: { "x-client-info": "sniper-v91" } }
    })
  : null;
...
if (supabaseEnabled) {
  ...
  console.log("⚠️ Supabase DISABLED — using local JSON fallback (check SUPABASE_URL and SUPABASE_SERVICE_KEY env vars)");
}
...
async function scan() {
  ...
  if (shouldScanCrypto()) {
    ...
    const breadthOk = checkedPairs < 5 || (greenPairs / checkedPairs) >= 0.45;
    if (!breadthOk) {
      log(`🚫 Crypto breadth weak — ${greenPairs}/${checkedPairs} green — skipping crypto scan`);
    } else {
...
server = app.listen(PORT, "0.0.0.0", async () => {
  ...
  await safeRun("bootTelegram", () => sendPrivate(lines([
    ...
    `\uD83D\uDDC4\uFE0F Supabase: ${supabaseEnabled ? "ENABLED \u2705" : "DISABLED \u274C — local JSON only"}`,
