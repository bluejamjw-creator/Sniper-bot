...
if (supabaseEnabled) {
  console.log("[DB] Supabase ENABLED");
} else {
  console.log("[DB] Supabase DISABLED (fallback active)");
}
...
function logDb(message, extra = "") {
  console.log(`[DB] ${message}${extra ? " | " + extra : ""}`);
}
...
function alertId(row) {
  return row.id || `${row.asset}-${row.sentAt}`;
}
...
async function loadAlerts() {
  if (!supabaseEnabled) {
    logDb("Supabase disabled, loading alerts from local fallback");
  ...
  try {
    ...
    logDb("Loaded alerts from Supabase", `rows=${data?.length || 0}`);
    return (data || []).map(normalizeAlert);
  } catch (e) {
    logDb("Alerts load failed, using local fallback", e.message);
    return loadLocal(FILES.alerts, []);
  }
}
...
async function loadTrades() {
  if (!supabaseEnabled) {
    logDb("Supabase disabled, loading trades from local fallback");
  ...
  try {
    ...
    if (error) throw error;
    logDb("Loaded trades from Supabase", `rows=${data?.length || 0}`);
    return (data || []).map(normalizeTrade);
  } catch (e) {
    logDb("Trades load failed, using local fallback", e.message);
    return loadLocal(FILES.trades, []);
  }
}
...
async function saveAlerts(alerts) {
  ...
  for (const a of alerts) {
    if (!a?.asset || !a?.sentAt) continue;
    const row = normalizeAlert(a);
  ...
  if (!supabaseEnabled) {
    logDb("Supabase disabled, saving alerts to local fallback", `rows=${rows.length}`);
    saveLocal(FILES.alerts, rows);
    return true;
  }
  ...
  try {
    ...
    logDb("Saved alerts to Supabase", `rows=${rows.length}`);
    saveLocal(FILES.alerts, rows);
    return true;
  } catch (e) {
    logDb("Alerts save failed, using local fallback", e.message);
    saveLocal(FILES.alerts, rows);
    return false;
  }
}
...
async function saveTrades(trades) {
  ...
  for (const t of trades) {
    if (!t?.id) continue;
    map.set(t.id, normalizeTrade(t));
  }

  const rows = Array.from(map.values());

  if (!supabaseEnabled) {
    logDb("Supabase disabled, saving trades to local fallback", `rows=${rows.length}`);
    saveLocal(FILES.trades, rows);
    return true;
  }
  ...
  try {
    ...
    logDb("Saved trades to Supabase", `rows=${rows.length}`);
    saveLocal(FILES.trades, rows);
    return true;
  } catch (e) {
    logDb("Trades save failed, using local fallback", e.message);
    saveLocal(FILES.trades, rows);
    return false;
  }
}
  ...
  try {
    const { data } = await retry(() =>
      axios.get("https://api.twelvedata.com/stocks", {
        params: { country: "United States", apikey: TWELVE_DATA_API_KEY },
        timeout: 15000
      })
    );
...
async function buildDynamicStockPool() {
  ...
  const seed = apiSymbols.length
    ? Array.from(new Set([...BASE_STOCK_POOL, ...apiSymbols.slice(0, 60)]))
    : BASE_STOCK_POOL;
  ...
  await mapWithConcurrency(seed, 4, async symbol => {
    ...
    const d21 = closes.at(-21);
    const d63 = closes.at(-63);
    ...
    const momentum1d = safePctChange(last, prev);
    const momentum1m = safePctChange(last, d21);
    const momentum3m = safePctChange(last, d63);
  ...
  const dynamicPool = scored
    .sort((a, b) => b.score - a.score)
    .slice(0, MAX_DYNAMIC_STOCKS)
    .map(x => x.symbol);
...
async function scan() {
  ...
  if (ENABLE_US && isUSMarketOpen()) {
    ...
    await mapWithConcurrency(activeStockPool, MAX_CONCURRENT_REQUESTS, asset =>
