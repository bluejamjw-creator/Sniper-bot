 ...
 * Architecture:    Growth Mode + V104 Execution Engine (ELITE/GOOD/WATCH/REJECT)
 * Status:          Validation phase -- do not change scoring or thresholds
 *                  until 100-200 completed trades exist in sniper_signal_decisions
 *
 * Historical development is maintained in Git history.
 * Major behavioural changes are documented in release notes, not in-file changelogs.
 */
...
console.log(`
🚀 SNIPER BUILD INFO
Version:    V104.0 (Growth Mode + Execution Engine: ELITE/GOOD/WATCH/REJECT + full telemetry)
...
const MIN_RR            = 1.75; // V103 GROWTH MODE: stocks 2.0->1.75 -- controlled loosening
...
const MIN_GLOBAL_SCORE = 75; // V103 GROWTH MODE: 85->75 -- captures 72-77 scoring candidates without going V63
const MIN_CRYPTO_SCORE = 75; // V103 GROWTH MODE: 88->75 -- EIGEN/TIA/INJ score range
const MIN_US_SCORE     = 70; // V103 PHASE2: slight raise from 68 to keep quality (was 80 at peak)
...
let lastNarrativeState = { sectors: "", stockMovers: "", cryptoMovers: "", lseMovers: "", btcTrend: "", qqqTrend: "" };
...
function resetDailyStats() {
  ...
  dailyStats.topPickToday   = null;   // { sym, theme, setAt, inMorningMovers }
  dailyStats.topPickReviewSent = false;
...
function reject(reason, asset = "", market = "", context = {}) {
  ...
  // V104: log ALL safety rejects to Supabase audit trail
  // This includes early exits (no_setup, momentum_low, extension, etc.)
  // so we can answer: "did momentum_low become tomorrow's winner?"
  // Fire-and-forget -- never blocks the scan cycle
  if (asset && supabaseEnabled) {
    // V104 FIX: supabase-js query builder does not reliably expose .catch()
    // on the chained insert() call across client versions. Wrap in an async
    // IIFE with try/catch instead -- fire-and-forget, never blocks the scan.
    (async () => {
      try {
        await supabase.from("sniper_signal_decisions").insert([{
          symbol:          asset,                    // existing col
          asset,                                     // new col
          market:          market || null,
          setup_type:      context.setupType || null,
          decision_class:  "SAFETY",
          decision:        "SAFETY",                 // existing col
          safety_gate:     reason,
          reject_reason:   reason,
          reason:          reason,                   // existing col
          quality_flags:   {},
          flag_count:      0,
          raw_sniper_score: context.score    || null,
          score:           context.score     || null, // existing col
          momentum:        context.momentum  || null,
          heat:            context.heat      || null,
          volume_ratio:    context.volRatio  || null,
          btc_regime:      btcTrend          || null, // existing col
          qqq_regime:      qqqTrend          || null, // existing col
          btc_trend:       btcTrend          || null,
          qqq_trend:       qqqTrend          || null,
          engine_version:  "V104",
        }]);
      } catch (e) {
        // silent fail -- never block the scan cycle on a logging error
      }
    })();
  }
}
...
async function sendRejectStatsSummary() {
  ...
  // Plain-English label for most common rejection
  const blockerLabels = {
    momentum_low:   "Low price momentum. Most assets didn\'t move enough to form a valid setup.",
    no_setup:       "No valid pattern detected. Price structure wasn\'t clean enough for an entry.",
    rr_too_low:     "Risk:reward below minimum. Potential gains weren\'t large enough vs the risk taken.",
    risk_pct:       "Stop-loss too wide. Position sizing would have exceeded the maximum risk per trade.",
    no_expansion:   "Insufficient room to target. Not enough space between entry and resistance.",
    global_floor:   "Setup quality below minimum threshold. Score didn\'t meet the required standard.",
    score_floor:    "Setup quality below market-specific threshold.",
    exec_score_low: "Composite execution score too low across all quality dimensions.",
  };
  const topLabel = blockerLabels[top?.[0]] || top?.[0]?.replace(/_/g," ") || "Unknown";

  const parts = [
    `📊 <b>DAILY BLOCK STATS</b>`,
    `<i>${new Date().toLocaleDateString("en-GB", {weekday:"long", day:"numeric", month:"long"})}</i>`,
    ``,
    `Candidates analysed: <b>${total}</b>`,
    ``,
    `<b>Overall</b>`,
    ...entries.slice(0,5).map(([k,v]) => `🚫 ${escapeHtml(k.replace(/_/g," "))}: <b>${v}</b> (${((v/total)*100).toFixed(1)}%)`),
    ``,
  ];
  ...
  for (const [mkt, stats] of Object.entries(rejectByMarket)) {
    ...
    mktEntries.forEach(([k,v]) => parts.push(`  🚫 ${escapeHtml(k.replace(/_/g," "))}: <b>${v}</b>`));
    parts.push(``);
  }

  parts.push(`🎯 <b>Biggest blocker today</b>`);
  parts.push(`${topLabel}`);
  parts.push(``);
  parts.push(`📈 <b>V104 Observation</b>`);
  parts.push(`No parameter changes recommended. Continue collecting data for 100–200 trades before reviewing thresholds.`);

  // ── Near miss section ────────────────────────────────────────────
  // Shows which specific assets were close to trading today and what blocked them.
  // This is the "fix it" data -- not aggregate counts but named candidates.
  if (nearMissLog.length > 0) {
    const sorted = [...nearMissLog].sort((a,b) => b.executionScore - a.executionScore);
    parts.push(``, `🔍 <b>Near misses today (${sorted.length})</b>`);
    parts.push(`<i>Reached Quality Assessment but didn't trade</i>`, ``);

    for (const n of sorted.slice(0, 8)) {
      const sym     = n.asset.replace("USDT","");
      const flag    = n.market === "CRYPTO" ? "🪙" : n.market === "LSE" ? "🇬🇧" : "🇺🇸";
      const cls     = n.classification === "GOOD" ? "🟡 GOOD" : "👁 WATCH";
      const blocked = n.flags.length > 0 ? n.flags.join(", ").replace(/_/g," ") : "exec score";
      parts.push(`${cls} ${flag} <b>${escapeHtml(sym)}</b>`);
      parts.push(`  Score: ${n.score} | execScore: ${n.executionScore} | R:R: ${n.rr}`);
      parts.push(`  Setup: ${n.setupType} | Blocked by: <b>${escapeHtml(blocked)}</b>`);
      parts.push(``);
    }

    if (sorted.length > 8) parts.push(`<i>...and ${sorted.length - 8} more. Full data in sniper_signal_decisions.</i>`);
  } else {
    parts.push(``, `🔍 <b>Near misses today</b>`, `None reached Quality Assessment.`);
  }

  await safeRun("rejectStats", () => sendPrivate(lines(parts)));
}

// ── V104: Signal Decision Logger ────────────────────────────────
// Writes every candidate that reaches Quality Assessment to Supabase.
// This is the execution audit trail -- no candidate disappears silently.
async function logSignalDecision(d) {
  if (!supabaseEnabled) return;
  try {
    const { error } = await supabase.from("sniper_signal_decisions").insert([{
      // ── Identity ─────────────────────────────────────────────
      symbol:           d.asset,                             // existing col
      asset:            d.asset,                             // new col
      market:           d.market,
      setup_type:       d.setupType       || null,

      // ── V104 classification ──────────────────────────────────
      decision_class:   d.classification,
      decision:         d.classification,                    // existing col
      safety_gate:      d.safetyGate      || null,
      reject_reason:    d.rejectReason    || null,
      reason:           d.rejectReason    || null,           // existing col

      // ── Quality flags ─────────────────────────────────────────
      quality_flags:    d.flags || {},
      flag_rr:          d.flags?.rr        || false,
      flag_risk_pct:    d.flags?.riskPct   || false,
      flag_expansion:   d.flags?.expansion || false,
      flag_resistance:  d.flags?.resistance|| false,
      flag_score:       d.flags?.score     || false,
      flag_grade_b:     d.flags?.gradeB    || false,
      flag_count:       d.flagCount        || 0,

      // ── Scores ───────────────────────────────────────────────
      raw_sniper_score: d.score            || null,
      score:            d.score            || null,          // existing col
      execution_score:  d.executionScore   || null,
      grade:            d.grade            || null,
      confidence:       d.grade            || null,          // existing col

      // ── Core metrics ─────────────────────────────────────────
      momentum:         d.momentum         || null,
      heat:             d.heat             || null,
      volume_ratio:     d.volRatio         || null,
      rr:               d.rr               || null,
      risk_pct:         d.riskPct          || null,
      expansion_atr:    d.expansionAtr     || null,
      setup_score:      d.execQuality      || null,
      score_breakdown:  d.scoreBreakdown    || null,         // V104.1: component telemetry
      ...
      // ── Levels ───────────────────────────────────────────────
      entry_price:      d.entry            || null,          // existing col
      stop_price:       d.sl               || null,          // existing col
      target_price:     d.tp               || null,          // existing col
      entry:            d.entry            || null,
      sl:               d.sl               || null,
      tp:               d.tp               || null,
    ...
    if (error) log(`⚠️ Signal decision log failed: ${error.message} (code: ${error.code})`);
  } catch(e) {
    log(`⚠️ Signal decision log exception: ${e.message || JSON.stringify(e)}`);
  }
}

// ── V104: Outcome Tracker ────────────────────────────────────────
// Runs at US close every day. Finds all unresolved GOOD/WATCH/ELITE
// decisions from the past 5 days, fetches current price, calculates
// the move since entry price was logged, writes outcome_pct back.
// This is the only way to validate classification thresholds with real data.
//
// Fires once per day. Never touches scoring or trade logic.
async function runOutcomeTracker() {
  if (!supabaseEnabled) return;

  // Only run once per day at US close (after 21:00 UK)
  const ukMins = getUkMinutes();
  if (ukMins < 1260 || ukMins > 1380) return; // 21:00-23:00 UK window

  const todayKey = new Date().toISOString().slice(0, 10);
  if (lastOutcomeTrackerDate === todayKey) return;
  lastOutcomeTrackerDate = todayKey;

  log("📊 Running outcome tracker...");

  try {
    // Fetch all unresolved decisions from the past 5 days that have an entry price
    const { data: rows, error } = await supabase
      .from("sniper_signal_decisions")
      .select("id, asset, market, entry_price, entry, decision_class, created_at")
      .eq("engine_version", "V104")
      .eq("outcome_filled", false)
      .in("decision_class", ["ELITE", "GOOD", "WATCH"])
      .gte("created_at", new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString())
      .not("entry_price", "is", null);

    if (error || !rows?.length) {
      log(`📊 Outcome tracker: ${error?.message || "no unresolved rows found"}`);
      return;
    }

    // Deduplicate by asset -- only fetch price once per asset
    const assetMap = new Map();
    for (const row of rows) {
      if (!assetMap.has(row.asset)) assetMap.set(row.asset, row.market);
    }

    // Fetch current prices
    const prices = new Map();
    for (const [asset, market] of assetMap.entries()) {
      try {
        const price = await fetchLivePrice(asset, market);
        if (price && isFinite(price)) prices.set(asset, price);
        await sleep(300); // gentle rate limiting
      } catch { /* skip if unavailable */ }
    }

    // Update each row with outcome_pct
    let updated = 0;
    let eliteWins = 0, eliteTotal = 0;
    let goodWins = 0, goodTotal = 0;

    for (const row of rows) {
      const currentPrice = prices.get(row.asset);
      if (!currentPrice) continue;

      const entryPrice = row.entry_price || row.entry;
      if (!entryPrice || !isFinite(entryPrice)) continue;

      const outcomePct = ((currentPrice - entryPrice) / entryPrice) * 100;
      ...
      updated++;
      if (row.decision_class === "ELITE") { eliteTotal++; if (outcomePct > 0) eliteWins++; }
      if (row.decision_class === "GOOD")  { goodTotal++;  if (outcomePct > 0) goodWins++;  }
    }

    log(`📊 Outcome tracker: ${updated} rows updated`);

    // Send Bluejam summary if we have enough data
    if (updated > 0) {
      const parts = [
        `📊 <b>OUTCOME TRACKER</b>`,
        `<i>${todayKey}</i>`,
        ``,
        `Rows updated: <b>${updated}</b>`,
        ``,
      ];

      if (eliteTotal > 0) {
        parts.push(`🟢 ELITE: ${eliteWins}/${eliteTotal} positive (${Math.round(eliteWins/eliteTotal*100)}%)`);
      }
      if (goodTotal > 0) {
        parts.push(`🟡 GOOD:  ${goodWins}/${goodTotal} positive (${Math.round(goodWins/goodTotal*100)}%)`);
      }
      if (eliteTotal > 0 && goodTotal > 0) {
        parts.push(``);
        parts.push(`<i>Not win rate — price direction at close. Full expectancy analysis needs TP/SL outcomes.</i>`);
      }
      if (eliteTotal + goodTotal < 10) {
        parts.push(``);
        parts.push(`<i>Sample too small for conclusions. Keep collecting.</i>`);
      }
    ...
    log(`⚠️ Outcome tracker error: ${e.message}`);
  }
}

// ── V103: Top Pick Missed-Move Review ──────────────────────────
// Fires at US close if today's top pick moved 8%+ without a trade.
// Bluejam only -- this is an execution engine tuning tool, not a public report.
async function sendTopPickReview() {
  const tp = dailyStats.topPickToday;
  if (!tp || dailyStats.topPickReviewSent) return;

  let endPrice = null;
  try {
    const data = await fetchLivePrice(tp.sym);
    endPrice = data?.price || data || null;
  } catch { /* skip if unavailable */ }

  if (!endPrice || !tp.priceAtPick) return;

  const movePct = ((endPrice - tp.priceAtPick) / tp.priceAtPick * 100).toFixed(1);
  const moveNum = parseFloat(movePct);
  if (Math.abs(moveNum) < 8) return; // only review significant moves

  // Check if a trade was taken on this asset today
  const trades = await loadTrades();
  const today  = new Date().toISOString().slice(0, 10);
  const tradeTaken = trades.some(t => {
    const sym    = t.asset || "";
    const sentAt = (t.sentAt || t.openedAt || "").slice(0, 10);
    return (sym === tp.sym || sym === tp.sym + "USDT") && sentAt === today;
  });
  if (tradeTaken) return; // trade was taken -- nothing to review

  const assetRejects = rejectLog
    .filter(r => r.asset === tp.sym || r.asset === tp.sym + "USDT")
    .slice(-10);
  const rejectSummary = assetRejects.length > 0
    ? assetRejects.map(r => `  • ${r.reason.replace(/_/g," ")}`).join("\n")
    : "  • No rejection data recorded today";

  const direction = moveNum > 0 ? `+${movePct}%` : `${movePct}%`;
  const emoji     = moveNum > 0 ? "📈" : "📉";

  const parts = [
    `🔍 <b>TOP PICK REVIEW</b>`,
    ``,
    `<b>${escapeHtml(tp.sym)}</b>${tp.theme || ""}`,
    ``,
    `🏆 Top Pick:        ✅`,
    `🌅 Morning Movers:  ${tp.inMorningMovers ? "✅" : "❌"}`,
    `🎯 Trade issued:    ❌`,
    ``,
    `${emoji} Today's move: <b>${direction}</b>`,
    ``,
    `<b>Why no trade?</b>`,
    rejectSummary,
    ``,
    `<i>Sniper identified this name. Review the rejection reasons above to determine whether any filter prevented a valid entry.</i>`,
  ];

  await sendPrivate(lines(parts));
  dailyStats.topPickReviewSent = true;
  log(`🔍 Top pick review sent: ${tp.sym} moved ${direction} with no trade`);
}
...
async function sendEndOfDayReport() {
  ...
  parts.push(`——————————`);
  parts.push(`📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`);
  parts.push(`@baretradesignals 🎯`);
...
let openingBellSentDate    = "";       // V103: date string -- Opening Bell fires once per trading day only
const goodCandidateLog     = new Map(); // V104: asset -> last alerted timestamp, 2hr dedup
const nearMissLog          = [];           // V104: GOOD/WATCH candidates today for EOD report
...
const SECTOR_SYMBOLS = {
  ...
  CLOUD:        ["MSFT","SNOW","DDOG","CRWD","PLTR","NET","ZS","COHR"],
  INFRA:        ["CAT","DE","URI","VMC","PWR","PRIM","FIX","NXT"],
  ...
  DEFI:         ["AAVEUSDT","UNIUSDT","MKRUSDT","ENAUSDT","MORPHOUSDT","PENDLEUSDT","AEROUSDT"],
  L1:           ["SOLUSDT","SUIUSDT","AVAXUSDT","ETHUSDT","APTUSDT","HBARUSDT","TONUSDT"],
  AI_CRYPTO:    ["TAOUSDT","FETUSDT","RENDERUSDT","NEARUSDT","GRASSUSDT"],
  ORACLES:      ["LINKUSDT","PYTHUSDT","ZROUSDT","QNTUSDT"],
  // UK sectors
...
// V103: per-sector emoji for intel posts -- one emoji per theme, never 📌 for everything
const SECTOR_EMOJI = {
  AI:         "🤖",
  SEMIS:      "💾",
  POWER:      "🔌",
...
const NARRATIVE_KEYWORDS = {
  // V103: SPACE keywords tightened -- "launch", "space", "satellite" too generic in financial news
  // Require compound/specific terms so ambient news language stops triggering SPACE every day
...
const STATIC_CRYPTO_FALLBACK = [
  // ── TIER 1: Core Leaders -- never removed, most likely to produce next major trend ──
  "BTCUSDT","ETHUSDT","SOLUSDT","HYPEUSDT","ONDOUSDT","TAOUSDT","LINKUSDT","SUIUSDT",
  // ── TIER 2: High Potential -- scan constantly, promoted by heat/momentum ──
  "HBARUSDT","QNTUSDT","SEIUSDT","INJUSDT","PENDLEUSDT","NEARUSDT","JUPUSDT","ENAUSDT",
  // ── TIER 3: Rotation Names -- here for dynamic injection eligibility, heat promotes them ──
  "MORPHOUSDT","ZROUSDT","GRASSUSDT","FETUSDT",
  // ── Proven performers retained from prior cycles ──
  "EIGENUSDT","TIAUSDT","AAVEUSDT","WLDUSDT","RENDERUSDT",
];

const STATIC_STOCK_FALLBACK = [
  // ── TIER 1: Core Leaders -- highest conviction, most likely next major trend ──
  "NVDA","AVGO","ANET","VRT","PLTR","META","MSFT",  // AI infra + software
  "RKLB","OKLO","LEU","PWR","FIX",                  // Space + nuclear + grid

  // ── TIER 2: High Potential -- scan constantly, strong narrative fit ──
  "TSM","AMD","ARM","CRWD","DDOG",                   // Chips + cybersecurity
  "ASTS","LUNR","BWXT","CEG","ETN",                  // Space + power
  "AVAV","CRDO","SMR",                               // Defence + semis + nuclear

  // ── TIER 3: Rotation Names -- promoted by heat/dynamic injection when active ──
  "SOUN","APP","HOOD","UBER","DUOL","HIMS",          // High beta + consumer momentum
  "IONQ","KTOS","RTX","NNE",                         // Quantum + defence + nuclear

  // Removed: MARA, RIOT, CLSK -- crypto proxies, covered by direct crypto scanning
  // Removed: SMCI, MRVL, SNOW, COHR, QBTS, RGTI, BBAI, JOBY, ACHR, RCAT, GEV
  // -- retained in SECTOR_SYMBOLS for dynamic injection when narratives activate
];

// LSE pool -- V103: expanded to 10 elite UK names.
// Kept small and focused -- LSE treated as secondary feed vs US/crypto.
// Only names with genuine momentum potential retained.
const LSE_POOL = [
  // Defence & aerospace -- narrative active, institutional flow
  "BA.L","RR.L",
  // Miners -- highest-beta names, commodity-driven swings
  "AAL.L","GLEN.L","FRES.L",
  // Tech/Growth -- momentum potential
  "SMT.L","OCDO.L","AUTO.L","WEIR.L","REL.L",
];
...
function addHeat(asset, points, reason) {
  ...
  // V103: mark dirty for batch persist -- avoids per-call Supabase writes
  dirtyHeatAssets.add(asset);
  // V103: append-only heat history -- explains why assets became leaders
  if (supabaseEnabled) {
    supabase.from("momentum_heat_history").insert({
      timestamp:   new Date(now).toISOString(),
      asset,
      heat_before: Math.round(decayed),
      heat_after:  Math.round(newScore),
      reason,
    }).then(({ error }) => {
      if (error) log(`⚠️ Heat history insert error: ${asset} -- ${error.message}`);
    });
  }
...
// V103: dirty set for batch heat persistence -- avoids per-call Supabase writes.
// addHeat() marks assets dirty; persistDirtyHeatScores() bulk-upserts every 5 mins.
const dirtyHeatAssets = new Set();

async function persistDirtyHeatScores() {
  if (!supabaseEnabled || dirtyHeatAssets.size === 0) return;
  const now = new Date().toISOString();
  const upserts = [];
  for (const asset of dirtyHeatAssets) {
    const h = HEAT_SCORES.get(asset);
    if (!h) continue;
    upserts.push({
      asset,
      heat_score: h.score,
      last_seen:  new Date(h.lastUpdated).toISOString(),
      updated_at: now,
    });
  }
  dirtyHeatAssets.clear(); // clear before await -- any new dirty writes during upsert are safe
  if (upserts.length === 0) return;
  const { error } = await supabase
    .from("momentum_heat")
    .upsert(upserts, { onConflict: "asset" });
  if (error) log(`⚠️ Heat batch persist error: ${error.message}`);
  else log(`💾 Heat persisted -- ${upserts.length} assets written to Supabase`);
}

// V103: restore heat on boot with decay applied for time elapsed since last_seen.
// Prevents artificially high heat after a restart -- if an asset was hot 3 days ago
// its score decays correctly rather than being restored at full strength.
async function loadHeatScoresFromSupabase() {
  if (!supabaseEnabled) return;
  try {
    const { data, error } = await supabase.from("momentum_heat").select("*");
    if (error) { log(`⚠️ Heat restore error: ${error.message}`); return; }
    if (!data || data.length === 0) { log("📊 Heat restore -- no prior state found"); return; }
    const now = Date.now();
    let loaded = 0;
    for (const row of data) {
      const lastSeen    = new Date(row.last_seen).getTime();
      const elapsedHrs  = (now - lastSeen) / 3600000;
      const decayedScore = row.heat_score * Math.max(0, 1 - (elapsedHrs / 8));
      if (decayedScore < 1) continue; // fully decayed -- not worth restoring
      HEAT_SCORES.set(row.asset, {
        score:       decayedScore,
    ...
    log(`✅ Heat restored -- ${loaded}/${data.length} assets loaded (decay applied)`);
  } catch (e) {
    log(`⚠️ Heat restore exception: ${e.message}`);
  }
}
...
let lastLeaderboardDigest = 0;    // V103: hourly digest throttle -- batches noise into one message
const pendingLeaderboardEvents = []; // V103: buffer for low-priority events (state_change, milestone, exit)
let lastHeatPersist      = 0;     // V103: throttle batch heat persistence (~5 min)
...
function getProtectionReason(market) {
  if (market === "CRYPTO") {
    if (btcMacroTrend === "risk_off") return "Bearish trend confirmed (4h)";
    if (btcTrend === "risk_off")      return "Bearish trend (4h)";
    return "Neutral (range-bound)";
  }
  if (market === "US") {
    if (qqqTrend === "risk_off") return "Bearish trend confirmed — staying selective";
    if (qqqBullCount < 2)        return "Range-bound — waiting for stronger confirmation";
    return "Conditions mixed — monitoring closely";
  }
  return "Mixed conditions — monitoring closely";
}
...
async function getCryptoMovers() {
  try {
    ...
    if (!Array.isArray(data)) return [];
    return data
      .filter(t => CRYPTO_PAIRS.includes(t.symbol) && +t.priceChangePercent >= 3) // V103: lowered from 5% -- discovery happens at 3-4% with volume
...
async function sendWatchlistAlert(alerts, trades) {
  ...
  parts.push(``, `——————————`, `📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`);
  // V104: watchlist is internal-only -- heat scoring above is kept, public Telegram broadcast removed.
  // Subscribers see leaderboard and Rising Stars instead. Internal log retained for diagnostics.
  log(`👀 Watchlist internal -- ${watching.length} setups tracked (heat scoring active, no public broadcast)`);
}

// ── Narrative change detection ───────────────────────────────────
// V103: Fully event-driven Intel gate. Three tiers:
//
// TIER 1 — Immediate (no minimum gap): genuine new information
//   BTC/QQQ regime change, new asset enters/leaves, narrative changes
//
// TIER 2 — Heat jump (no minimum gap): asset strengthening significantly
//   Any tracked asset heat increases ≥15 since last post
//
// TIER 3 — 2hr heartbeat (last resort, requires actual change):
//   Only fires if 2hrs have elapsed AND something is materially different.
//   Silence is acceptable. Never repeats identical content.
function narrativeHasChanged(sectors, stockMovers, cryptoMovers, lseMovers = []) {
  ...
  // Hard cap -- max 8 intel posts per day
  if (narrativePostCount >= 8) return false;
  ...
  // ── TIER 1: Immediate triggers (no minimum gap) ──────────────────
  const newSector    = newState.sectors !== lastNarrativeState.sectors;

  const prevMovers   = (lastNarrativeState.stockMovers || "").split(",").filter(Boolean);
  const curMovers    = newState.stockMovers.split(",").filter(Boolean);
  const newStockIn   = curMovers.some(s => !prevMovers.includes(s));
  const stockLeft    = prevMovers.some(s => !curMovers.includes(s));

  const prevCrypto   = (lastNarrativeState.cryptoMovers || "").split(",").filter(Boolean);
  const curCrypto    = newState.cryptoMovers.split(",").filter(Boolean);
  const newCryptoIn  = curCrypto.some(s => !prevCrypto.includes(s));
  const cryptoLeft   = prevCrypto.some(s => !curCrypto.includes(s));

  // Only count regime as changed if it's genuinely different from last posted state
  // Prevents rapid BTC neutral→bull→neutral oscillation from spamming Intel
  const regimeChanged = (lastNarrativeState.btcTrend || "") !== (btcTrend || "") ||
                        (lastNarrativeState.qqqTrend || "") !== (qqqTrend || "");

  // Extra gate: regime change must be stable -- don't fire if same regime posted within 4hrs
  const fourHrs = 4 * 60 * 60 * 1000;
  const regimeRecentlyPosted = regimeChanged &&
    (Date.now() - (lastNarrativeState.lastRegimePost || 0)) < fourHrs &&
    (lastNarrativeState.btcTrendPosted === btcTrend) &&
    (lastNarrativeState.qqqTrendPosted === qqqTrend);
  const regimeTrigger = regimeChanged && !regimeRecentlyPosted;

  const tier1 = newSector || newStockIn || stockLeft || newCryptoIn || cryptoLeft || regimeTrigger;

  // ── TIER 2: Heat jump (no minimum gap) ───────────────────────────
  // Fires when a tracked asset's heat increases ≥15 since last post.
  // Signals a strengthening asset before a full trade alert arrives.
  const HEAT_JUMP_THRESHOLD = 15;
  let heatJumped = false;
  let heatJumpAsset = "";
  if (!tier1) {
    const lastHeatSnapshot = lastNarrativeState.heatSnapshot || {};
    for (const [asset, h] of HEAT_SCORES.entries()) {
      const currentHeat = Math.round(getHeat(asset));
      const priorHeat   = lastHeatSnapshot[asset] || 0;
      if (currentHeat - priorHeat >= HEAT_JUMP_THRESHOLD && currentHeat >= 30) {
        heatJumped    = true;
        heatJumpAsset = asset.replace("USDT","");
        break;
      }
    }
  }

  // ── TIER 3: 2hr heartbeat (last resort) ──────────────────────────
  // Only fires if: 2hrs elapsed AND content is materially different.
  // Never sends identical content twice. Silence is fine.
  const anyChange =
    ...
    regimeChanged;
  // Tier 3: scheduled mid-session check -- 15:30 UK only (1hr after US open)
  // Not a rolling 2hr timer -- one deliberate mid-session pulse per day
  const ukM = getUkMinutes ? getUkMinutes() : 0;
  const inMidSessionWindow = ukM >= 930 && ukM <= 960; // 15:30-16:00 UK
  const midSessionNotSentToday = (lastNarrativeState.midSessionSent || "") !== new Date().toISOString().slice(0,10);
  const tier3 = !tier1 && !heatJumped && inMidSessionWindow && midSessionNotSentToday && anyChange;
  if (tier3) lastNarrativeState.midSessionSent = new Date().toISOString().slice(0,10);

  const shouldPost = tier1 || heatJumped || tier3;

  const postReason = newSector       ? "new narrative"
                   : regimeChanged   ? "regime change"
                   : newStockIn      ? "new stock entered"
                   : stockLeft       ? "stock dropped out"
                   : newCryptoIn     ? "new crypto entered"
                   : cryptoLeft      ? "crypto dropped out"
                   : heatJumped      ? `heat jump (${heatJumpAsset} +${HEAT_JUMP_THRESHOLD}+)`
                   : tier3           ? "2hr heartbeat (content changed)"
                   : "no change";

  if (shouldPost) {
    // Snapshot current heat for next comparison
    const heatSnapshot = {};
    for (const [asset] of HEAT_SCORES.entries()) {
      heatSnapshot[asset] = Math.round(getHeat(asset));
    }
    lastNarrativeState     = { ...lastNarrativeState, ...newState, btcTrend, qqqTrend, heatSnapshot,
                               lastRegimePost: regimeTrigger ? Date.now() : (lastNarrativeState.lastRegimePost || 0),
                               btcTrendPosted: regimeTrigger ? btcTrend : (lastNarrativeState.btcTrendPosted || ""),
                               qqqTrendPosted: regimeTrigger ? qqqTrend : (lastNarrativeState.qqqTrendPosted || "") };
    lastNarrativeAlertTime = now;
    lastNarrativeSectors   = sectorsStr;
    narrativePostCount++;
    log(`🧠 Intel post #${narrativePostCount} today (${postReason})`);
  } else {
    log(`🧠 Intel skipped -- ${postReason} (${Math.round(timeSinceLast/60000)}min since last post)`);
  }
  return shouldPost ? postReason : false; // V103: return reason string so sendNarrativeAlert can pick message type
}

// ── Narrative alert ──────────────────────────────────────────────
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = [], triggerReason = "") {
  if (!Array.isArray(topMovers))    topMovers    = [];
  if (!Array.isArray(cryptoMovers)) cryptoMovers = [];
  if (!Array.isArray(lseMovers))    lseMovers    = [];

  // V103: Opening Bell fires once per trading day
  const ukMins   = getUkMinutes();
  const todayStr = new Date().toISOString().slice(0, 10);
  const inOpeningWindow = !isWeekend() && isUSMarketOpen() && ukMins >= 870 && ukMins < 900;
  const openingBellDue  = inOpeningWindow && openingBellSentDate !== todayStr;
  if (openingBellDue) openingBellSentDate = todayStr;

  // ── Helper functions ─────────────────────────────────────────────
  function heatBar(heat) {
    const filled = Math.min(10, Math.round((heat / 150) * 10));
    return "█".repeat(filled) + "░".repeat(10 - filled);
  }
  function convictionStars(heat, streakDays) {
    const score = Math.min(100,
      Math.min(streakDays / 7, 1) * 40 +
      (heat / 150) * 40 +
      (streakDays >= 3 ? 20 : streakDays >= 1 ? 10 : 0)
    );
    if (score >= 80) return "★★★★★";
    if (score >= 65) return "★★★★☆";
    if (score >= 50) return "★★★☆☆";
  ...
  function mktFlag(sym) {
    if (sym.endsWith("USDT")) return "🪙";
    if (sym.endsWith(".L"))   return "🇬🇧";
    return "🇺🇸";
  }
  function regimeLine() {
    const btcE = btcTrend==="strong_bull"?"🟢🟢":btcTrend==="bull"?"🟢":btcTrend==="bear"?"🔴":"🟡";
    const qqqE = qqqTrend==="bull"?"🟢":qqqTrend==="risk_off"?"🔴":"🟡";
    const lines = [`₿ BTC: ${btcTrend.replace("_"," ").toUpperCase()} ${btcE}`];
    if (!isWeekend()) lines.push(`📈 QQQ: ${qqqTrend.replace("_"," ").toUpperCase()} ${qqqE}`);
    return lines.join("  ·  ");
  }

  // ── EVENT-SPECIFIC MESSAGE BUILDERS ─────────────────────────────
  // Each trigger type gets its own focused message format.
  // One event, one clear message, one thing to act on.

  const FOOTER = [`——————————`, `📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`];

  // REGIME CHANGE -- most important, always specific
  // ── CONFIDENCE HELPER ────────────────────────────────────────────
  function confidenceLabel(stars) {
    if (stars === "★★★★★") return "High";
    if (stars === "★★★★☆") return "Building strongly";
    if (stars === "★★★☆☆") return "Building";
    if (stars === "★★☆☆☆") return "Early stage";
    return "Very early stage";
  }

  // ── STANDARD CLOSING LINE ─────────────────────────────────────────
  // Repeating this on every Intel post teaches the Intel vs Trade distinction
  // without ever having to explain it explicitly.
  // V103: INTEL_CLOSING removed -- the Action line teaches the same concept more naturally
  const NO_ACTION     = `👉 <b>Action:</b> Nothing to do right now. If this develops into a Trade Alert, we'll notify you immediately.`;

  // REGIME CHANGE 🔴 HIGH PRIORITY
  if (triggerReason === "regime change") {
    const prevBtc    = lastNarrativeState.btcTrend || "neutral";
    const prevQqq    = lastNarrativeState.qqqTrend || "neutral";
    const btcChanged = prevBtc !== btcTrend;
    const qqqChanged = prevQqq !== qqqTrend;
    const parts = [`🌍 <b>MARKET REGIME CHANGE</b>`, ``];

    // Always show current regime state -- even if prevBtc is unknown (first boot)
    if (btcChanged || !lastNarrativeState.btcTrend) {
      const fromLabel = prevBtc === "strong_bull" ? "strongly bullish" : prevBtc === "bull" ? "bullish" : prevBtc === "bear" ? "bearish" : "neutral";
      const toLabel   = btcTrend === "strong_bull" ? "strongly bullish" : btcTrend === "bull" ? "bullish" : btcTrend === "bear" ? "bearish" : "neutral";
      const toE       = btcTrend==="bull"||btcTrend==="strong_bull"?"🟢":btcTrend==="bear"?"🔴":"🟡";
      const changeStr = btcChanged ? `changed from ${fromLabel} to ${toLabel}` : `is ${toLabel}`;
      parts.push(`₿ <b>Bitcoin</b> ${changeStr} ${toE}`, ``);
      parts.push(`🧠 <b>What does that mean?</b>`);
      if (btcTrend==="bull"||btcTrend==="strong_bull")
        parts.push(`The overall crypto market is becoming healthier. More quality trading opportunities tend to appear when Bitcoin is in a bullish trend.`);
      else if (btcTrend==="bear")
        parts.push(`Bitcoin is under selling pressure. Sniper becomes more selective with crypto setups to protect against false signals in a weak market.`);
      else
        parts.push(`Bitcoin is moving sideways without a clear direction. Sniper stays cautious and only acts on the strongest crypto setups.`);
    }

    if (qqqChanged || !lastNarrativeState.qqqTrend) {
      const fromLabel = prevQqq === "bull" ? "bullish" : prevQqq === "risk_off" ? "bearish" : "neutral";
      const toLabel   = qqqTrend === "bull" ? "bullish" : qqqTrend === "risk_off" ? "bearish" : "neutral";
      const toE       = qqqTrend==="bull"?"🟢":qqqTrend==="risk_off"?"🔴":"🟡";
      const changeStr = qqqChanged ? `changed from ${fromLabel} to ${toLabel}` : `is ${toLabel}`;
      parts.push(``, `📈 <b>US stocks (QQQ)</b> ${changeStr} ${toE}`, ``);
      parts.push(`🧠 <b>What does that mean?</b>`);
      if (qqqTrend==="bull") parts.push(`The US stock market is in a positive trend. More stock setups are likely to emerge.`);
      else if (qqqTrend==="risk_off") parts.push(`US stocks are weakening. Sniper filters become more selective to avoid entering a falling market.`);
      else parts.push(`US stocks are mixed. Sniper focuses on the strongest individual setups rather than broad market moves.`);
    }

    // Show which sectors and assets are currently leading
    const hotSectors = [...HOT_SECTORS].filter(s => !s.startsWith("UK_")).slice(0, 3);
    if (hotSectors.length > 0) {
      parts.push(``, `🔥 <b>Currently leading:</b>`);
      for (const s of hotSectors) {
        const emoji = SECTOR_EMOJI[s] || "📌";
        const syms  = (SECTOR_SYMBOLS[s]||[]).filter(t => !t.endsWith("USDT") && !t.includes(".")).slice(0,3).join(", ");
        parts.push(`${emoji} ${s.replace(/_/g," ")}${syms ? ` — ${syms}` : ""}`);
      }
    }

    // Show hot crypto assets
    const hotCrypto = [...HEAT_SCORES.entries()]
      .filter(([a]) => CRYPTO_PAIRS.includes(a))
      .map(([a]) => ({ sym: a.replace("USDT",""), heat: Math.round(getHeat(a)) }))
      .filter(c => c.heat >= 40)
      .sort((a,b) => b.heat - a.heat)
      .slice(0, 4);
    if (hotCrypto.length > 0) {
      parts.push(``, `🪙 <b>Crypto on watch:</b>`);
      parts.push(hotCrypto.map(c => `${escapeHtml(c.sym)} (heat ${c.heat})`).join(" · "));
    }

    parts.push(``, `👀 <b>What happens next?</b>`);
    parts.push(`Sniper adjusts its filters automatically based on market conditions. Trade alerts are still only sent when every safety check passes.`);
    parts.push(``, NO_ACTION);
    parts.push(...FOOTER);
    await safeRun("telegramNarrative", () => sendChannel(lines(parts)));
    log(`🧠 Intel: regime change posted`);
    return;
  }

  // HEAT JUMP 🟡 MEDIUM PRIORITY
  if (triggerReason && triggerReason.startsWith("heat jump")) {
    const heatSnapshot = lastNarrativeState.heatSnapshot || {};
    const jumpers = [];
    for (const [asset] of HEAT_SCORES.entries()) {
      const cur   = Math.round(getHeat(asset));
      const prior = heatSnapshot[asset] || 0;
      if (cur - prior >= 15 && cur >= 30) jumpers.push({ asset, cur, prior, delta: cur - prior });
    }
    if (jumpers.length > 0) {
      jumpers.sort((a, b) => b.delta - a.delta);
      const parts = [`⚡ <b>MOMENTUM BUILDING</b>`, ``, regimeLine(), ``];
      for (const j of jumpers.slice(0, 3)) {
        const sym    = j.asset.replace("USDT","");
        const flag   = mktFlag(j.asset);
        const stars  = convictionStars(j.cur, 1);
        const conf   = confidenceLabel(stars);
        const bar    = heatBar(j.cur);
        const theme  = [...HOT_SECTORS].find(s => (SECTOR_SYMBOLS[s]||[]).includes(j.asset.replace("USDT",""))) || "";
        const tLabel = theme ? (SECTOR_EMOJI[theme]||"")+" "+theme.replace("_"," ") : "";
        parts.push(`<b>${escapeHtml(sym)}</b> ${flag}${tLabel ? "  "+tLabel : ""}`);
        parts.push(`${bar}  ${stars}  Current confidence: <b>${conf}</b>`);
        parts.push(``);
        parts.push(`🧠 <b>What changed?</b>`);
        parts.push(`Buying interest increased noticeably (${j.prior} → ${j.cur}). This is <b>not</b> a buy signal yet.`);
        parts.push(``);
        parts.push(`💡 <b>Why does it matter?</b>`);
        parts.push(`Assets that continue attracting buyers over several scans are more likely to develop a quality setup.`);
        parts.push(``);
      }
      const needsMore = jumpers[0].cur < 60;
      parts.push(`👀 <b>What happens next?</b>`);
      parts.push(needsMore
        ? `Sniper will keep monitoring. If buying interest keeps increasing and volume expands, a Trade Alert may follow.`
        : `Momentum is strong. Sniper is now watching for price structure and volume to confirm before issuing a Trade Alert.`);
      parts.push(``, NO_ACTION);
          parts.push(...FOOTER);
      await safeRun("telegramNarrative", () => sendChannel(lines(parts)));
      log(`🧠 Intel: heat jump posted (${jumpers[0].asset})`);
      return;
    }
  }
  ...
  // ASSET IN/OUT
  const prevCrypto   = (lastNarrativeState.cryptoMovers || "").split(",").filter(Boolean);
  const curCrypto    = cryptoMovers.filter(c => isApprovedCrypto(c.symbol)).map(c => c.symbol);
  const newCryptoIn  = curCrypto.filter(s => !prevCrypto.includes(s));
  const cryptoLeft   = prevCrypto.filter(s => !curCrypto.includes(s));
  const prevStocks   = (lastNarrativeState.stockMovers || "").split(",").filter(Boolean);
  const curStocks    = topMovers.slice(0,6);
  const newStockIn   = curStocks.filter(s => !prevStocks.includes(s));
  const stockLeft    = prevStocks.filter(s => !curStocks.includes(s));

  // NEW ASSET 🟡 MEDIUM PRIORITY
  if ((newCryptoIn.length > 0 || newStockIn.length > 0) &&
      (triggerReason === "new crypto entered" || triggerReason === "new stock entered")) {
    const parts = [`🆕 <b>NEW MOMENTUM DETECTED</b>`, ``, regimeLine(), ``];
    for (const sym of [...newCryptoIn.map(s=>s.replace("USDT","")), ...newStockIn]) {
      const assetFull = newCryptoIn.find(s => s.replace("USDT","") === sym) || sym;
      const heat      = Math.round(getHeat(assetFull.endsWith("USDT") ? assetFull : sym));
      const stars     = convictionStars(heat, 1);
      const conf      = confidenceLabel(stars);
      const bar       = heatBar(heat);
      const flag      = mktFlag(assetFull);
      const theme     = [...HOT_SECTORS].find(s => (SECTOR_SYMBOLS[s]||[]).includes(assetFull.replace("USDT",""))) || "";
      const tLabel    = theme ? (SECTOR_EMOJI[theme]||"")+" "+theme.replace("_"," ") : "Emerging";
      parts.push(`<b>${escapeHtml(sym)}</b> ${flag}`);
      parts.push(`${bar}  ${stars}  Current confidence: <b>${conf}</b>`);
      parts.push(`${tLabel}`);
      parts.push(``);
      parts.push(`🧠 <b>What changed?</b>`);
      parts.push(`${sym} has appeared on the Sniper Intel board for the first time today.`);
      parts.push(``);
      parts.push(`💡 <b>Why is that interesting?</b>`);
      parts.push(`More buyers are beginning to take interest in this ${flag === "🪙" ? "coin" : "stock"}. Some assets lose momentum within hours. Others continue strengthening over several days. Sniper tracks that progress automatically.`);
      parts.push(``);
    }
    parts.push(`📊 <b>Why Sniper is watching it</b>`);
    parts.push(`It has passed Sniper\'s first momentum checks, but hasn\'t met the requirements for a Trade Alert yet.`);
    parts.push(``);
    parts.push(`👀 <b>What happens next?</b>`);
    parts.push(`If buying interest continues to grow, Sniper will keep checking for a complete setup.`);
    parts.push(``, NO_ACTION);
    parts.push(...FOOTER);
    await safeRun("telegramNarrative", () => sendChannel(lines(parts)));
    log(`🧠 Intel: new asset detected`);
    return;
  }

  // ASSET LEFT 🟡 MEDIUM PRIORITY
  if ((cryptoLeft.length > 0 || stockLeft.length > 0) &&
      (triggerReason === "crypto dropped out" || triggerReason === "stock dropped out")) {
    const parts = [`📉 <b>MOMENTUM FADING</b>`, ``, regimeLine(), ``];
    for (const sym of [...cryptoLeft.map(s=>s.replace("USDT","")), ...stockLeft]) {
      const flag  = cryptoLeft.some(s => s.replace("USDT","") === sym) ? "🪙" : "🇺🇸";
      const type  = flag === "🪙" ? "coin" : "stock";
      parts.push(`<b>${escapeHtml(sym)}</b> ${flag}`, ``);
      parts.push(`🧠 <b>What changed?</b>`);
      parts.push(`${sym} has dropped below the momentum threshold. Buying interest has faded on the latest scans.`);
      parts.push(``);
      parts.push(`💡 <b>Why does it matter?</b>`);
      parts.push(`This ${type} is no longer showing the strength needed for Sniper to consider a trade setup. It may recover — or it may continue weakening.`);
      parts.push(``);
    }
    parts.push(`👀 <b>What happens next?</b>`);
    parts.push(`Sniper will continue watching. If momentum rebuilds over the next few sessions, it may reappear on the Intel board.`);
    parts.push(``, NO_ACTION);
      parts.push(...FOOTER);
    await safeRun("telegramNarrative", () => sendChannel(lines(parts)));
    log(`🧠 Intel: asset dropped out`);
    return;
  }

  // NARRATIVE ROTATION 🔴 HIGH PRIORITY
  if (triggerReason === "new narrative") {
    const newSectors = HOT_SECTORS.filter(s => !s.startsWith("UK_")).slice(0,3);
    const parts = [`🔄 <b>SECTOR ROTATION</b>`, ``, regimeLine(), ``];
    parts.push(`🧠 <b>What changed?</b>`);
    if (newSectors.length > 0) {
      const top = newSectors[0];
      const emoji = SECTOR_EMOJI[top] || "📌";
      const syms  = (SECTOR_SYMBOLS[top]||[]).filter(t => !t.includes(".") && !t.endsWith("USDT")).slice(0,3).join(", ");
      parts.push(`Investors are rotating into <b>${top.replace("_"," ")}</b> ${emoji}`);
      if (syms) parts.push(`Leading names: ${syms}`);
    }
    parts.push(``);
    if (newSectors.length >= 2) {
      parts.push(`<b>Current sector ranking:</b>`);
      for (let i = 0; i < Math.min(newSectors.length, 3); i++) {
        const s    = newSectors[i];
        const emoji = SECTOR_EMOJI[s] || "📌";
        const rank  = ["🥇","🥈","🥉"][i];
        parts.push(`${rank} ${emoji} ${s.replace("_"," ")}`);
      }
      parts.push(``);
    }
    parts.push(`💡 <b>Why does it matter?</b>`);
    parts.push(`When money flows into a specific sector, individual stocks and coins within that sector often follow. Sniper tracks these rotations to spot setups before they become obvious.`);
    parts.push(``);
    parts.push(`👀 <b>What happens next?</b>`);
    parts.push(`Sniper is scanning the leading sector for confirmed setups. A Trade Alert fires only when price structure, volume and risk-reward all align.`);
    parts.push(``, NO_ACTION);
      parts.push(...FOOTER);
    await safeRun("telegramNarrative", () => sendChannel(lines(parts)));
    log(`🧠 Intel: narrative rotation posted`);
    return;
  }

  // ── OPENING BELL / SCHEDULED -- full format ───────────────────────
  // Used for: Opening Bell, Midday, Closing Bell, 2hr heartbeat with real changes.
  const isOpeningBell = openingBellDue;
  const isMidday      = !isWeekend() && isUSMarketOpen() && ukMins >= 1650 && ukMins < 1710;
  const isClosingBell = !isWeekend() && isUSMarketOpen() && ukMins >= 1920 && ukMins < 1980;
  const intelHeader   = isOpeningBell ? "🔔🟢 <b>OPENING BELL INTEL</b>"
                      : isMidday      ? "🌤️ <b>MIDDAY INTEL</b>"
                      : isClosingBell ? "🌆 <b>CLOSING BELL INTEL</b>"
                      : "🧠 <b>MARKET INTEL UPDATE</b>";
  const parts = [intelHeader, "", regimeLine(), ""];

  // Regime already shown in header via regimeLine() -- no repeat needed
  // ── ACTIVE THEMES ───────────────────────────────────────────────
  if (!isWeekend() && HOT_SECTORS.length > 0) {
    ...
    if (usSectors.length > 0) {
      ...
      for (const s of usSectors) {
        const syms = (SECTOR_SYMBOLS[s]||[])
          .filter(sym => !sym.includes(".") && !sym.endsWith("USDT")) // US stocks only in narrative line
          .slice(0,4).join(", ");
        const emoji = SECTOR_EMOJI[s] || "📌";
        const label = s.replace("_"," ");
        parts.push(`  ${emoji} <b>${escapeHtml(label)}</b> → ${escapeHtml(syms)}`);
      }
      parts.push("");
    }
  }
  ...
  if (!isWeekend() && topMovers.length > 0) {
    ...
    // V103: persist intel mentions for future performance analysis
    if (supabaseEnabled && ranked.length > 0) {
      const ts = new Date().toISOString();
      const intelRows = ranked.map(m => ({
        timestamp: ts,
        asset: m.sym,
        market: "US",
        mention_type: "intel",
        pct_change: m.pmPct || m.pct || 0,
        in_sector: m.inSector || false,
        heat_at_mention: Math.round(getHeat(m.sym)),
        btc_regime: btcTrend || "unknown",
        qqq_regime: qqqTrend || "unknown",
      }));
      supabase.from("intel_mention_history").insert(intelRows)
        .then(({ error }) => { if (error) log(`⚠️ Intel history error: ${error.message}`); });
    }

    parts.push("🇺🇸 <b>Stocks attracting attention:</b>");
    ...
    if (topPick) {
      const topTheme = HOT_SECTORS.find(s => (SECTOR_SYMBOLS[s]||[]).includes(topPick.sym));
      const themeStr = topTheme ? ` (${topTheme.replace("_"," ")})` : "";
      parts.push(``);
      parts.push(`🏆 <b>Top pick: ${escapeHtml(topPick.sym)}${themeStr}</b>`);
      parts.push(`It currently has the strongest combination of price movement, buying interest and market structure${themeStr ? " within the leading narrative" : ""}. Sniper will keep monitoring for a confirmed entry.`);
      // V103: track today's top pick for end-of-day missed-move review
      if (!dailyStats.topPickToday || dailyStats.topPickToday.sym !== topPick.sym) {
        dailyStats.topPickToday = {
          sym:            topPick.sym,
          theme:          themeStr,
          setAt:          nowIso(),
          priceAtPick:    topPick.last || null,
          inMorningMovers: false, // updated by sendMorningBriefing if it appears
        };
        log(`🏆 Top pick recorded: ${topPick.sym}`);
      }
    }
    parts.push("");
  }
  ...
  if (filteredCryptoMovers.length > 0) {
    ...
    // V103: persist crypto intel mentions
    if (supabaseEnabled && filteredCryptoMovers.length > 0) {
      const ts = new Date().toISOString();
      const cryptoRows = filteredCryptoMovers.map(c => ({
        timestamp: ts,
        asset: c.symbol,
        market: "CRYPTO",
        mention_type: "crypto-intel",
        pct_change: c.changePct || 0,
        in_sector: HOT_SECTORS.some(s => (SECTOR_SYMBOLS[s]||[]).includes(c.symbol.replace("USDT",""))),
        heat_at_mention: Math.round(getHeat(c.symbol)),
        btc_regime: btcTrend || "unknown",
        qqq_regime: qqqTrend || "unknown",
      }));
      supabase.from("intel_mention_history").insert(cryptoRows)
        .then(({ error }) => { if (error) log(`⚠️ Crypto intel history error: ${error.message}`); });
    }

    parts.push("\uD83E\uDE99 <b>Crypto to watch:</b>");
    for (const c of filteredCryptoMovers) {
      const sym    = c.symbol.replace("USDT","");
      const what   = c.changePct >= 15 ? "Price jumped sharply — buyers are highly active."
                   : c.changePct >= 8  ? "Strong buying interest detected."
                   : c.changePct >= 3  ? "Momentum is beginning to improve."
                   : c.changePct <= -15 ? "Sharp selling pressure — buyers stepping back."
                   : c.changePct <= -8  ? "Selling pressure increasing."
                   : "Pulling back slightly.";
      const next   = c.changePct >= 5  ? "If buying pressure continues, it could move onto Sniper's priority watchlist."
                   : c.changePct >= 3  ? "Sniper is watching for a second session of strength before upgrading its status."
                   : "Sniper will continue monitoring.";
      const badge  = c.changePct >= 15 ? "🔥" : c.changePct >= 8 ? "⚡" : c.changePct >= 3 ? "📈" : "📉";
      parts.push(`  ${badge} <b>${escapeHtml(sym)}</b> +${c.changePct.toFixed(1)}%`);
      parts.push(`  <i>${what} ${next}</i>`);
    }
    parts.push("");
  }
  ...
  parts.push(`👉 <b>No action needed yet.</b>`);
  parts.push(`Sniper is monitoring these assets and will only publish a Trade Alert if all entry conditions are confirmed.`);
  parts.push(``, `\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014`, `📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`);
...
async function refreshDynamicPools(forceNarrativeAlert = false) {
  ...
  try {
    ...
    CRYPTO_PAIRS = dedup([...STATIC_CRYPTO_FALLBACK, ...trendingCoins]).slice(0, 26);
    ...
    if (!isWeekend()) {
      ...
      if (shouldSendStockAlerts()) {
        ...
        const intelReason = forceNarrativeAlert ? "opening bell" : narrativeHasChanged(HOT_SECTORS, topMovers, cryptoMovers, lseMovers);
        if (intelReason) {
          if (forceNarrativeAlert) {
            lastNarrativeAlertTime = 0;
            narrativeHasChanged(HOT_SECTORS, topMovers, cryptoMovers, lseMovers);
          }
          await sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts, lseMovers, typeof intelReason === "string" ? intelReason : "scheduled change");
        }
      }
    } else {
      log(`✅ Weekend -- Crypto: ${CRYPTO_PAIRS.length} pairs`);
      const cryptoMovers = await getCryptoMovers();
      const cryptoIntelReason = forceNarrativeAlert ? "opening bell" : narrativeHasChanged([],[],cryptoMovers,[]);
      if (cryptoIntelReason && (forceNarrativeAlert || shouldSendCryptoAlerts())) {
        await sendNarrativeAlert([], cryptoMovers, {}, [], typeof cryptoIntelReason === "string" ? cryptoIntelReason : "scheduled change");
      }
    }
...
async function sendWeeklyAudit() {
  ...
  const parts = [
    ...
    total > 0
      ? `<b>Long-term outlook:</b> If Sniper continues performing like this over many trades, it would be expected to make money overall.`
      : ``,
    total > 0
      ? `Expected Value per signal: ${evSign}${ev}%`
      : ``,
    ``,
    `<b>🔍 Filter activity</b>`,
    ...
    ``,
    `——————————`,
    `📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`,
    `@baretradesignals 🎯`,
  ];

  await sendPrivate(lines(parts.filter(Boolean)));
  log("📊 Weekly self-audit sent to Bluejam");
}

// ── V103: Weekly health dashboard (Bluejam only) ─────────────────
// Five metrics that tell you whether Sniper is healthy.
// Fires Sunday alongside weekly audit. Private only -- ops visibility.
async function sendV104HealthDashboard() {
  const isSunday = new Date().getDay() === 0;
  if (!isSunday) return;
  const today = new Date().toISOString().slice(0, 10);
  if (lastHealthDashboardDate === today) return;
  lastHealthDashboardDate = today;

  // Trade performance from closed trades this week
  const trades   = await loadTrades();
  const thisWeek = trades.filter(t => {
    if (!t.closedAt && !t.sentAt) return false;
    return (Date.now() - new Date(t.closedAt || t.sentAt)) < 7 * 24 * 60 * 60 * 1000;
  });
  const alerts  = thisWeek.filter(t => t.sentToChannel);
  const closed  = thisWeek.filter(t => t.outcome !== "OPEN" && t.outcome !== "PENDING" && t.realizedR !== null);
  const wins    = closed.filter(t => t.realizedR > 0);
  const winRate = closed.length > 0 ? Math.round((wins.length / closed.length) * 100) : null;

  // Top rejection reason from today's in-memory stats
  const topReject  = Object.entries(rejectStats).sort((a,b) => b[1]-a[1])[0];
  const topBlocker = topReject ? `${topReject[0].replace(/_/g," ")} (${topReject[1]})` : "none";

  // Highest score seen this week -- query sniper_signal_decisions
  let topScore = null;
  let goodCount = 0, watchCount = 0, eliteCount = 0;
  if (supabaseEnabled) {
    try {
      const { data } = await supabase
        .from("sniper_signal_decisions")
        .select("raw_sniper_score, decision_class")
        .eq("engine_version", "V104")
        .gte("created_at", new Date(Date.now() - 7*24*60*60*1000).toISOString());
      if (data?.length) {
        const scores = data.map(r => r.raw_sniper_score).filter(Boolean);
        topScore  = scores.length ? Math.max(...scores) : null;
        goodCount  = data.filter(r => r.decision_class === "GOOD").length;
        watchCount = data.filter(r => r.decision_class === "WATCH").length;
        eliteCount = data.filter(r => r.decision_class === "ELITE").length;
      }
    } catch { /* skip if unavailable */ }
  }

  const parts = [
    `📊 <b>V104 HEALTH DASHBOARD</b>`,
    `<i>Week ending ${new Date().toLocaleDateString("en-GB", {day:"numeric", month:"short"})}</i>`,
    ``,
    `🎯 <b>Trade alerts (ELITE):</b>  ${alerts.length}  ${alerts.length >= 2 ? "✅" : alerts.length >= 1 ? "🟡" : "🔴"}`,
    `   Target: 2–4 per week`,
    ``,
    `✅ <b>Win rate:</b>  ${winRate !== null ? winRate+"%" : "No closed trades yet"}  ${winRate !== null ? (winRate >= 60 ? "✅" : winRate >= 50 ? "🟡" : "🔴") : "⚪"}`,
    `   Target: ≥60%`,
    ``,
    `🧠 <b>Intel posts today:</b>  ${narrativePostCount}`,
    `   Target: event-driven, not repetitive`,
    ``,
    `📋 <b>This week's classifications:</b>`,
    `   🟢 ELITE: ${eliteCount} | 🟡 GOOD: ${goodCount} | 👁 WATCH: ${watchCount}`,
    ``,
    `🚫 <b>Top rejection reason:</b>  ${topBlocker}`,
    `   Watch for: resistance or rr dominating near misses`,
    ``,
    `📈 <b>Highest score seen:</b>  ${topScore !== null ? topScore : "No data yet"}`,
    `   Watch for: scores clustering near 75 (Growth Mode floor)`,
    ``,
    `——————————`,
    `<i>V104 · ELITE→public · GOOD→Bluejam · Freeze until 100 trades</i>`,
  ];

  await sendPrivate(lines(parts));
  log("📊 V104 health dashboard sent to Bluejam");
}
...
const LEADERBOARD_DISPLAY_LIMIT = { CRYPTO: 5, US: 3 };
  ...
  try {
    ...
    for (const asset of trackedAssets) {
      ...
      upserts.push({
        ...
        current_score: score,
        streak_scans: streak,
        prev_streak_scans: prior.streak_scans || streak, // V103: enables Δ Streak in league table
    ...
    for (const market of ["CRYPTO", "US"]) {
      ...
      if (entered.length > 0 || exited.length > 0) {
        events.push({ type: "topN_change", market, limit, entered, exited, newTopN: afterTopN });
      }
    }
...
const LEADERBOARD_DISCLAIMER = `📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>\n@baretradesignals 🎯`
// V103: standard footer for all public alerts -- disclaimer + standalone signature
...
// V103: leaderboard alert compression.
// HIGH PRIORITY (immediate): new_leader_entry, topN_change -- meaningful structural events.
// LOW PRIORITY (buffered): state_change, milestone, exit -- internal telemetry, rolled into hourly digest.
async function postLeaderboardEvent(event) {
  ...
  // ── Immediate alert -- new #1 only ──────────────────────────────
  // Everything else rolls into the hourly league table. The table already
  // shows rank, heat, streak and trend -- individual event pings add noise.
  if (event.type === "new_leader_entry") {
    await sendPrivate(lines([
  ...
  // ── Everything else -- buffer for hourly league table ────────────
  // topN_change, state_change, milestone, exit: all covered by the
  // hourly digest which shows the full picture in one message.
  if (["topN_change","state_change","milestone","exit"].includes(event.type)) {
    let label;
    if (event.type === "topN_change") {
      const entered = (event.entered||[]).map(a => `⬆ ${a.replace("USDT","")}`).join(", ");
      const exited  = (event.exited||[]).map(a => `⬇ ${a.replace("USDT","")}`).join(", ");
      label = [entered, exited].filter(Boolean).join(" | ") || "Top 5 shuffle";
    } else if (event.type === "state_change") {
      label = `${sym} ${event.from||"?"} ➜ ${event.to}`;
    } else if (event.type === "milestone") {
      label = `${sym} held ${event.state} for ${event.streak} scans`;
    } else {
      label = `${sym} exited (was ${event.previousState}, ${event.streak} scans)`;
    }
    pendingLeaderboardEvents.push({ label, market: event.market, ts: Date.now() });
  }
}

// V103: hourly momentum league table for Bluejam.
// Replaces 20+ individual state_change/milestone/exit pings with one clean snapshot.
// Reads live Supabase leaderboard state -- crypto and stocks kept separate.
// Also shows rank movers (biggest climbers/fallers since last snapshot).
async function sendLeaderboardDigest() {
  const now = Date.now();
  if (now - lastLeaderboardDigest < 60 * 60 * 1000) return;

  // Only send if there are buffered events OR it's been 2+ hours (scheduled refresh)
  const hasEvents = pendingLeaderboardEvents.length > 0;
  const scheduledRefresh = now - lastLeaderboardDigest >= 2 * 60 * 60 * 1000;
  if (!hasEvents && !scheduledRefresh) return;

  lastLeaderboardDigest = now;
  pendingLeaderboardEvents.length = 0; // clear buffer regardless
  ...
  try {
    const { data, error } = await supabase
      .from("momentum_leaderboard")
      .select("asset, market, state, streak_scans, current_score, rank")
      .order("current_score", { ascending: false })
      .limit(20);

    if (error || !data || data.length === 0) {
      log(`📊 League table skipped -- no leaderboard data yet`);
      return;
    }

    const rankEmoji = ["🥇","🥈","🥉","4️⃣","5️⃣","6️⃣","7️⃣","8️⃣"];
    const stateEmoji = { Leader:"🔥", Hold:"✅", Cooling:"❄️", Accelerating:"🚀" };

    // V103: trend arrow -- purely factual, compares current state to previous state
    const stateRank = { Accelerating:4, Leader:3, Hold:2, Cooling:1, Emerging:0 };
    function trendArrow(r) {
      if (!r.previous_state || r.previous_state === r.state) return "→";
      return (stateRank[r.state]||0) > (stateRank[r.previous_state]||0) ? "↑" : "↓";
    }

    const { data: fullData, error: fullErr } = await supabase
      .from("momentum_leaderboard")
      .select("asset, market, state, previous_state, streak_scans, prev_streak_scans, current_score, rank")
      .order("current_score", { ascending: false })
      .limit(20);

    const rows   = fullErr ? data : (fullData || data);
    const crypto = rows.filter(r => r.market === "CRYPTO").slice(0, 5);
    const stocks  = rows.filter(r => r.market !== "CRYPTO").slice(0, 5);

    const parts = [`📊 <b>MOMENTUM LEAGUE TABLE</b>`, ``];

    // Row format: 🥇 EIGEN   Heat 84 ↑   17 scans  (Δ+2)
    function formatRow(r, i, isCrypto) {
      const sym    = isCrypto ? r.asset.replace("USDT","") : r.asset;
      const heat   = Math.round(r.current_score || 0);
      const scans  = r.streak_scans || 1;
      const prev   = r.prev_streak_scans || scans;
      const delta  = scans - prev;
      const deltaStr = delta === 0 ? "(Δ0)" : delta > 0 ? `(Δ+${delta})` : `(Δ${delta})`;
      const arrow  = trendArrow(r);
      const rank   = rankEmoji[i] || `${i+1}.`;
      const symPad = sym.padEnd(6," ");
      return `${rank} <b>${escapeHtml(symPad)}</b>   Heat ${heat} ${arrow}   ${scans} scan${scans===1?"":"s"}  ${deltaStr}`;
    }

    if (crypto.length > 0) {
      parts.push(`🪙 <b>Crypto</b>`);
      crypto.forEach((r, i) => parts.push(formatRow(r, i, true)));
      parts.push(``);
    }

    if (stocks.length > 0) {
      parts.push(`🇺🇸 <b>Stocks</b>`);
      stocks.forEach((r, i) => parts.push(formatRow(r, i, false)));
      parts.push(``);
    }

    if (rows.length === 0) {
      parts.push(`<i>No leaders yet — heat still building.</i>`, ``);
    }

    parts.push(LEADERBOARD_DISCLAIMER);
    await sendPrivate(lines(parts));
    log(`📊 League table sent -- ${crypto.length} crypto, ${stocks.length} stock leaders`);

  } catch (e) {
    log(`⚠️ League table exception: ${e.message}`);
  }
}
...
async function runMomentumLeaderboardCheck() {
  try {
    ...
    const heatSummary = [...HEAT_SCORES.entries()]
      .map(([a, h]) => `${a.replace("USDT","")}=${Math.round(getHeat(a))}`)
      .sort((x, y) => parseFloat(y.split("=")[1]) - parseFloat(x.split("=")[1]))
      .join(" | ");
    log(`📊 HEAT SNAPSHOT: ${heatSummary || "empty"} (threshold=60)`);
  } catch (e) {
...
async function checkMarketAlerts() {
  ...
  for (const { key, isOpen } of checks) {
    ...
    if (nowOpen && !marketWasOpen[key]) {
      ...
      if (key !== "CRYPTO") {
        // V104: Market Open removed from public -- Opening Bell Intel covers it.
        // Brief ops note to Bluejam only.
        const msgs = {
          US:  lines([`🟢🇺🇸 US MARKET OPEN`, `QQQ: ${qqqTrend} · BTC: ${btcTrend} · ${STOCK_POOL.length} stocks scanning`]),
          LSE: lines([`🟢🇬🇧 LSE MARKET OPEN`, `${LSE_POOL.length} stocks scanning`])
        };
        await safeRun(`open${key}`, () => sendPrivate(msgs[key]));
    ...
    if (!nowOpen && marketWasOpen[key]) {
      ...
      if (key !== "CRYPTO") {
        // V103: Market Closed removed from public channel.
        // Daily Report fires shortly after and covers the session summary.
        // Close notification logged to Bluejam only for ops awareness.
        const closeMsgs = {
          US:  lines([`🔴🇺🇸 <b>US SESSION ENDED</b>`, `Daily report will follow shortly.`]),
          LSE: lines([`🔴🇬🇧 <b>LSE SESSION ENDED</b>`])
        };
        await safeRun(`close${key}`, () => sendPrivate(closeMsgs[key]));
        ...
        if (key === "US") await safeRun("outcomeTracker", runOutcomeTracker);
        if (key === "US") await safeRun("topPickReview", sendTopPickReview);
        // V104: EOD report only fires if a trade alert was sent today or we have near misses
        if (key === "US" && (dailyStats.signalsFired > 0 || nearMissLog.length > 0)) {
          await safeRun("eodReport", sendEndOfDayReport);
        } else if (key === "US") {
          log("📊 EOD report skipped -- no trades or near misses today");
        }
      }
    }
  ...
  await safeRun("healthDashboard", sendV104HealthDashboard);
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  ...
  // V103 BUG FIX: minsIntoSession declared here -- must be before first use in ignitionSignal below
  ...
  // V103 PHASE2: BTC neutral is advisory not a hard block -- V65 era didn't block on regime
  // Score penalty applied upstream; let the score decide rather than hard-reject here
  const btcNotBull = market === "CRYPTO" && btcTrend !== "bull" && btcTrend !== "strong_bull";
  if (false && btcNotBull && setupType === "PULLBACK_CONTINUATION") { // disabled -- regime is score modifier
  ...
  // V103: conditional buy fires when BTC bull OR strong individual momentum (vol >= 1.5x)
  // Lowered from 1.8 -- SQL audit showed INJ/EIGEN/ONDO/TIA all lost to limit entries on neutral BTC
  // Evidence: multiple DRIFTED_TOO_FAR trades were at or beyond TP when cancelled
  const useConditional = market === "CRYPTO" && (isBtcBull || volRatio >= 1.5);
  ...
  const maxRisk = market === "CRYPTO" ? 0.08
                ...
                : market === "US" ? 0.04
                : 0.06; // LSE
  // V104: risk_pct moves from hard stop to quality flag
  // riskPct <= 0 still kills (invalid stop placement — safety gate)
  if (riskPct <= 0) { reject("risk_pct_invalid", asset, market); return null; }
  const qualityFlags = {};
  if (riskPct > maxRisk) {
    qualityFlags.riskPct = true;
    log(`📋 ${asset} quality flag: risk_pct ${(riskPct*100).toFixed(1)}% > max ${(maxRisk*100).toFixed(0)}%`);
  }
  ...
  const minRRforMarket = market === "CRYPTO" ? 1.6 : MIN_RR; // V103 GROWTH MODE: crypto 1.7->1.6
  // V104: rr_too_low moves from hard stop to quality flag
  if (rr < minRRforMarket) {
    qualityFlags.rr = true;
    log(`📋 ${asset} quality flag: rr ${rr} < min ${minRRforMarket}`);
  }
  ...
  // V104: expansion checks move from hard stops to quality flags
  const expansionAtr = atr > 0 ? targetDistance / atr : 0;
  if (targetDistance < atr * 1.5) {
    checkpoint("expansion", false, `target ${targetDistance.toFixed(4)} < 1.5 ATR -- quality flag`);
    qualityFlags.expansion = true;
    log(`📋 ${asset} quality flag: expansion ${expansionAtr.toFixed(2)} ATR (min 1.5)`);
  } else {
  ...
  if (resistanceRoom < atr * 1.0 && setupType !== "BREAKOUT_CONTINUATION") {
    qualityFlags.resistance = true;
    log(`📋 ${asset} quality flag: resistance ${resistanceRoom.toFixed(4)} < 1 ATR`);
  }
  ...
  // V103 PHASE2: crypto grade gate aligned with MIN_CRYPTO_SCORE=74
  const cryptoGradeOk = market !== "CRYPTO" || score >= MIN_CRYPTO_SCORE;

  const scoreFloor = minScoreForMarket(market, asset);

  // ── V104: DECISION ENGINE ────────────────────────────────────────
  // Score floors move from hard stops to quality flags.
  // Every candidate that reaches this point is classified, logged, and audited.
  // No candidate disappears without a trail.

  if (score < MIN_GLOBAL_SCORE) qualityFlags.score = true;
  if (score < scoreFloor)       qualityFlags.score = true;
  if (market === "CRYPTO" && !cryptoGradeOk) qualityFlags.score = true;
  if (grade === "B")            qualityFlags.gradeB = true;

  const flagCount = Object.values(qualityFlags).filter(Boolean).length;

  // ── Execution Score (composite) ───────────────────────────────
  // A single 0-100 score combining four independent quality dimensions.
  // Classification is based on THIS score, not directly on raw Sniper score.
  // This means future signals (Heat Velocity, Intel Persistence, etc.)
  // can influence classification without the raw score dominating.
  //
  // Dimensions:
  //   Setup Quality (35%)  — raw Sniper score normalised
  //   Execution Window (25%) — R:R and expansion quality
  //   Regime Alignment (20%) — BTC/QQQ tailwind
  //   Conviction (20%)    — heat + volume alignment
  //
  const setupQuality = Math.min(100, Math.max(0, score - 50)) / 50 * 100; // 50-100 maps to 0-100
  const rrQuality    = qualityFlags.rr ? 0 : Math.min(100, (rr / (minRRforMarket * 2)) * 100);
  const expQuality   = qualityFlags.expansion ? 0 : Math.min(100, (expansionAtr / 3) * 100);
  const execWindow   = (rrQuality + expQuality) / 2;
  const regimeScore  = market === "CRYPTO"
    ? (btcTrend === "strong_bull" ? 100 : btcTrend === "bull" ? 80 : btcTrend === "neutral" ? 50 : 20)
    : (qqqTrend === "bull" && qqqBullCount >= 2 ? 100 : qqqTrend === "bull" ? 75 : qqqTrend === "neutral" ? 50 : 20);
  const heatNorm     = Math.min(100, getHeat(asset));
  const volNorm      = Math.min(100, (volRatio - 1) / 2 * 100);
  const conviction   = (heatNorm * 0.6 + volNorm * 0.4);
  ...
  // ── Classification (based on executionScore, not raw score) ──────
  // ELITE:  executionScore ≥ 65, no quality flags → public trade
  // GOOD:   executionScore ≥ 45, ≤ 1 non-score flag → internal queue
  // WATCH:  executionScore ≥ 25 → internal monitoring
  // REJECT: executionScore < 25 → logged, discarded
  let classification;
  let primaryRejectReason = null;
  const scoreFlags = qualityFlags.score || qualityFlags.gradeB;

  if (flagCount === 0 && executionScore >= 65) {
    classification = "ELITE";
    checkpoint("exec", true, `execScore=${executionScore} flags=0 → ELITE`);
  } else if (flagCount <= 1 && !scoreFlags && executionScore >= 45) {
    classification = "GOOD";
    primaryRejectReason = Object.keys(qualityFlags).find(k => qualityFlags[k]);
    checkpoint("exec", false, `execScore=${executionScore} → GOOD (${primaryRejectReason || "clean"})`);
  } else if (executionScore >= 25) {
    classification = "WATCH";
    primaryRejectReason = Object.keys(qualityFlags).find(k => qualityFlags[k]);
    checkpoint("exec", false, `execScore=${executionScore} flags=${flagCount} → WATCH`);
  } else {
    classification = "REJECT";
    primaryRejectReason = scoreFlags ? "score_floor" : Object.keys(qualityFlags).find(k => qualityFlags[k]) || "exec_score_low";
    checkpoint("exec", false, `execScore=${executionScore} → REJECT`);
    reject(primaryRejectReason, asset, market, { score, momentum, heat: getHeat(asset), volRatio });
  }

  // ── Log every candidate that reached Quality Assessment ────────
  const heatAtDecision = getHeat(asset);
  // V104.1: store both the rolled-up scores AND the raw factor values that
  // produced them. Rolled-up scores answer "what was the composite?".
  // Raw values answer "which underlying factor actually drove outcomes?" --
  // e.g. does RVOL > 2.0 matter, is ATR expansion predictive, etc.
  // Pure telemetry -- no scoring logic changes.
  const scoreBreakdown = {
    // Rolled-up composite components (0-100, weighted into executionScore)
    setupQuality,   // weight 35%
    execWindow,     // weight 25% (rr + expansion components)
    regimeScore,    // weight 20%
    conviction,     // weight 20% (heat + volume components)
    rrQuality,      // raw component of execWindow
    expQuality,     // raw component of execWindow
    heatNorm,       // raw component of conviction
    volNorm,        // raw component of conviction

    // Raw factor values -- the actual numbers behind the above scores
    rvol:           volRatio,                              // raw volume ratio
    rr,                                                     // raw reward:risk
    atrExpansion:   atr > 0 ? (tp - entry) / atr : null,    // target distance in ATR units
    heat:           heatAtDecision,                         // raw heat 0-150
    momentum,                                                // raw momentum value
    riskPct,                                                 // raw risk % of account
    setupType,                                               // pattern classification
    rawSniperScore: score,                                  // pre-execution-score Sniper score
  };

  safeRun("logDecision", () => logSignalDecision({
    asset, market, setupType, classification,
    rejectReason:    primaryRejectReason,
    flags:           qualityFlags,
    flagCount,
    score,           // raw_sniper_score
    executionScore,  // composite exec quality score
    scoreBreakdown,  // V104.1: full component + raw factor telemetry
    grade,
    momentum,
    heat:            heatAtDecision,
    volRatio,
    rr,
    riskPct,
    expansionAtr:    atr > 0 ? (tp - entry) / atr : null,
    execQuality,
    entry, sl, tp,
  }));

  // WATCH and REJECT do not produce trade signals
  // V104: record near misses for EOD reporting
  if (classification === "GOOD" || classification === "WATCH") {
    const today = new Date().toISOString().slice(0, 10);
    // Only keep today's entries, clear old ones
    if (nearMissLog.length > 0 && nearMissLog[0].date !== today) nearMissLog.length = 0;
    // Deduplicate -- keep highest score per asset per day
    const existing = nearMissLog.findIndex(n => n.asset === asset);
    const entry = {
      date, asset, market, setupType, classification,
      score, executionScore, grade,
      flags: Object.keys(qualityFlags).filter(k => qualityFlags[k]),
      rr: Math.round(rr * 100) / 100,
      volRatio: Math.round(volRatio * 100) / 100,
    };
    if (existing >= 0) {
      if (score > nearMissLog[existing].score) nearMissLog[existing] = entry;
    } else {
      nearMissLog.push(entry);
    }
  }
  if (classification === "WATCH" || classification === "REJECT") return null;

  // GOOD goes to internal priority queue only -- no public trade alert
  // Until historical data proves GOOD performs as well as ELITE
  if (classification === "GOOD") {
    log(`🟡 GOOD candidate: ${asset} score=${score} flags=${JSON.stringify(qualityFlags)}`);
    // 2hr dedup -- same asset only alerts once per 2 hours to avoid spam
    const lastGoodAlert = goodCandidateLog.get(asset) || 0;
    const goodCooldown  = 2 * 60 * 60 * 1000;
    if (Date.now() - lastGoodAlert > goodCooldown) {
      goodCandidateLog.set(asset, Date.now());
      safeRun("goodQueue", () => sendPrivate(
        `🟡 <b>GOOD CANDIDATE</b>\n\n` +
        `<b>${escapeHtml(asset.replace("USDT",""))}</b> ${market}\n` +
        `Score: ${score} | Grade: ${grade}\n` +
        `R:R: ${rr} | Setup: ${setupType}\n` +
        `Flags: ${Object.keys(qualityFlags).filter(k=>qualityFlags[k]).join(", ")}\n\n` +
        `<i>Internal only — collecting evidence before public promotion</i>`
      ));
    }
    return null;
  }

  // ELITE continues to trade signal
  checkpoint("score", true, `${score} >= floor ${scoreFloor} grade=${grade} → ELITE → trade`);
...
function buildSignalLesson(signal) {
  const parts = ["🧠 <b>Why Sniper liked this setup</b>", ""];
  if (signal.setupType === "GAP_CONTINUATION") {
    parts.push(`🚀 <b>Gap Continuation</b>`);
  parts.push(`The price opened sharply higher and managed to hold those gains instead of giving them back. This signals strong buying interest from the start of the session.`);
  } else if (signal.setupType === "TREND_RESUMPTION") {
    parts.push(`🔄 <b>Trend Resumption</b>`);
  parts.push(`The price paused after an initial move higher, then held its ground rather than falling back. Large investors appeared to be buying gradually rather than chasing the price up — a sign of steady, organised demand.`);
  } else if (signal.setupType === "PRE_BREAKOUT_EXPANSION") {
    parts.push(`📈 <b>Early Momentum Building</b>`);
  parts.push(`Buying pressure is increasing as the price approaches a level it has struggled to break through before. This is an early-stage setup — higher potential reward, but also higher risk. Sniper sized the position accordingly.`);
  } else if (signal.setupType === "BREAKOUT_CONTINUATION") {
    parts.push(`💥 <b>Breakout Continuation</b>`);
  parts.push(`The price broke through a level that had previously stopped it, and did so with noticeably higher trading volume. When more people are buying at a breakout, it tends to be more reliable.`);
  } else if (signal.setupType === "PULLBACK_CONTINUATION") {
    parts.push(`🎯 <b>Pullback Continuation</b>`);
  parts.push(`The price briefly pulled back after a move higher, then buyers stepped in again. This often happens in healthy uptrends — a small dip followed by renewed buying interest. Sniper looks for exactly this pattern.`);
  } else if (signal.setupType === "MOMENTUM_BREAKOUT") {
    parts.push(`🔥 <b>Momentum Breakout</b>`);
  parts.push(`A surge in buying pushed the price through a key level with significantly higher volume than usual. The increase in trading activity confirms this is genuine demand, not just a random move.`);
  } else {
    parts.push(`⚡ <b>Momentum Ignition</b>`);
  parts.push(`A sudden burst of buying pushed the price sharply higher with exceptional volume. These setups move fast — Sniper entered quickly. Higher risk profile, so position size was kept smaller than usual.`);
  }

  if (signal.volRatio >= 3.0) {
    parts.push(`📊 Trading volume was ${signal.volRatio.toFixed(1)}x higher than usual — an exceptionally strong signal that many buyers were active at once.`);
  } else if (signal.volRatio >= 2.0) {
    parts.push(`📊 Trading volume was ${signal.volRatio.toFixed(1)}x higher than usual — significantly above average, suggesting real conviction from buyers.`);
  } else {
    parts.push(`📊 Trading volume was ${signal.volRatio.toFixed(1)}x higher than usual — above average, showing active participation from buyers.`);
  }
...
✅ <b>All conditions aligned</b> — this was one of Sniper's highest-confidence setups.`);
  else if (signal.grade === "A") parts.push(`
✅ <b>Strong signal</b> — most conditions were well aligned.`);
...
function buildSignalMessage(signal, size) {
  ...
  // ── PUBLIC: clean, concise, action-focused ───────────────────────
  // Educational lesson moves to Bluejam once track record is established.
  return lines([
    `🎯 <b>SNIPER TRADE ALERT</b>${ntag}`,
    ``,
    ...
    `🎯 Target  ${escapeHtml(formatPrice(signal.tp, signal.market))} <b>(+${escapeHtml(tpPct)}%)</b>`,
    `🚀 Extended ${escapeHtml(formatPrice(tp2, signal.market))} <b>(+${escapeHtml(tp2Pct)}%)</b>`,
    `━━━━━━━━━━━━━━`,
    `📊 Vol: ${escapeHtml(signal.volRatio.toFixed(1))}x above average  ·  Risk: ${escapeHtml(riskPct)}% of account`,
    ``,
    `👉 <b>Action:</b> Sniper has flagged this setup. Entry, stop and target levels are shown above. Always manage your own risk.`,
    ``,
    `——————————`,
    `📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`,
    `@baretradesignals 🎯`,
...
// ── Bluejam version: full detail + educational lesson ────────────
// Sent to private channel alongside public alert.
// Contains execution plan, setup analysis, technical notes.
// Will move to public channel once 20-30 trade track record established.
function buildSignalMessagePrivate(signal, size) {
  const sym       = signal.asset.replace("USDT","");
  const matched   = HOT_SECTORS.filter(s => (SECTOR_SYMBOLS[s]||[]).includes(sym));
  const ntag      = matched.length>0 ? `\n🧠 <b>${escapeHtml(matched.join(" + "))}</b>` : "";
  const slPct     = (((signal.entry - signal.sl)/signal.entry)*100).toFixed(1);
  ...
  return lines([
    `🎯 <b>SNIPER SIGNAL — FULL DETAIL</b>${ntag}`,
    ``,
    `${marketEmoji(signal.market)} <b>${escapeHtml(signal.market)} | ${escapeHtml(sym)}</b>`,
    ...
    `━━━━━━━━━━━━━━`,
    `💰 Entry   ${escapeHtml(formatPrice(signal.entry, signal.market))}`,
    `🛡️ Stop    ${escapeHtml(formatPrice(signal.sl, signal.market))} (-${escapeHtml(slPct)}%)`,
    `🎯 Target1 ${escapeHtml(formatPrice(signal.tp, signal.market))} (+${escapeHtml(tpPct)}%)`,
    `🚀 Target2 ${escapeHtml(formatPrice(tp2, signal.market))} (+${escapeHtml(tp2Pct)}%)`,
    `⚖️ R:R ${escapeHtml(String(signal.rr))} | Hunter: ${escapeHtml(String(signal.hunterScore || signal.score))} | Score: ${escapeHtml(String(signal.score))}`,
    `━━━━━━━━━━━━━━`,
    `📋 <b>EXECUTION PLAN</b>`,
    `${escapeHtml(signal.execEntryNote || "")}`,
    `${escapeHtml(signal.execCancelCond || "")}`,
    ``,
    `📊 Vol: ${escapeHtml(signal.volRatio.toFixed(2))}x  Risk: ${escapeHtml(riskPct)}%  execQ: ${escapeHtml(String(signal.execQuality||"?"))}`,
    ``,
...
function buildCloseMessage(trade, reason, closePrice) {
  ...
  if (isWin) {
    parts.push(``, `✅ <b>Target reached.</b>`);
    parts.push(`🧠 <b>What happened?</b>`);
    parts.push(`The price continued higher and reached the target level. Sniper closed the trade automatically at the planned exit.`);
    if (rVal) parts.push(`📈 Result: +${rVal}R`);
  }
  if (isLoss) {
    parts.push(``, `🛑 <b>Stop level reached.</b>`);
    parts.push(`🧠 <b>What happened?</b>`);
    parts.push(`The price moved against the trade and reached the stop level. Sniper closed automatically to protect capital. Losses are a normal part of any trading system — what matters is keeping them smaller than the wins.`);
    if (rVal) parts.push(`📉 Result: ${rVal}R`);
  }
...
function pickTopSignals(results) {
  ...
  // V103 GROWTH MODE: elite threshold is now purely for logging/diagnostics.
  // Gate 2 no longer rejects candidates -- score floor + RR already did that work.
  // Ranking selects best per market from all candidates that cleared Gate 1.
  // This removes the hidden floating ceiling that was blocking 74-77 score trades.
  const eliteThreshold = Math.max(55, secondScore + 2); // kept for log visibility only
  log(`🎯 Elite threshold (info only): ${eliteThreshold.toFixed(0)} (top=${topScore.toFixed(0)} second=${secondScore.toFixed(0)})`);

  // Take best candidate per market from everyone who cleared the score floor
  // No additional score gate here -- ranking, not filtering
  const selected  = [];
  ...
  if (selected.length === 0) {
    log("🚫 NO CANDIDATES -- none cleared score floor or RR gate");
  } else {
...
async function sendPerformanceReport() {
  try {
    ...
    const parts = [
      ...
      "📚 <i>Sniper shares market intelligence and educational commentary.</i>\n@baretradesignals 🎯"
    ];
...
async function manageTrades() {
  ...
  for (const trade of trades) {
    ...
    if (trade.status === "PENDING") {
      ...
      // V103: momentum setups get 6% drift room instead of 4% -- SQL audit showed DRIFTED trades
      // were already at/beyond TP when cancelled. 6% is a measured test (data clustered at 4-4.3%).
      const momentumSetup = ["MOMENTUM_IGNITION","BROAD_MOMENTUM","PULLBACK_CONTINUATION"].includes(trade.setupType);
      const driftThreshold = momentumSetup ? 1.06 : 1.04;
      const drifted  = trade.entryType==="LIMIT BUY"        && livePrice > trade.entry * driftThreshold;
...
async function processSignals(signals) {
  ...
  for (const signal of signals) {
    ...
    await safeRun(`signalDetail${signal.asset}`, () => sendPrivate(buildSignalMessagePrivate(signal, size)));
...
async function sendPreMarketAlert() {
  ...
  const msg = [
    ...
    ...moverLines,
    "",
    `💡 Sniper continues monitoring. A trade alert is only issued if all entry conditions confirm.`,
    ``,
    `——————————`,
    `📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`,
    `@baretradesignals 🎯`
...
let lastHealthDashboardDate = "";
let lastOutcomeTrackerDate  = "";       // V104: outcome tracker fires once per day at US close
...
async function sendMorningBriefing() {
  ...
  const msgParts = [
    ...
    `<i>High Momentum Watchlist — US open in ~${Math.round((870 - getUkMinutes()))} mins</i>`,
    "",
  ];
  ...
  msgParts.push(`🔍 Sniper scans begin at the US open. A trade alert is only issued if all entry conditions confirm.`);

  msgParts.push(``, `——————————`, `📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`);
  const msg = msgParts.filter(l => l !== undefined).join("\n");
  await sendChannel(msg);
  // V103: flag if today's top pick appeared in morning movers
  if (dailyStats.topPickToday) {
    const tp = dailyStats.topPickToday.sym;
    if (movers.some(m => m.ticker === tp || m.sym === tp)) {
      dailyStats.topPickToday.inMorningMovers = true;
      log(`🌅 Top pick ${tp} confirmed in morning movers`);
    }
  }
...
function updateSniperRoster() {
  ...
  if (shouldSendStockAlerts() || shouldSendCryptoAlerts()) {
    ...
    for (const { asset, heat } of warmCandidates) {
      ...
      // V104: conviction alerts removed from public channel -- heat ≥60 is meaningful to the system
      // but not to subscribers. Leaderboard and Rising Stars carry this information instead.
      log(`👀 Conviction internal: ${asset} heat=${Math.round(heat)} (no public broadcast)`);
    }
  }
}
...
async function safeCycle() {
  ...
  try {
    ...
    if (nowProtecting && !macroProtecting && alertCooldown) {
      ...
      lines2.push(``);
      // Determine tone based on what's blocked
      const bothBlocked = cryptoBlocked && usBlocked;
      const cryptoOnly  = cryptoBlocked && !usBlocked;
      const usOnly      = !cryptoBlocked && usBlocked;
      if (bothBlocked) {
        lines2.push(`Risk conditions are elevated. Sniper is staying selective and waiting for higher-probability confirmations before issuing trade alerts.`);
      } else if (cryptoOnly) {
        lines2.push(`US equities remain the primary focus while crypto requires stronger confirmation.`);
      } else if (usOnly) {
        lines2.push(`Crypto opportunities remain active. Sniper continues scanning while awaiting a clearer US trend.`);
      }
      lines2.push(``);
      lines2.push(`Intel and market analysis continue as normal.`);
      lines2.push(``);
      lines2.push(`——————————`);
      lines2.push(`📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`);
      lines2.push(`@baretradesignals 🎯`);
    ...
    if (Date.now() - lastLeaderboardCheck > 12 * 60 * 1000) {
      ...
      await safeRun("leaderboardDigest",   sendLeaderboardDigest); // V103: hourly digest replaces per-event spam
    }
    // V103: batch-persist dirty heat scores every 5 minutes
    if (Date.now() - lastHeatPersist > 5 * 60 * 1000) {
      lastHeatPersist = Date.now();
      await safeRun("persistHeat", persistDirtyHeatScores);
    }
    ...
    // V104: hourly reject summary removed -- daily block stats at US close covers this
...
server = app.listen(PORT, "0.0.0.0", async () => {
  ...
  await safeRun("bootTelegram", () => sendPrivate(lines([
    "🎯 SNIPER V104 LIVE — GROWTH MODE · EXECUTION ENGINE · VALIDATION PHASE",
    "",
  ...
  await safeRun("restoreHeat",   loadHeatScoresFromSupabase); // V103: restore heat before first scan
  await safeRun("initialPools",  refreshDynamicPools);
  ready = true;
  log("✅ SNIPER V104 READY -- GROWTH MODE · COLLECT DATA · DO NOT OPTIMISE EARLY");
