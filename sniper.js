// ================================================================
// SNIPER V72
// Hybrid worker + health server + capital allocator
//
// DATA
...
console.log("🚀 SNIPER V72 STARTING...");
...
app.get("/", (_req, res) => {
  res.status(200).send("SNIPER V72 alive");
});
...
function shouldReplaceTrade(newSignal, currentTrade, price) {
  ...
  const scoreBetter = newSignal.score > (currentTrade.score || 0) + 8;
  const isStalling = progress < 0.3;
  const stale = hoursAgo(currentTrade.filledAt) > 6;
...
async function buildDynamicStockPool() {
  ...
  await mapWithConcurrency(seed, 4, async symbol => {
    ...
    if (momentum1d < 0.01) return;
    if (momentum1m <= 0) return;
    if (momentum3m <= 0) return;
    if (last < ema20v || last < ema50v) return;
    if (volRatio < 1.05) return;
...
async function scan() {
  ...
  if (ENABLE_US && isUSMarketOpen()) {
    const activeStockPool = DYNAMIC_STOCK_UNIVERSE
      ? await buildDynamicStockPool()
      : STOCK_POOL;
...
async function processSignals(signals) {
  ...
  for (const s of signals) {
    ...
    if (currentTrade) {
      ...
      if (!replace) {
        console.log(`IGNORED ${s.asset} — current ${s.market} position stronger or too advanced`);
        continue;
      }
...
👉 Action: REPLACE`;
    ...
    if (!currentTrade && (openRiskFraction + riskFraction) > MAX_PORTFOLIO_RISK) {
...
app.listen(PORT, "0.0.0.0", async () => {
  ...
  console.log("🚀 SNIPER V72 LIVE");

  await send(
`🚀 SNIPER V72 LIVE

✅ dynamic stock universe
✅ top 1 crypto allocator
✅ top 1 stock allocator
