...
let cycleCount      = 0;
let lastSignalTime  = 0; // timestamp of last fired signal — watchlist suppressed if recent
...
async function processSignals(signals) {
  ...
  for (const signal of signals) {
    ...
    lastSignalTime = Date.now(); // reset quiet timer
...
async function safeCycle() {
  ...
  try {
    ...
    if (Array.isArray(signals) && signals.length > 0) {
      ...
      // Watchlist alert: only after 90 mins of no signals AND every 5 cycles
      const quietMs = Date.now() - lastSignalTime;
      const quietEnough = lastSignalTime === 0 || quietMs > 90 * 60 * 1000;
      if (quietEnough && cycleCount % 5 === 0) {
        const alerts = await loadAlerts();
        const trades = await loadTrades();
