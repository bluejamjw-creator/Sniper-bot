// ================================================================
// SNIPER V97 -- MOMENTUM MATURITY ENGINE
// ----------------------------------------------------------------
// V97 changes from V96:
//   - Separate rulesets per setup archetype
//     HIGH_BETA opening window gets fully relaxed rules
//     Extension, risk, score floor all setup-specific
//   - Rejection stat logging -- daily summary of what blocked signals
//     Posted to Bluejam so you can see the bottleneck
//   - HIGH_BETA risk limit restored to 15% (runners need room)
//   - Score floors lowered: US 72->68, HIGH_BETA 70->65
//   - Extension limits per setup type not universal cap
//   - Regime override removed entirely -- BTC bull only for crypto
//   - QQQ momentum pre-fetched once per cycle not per asset
// ================================================================
...
console.log("🚀 SNIPER V97 STARTING...");
...
app.get("/",       (_req, res) => res.status(200).send("SNIPER V97 alive"));
...
const MAX_PORTFOLIO_RISK  = 0.08; // V97: raised from 5% -- allows 2-3 concurrent signals
const MAX_POSITIONS = { CRYPTO: 3, US: 2, LSE: 2 }; // V97: raised for subscription -- subscribers miss signals at 1
...
const MIN_US_SCORE_HB       = 65;   // V97: lowered from 70 -- runners need room to score
...
const MIN_GLOBAL_SCORE = 65;        // V97: lowered from 70 -- give stocks room
const MIN_CRYPTO_SCORE = 78;        // kept high -- crypto quality over quantity
const MIN_US_SCORE     = 68;        // V97: lowered from 72
const MIN_LSE_SCORE    = 60;        // V97: lowered from 64

const PENDING_EXPIRY_HOURS_US  = 12;
const PENDING_EXPIRY_HOURS_LSE = 24;
const MIN_CANDLES              = 40;  // standard requirement
const MIN_CANDLES_OPENING      = 8;   // V97: opening window only has 8-16 bars at 15min -- must accept fewer
const ATR_PERIOD               = 14;
...
// V97: MAX_POSITIONS defined above at line 117
...
// V97: TwelveData per-minute rate limit guard
...
// ── Regime override ──────────────────────────────────────────────
const REGIME_OVERRIDE_MIN_VOL      = 2.5;  // V96: raised from 1.6 -- must be truly exceptional
const REGIME_OVERRIDE_MIN_MOMENTUM = 0.020; // V96: raised from 1.0% to 2.0% -- real ignition only
...
// V97: tracks why signals are blocked so you can see the bottleneck
...
const SECTOR_SYMBOLS = {
  ...
  SPACE:      ["RKLB","LUNR","ASTS","ACHR","JOBY","SPCE"],
...
// LSE pool rebuilt for V94 -- momentum names, not just FTSE 100 blue chips
// Mix of defence (BA.L, RR.L, AVON.L), miners on trend (AIM/FTSE),
// growth (EXPN.L, AUTO.L, MNDI.L), plus liquid large caps that can trend
const LSE_POOL = [
  // Defence & aerospace -- narrative active
  "BA.L","RR.L","QQ.L",
  // Miners -- commodity momentum
  "RIO.L","AAL.L","GLEN.L","FRES.L",
  // Banks -- rate sensitivity, can trend
  "BARC.L","LLOY.L","NWG.L",
...
async function getTrendingStocks() {
  ...
  const core = ["NVDA","PLTR","AMD","ASTS","IONQ","MSFT","META","AMZN","APP","MSTR",
...
async function getTopMovers() {
  ...
  const candidates = ["NVDA","ASTS","IONQ","PLTR","AMD","COIN","MSTR","AVAV","SMR","OKLO",
    "CRWD","APP","RKLB","LUNR","ACHR","SMCI","TSLA","HOOD","MRNA","INTC"];
  const movers = [];
...
async function sendWatchlistAlert(alerts, trades) {
    ...
    for (const sym of CRYPTO_PAIRS.slice(0, 15)) {
      try {
        ...
        if (nearEMA && risingEMA && volBuilding) {
          ...
          watching.push({
            label: `\uD83E\uDE99 <b>${escapeHtml(name)}</b> \u2014 pullback to EMA`,
            key: name,
            priority: volRat
          });
        }
  ...
  parts.push(``, `\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014`, `\u26A0\uFE0F <i>Educational only · Not financial advice · Not FCA regulated · @baretradesignals</i>`);
...
async function sendNarrativeAlert(topMovers, cryptoMovers, stockMoverPcts = {}, lseMovers = []) {
  ...
  const isOpeningBell = !isWeekend() && isUSMarketOpen() && getUkMinutes() < 880; // within 10 mins of open
  const parts = [isOpeningBell ? "\uD83D\uDD14\uD83D\uDFE2 <b>OPENING BELL INTEL</b>" : "\uD83E\uDDE0\uD83D\uDD25 <b>MARKET INTEL UPDATE</b>", ""];

  // ── REGIME SECTION ──────────────────────────────────────────────
  const btcLabel = btcTrend==="strong_bull" ? "STRONG BULL \uD83D\uDFE2"
                 : btcTrend==="bull"        ? "BULL \uD83D\uDFE2"
                 : btcTrend==="bear"        ? "BEAR \uD83D\uDD34"
                 : "NEUTRAL \uD83D\uDFE1";

  const btcContext = btcTrend==="strong_bull"
    ? "Trend strong above key moving averages. Volume expanding. Full signal mode active."
    : btcTrend==="bull"
    ? "Momentum building above EMAs. Pullback entries and ignition setups active."
    : btcTrend==="bear"
    ? "Below key moving averages. No crypto signals active. Capital preservation mode."
    : "Holding range without conviction. Selective setups only. Reduced signal mode.";

  parts.push(`\u20BF <b>BITCOIN REGIME: ${btcLabel}</b>`);
  parts.push(`<i>${btcContext}</i>`);
  parts.push("");

  if (!isWeekend()) {
    const qqqLabel = qqqTrend==="bull"     ? "BULL \uD83D\uDFE2"
                   : qqqTrend==="risk_off" ? "RISK OFF \uD83D\uDD34"
                   : "NEUTRAL \uD83D\uDFE1";

    const qqqContext = qqqTrend==="bull"
      ? "Indices expanding. HIGH_BETA momentum setups active. Full US signal mode."
      : qqqTrend==="risk_off"
      ? "Indices under pressure. No US stock signals. Defensive posture."
      : "Indices holding range. Selective strength only. Focus on sector leaders.";

    parts.push(`\uD83D\uDCC8 <b>STOCK MARKET REGIME: ${qqqLabel}</b>`);
    parts.push(`<i>${qqqContext}</i>`);
    parts.push("");
  }

  const cryptoBull = btcTrend==="bull"||btcTrend==="strong_bull";
  const stocksBull = qqqTrend==="bull";
  const signalMode = cryptoBull && stocksBull ? "\uD83D\uDFE2 FULL AGGRESSION — all setups active"
                   : cryptoBull              ? "\uD83D\uDFE2 Crypto active · \uD83D\uDFE1 Stocks selective"
                   : stocksBull              ? "\uD83D\uDFE1 Crypto selective · \uD83D\uDFE2 Stocks active"
                   : "\uD83D\uDFE1 SELECTIVE MODE — quality setups only";

  parts.push(`\uD83C\uDFAF <b>SIGNAL MODE: ${signalMode}</b>`);
  parts.push("");
  ...
  if (!isWeekend() && topMovers.length > 0) {
    const ranked = topMovers.slice(0, 6).map(sym => {
      ...
      return { sym, pmPct, pct, strength, inSector };
    }).sort((a, b) => b.strength - a.strength);

    parts.push("\uD83C\uDDFA\uD83C\uDDF8 <b>Hot stocks right now:</b>");
    for (const m of ranked) {
      const pmTag  = m.pmPct >= 5 ? ` \uD83D\uDD25 +${m.pmPct.toFixed(1)}% pre`
                   : m.pmPct >= 3 ? ` \u26A1 +${m.pmPct.toFixed(1)}% pre`
                   : m.pct >= 3   ? ` +${m.pct.toFixed(1)}%`
                   : "";
      const narTag = m.inSector ? " \uD83E\uDDE0" : "";
      parts.push(`  \uD83D\uDE80 <b>${escapeHtml(m.sym)}</b>${pmTag}${narTag}`);
    }
    if (ranked.length > 0) {
      parts.push(``, `\uD83C\uDFC6 <b>Top pick: ${escapeHtml(ranked[0].sym)}</b> \u2014 strongest setup heading into session`);
    }
    parts.push("");
  }
  ...
  // V97: DexScreener early momentum -- catches DEX moves before CEX
  try {
    ...
    if (dexMovers.length > 0) {
      ...
      for (const d of dexMovers.slice(0,3)) {
        parts.push(`  \uD83D\uDD25 <b>${escapeHtml(d.symbol)}</b> +${d.change1h.toFixed(1)}%/1h \u2014 ${escapeHtml(d.chain)}`);
      }
      parts.push("");
    }
  ...
  // ── AVOID SECTION ───────────────────────────────────────────────
  const avoidReasons = [];
  if (!cryptoBull) avoidReasons.push("Weak altcoins without BTC confirmation");
  if (!stocksBull) avoidReasons.push("Extended breakouts without broad volume");
  avoidReasons.push("Low conviction setups — wait for A* grade only");
  parts.push("\u26A0\uFE0F <b>Avoid right now:</b>");
  for (const r of avoidReasons) parts.push(`  \u2022 ${r}`);
  parts.push("");
...
async function refreshDynamicPools(forceNarrativeAlert = false) {
  ...
  try {
    ...
    // V97: DexScreener removed from pool injection -- returns junk meme tokens
  ...
  for (const { key, isOpen } of checks) {
    ...
    if (nowOpen && !marketWasOpen[key]) {
      ...
      if (key !== "CRYPTO") {
        ...
        // V97: fire fresh intel at market open -- pre-market intel goes stale by 14:30
    ...
    if (!nowOpen && marketWasOpen[key]) {
      ...
      if (key !== "CRYPTO") {
        ...
        // V97: send rejection stats to Bluejam on US close so you can see bottleneck
...
async function fetchTwelveDataSeries(symbol, interval="1h", outputsize=70) {
  ...
  // V97: per-minute rate limit guard -- free tier = 8/min hard limit
  ...
  try {
    ...
    // V97: log TD errors so we can see rate limit messages
...
// Free paper trading account gives real market data
// Much better than Yahoo 1h for intraday structure
// Primary source for HIGH_BETA during opening window
async function fetchAlpacaBars(symbol, timeframe = "15Min", limit = 60) {
  ...
  try {
    ...
    log(`🦙 Alpaca ${symbol} -- ${candles.length} bars`);
    setCache(cacheKey, candles);
    return candles;
  } catch (e) {
...
async function fetchPolygonBars(symbol, timespan = "hour", multiplier = 1, days = 30) {
  ...
  try {
    ...
    const { data } = await retry(() =>
      axios.get(`https://api.polygon.io/v2/aggs/ticker/${symbol}/range/${multiplier}/${timespan}/${from}/${to}`, {
...
async function fetchYahooFinanceSeries(symbol, interval="1h") {
  ...
  const range         = yahooInterval==="5m"?"2d":yahooInterval==="15m"?"10d":"60d"; // V97: 15m range 2d->10d for more candles
  try {
    const { data } = await retry(() =>
...
// V97: Yahoo 5-min aggregated to 15-min for HIGH_BETA during opening window
...
// Top HIGH_BETA names for Yahoo 5m fallback (rate limit protection)
const TOP_HIGHBETA_5M = ["IONQ","ASTS","RKLB","LUNR","CLSK"];
...
async function fetchUSData(symbol) {
  ...
  // V97: Alpaca primary for HIGH_BETA during opening window
  ...
  // V97: Polygon as primary for standard US stocks -- better than Yahoo 1h
  ...
  if (!candles) {
    ...
    if (candles && candles.length > 0) {
      ...
      // Yahoo throttled -- try Polygon as backup for standard stocks
...
async function fetchLSEData(symbol) {
  ...
  const cached   = getCached(cacheKey);
  if (cached) return cached;
  // LSE uses Yahoo only -- TD credits reserved exclusively for US opening window
  let candles = await fetchYahooFinanceSeries(symbol,"1h");
  if (candles) setCache(cacheKey, candles);
  return candles;
}
...
async function updateQQQTrend() {
  ...
  // V97: if Yahoo fails QQQ fetch, keep last known trend instead of resetting to neutral
...
async function analyse(asset, candles, market, prefetchedQqqMom = null) {
  // V97: opening window stocks only have 8-16 bars at 15min -- use lower minimum
  ...
  // V97: verbose checkpoint logging for HIGH_BETA US stocks
  ...
  if (momentum <= minMomentum) {
    ...
    if (market !== "LSE") reject("momentum_low", asset, market); return null;
    return null;
  }
  ...
  if (useMode2 && inUSOpen) {
    ...
    // V97: relaxed crypto EMA -- price above EMA20 too strict for sideways market
  ...
  if (!emaOk && !isTrendResumptionCandidate) {
    ...
    reject("ema_fail", asset, market); return null;
  }
  checkpoint("EMA", true);
  ...
  // If volume data is missing (Yahoo often returns 0s for recent candles),
  // fall back to price range ratio as a volume proxy
  ...
  // V96: guard against NaN/Infinity from bad data -- kills downstream comparisons
  ...
  if (!regimePass(market) && !exceptionalCrypto) {
    ...
    reject("regime", asset, market); return null;
  }
  checkpoint("regime", true, `btc=${btcTrend} qqq=${qqqTrend}`);
  const isRegimeOverride = exceptionalCrypto && !regimePass(market);
  if (isRegimeOverride) {
  ...
  // No regime override for ignition -- if BTC isn't bull, real ignition is rare
  const ignitionMomThreshold = market==="CRYPTO" ? 0.03 : (inUSOpen||highBeta) ? 0.025 : 0.035;
  const ignitionBtcOk = market==="CRYPTO" ? (btcTrend==="bull"||btcTrend==="strong_bull") : true;
  ...
  // V97: LATE MOVE GUARD -- block BREAKOUT_CONTINUATION when already extended on day
  // TON +7.8% then firing BREAKOUT at the top = late entry, likely to reverse
  // Real breakouts fire early -- if 24hr move > 8% the breakout already happened
  if (setupType === "BREAKOUT_CONTINUATION" && market === "CRYPTO") {
    if (pct24h >= 0.08 && momentum < 0.015) {
      reject("late_breakout", asset, market); return null;
    }
  }

  // Without BTC bull, pullback entries have no trend to continue into
  if (isRegimeOverride && setupType === "PULLBACK_CONTINUATION") {
  ...
  const entryType = setupType === "PULLBACK_CONTINUATION"
    ? (useConditional ? "CONDITIONAL BUY" : "LIMIT BUY")
    : "MARKET BUY";

  // CONDITIONAL BUY: trigger set 0.5% above current price -- fills on upside break
  // LIMIT BUY: set at EMA20 * 1.008 -- waits for pullback
  const entry = setupType === "PULLBACK_CONTINUATION"
  ...
  // V97: setup-specific risk limits
  // HIGH_BETA runners need wider stops -- they gap and pullback before running
  // Crypto and standard stocks keep tighter 8% cap
  const maxRisk = (market === "US" && highBeta) ? 0.15 : 0.08;
  if (riskPct <= 0 || riskPct > maxRisk) { reject("risk_pct", asset, market); return null; }

  const tpRaw = entry + riskAmt * TARGET_R;
  // V97: dynamic crypto TP based on BTC regime
  // BTC bull = full R:R target -- trend has conviction, 8-10% moves realistic
  // BTC neutral/bear = cap at 5% -- momentum fades faster, take profit quickly
  const cryptoTpCap = (btcTrend === "bull" || btcTrend === "strong_bull") ? 1.10 : 1.05;
  const tp = market === "CRYPTO" ? Math.min(tpRaw, entry * cryptoTpCap) : tpRaw;
  ...
  // V97: use pre-fetched QQQ from scan cycle -- no per-asset await
  ...
  if (score < MIN_GLOBAL_SCORE || score < scoreFloor) {
    ...
    reject("score_floor", asset, market); return null;
  }
  checkpoint("score", true, `${score} >= floor ${scoreFloor}`);
...
function buildSignalLesson(signal) {
  ...
  if (signal.setupType === "GAP_CONTINUATION") {
    ...
    parts.push("\uD83D\uDD04 <b>Trend Resumption</b> \u2014 gapped up at open, consolidated without giving back the move, now resuming higher. Institutional buyers defending the gap.");
...
function buildSignalMessage(signal, size) {
  ...
  const isConditional = signal.entryType === "CONDITIONAL BUY";
  ...
  return lines([
    ...
    ``,
    isMarket      ? `\u26A1 <b>MARKET BUY NOW</b> ~${escapeHtml(formatPrice(signal.entry, signal.market))}`
    : isConditional ? `\uD83D\uDFE1 <b>CONDITIONAL BUY</b> if price breaks above ${escapeHtml(formatPrice(signal.entry, signal.market))}`
    ...
    ``,
    `\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014`,
    `\u26A0\uFE0F <i>Educational only · Not financial advice · Not FCA regulated</i>`,
    `\uD83D\uDCF8 <i>Signal by @baretradesignals</i>`
  ]);
}
...
function buildCloseMessage(trade, reason, closePrice) {
  ...
  if (isWin)  parts.push(``, `\uD83C\uDF7E <b>Target hit.</b>${rVal?` Made ${rVal}x the risk.`:""} Setup played out as planned.`);
  if (isLoss) parts.push(``, `\uD83D\uDEE1\uFE0F <b>Stop did its job.</b> Loss capped. Capital protected for the next setup.`);
  if (reason==="FORCE CLOSE")     parts.push(``, `\u23F1\uFE0F <b>Force closed.</b> No progress after ${heldHrs}hrs.`);
  if (reason==="PENDING_EXPIRED") parts.push(``, `\u231B <b>Order expired.</b> Entry level never reached.`);

  if (isWin || isLoss) {
    parts.push(``, `\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014\u2014`, `\u26A0\uFE0F <i>Educational only · Not financial advice · Not FCA regulated</i>`, `\uD83D\uDCF8 <i>Signal by @baretradesignals</i>`);
  }
  return lines(parts);
}
...
function shouldForceClose(trade, livePrice) {
  ...
  // V96: MOMENTUM_IGNITION -- prove immediate expansion or die
  // Real momentum moves fast. 0.00% after 15 mins = dead capital.
  if (trade.setupType === "MOMENTUM_IGNITION") {
    if (ageMins >= 15 && mfePct < 0.005) return true; // 15 mins < 0.5% up → close
    if (ageMins >= 30 && mfePct < 0.010) return true; // 30 mins < 1.0% up → close
    if (ageMins >= 60)                   return true; // 60 mins max regardless
    return false;
  }

  // PRE_BREAKOUT_EXPANSION -- needs fast confirmation too
  if (trade.setupType === "PRE_BREAKOUT_EXPANSION") {
    if (ageMins >= 30 && mfePct < 0.005) return true;
    if (ageMins >= 90)                   return true;
    return false;
  }

  // PULLBACK_CONTINUATION -- slower, trend-following tolerance
  if (trade.setupType === "PULLBACK_CONTINUATION") {
    return ageHrs > 3 && progressToTarget(trade, livePrice) < FORCE_CLOSE_PROGRESS;
  }

  // Default
  return ageHrs > FORCE_CLOSE_HOURS && progressToTarget(trade, livePrice) < FORCE_CLOSE_PROGRESS;
}
...
async function closeTrade(trades, trade, closePrice, reason) {
  ...
  // Winners and losses go to channel -- subscribers want to see results
  // Admin closes (replaced, expired, structure invalid) private only
...
async function scan() {
  ...
  // V97: pre-fetch QQQ momentum once per cycle not per asset
...
async function processSignals(signals) {
  ...
  for (const signal of signals) {
    ...
    if (pendingTrades.length >= 2) continue; // V97: raised from 1 -- subscription product needs concurrent signals
...
async function sendMorningBriefing() {
  ...
  const lines = movers.slice(0, 8).map(m => {
    const pct   = m.changePct.toFixed(1);
    const arrow = m.changePct >= 0 ? "\uD83D\uDFE2" : "\uD83D\uDD34";
    const fire  = Math.abs(m.changePct) >= 8 ? " \uD83D\uDD25" : Math.abs(m.changePct) >= 5 ? " \u26A1" : "";
    return `${arrow} <b>${escapeHtml(m.sym)}</b> ${m.changePct >= 0 ? "+" : ""}${pct}%${fire} \u2014 $${m.price.toFixed(2)}`;
  });

  const upCount   = movers.filter(m => m.changePct >= 3).length;
  const downCount = movers.filter(m => m.changePct <= -3).length;
  ...
  const msg = [
    ...
    ...lines,
    "",
    upCount   > 0 ? `\uD83D\uDCC8 ${upCount} stock${upCount > 1 ? "s" : ""} showing pre-market strength` : "",
    downCount > 0 ? `\uD83D\uDCC9 ${downCount} stock${downCount > 1 ? "s" : ""} under pressure` : "",
    "",
    "\uD83D\uDD0D Scanner primes at 14:30 \u2014 entry fires if setup confirms at open"
  ].filter(l => l !== "").join("\n");
...
server = app.listen(PORT, "0.0.0.0", async () => {
  log(`🚀 SNIPER V97 on port ${PORT}`);
  await safeRun("bootTelegram", () => sendPrivate(lines([
    "\uD83D\uDE80 SNIPER V97 LIVE -- MOMENTUM MATURITY ENGINE",
    "",
    "V97: Alpaca 15-min bars for HIGH_BETA -- proper intraday data",
    "V97: Bybit fallback for crypto candles",
    "V97: DexScreener early momentum detection",
    "V97: Rejection stats posted daily -- see what's blocking",
    "V97: Score floors lowered (global 65, US 68, HB 65)",
    "V97: HIGH_BETA risk limit restored to 15%",
    "V97: QQQ pre-fetched once per scan cycle",
