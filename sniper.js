...
function reject(reason, asset = "", market = "", context = {}) {
  ...
  if (asset && supabaseEnabled) {
    supabase.from("sniper_signal_decisions").insert([{
      ...
      engine_version:  "V104",
    }]).catch(() => {}); // silent fail -- never block the scan
  }
}
...
const ALERT_FOOTER = [`——————————`, `📚 <i>Sniper shares market intelligence and educational commentary. It is not personalised financial advice or a recommendation to buy or sell.</i>`, `@baretradesignals 🎯`];
